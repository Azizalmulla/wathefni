...
def register_attendance_ops_routes(
    ...
    *,
    ...
    def _role(context: dict[str, Any]) -> str:
        ...
        if role in {"owner", "hr", "admin", "hr_admin", "hr_manager"}:
            return "hr"
    ...
    def attendance_ops_state_model(context: dict[str, Any] = Depends(dashboard_context)):
        ...
        require_entitlement(context, "attendance", "attendance.read")
    ...
    def attendance_ops_open_exception(
        ...
        company = _company(context)
        _require_ops(company)
        require_entitlement(context, "attendance", "attendance.manage")
    ...
    def attendance_ops_assign(
        ...
        _audit(context, "attendance_ops_exception_assigned", "Assigned an attendance exception.", "attendance_exception", exception_id, result)
        return json_safe(result)
    ...
    def attendance_ops_reopen(
        ...
        if not result.get("ok"):
            code = 409 if result.get("error") == "stale_row_version" else 400
            raise HTTPException(status_code=code, detail=result)
        _audit(context, "attendance_ops_exception_reopened", "Reopened an attendance exception.", "attendance_exception", exception_id, result)
        return json_safe(result)
    ...
    def attendance_ops_request_correction(
        ...
        if not result.get("ok"):
            raise HTTPException(status_code=400, detail=result)
        _audit(context, "attendance_ops_correction_requested", "Requested an attendance correction.", "employee", body.employee_key, result)
        return json_safe(result)
    ...
    def attendance_ops_review_case(
        ...
        if not result.get("ok"):
            code = 409 if result.get("error") == "stale_row_version" else 400
            raise HTTPException(status_code=code, detail=result)
        _audit(context, "attendance_ops_case_reviewed", "Reviewed an attendance case.", "attendance_case", case_id, result)
        return json_safe(result)
    ...
    def attendance_ops_apply_case(
        ...
        if not result.get("ok"):
            code = 409 if result.get("error") == "stale_row_version" else 400
            raise HTTPException(status_code=code, detail=result)
        _audit(context, "attendance_ops_case_applied", "Applied an attendance case.", "attendance_case", case_id, result)
        return json_safe(result)
    ...
    def attendance_ops_raise_dispute(
        ...
        if not result.get("ok"):
            raise HTTPException(status_code=400, detail=result)
        _audit(context, "attendance_ops_dispute_raised", "Raised an attendance dispute.", "employee", body.employee_key, result)
        return json_safe(result)
    ...
    def attendance_ops_resolve_dispute(
        ...
        if not result.get("ok"):
            code = 409 if result.get("error") == "stale_row_version" else 400
            raise HTTPException(status_code=code, detail=result)
        _audit(context, "attendance_ops_dispute_resolved", "Resolved an attendance dispute.", "attendance_dispute", dispute_id, result)
        return json_safe(result)
    ...
    def attendance_ops_comment(
        ...
        if not result.get("ok"):
            raise HTTPException(status_code=400, detail=result)
        _audit(context, "attendance_ops_comment_added", "Added an attendance operations comment.", body.entity_type, body.entity_id, result)
        return json_safe(result)
    ...
    def attendance_ops_attachment(
        ...
        if not result.get("ok"):
            raise HTTPException(status_code=400, detail=result)
        _audit(context, "attendance_ops_attachment_added", "Added attendance operations evidence.", body.entity_type, body.entity_id, result)
        return json_safe(result)
