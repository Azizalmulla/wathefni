...
def runtime_gate_for_company(company_code: str | None) -> dict[str, Any]:
    ...
    if not c1.slice_allowlist_admits(company, "WATHEFNI_HR_INTELLIGENCE_RECRUITING_COMPANIES"):
        return {
            "ok": False,
            "enabled": False,
...
def _c3_settings(cur, company_code: str) -> dict[str, Any]:
    cur.execute(
        "SELECT * FROM hr_intelligence_c3_company_settings WHERE company_code=%s",
...
def _handler_gate(cur, company_code: str) -> dict[str, Any] | None:
    """C3 company entitlement — C1 evaluate alone is not enough after C3 disable."""
    ent = _entitled(cur, company_code)
    if not ent.get("ok"):
        return {
            "status": "unavailable",
            "value": None,
            "population_ids": [],
            "error": ent.get("error") or "company_recruiting_intelligence_disabled",
            "explain": {"c3_required": True},
        }
    return None
...
def _begin(cur, company_code, *, module_flag: str | None = None, module_name: str | None = None):
    gated = _handler_gate(cur, company_code)
    if gated:
        return None, gated
    settings = _c3_settings(cur, company_code)
    if module_flag and module_name:
        unavail = _module_unavailable(settings, module_flag, module_name)
        if unavail:
            return settings, unavail
...
def _handler_ttf(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    _c3s, unavail = _begin(cur, company_code, module_flag="recruiting_module_enabled", module_name="recruiting")
    if unavail:
        return unavail
    _ = settings
...
def _handler_apps_by_source(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    ...
    for a in _active_apps(cur, company_code):
        ...
        if source_filter and a.get("source_bucket") != source_filter:
            continue
        members.append(a)
    # Small-source suppression only when explicitly requested (not ordinary HR company counts)
    if filters.get("apply_min_cohort_suppression") and source_filter:
        try:
...
def _handler_preboard_rate(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    _c3s, unavail = _begin(cur, company_code, module_flag="preboarding_module_enabled", module_name="preboarding")
    if unavail:
        return unavail
    _ = settings
    start, end = _parse_window(time_window)
...
def _handler_onboard_rate(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    _c3s, unavail = _begin(cur, company_code, module_flag="onboarding_module_enabled", module_name="onboarding")
    if unavail:
        return unavail
    _ = settings
    start, end = _parse_window(time_window)
...
def _handler_probation(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    _c3s, unavail = _begin(cur, company_code, module_flag="probation_module_enabled", module_name="probation")
    if unavail:
        return unavail
    _ = settings
    start, end = _parse_window(time_window)
...
def seed_recruiting_definitions(cur: Any, *, actor_phone: str) -> dict[str, Any]:
    ...
    def _ensure(key: str, **kwargs):
        ...
        if ex:
            ...
            if isinstance(fc, str):
                fc = json.loads(fc)
            want_fc = kwargs.get("formula_contract") or {}
            needs_refresh = False
            if want_kind and (not fc or fc.get("kind") != want_kind):
                needs_refresh = True
            elif want_fc and fc and fc.get("sensitive_aggregate") != want_fc.get("sensitive_aggregate"):
                needs_refresh = True
            if needs_refresh:
    ...
    _ensure(
        ...
        formula_contract={"kind": "recruiting_applications_by_source", "sensitive_aggregate": False},
        unit="count", time_semantics="event_count", permission_class="workforce_general", status="published",
    ...
    _ensure(
        ...
        formula_contract={"kind": "recruiting_funnel_events", "sensitive_aggregate": False},
        unit="count", time_semantics="event_count", permission_class="workforce_general", status="published",
