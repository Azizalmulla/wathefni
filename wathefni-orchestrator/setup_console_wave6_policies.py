...
def honesty_payload() -> dict[str, Any]:
    return {
        ...
        "engagement_customer_enableable": True,
        "comp_planning_customer_enableable": True,
        "workforce_planning_customer_enableable": True,
        "remaining_wave6_not_customer_enableable": False,
...
def _wrap_patch(key: str, result: dict[str, Any], policy: dict[str, Any]) -> dict[str, Any]:
    out = {
...
def _setup_gate_requires_company_ja(cur: Any, company: str, gate: dict[str, Any]) -> dict[str, Any]:
    """Setup display: JA must be company-enabled, not only deployment-available."""
    if not gate.get("ok"):
        return gate
    if ja.module_enabled_for_company(cur, company):
        return gate
    return {
        "ok": False,
        "enabled": False,
        "error": "ja_must_be_enabled",
        "ja_is_hard": True,
...
def get_wave6_module_policy(cur: Any, company_code: str, module_key: str) -> dict[str, Any]:
    ...
    if key == "comp_planning":
        ...
        return {
            ...
            "label_ar": status_label(key, lang="ar"),
            "policy": policy,
            "runtime_gate": _setup_gate_requires_company_ja(
                cur, company, cp.runtime_gate_for_company(company)
            ),
            "platform_capability": False,
            "commercial_sku": True,
    ...
    if key == "workforce_planning":
        ...
        return {
            ...
            "policy": policy,
            "runtime_gate": _setup_gate_requires_company_ja(
                cur, company, wfp.runtime_gate_for_company(company)
            ),
            "platform_capability": False,
            "commercial_sku": True,
...
def patch_wave6_module_policy(
    ...
    *,
    ...
    if key == "job_architecture":
        ...
        return _wrap_patch(key, result, get_wave6_module_policy(cur, company, key))
    if key == "learning":
        enabled = body.get("enabled")
        policy_kwargs = {
            k: body[k]
    ...
    if key == "engagement":
        ...
        now_enabled = eg.module_enabled_for_company(cur, company)
        try:
            cur.execute(
                """
                INSERT INTO company_modules (company_code, module_key, enabled, source, settings, updated_at)
                VALUES (%s, 'engagement', %s, 'wave6_setup', '{}'::jsonb, now())
                ON CONFLICT (company_code, module_key)
                DO UPDATE SET enabled=EXCLUDED.enabled, source='wave6_setup', updated_at=now()
                """,
                (company, bool(now_enabled)),
    ...
    if key == "comp_planning":
        ...
        now_enabled = cp.module_enabled_for_company(cur, company)
        try:
            cur.execute(
                """
                INSERT INTO company_modules (company_code, module_key, enabled, source, settings, updated_at)
                VALUES (%s, 'comp_planning', %s, 'wave6_setup', '{}'::jsonb, now())
                ON CONFLICT (company_code, module_key)
                DO UPDATE SET enabled=EXCLUDED.enabled, source='wave6_setup', updated_at=now()
                """,
                (company, bool(now_enabled)),
