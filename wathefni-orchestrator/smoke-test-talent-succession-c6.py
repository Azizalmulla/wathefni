...
def main() -> int:
    ...
    empty_gate = c6.runtime_gate_for_company(COMPANY)
    try:
        import capability_readiness as _ready

        if _ready.customer_enableable("talent"):
            check("empty allowlist admits after R5C", empty_gate.get("ok") is True)
        else:
            check("empty allowlist", "allowlist" in str(empty_gate.get("gate")))
    except Exception:
        check("empty allowlist", "allowlist" in str(empty_gate.get("gate")))
