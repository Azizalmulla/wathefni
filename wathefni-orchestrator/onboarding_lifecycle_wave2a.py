...
def actions_for_hr_item(
    ...
    *,
    ...
    if mode == "ess_encrypted" or item_id == "bank_details":
        # Under review / approved-not-applied → HR reviews via Bank ESS panel.
        if status in REVIEW_STATES | {STATE_SUBMITTED, STATE_PROCESSING}:
            actions.append("review_bank")
        if can_mutate and status not in COMPLETE_STATES | REVIEW_STATES:
...
def actions_for_item(
    ...
    *,
    ...
    if item.get("file_id") or item.get("current_file_id"):
        ...
        actions.append("view_versions")
    if mode == "ess_encrypted" or item_id == "bank_details":
        # Canonical Bank ESS eligibility owns the open action. Never advertise
        # open_bank when /app/bank would refuse the same employee.
        if bank_ess_eligible and not is_lifecycle_complete(status):
            actions.append("open_bank")
        return actions
...
def decorate_item(
    ...
    *,
    ...
    row["bank_ess_eligible"] = bool(bank_ess_eligible) if (
        str(row.get("item_id") or "").lower() == "bank_details"
        or str(row.get("collection_mode") or "").lower() == "ess_encrypted"
    ) else None
    row["actions"] = actions_for_item(
        row,
