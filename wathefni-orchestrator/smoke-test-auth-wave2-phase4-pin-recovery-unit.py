...
def main() -> int:
    ...
    check(
        ...
        en.get("pin.forgotConfirm")
        == "You'll need to sign in again with a new activation code, then create a new PIN.",
    )
    check("EN confirm title", en.get("pin.forgotTitle") == "Reset your PIN?")
    check("EN confirm action Continue", en.get("pin.forgotConfirmAction") == "Continue")
    ...
    check("Forgot shows Alert before recover", "Alert.alert" in flows and "pin.forgotConfirm" in flows)
    check("Cancel button in confirm", "common.cancel" in flows)
    check("Continue only calls onForgotPin", "onForgotPin()" in flows)
