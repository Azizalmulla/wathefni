...
def refuse_nonsynthetic_mutation(
    *,
    employee_key: str | None = None,
    phone: str | None = None,
    name: str | None = None,
    action: str = "payroll_mutation",
    cur: Any = None,
    company_code: str | None = None,
) -> dict[str, Any] | None:
    """When SYNTHETIC_ONLY, refuse mutations on non-marker subjects.

    P6 company entitlement may admit allowlisted / authoritative employees.
    """
    if not payroll_wave1_synthetic_only():
        return None
    if is_payroll_synthetic_employee(employee_key=employee_key, phone=phone, name=name):
        return None
    if cur is not None and company_code:
        try:
            ...
            if p6.employee_production_mutations_allowed(
                cur, company_code=company_code, employee_key=str(employee_key or "")
            ):
                return None
        except Exception:
            pass
    return {
        "ok": False,
        "error": "payroll_synthetic_only",
        "message": "Payroll Wave 1 is synthetic-only; real employees are blocked.",
        "action": str(action or ""),
        "employee_key": employee_key,
    }


def assert_payment_processing_disabled(settings: dict[str, Any] | None = None) -> dict[str, Any] | None:
    row = settings or {}
    if str(row.get("payment_processing") or "disabled") != "disabled":
        return {"ok": False, "error": "payment_processing_must_remain_disabled"}
    return None
...
def create_contract_draft(
    ...
    *,
    ...
    if denied:
        return denied
    synth_ban = refuse_nonsynthetic_mutation(
        employee_key=employee_key,
...
def seed_contract_from_offer(
    ...
    *,
    ...
    synth_ban = refuse_nonsynthetic_mutation(
        employee_key=employee_key,
        action="seed_contract_from_offer",
        cur=cur,
        company_code=company_code,
    )
    if synth_ban:
...
def approve_contract(
    ...
    *,
    ...
    if creator_ban:
        return creator_ban
    synth_ban = refuse_nonsynthetic_mutation(
        employee_key=str(contract.get("employee_key") or ""),
        phone=subject_phone,
        action="approve_contract",
        cur=cur,
        company_code=company_code,
    ...
    *,
    ...
    if subject_phone:
        ...
        if ban:
            return ban
    synth_ban = refuse_nonsynthetic_mutation(
        employee_key=str(previous.get("employee_key") or ""),
        phone=subject_phone,
        action="replace_contract",
        cur=cur,
        company_code=company_code,
    )
    if synth_ban:
...
def reopen_period(cur: Any, **kwargs: Any) -> dict[str, Any]:
    ...
    # Wave 2 C4: authoritative sealed periods cannot silently reopen.
    try:
        import payroll_authoritative_c4 as _c4

        denied = _c4.forbid_period_reopen_if_authoritative(
            cur,
            company_code=kwargs["company_code"],
            period_id=str(kwargs.get("period_id") or period.get("period_id") or ""),
