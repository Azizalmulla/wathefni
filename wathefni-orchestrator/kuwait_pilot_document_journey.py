...
def upload_synced_compliance_types() -> set[str]:
    tm = _doc_type_map()
    if tm is not None:
        types = set(tm.UPLOAD_SYNCED_COMPLIANCE_TYPES)
    else:
        types = {"civil_id", "passport", "medical", "education_cert", "residence", "work_permit"}
    types |= {"employment_contract", "contract_amendment", "education_cert", "residence", "medical"}
    return types


def is_disposable_canary_document_type(document_type: str | None) -> bool:
    """Isolated canary lanes must dual-write without touching real civil_id/passport rows."""
    raw = str(document_type or "").strip().lower()
    if not raw:
        return False
    if raw.startswith("docs_qual_") or raw.endswith("_canary") or "canary" in raw:
        return True
    try:
        import onboarding_doc_validation_parity as docval

        if raw in getattr(docval, "CANARY_VALIDATION_ALIASES", {}):
            return True
    except Exception:
        pass
    return False
...
def should_dual_write_compliance(document_type: str | None) -> bool:
    if is_disposable_canary_document_type(document_type):
        return True
...
def _upsert_compliance_document_row(
    cur: Any,
    *,
    company: str,
    employee_key: str,
    ...
    cur.execute(
        ...
        VALUES (
            %s,%s,%s,%s,%s,%s::jsonb,%s,NULL,NULL,NULL,
...
def dual_write_compliance_from_upload(
    ...
    *,
    ...
    # Disposable canaries stay on their own compliance + governed lane (never alias into real types).
    if is_disposable_canary_document_type(raw):
        ctype = raw
        target_type = raw
        compliance_metadata = {
            **metadata,
    ...
    if ocr_proposal:
        ...
        if expiry_date:
            compliance_metadata["ocr_proposed_expiry_date"] = str(expiry_date)
        if issued_date:
            compliance_metadata["ocr_proposed_issued_date"] = str(issued_date)

    return _upsert_compliance_document_row(
        cur,
        company=company,
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            try:
                ...
                # Reload parts_schema columns if migration just added them.
                if "parts_schema" not in old:
                    cur.execute(
                        "SELECT parts_schema, parts_complete FROM governed_document_versions WHERE version_id=%s",
                        (old["version_id"],),
                    )
                    extra = cur.fetchone() or {}
                    old.update(dict(extra))
                _dual.assert_version_approvable(cur, old)
            except Exception as exc:
                if getattr(exc, "code", None):
                    raise DocumentJourneyError(
                        str(getattr(exc, "code")),
                        str(getattr(exc, "message", "") or getattr(exc, "code")),
            ...
            # Wave 2A: keep onboarding_items in sync (accepted). Never leave status=received.
            try:
                import onboarding_lifecycle_wave2a as _lc

                if _lc.lifecycle_enabled(company_code=company, employee_key=employee_key):
                    _lc.ensure_lifecycle_schema(cur)
                    _lc.apply_hr_approve_to_checklist(
                        cur, employee_key=employee_key, document_type=ctype
                    )
                    # Also match original onboarding item_id when distinct from canonical type
                    if document_type and str(document_type) != ctype:
                        _lc.apply_hr_approve_to_checklist(
                            cur, employee_key=employee_key, document_type=str(document_type)
                        )
            except Exception:
                import logging

                logging.getLogger(__name__).exception(
                    "wave2a checklist sync failed on approve employee=%s type=%s",
                    employee_key,
                    ctype,
                )
        conn.commit()
    ...
    *,
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            # Wave 2A: HR rejection → replacement_required on the same checklist item.
            try:
                import onboarding_lifecycle_wave2a as _lc

                if _lc.lifecycle_enabled(company_code=company, employee_key=employee_key):
                    _lc.ensure_lifecycle_schema(cur)
                    _lc.apply_hr_reject_to_checklist(
                        cur,
                        employee_key=employee_key,
                        document_type=ctype,
                ...
                logging.getLogger(__name__).exception(
                    "wave2a checklist sync failed on reject employee=%s type=%s",
                    employee_key,
                    ctype,
                )
        conn.commit()
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            for crow in compliance_rows:
                ...
                # Strip raw model dumps; keep role-appropriate proposal fields for HR summary UI.
                ocr_safe = {k: v for k, v in ocr.items() if k != "raw"} if ocr else None
                verified_status = str((current or {}).get("review_status") or "")
                verified_block = None
                if verified_status in {STATUS_HR_REVIEWED, "accepted"} or display in {"valid", "hr_reviewed", STATUS_HR_REVIEWED}:
                    verified_block = {
                        "document_number": (current or crow).get("document_number"),
                        ...
                        "review_status": verified_status or display,
                        "source": "hr_reviewed_version",
                    }
                out.append(
                    {
                        "document_type": ctype,
                        ...
                        "expiry_date": str((current or crow).get("expiry_date") or "") or None,
                        "issue_date": str((current or crow).get("issue_date") or crow.get("issued_date") or "") or None,
                        "document_number": (pending or current or crow).get("document_number"),
                        ...
                        "ocr_proposal": ocr_safe,
                        "ocr_authoritative": False,
                        "verified_fields": verified_block,
                        "extraction_vs_verified": {
                            "extracted_are_proposals": True,
                            "never_auto_overwrite_employee_profile": True,
                            "hr_must_confirm_explicitly": True,
                        },
