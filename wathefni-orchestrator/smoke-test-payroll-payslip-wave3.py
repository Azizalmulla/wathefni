...
def main() -> int:
    ...
    # UX EN/AR presence (optional when dashboard sources not co-located)
    dash_candidates = [
    ...
    dash = next((p for p in dash_candidates if (p / "payrollPayslipUx.ts").exists()), None)
    if dash:
        ux = (dash / "payrollPayslipUx.ts").read_text(encoding="utf-8")
        ...
        print("      SKIP  ux files not co-located (covered by smoke-test-payroll-payslip-wave3-ux.py)")
