...
def main() -> int:
    ...
    company = "WATHEFNI"
    other = "OTHERCO"
    # Phone suffixes must be digits-only — ops digits() strips a-f from hex tags.
    suffix = f"{int(tag, 16) % 10000:04d}"
    emp_key = f"W3-SYNTH|{tag}"
    emp_phone = f"96552470{suffix}"
    mgr_phone = f"96552471{suffix}"
    mgr2_phone = f"96552472{suffix}"
    hr_phone = f"96552473{suffix}"
    out_mgr = f"96552474{suffix}"
    emp = {"employee_key": emp_key, "phone": emp_phone, "name": f"Wave3 Canary {tag}", "company_code": company}
    emp_other = {"employee_key": f"W3-SYNTH|OUT-{tag}", "phone": f"96552475{suffix}", "name": "Other", "company_code": other}
    ...
    configured = {mgr_phone, mgr2_phone, hr_phone, out_mgr}
    scope_map = {
        mgr_phone: {emp_key},
        mgr2_phone: {emp_key},
    ...
    auth_store = auth_pg.PostgresAuthorityStore(app.db_connect)
    authority = core.AttendanceAuthorityService(store=auth_store)
    ops_store = ops_pg.PostgresOpsStore(connect_fn=app.db_connect)
    ...
    try:
        ...
        miss_id = str(miss["exception_id"])
        assigned = svc.assign_exception(
            company_code=company, exception_id=miss_id, owner_phone=mgr_phone,
            actor_phone=hr_phone, expected_row_version=int(miss["row_version"]), actor_role="hr",
            priority="high",
        )
        check(
            "exception assigned",
            assigned.get("ok") is True and str(assigned.get("exception", {}).get("owner_phone") or "") == mgr_phone,
            assigned,
        )
        ...
        def _apply_same() -> None:
            try:
                ...
                same_key_results.append({"ok": False, "error": "thread_exception", "detail": str(exc)})
        ...
        check(
            ...
            len(same_key_results) == 2
            and all(r.get("ok") for r in same_key_results)
            and any(r.get("applied") or r.get("idempotent_replay") or r.get("already_applied") for r in same_key_results),
            same_key_results,
        )
