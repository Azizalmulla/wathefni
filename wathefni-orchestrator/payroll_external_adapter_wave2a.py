...
# Wave 2A-D — operability packaging only (no money/authority contract change).
PAYROLL_WAVE2AD_OPERABILITY_VERSION = "1.0.0"
...
def package_contents_payload() -> dict[str, Any]:
    """What the external input export package contains today (honesty for operators)."""
    return {
        "included": [
            "employee_key",
            "approved_compensation_components",
            "contract_amounts",
            "pay_period_dates",
        ],
        "not_included_yet": [
            "attendance_minutes_live",
            "leave_classifications_live",
            "shifts_live",
            "bank_files",
            "wps",
            "pifss_remittance",
        ...
        "export_csv_headers": list(EXPORT_CSV_HEADERS),
        "import_csv_headers": list(IMPORT_CSV_HEADERS),
...
def honesty_payload() -> dict[str, Any]:
    return {
        ...
        "payroll_wave2ad_operability_version": PAYROLL_WAVE2AD_OPERABILITY_VERSION,
        ...
        "wave2a_adapter_contracts_unchanged": True,
        "package_contents": package_contents_payload(),
...
def ensure_payroll_wave2a_schema(cur: Any, *, force: bool = False) -> None:
    ...
    try:
        ...
        # Wave 2A-D additive quarantine acknowledgement columns (soft only; no money change).
        for stmt in (
            "ALTER TABLE payroll_adapter_quarantine ADD COLUMN IF NOT EXISTS acknowledged_at timestamptz",
            "ALTER TABLE payroll_adapter_quarantine ADD COLUMN IF NOT EXISTS acknowledged_by_phone text",
            "ALTER TABLE payroll_adapter_quarantine ADD COLUMN IF NOT EXISTS acknowledgement_reason text",
        ):
            cur.execute(stmt)
        _SCHEMA_READY = True
    finally:
...
def list_quarantine(cur: Any, *, company_code: str, limit: int = 100) -> list[dict[str, Any]]:
    ...
    cur.execute(
        ...
        SELECT quarantine_id::text, source_kind, export_run_id::text, import_run_id::text,
               ...
               acknowledged_at, acknowledged_by_phone, acknowledgement_reason
...
def acknowledge_quarantine(
    ...
    *,
    ...
    """Soft-acknowledge a quarantine row. Does not admit money or mutate imports."""
    denied = require_audit_reason(reason)
    if denied:
        return denied
    ensure_payroll_wave2a_schema(cur)
    ...
    cur.execute(
        """
        SELECT * FROM payroll_adapter_quarantine
        WHERE company_code=%s AND quarantine_id=%s::uuid AND hard_deleted=false
        """,
        (company, quarantine_id),
    )
    row = _row(cur)
    if not row:
    ...
    if row.get("acknowledged_at"):
        return {
            "ok": True,
            "idempotent": True,
            "quarantine": _json_safe(row),
            "money_authority_unchanged": True,
            "authoritative_in_wathefni": False,
            **honesty_payload(),
        }
    cur.execute(
        """
        UPDATE payroll_adapter_quarantine
        SET acknowledged_at=now(),
            acknowledged_by_phone=%s,
            acknowledgement_reason=%s,
            payload = COALESCE(payload, '{}'::jsonb) || %s::jsonb
        WHERE company_code=%s AND quarantine_id=%s::uuid AND acknowledged_at IS NULL
        RETURNING *
        """,
        (
            digits_phone(actor_phone),
    ...
    updated = _row(cur)
    if not updated:
        return {"ok": False, "error": "quarantine_ack_race"}
    _event(
        cur,
        company_code=company,
        event_type="quarantine_acknowledged",
        export_run_id=updated.get("export_run_id"),
        import_run_id=updated.get("import_run_id"),
        payload={
            "quarantine_id": str(quarantine_id),
            "acknowledgement_reason": str(reason).strip(),
            "money_authority_unchanged": True,
        },
        actor_phone=actor_phone,
    )
...
def period_readiness(
    ...
    *,
        ...
        blockers.append(
            {
                ...
                "message": "Payroll mode is not set to External. Ask an admin to switch mode to External before this run.",
            }
        )
    ...
    if not p_start or not p_end:
        blockers.append({"code": "period_missing", "message": "Choose a pay period (start and end dates)."})
            ...
            blockers.append(
                {
                    ...
                    "message": "No approved compensation contracts cover this period. Approve contracts before exporting.",
                }
            )
    ...
    period_count = 0
    cur.execute(
        "SELECT COUNT(*) AS n FROM payroll_periods WHERE company_code=%s",
        (company,),
    )
    period_count = int((_row(cur) or {}).get("n") or 0)
    if p_start and p_end:
        cur.execute(
            """
                ...
                warnings.append(
                    {
                        ...
                        "message": "An input package already exists for this period. Creating another may replace it if people or contracts changed.",
                    }
                )
    ...
    setup = {
        "payroll_mode": mode,
        "mode_ok": mode in ("external", "parallel_shadow"),
        "payment_processing": "disabled",
        "payment_processing_ok": str(settings.get("payment_processing") or "") == "disabled",
        "approved_contract_count": contract_count,
        "contracts_ok": contract_count > 0,
        "period_count": period_count,
        "period_selected": bool(p_start and p_end),
        "period_start": p_start or None,
        "period_end": p_end or None,
    }
    return {
        "ok": True,
        "ready": ready,
...
def workspace_bootstrap(cur: Any, *, company_code: str) -> dict[str, Any]:
    ...
    open_quarantine = [q for q in quarantine if not q.get("acknowledged_at")]
    cur.execute(
        """
    ...
    mode = str(settings.get("payroll_mode") or "")
    # Setup status (read-only diagnosis for operators — no mode mutation here).
    cur.execute(
        """
    ...
    approved_contracts_total = int((_row(cur) or {}).get("n") or 0)
    setup = {
        "payroll_mode": mode,
        "mode_label": mode or "unset",
        "mode_ok": mode in ("external", "parallel_shadow"),
        "payment_processing": "disabled",
        "payment_processing_ok": True,
        "approved_contract_count": approved_contracts_total,
        "contracts_ok": approved_contracts_total > 0,
        "period_count": len(periods),
        "periods_ok": len(periods) > 0,
        "money_authority": "external",
        "vendor_claimed": False,
    }
    return {
        ...
        "counts": {
            "exports": len(exports),
            "imports": len(imports),
            "quarantine_open": len(open_quarantine),
