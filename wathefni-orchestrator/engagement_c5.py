...
def runtime_gate_for_company(company_code: str | None) -> dict[str, Any]:
    ...
    if company not in allow:
        try:
            ...
            entitled = _cr.engagement_runtime_allowlist_admits(company, allow)
        except Exception:
            entitled = False
        if not entitled:
            return {
...
def create_survey_template(
    ...
    _audit(
        ...
        entity_type="survey",
        entity_id=survey_id,
        detail={"code": code.strip()},
    )
    return {"ok": True, "survey": survey, "stable_id": survey_id, "recognition_absent": True}
...
def create_survey_version(
    ...
    *,
    ...
    _audit(
        ...
        entity_id=survey_version_id,
        detail={"survey_id": survey_id, "version_no": int(version_no), "privacy_mode": mode},
    )
    ...
    *,
    ...
    _audit(
        ...
        detail={
            ...
            "audience_count": len(snapshot),
            "privacy_mode": ver["privacy_mode"],
        },
    )
    return {"ok": True, "campaign": campaign, "audience_count": len(snapshot)}
...
def launch_campaign(cur: Any, *, company_code: str, actor_phone: str, campaign_id: str) -> dict[str, Any]:
    ...
    _audit(
        ...
        entity_id=campaign_id,
        detail={
            "survey_version_id": str(camp["survey_version_id"]),
...
def compute_enps(cur: Any, *, company_code: str, campaign_id: str, question_id: str) -> dict[str, Any]:
    ...
    if scale_min is None or scale_max is None or int(scale_min) != 0 or int(scale_max) != 10:
        return {"ok": False, "error": "enps_requires_0_to_10_scale"}
    cur.execute(
        """
...
def create_action_plan(
    ...
    *,
    ...
    _audit(
        ...
        entity_id=plan_id,
        detail={"campaign_id": campaign_id, "open_items": len(items)},
    )
    return {
        "ok": True,
        "action_plan": plan,
