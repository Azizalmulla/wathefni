...
def main() -> int:
    ...
    check("hub pending→pending_start", w3.hub_status_for_lifecycle("pending_start") == "pending_start")
    check("pending_start is not active hub", w3.is_active_hub_status("pending_start") is False)
    check("active hub is active", w3.is_active_hub_status("active") is True)
