#!/usr/bin/env python3
"""Payroll Wave 2A-D-B — production synthetic External Run Operability canary.

Synthetic subjects only (PYW2ADB / 965540*).
Proves Wave 2A-C flow plus Wave 2A-D operability:
  setup status, package-content honesty, quarantine acknowledgement,
  rollback concurrency token, EN/AR route surface, residual 0.

Does NOT: real vendor, bank/WPS/PIFSS/EOS/payments, attendance package expansion, AI.
...
os.environ.setdefault(
    ...
    "PYW2ADB,PYW2ADB-SYNTH|,PYW2ACB,PYW2ACB-SYNTH|,PYW2AB,PYW2AB-SYNTH|,PYW2A,PYW2A-SYNTH|,PYW1,PYW1-SYNTH|",
)
...
COMPANY = "WATHEFNI"
TAG = os.environ.get("PYW2ADB_TAG") or uuid.uuid4().hex[:8]
TAG_DIGITS = ("".join(ch for ch in TAG if ch.isdigit()) + "00000")[:5]
EMP_KEY = f"WATHEFNI-PYW1-W2ADB-{TAG}"
REAL_KEY = "WATHEFNI-96566363363"
CREATOR = f"9655405{TAG_DIGITS}"
APPROVER = f"9655406{TAG_DIGITS}"

PASS = FAIL = 0
RESULTS: list[dict[str, Any]] = []
EVID = Path(os.environ.get("PYW2ADB_EVID") or f"/tmp/payroll-w2adb-{TAG}")
...
def main() -> int:
    print(f"payroll wave2adb prod synthetic canary tag={TAG}")
    ...
    check("wave2a adapter contracts unchanged", honesty.get("wave2a_adapter_contracts_unchanged") is True)
    check("operability version", bool(honesty.get("payroll_wave2ad_operability_version")))
    ...
    check("attendance not packaged", pkg.get("attendance_leave_shifts_packaged") is False)
    ...
    check("route quarantine acknowledge", any(
        isinstance(p, str) and "quarantine" in p and p.endswith("/acknowledge") for p in paths
    ))
    ...
    check("rollback requires concurrency helper", callable(pyw1.require_concurrency))
    miss = pyw1.require_concurrency(expected_row_version=None, actual_row_version=1)
    check("rollback concurrency required", isinstance(miss, dict) and miss.get("error") == "concurrency_token_required")

    # UI operability surface (src on prod host or local checkout)
    ui_candidates = [
        Path("/opt/wathefni/apps/wathefni-dashboard/src/posthire"),
        ROOT.parent / "apps" / "wathefni-dashboard" / "src" / "posthire",
    ]
    posthire = next((p for p in ui_candidates if p.is_dir()), None)
    if posthire:
        ws = (posthire / "ExternalPayrollWorkspace.tsx").read_text(encoding="utf-8", errors="ignore")
        ux = (posthire / "payrollExternalUx.ts").read_text(encoding="utf-8", errors="ignore")
        payslip = (posthire / "PayslipWorkspace.tsx").read_text(encoding="utf-8", errors="ignore")
        close = (posthire / "CloseExportWorkspace.tsx").read_text(encoding="utf-8", errors="ignore")
        post = (posthire / "PostHire.tsx").read_text(encoding="utf-8", errors="ignore")
        check("UI setup panel", "external-payroll-setup" in ws)
        check("UI checklist", "external-payroll-checklist" in ws)
        check("UI package honesty", "external-payroll-package" in ws)
        check("UI csv guide", "external-payroll-csv-help" in ws)
        check("UI rollback concurrency", "expected_row_version" in ws)
        check("UI EN who pays", "Who pays: external payroll system" in ux)
        check("UI AR title", "تشغيل الرواتب الخارجية" in ux)
        check("UI hours review tab", "Hours review" in ux and "مراجعة الساعات" in ux)
        check("UI payslip picker", "payslip-import-run-picker" in payslip)
        check("UI close picker", "close-import-run-picker" in close)
        check("UI tab separation", "tabTimesheetsHint" in post or "payroll-tab-hours-review" in post)
        check("UI rtl", "dir={isAr" in ws)
        check("UI mobile wrap", "flex-wrap" in ws)
    else:
        check("UI sources present for operability checks", False, ui_candidates)
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            pyw1.set_payroll_mode(
                cur, company_code=COMPANY, mode="external", actor_phone=APPROVER, reason=f"w2adb_mode_{TAG}"
            )
            ...
            check("setup status present", isinstance(boot.get("setup"), dict), boot.get("setup"))
            check("setup payment ok flag", (boot.get("setup") or {}).get("payment_processing_ok") is True)
            check("package contents present", isinstance(boot.get("package_contents"), dict))
            check(
                "readiness has setup",
                isinstance(ready.get("setup"), dict) and ready.get("package_contents"),
                ready,
            )
            ...
            result_csv = w2a.build_synthetic_result_csv(
                export_payload=payload, external_run_id=f"EXT-W2ADB-{TAG}"
            )
            ...
            replay = w2a.import_external_results(
                ...
                csv_text=result_csv,
                actor_phone=APPROVER,
                reason=f"w2adb_import_replay_{TAG}",
                expected_input_fingerprint=fp,
            )
            ...
            bad = w2a.import_external_results(
                ...
                csv_text="not,valid\n1,2",
                actor_phone=APPROVER,
                reason=f"w2adb_malformed_{TAG}",
            )
            check("malformed quarantined", bad.get("ok") is False and bool(bad.get("quarantine")), bad)
            qid = str(((bad.get("quarantine") or {}).get("quarantine_id")) or "")
            if qid:
                ...
                ack = w2a.acknowledge_quarantine(
                    ...
                    quarantine_id=qid,
                    actor_phone=APPROVER,
                    reason=f"w2adb_ack_{TAG}",
                )
                check("quarantine acknowledge ok", ack.get("ok") is True, ack)
                ...
                ack2 = w2a.acknowledge_quarantine(
                    ...
                    quarantine_id=qid,
                    actor_phone=APPROVER,
                    reason=f"w2adb_ack_again_{TAG}",
                )
                check("quarantine ack idempotent", ack2.get("idempotent") is True, ack2)
            ...
            replace_csv = w2a.build_synthetic_result_csv(
                export_payload=payload, external_run_id=f"EXT-W2ADB-RPL-{TAG}"
            )
            ...
            check(
                "quarantine ack in audit",
                any(e.get("event_type") == "quarantine_acknowledged" for e in w2a.list_events(cur, company_code=COMPANY, limit=120)),
            )

            # Rollback concurrency: missing token fails; valid token works
            run_row = w2a.get_export_run(cur, company_code=COMPANY, export_run_id=eid) or {}
            rv = int(run_row.get("row_version") or 0)
            no_tok = w2a.rollback_export_run(
                cur,
                company_code=COMPANY,
                export_run_id=eid,
                actor_phone=APPROVER,
                reason=f"w2adb_rb_notoken_{TAG}",
                expected_row_version=None,
            )
            check("rollback without token denied", no_tok.get("error") == "concurrency_token_required", no_tok)
            rolled = w2a.rollback_export_run(
                cur,
                company_code=COMPANY,
                export_run_id=eid,
                actor_phone=APPROVER,
                reason=f"w2adb_rb_{TAG}",
                expected_row_version=rv,
            )
            check("rollback with concurrency ok", rolled.get("ok") is True, rolled)
            check("rollback money still external", (rolled.get("export_run") or {}).get("money_authority") == "external" or rolled.get("money_authority") == "external" or honesty.get("money_authority") == "external")

            # Native mode readiness blocker then restore
            pyw1.set_payroll_mode(
                cur, company_code=COMPANY, mode="native", actor_phone=APPROVER, reason=f"w2adb_native_{TAG}"
            )
            ...
            pyw1.set_payroll_mode(
                cur, company_code=COMPANY, mode="external", actor_phone=APPROVER, reason=f"w2adb_restore_{TAG}"
            )
            ...
            qual = {
                "wave": "payroll_wave2adb_prod_synthetic",
                "tag": TAG,
                "passed": PASS,
                "failed": FAIL,
