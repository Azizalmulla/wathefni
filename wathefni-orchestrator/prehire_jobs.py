...
-- Normalize legacy active/published to open, inactive/blank unknown to closed.
UPDATE positions SET status = 'open' WHERE LOWER(COALESCE(status, '')) IN ('', 'active', 'published');
UPDATE positions SET status = 'closed' WHERE LOWER(COALESCE(status, '')) IN ('inactive');
...
def normalize_status(value: str | None) -> str:
    """Canonical job status via shared Jobs status-alias contract."""
    from jobs_queue_contract import normalize_job_status
...
def serialize_job(row: dict[str, Any], *, vacancy: dict[str, Any] | None = None, include_salary: bool = True) -> dict[str, Any]:
    ...
    out = {
        ...
        "hiring_manager_name": row.get("hiring_manager_name") or None,
        "recruiter_ownership_state": "assigned" if row.get("recruiter_user_id") else "unassigned",
        "hiring_manager_ownership_state": "assigned" if row.get("hiring_manager_user_id") else "unassigned",
        "ownership_unassigned": not bool(row.get("recruiter_user_id")),
        "status": status,
...
def create_job(
    *,
    ...
    recruiter_user_id, recruiter_reason = _own.resolve_job_recruiter_on_create(
        actor_user_id=actor_user_id,
        actor_role=actor_role,
        payload=payload,
    )
    hiring_manager_user_id = _own.resolve_job_hiring_manager_on_write(payload=payload, updating=False)
    with db_connect() as conn:
        with conn.cursor() as cur:
            import prehire_team_directory as _td

            for uid, purpose in (
                (recruiter_user_id, _td.PURPOSE_RECRUITER),
                (hiring_manager_user_id, _td.PURPOSE_HIRING_MANAGER),
            ):
                if not uid:
                    continue
                try:
                    _td.validate_active_assignee(cur, company=company, user_id=uid, purpose=purpose)
                except ValueError as exc:
                    raise JobsError(
                        str(exc),
                        "Choose an eligible active team member for this ownership field.",
                        http_status=422,
                        details={"field": purpose, "user_id": uid},
                    ) from exc
            cur.execute(
            ...
            if status == "open":
                ...
                try:
                    ...
                    gate = _requisitions.assert_job_publish_allowed(
                        cur,
                        company_code=company,
                        position_code=position_code,
                        requisition_id=str(payload.get("requisition_id") or "") or None,
                    )
                    if not gate.get("allowed"):
                        raise JobsError(
                            str(gate.get("error") or "requisition_required"),
                            str(
                                gate.get("message")
                                or "An approved requisition is required before publishing this job."
                            ),
                            http_status=422,
                            details={"requisition_gate": gate},
                        )
                except JobsError:
                    raise
                except Exception:
            ...
            if recruiter_user_id:
                _own.record_position_ownership_event(
                    cur,
                    company_code=company,
                    position_code=position_code,
                    event_type="recruiter_assigned" if recruiter_reason != "auto_recruiter" else "created",
                    field="recruiter",
                    from_user_id=None,
                    to_user_id=recruiter_user_id,
                    actor_user_id=actor_user_id,
                    reason=recruiter_reason,
                    metadata={"source": "create_job"},
                )
            else:
                _own.record_position_ownership_event(
                    cur,
            ...
            if hiring_manager_user_id:
                _own.record_position_ownership_event(
                    ...
                    from_user_id=None,
                    to_user_id=hiring_manager_user_id,
                    actor_user_id=actor_user_id,
                    reason="explicit",
                    metadata={"source": "create_job"},
                )
    ...
    *,
    ...
    with db_connect() as conn:
        with conn.cursor() as cur:
            ...
            prev_recruiter = str(current.get("recruiter_user_id") or "") or None
            prev_hm = str(current.get("hiring_manager_user_id") or "") or None
            if "recruiter_user_id" in payload:
                provided = payload.get("recruiter_user_id")
                next_recruiter = str(provided).strip() or None if provided is not None else None
                if next_recruiter == "":
                    next_recruiter = None
            else:
                next_recruiter = prev_recruiter
            next_hm = _own.resolve_job_hiring_manager_on_write(
                payload=payload,
                current=prev_hm,
                updating=True,
            )
            ...
            for uid, purpose in (
                (next_recruiter, _td.PURPOSE_RECRUITER),
                (next_hm, _td.PURPOSE_HIRING_MANAGER),
            ):
                if not uid:
                    continue
                try:
            ...
            recruiter_evt = _own.recruiter_event_type(previous=prev_recruiter, new=next_recruiter)
            if recruiter_evt:
                _own.record_position_ownership_event(
                    cur,
                    company_code=company,
                    position_code=code,
                    event_type=recruiter_evt,
                    field="recruiter",
                    from_user_id=prev_recruiter,
                    to_user_id=next_recruiter,
                    actor_user_id=actor_user_id,
                    reason="explicit_update",
                    metadata={"source": "update_job", "actor_role": actor_role},
                )
            hm_evt = _own.hm_event_type(previous=prev_hm, new=next_hm)
            if hm_evt:
                _own.record_position_ownership_event(
                    cur,
                    company_code=company,
                    ...
                    event_type=hm_evt,
                    field="hiring_manager",
                    from_user_id=prev_hm,
                    to_user_id=next_hm,
                    actor_user_id=actor_user_id,
                    reason="explicit_update",
                    metadata={"source": "update_job", "actor_role": actor_role},
                )
            # When recruiter is set/changed, inherit onto still-unassigned applications for this job.
            if next_recruiter and next_recruiter != prev_recruiter:
                cur.execute(
                    """
                    SELECT app_key FROM applications
                    WHERE company_code=%s AND position_code=%s AND owner_user_id IS NULL
                    LIMIT 500
                    """,
                    (company, code),
                )
                for app_row in cur.fetchall() or []:
                    _own.apply_application_owner_inherit(
                        cur,
                        company_code=company,
                        app_key=str(app_row.get("app_key") or ""),
                        position_code=code,
                        actor_user_id=actor_user_id,
                        reason="inherit_after_job_recruiter_reassignment",
                    )
        conn.commit()
    ...
    with db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if action == "publish" or action == "reopen" or action == "resume":
                ...
                # OPTIONAL_INTEGRATION: approved requisition gate (company-scoped; dark by default).
                try:
                    import requisitions as _requisitions
                    ...
                    gate = _requisitions.assert_job_publish_allowed(
                        ...
                        company_code=company,
                        position_code=code,
                        requisition_id=str(current.get("requisition_id") or "") or None,
                    )
                    if not gate.get("allowed"):
                        raise JobsError(
                            str(gate.get("error") or "requisition_required"),
                    ...
                    # Gate module unavailable → do not block legacy publish paths.
                    pass
                if not published_at:
...
def _assert_fresh(row: dict[str, Any], *, expected_updated_at: str | None, expected_version: int | None) -> None:
    ...
    try:
        _cs.assert_fresh(
            ...
            last_changed_by={
                "user_id": row.get("updated_by_user_id") or row.get("recruiter_user_id"),
                "name": row.get("updated_by_name"),
            },
            entity_type="job",
            entity_id=str(row.get("position_code") or ""),
            code_version="stale_job_version",
            code_updated_at="stale_job_update",
            require_token=True,
        )
    except _cs.ConcurrencyError as exc:
        raise JobsError(exc.code, exc.message, http_status=exc.http_status, details=exc.as_detail()) from exc
...
def get_job(*, company: str, position_code: str, db_connect: Callable[[], Any]) -> dict[str, Any]:
    ...
    with db_connect() as conn:
        with conn.cursor() as cur:
            ...
            names = _td.ownership_display_names(
                cur,
                company=company,
                user_ids=[data.get("recruiter_user_id"), data.get("hiring_manager_user_id")],
            )
            rid = str(data.get("recruiter_user_id") or "")
            hid = str(data.get("hiring_manager_user_id") or "")
            data["recruiter_name"] = names.get(rid)
            data["hiring_manager_name"] = names.get(hid)
