...
SCHEMA_VERSION = "employees360-wave2-authority-v1"

def authority_v2_enabled(company_code: str | None = None) -> bool:
    """Global flag + tenant allowlist. Default allowlist is WATHEFNI only."""
    raw = str(os.environ.get("WATHEFNI_EMPLOYEE_AUTHORITY_V2") or "").strip().lower()
    if raw not in {"1", "true", "yes", "on"}:
        return False
    allow = str(os.environ.get("WATHEFNI_EMPLOYEE_AUTHORITY_V2_COMPANIES") or "WATHEFNI")
...
def ensure_authority_schema(cur) -> None:
    ...
    # Lightweight schema marker for ops evidence (idempotent).
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS employee_authority_schema_meta (
    ...
    cur.execute(
        """
        INSERT INTO employee_authority_schema_meta (schema_name, schema_version)
        VALUES ('employees360_wave2', %s)
        ON CONFLICT (schema_name) DO UPDATE SET schema_version=EXCLUDED.schema_version, applied_at=now()
        """,
        (SCHEMA_VERSION,),
    )
...
def sync_authority_from_hub_employee(legacy: Any, employee: dict[str, Any], *, hire_source: str = "hub_sync") -> dict[str, Any] | None:
    """Best-effort shadow sync used by create/update/hire when V2 enabled for tenant."""
    if not authority_v2_enabled(employee.get("company_code")):
        return None
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ensure_authority_schema(cur)
