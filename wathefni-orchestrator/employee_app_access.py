...
def employment_eligible(employee: dict[str, Any] | None) -> bool:
    """Normal ESS eligibility — pending_start / joining is not employment-eligible (P5)."""
    if not employee:
        return False
    status = str(employee.get("employment_status") or "active").strip().lower()
    if status in {"pending_start", "joining", "future_start", "left", "terminated"}:
        return False
...
def is_employee_app_access_eligible(
    ...
    *,
    ...
    if employment_eligible(employee):
        if bool((employee or {}).get("app_access_enabled")):
            return True, "employee_enabled"
        return False, "employee_access_disabled"
    # P5 — pending_start joiners: preboarding-scoped access only when invited + module on.
    status = str((employee or {}).get("employment_status") or "").strip().lower()
    if status in {"pending_start", "joining", "future_start"}:
        try:
            preboarding_on = bool(legacy.company_has_module(company, "preboarding"))
        except Exception:
            preboarding_on = False
        if preboarding_on and bool((employee or {}).get("app_access_enabled")):
            return True, "employee_preboarding_only"
