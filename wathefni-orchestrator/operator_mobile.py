...
# Client SecureStore contract (documented; client not built in HR-1).
SECURE_STORE_CONTRACT = {
    "access_token_key": "wathefni.hr.access_token",
    "refresh_token_key": "wathefni.hr.refresh_token",
    ...
    "rules": [
        ...
        "On 401 session_expired or session_revoked clear SecureStore and return to login.",
...
def build_hr_workspace_capabilities(app_mod: Any, context: dict[str, Any]) -> dict[str, Any]:
    ...
    if role == "manager" and scope_blocked:
        ...
        keys = [
            "hr_tasks", "leave_approvals", "onboarding_review", "document_review",
            "attendance_exceptions", "today_shifts", "shift_swap_decisions",
            "employee_search", "employee_quick_profile", "delivery_alerts",
        ]
    ...
    return {
        "hr_tasks": _feature(
            enabled=any_posthire and _has(perms, "leave.read", "attendance.read", "onboarding.read", "compliance.read", "shifts.read", "payroll.read"),
            actions=["read"] if any_posthire and _has(perms, "leave.read", "attendance.read", "onboarding.read", "compliance.read", "shifts.read", "payroll.read") else [],
        ...
        "delivery_alerts": _feature(
            enabled=any_posthire and _has(perms, "leave.read", "attendance.read", "onboarding.read", "compliance.read", "shifts.read"),
            actions=["read"] if any_posthire and _has(perms, "leave.read", "attendance.read", "onboarding.read", "compliance.read", "shifts.read") else [],
            reason="module_disabled" if not any_posthire else "action_forbidden",
        ),
    }
...
def build_recruiting_workspace_capabilities(app_mod: Any, context: dict[str, Any]) -> dict[str, Any]:
    ...
    can_interview = prehire and "interview.manage" in perms
