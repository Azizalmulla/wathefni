...
# People-surface modules unlock Employees 360 (Employees is not a purchasable SKU).
PEOPLE_SURFACE_MODULES = frozenset(
    {"onboarding", "compliance", "attendance", "shifts", "leave", "payroll"}
)
POSTHIRE_INBOX_SOURCE_MODULES = frozenset(
    {"analytics", "compliance", "onboarding", "attendance", "leave", "shifts", "payroll"}
)


def _module_on(enabled: set[str], module: str) -> bool:
    """True when the capability's module entitlement is on for this tenant.

    Synthetic keys:
      - employees → any people-surface module
      - action_inbox → any inbox-composable post-hire module
      - setup_console → settings surface (permission-gated separately)
    Pre-Hiring is NOT always enabled.
    """
    if module == "employees":
        return bool(enabled & PEOPLE_SURFACE_MODULES)
    if module == "action_inbox":
        return bool(enabled & POSTHIRE_INBOX_SOURCE_MODULES)
    if module == "setup_console":
        return True
    return module in enabled


def _email_env_transport_configured() -> bool:
    """Kept for tests/back-compat; canonical probe lives in assistant_channel_readiness."""
    import assistant_channel_readiness as acr

    return bool(acr._email_env_transport_configured())


def _email_configured(legacy: Any, company_code: str | None = None) -> bool:
    """Delegate to canonical assistant_channel_readiness (Setup → catalog → execute)."""
    import assistant_channel_readiness as acr

    return bool(acr.email_ready(legacy, company_code))


def _whatsapp_configured(legacy: Any, company_code: str) -> bool:
    import assistant_channel_readiness as acr

    return bool(acr.whatsapp_candidate_ready(legacy, company_code))
...
def build_assistant_capability_catalog(
    *,
    ...
    try:
        ...
        push_ok = bool(acr.push_ready(legacy, company))
        push_inbox_ok = bool(acr.in_app_inbox_ready(legacy, company))
    except Exception:
        push_ok = False
        push_inbox_ok = False
    ...
    def from_tool(
        ...
        *,
        ...
        if not _module_on(enabled, module):
    ...
    capabilities: dict[str, dict[str, Any]] = {
        ...
        "assessments": from_tool("list_assessment_attempts", module="assessments", permission="assessment.manage")
        if "list_assessment_attempts" in tools
        else from_tool("send_assessment", module="assessments", permission="assessment.manage"),
        ...
        "interviews_read": from_tool("list_live_interviews", module="interviews", permission="interview.manage"),
        ...
        "video_interviews": from_tool(
            "list_video_interviews",
            module="video_interviews",
            permission="interview.manage",
        )
        if "list_video_interviews" in tools
        else from_tool("send_video_interview", module="video_interviews", permission="interview.manage"),
        "employment_offers": from_tool(
        ...
        "employee_app": entry(
            status=(
                STATUS_AVAILABLE
                if (_module_on(enabled, "employee_app") and push_inbox_ok)
                else (
                    STATUS_MODULE_OFF
                    if not _module_on(enabled, "employee_app")
                    else STATUS_NOT_CONFIGURED
                )
            ),
            module="employee_app",
            tools_needed=[],
            provider="employee_app",
            note=(
                "Employee App / in-app inbox awareness only — Assistant does not send push directly; "
                "employee notifications use the outbound delivery ladder when flows are enabled."
                if _module_on(enabled, "employee_app")
                else None
            ),
        ),
        "push_notifications": entry(
            status=(
                STATUS_AVAILABLE
                if push_ok
                else (
            ...
            tools_needed=[],
            provider="push",
            note="Push is not an Assistant send tool — refuse SMS/Telegram/Teams-chat; push rides employee outbound ladder only.",
        ),
        ...
        "posthire_employees_360": from_tool(
            "summarize_employee_360", module="employees", permission="employees.read"
        ),
        "posthire_setup_readiness": from_tool(
            "get_launch_readiness_summary", module="setup_console", permission="settings.read"
        ),
        ...
        "posthire_performance": from_tool("get_okr_alignment", module="performance", permission="performance.read"),
        "posthire_talent": from_tool("explain_talent_classification", module="talent", permission="talent.read"),
    }
    ...
    return {
        ...
        "providers": {
            ...
            "push": push_ok,
            "in_app_inbox": push_inbox_ok,
        },
        "unsupported_messaging": ["sms", "telegram", "teams_chat"],
...
def capability_prompt_block(catalog: dict[str, Any] | None) -> str:
    ...
    if not isinstance(catalog, dict):
        return acr.unsupported_channels_prompt_block()
    ...
    lines.append(acr.unsupported_channels_prompt_block())
    return "\n".join(lines)
