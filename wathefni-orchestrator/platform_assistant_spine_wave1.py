...
def wave1_enabled_for_company(company_code: str | None) -> bool:
    """Offer Wave1 spine tools when the wave flag is on and company is in the
    env allowlist. Setup + env list drive offerability — no hard single-tenant
    WATHEFNI-only equality (global kill switches still apply).
    """
    ...
    if not company:
        return False
    return company in wave1_companies()
