...
def main() -> int:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # Retries without idempotency key must not create a second open task
            dedupe = wts.create_workflow_task(
                cur,
                company_code=company,
                task_type="preboard_item",
                title="Canary preboard item retry",
                subject_id=f"WFT-SYNTH|{SUFFIX}|item",
            )
            check(
                "one open task per subject/action",
                dedupe.get("ok") is True
                and dedupe.get("deduped_open") is True
                and dedupe["task"]["task_id"] == created["task"]["task_id"],
                dedupe,
            )
            missing_subject = wts.create_workflow_task(
                cur,
                company_code=company,
                task_type="sla_breach",
                title="missing subject",
            )
            check("subject required fail-closed", missing_subject.get("error") == "subject_required", missing_subject)

            from_appr = wts.create_task_from_approval(
                cur,
                company_code=company,
                task_type="requisition_approval",
                title="Approve req",
                approval_instance_id=f"inst-{SUFFIX}",
                subject_type="requisition",
                subject_id=f"WFT-SYNTH|{SUFFIX}|req",
            )
            check(
                "approval→task linkage preserved",
                from_appr.get("ok") is True
                and from_appr["task"]["metadata"].get("approval_instance_id") == f"inst-{SUFFIX}"
                and from_appr["task"]["metadata"].get("origin") == "workflow_approvals",
                from_appr,
            )
            task = created["task"]
            ...
            check("resolve does not mutate subject", resolved.get("mutates_subject") is False, resolved)
            ...
            tick_b2 = wts.tick_sla_clock(
                cur, company_code=company, clock_id=clock["clock_id"], now=past + timedelta(hours=1)
            )
            check(
                "repeated breach evaluation idempotent",
                tick_b2.get("ok") is True
                and tick_b2.get("replayed") is True
                and tick_b2["clock"].get("breach_task_id") == tick_b["clock"].get("breach_task_id"),
                tick_b2,
            )
            cur.execute(
                """
                SELECT count(*) AS c FROM hr_tasks
                 WHERE company_code=%s AND task_type='sla_breach'
                   AND subject_id=%s AND metadata->>'sla_clock_id'=%s
                """,
                (company, f"WFT-SYNTH|{SUFFIX}|subj", clock["clock_id"]),
            )
            breach_count = int(dict(cur.fetchone())["c"])
            check("no duplicate sla_breach tasks", breach_count == 1, breach_count)

            post = wts.satisfy_sla_clock(
                cur,
                company_code=company,
                clock_id=clock["clock_id"],
                expected_row_version=tick_b["clock"]["row_version"],
            )
            check(
                "satisfy after breach deterministic",
                post.get("ok") is True
                and post.get("from_status") == "breached"
                and post["clock"]["status"] == "satisfied",
                post,
            )
