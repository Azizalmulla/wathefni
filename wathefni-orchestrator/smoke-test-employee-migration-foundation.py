...
def main() -> None:
    ...
    if not honesty.get("mapping_profiles") or "canonical" not in (honesty.get("field_layers") or []):
        _fail(f"P2 field model honesty incomplete: {honesty}")
    if not honesty.get("onboarding_migration_p3"):
        _fail(f"P3 onboarding honesty incomplete: {honesty}")
    try:
