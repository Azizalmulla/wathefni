...
def main() -> None:
    ...
    check("hr main hides dormant optional", not lc._hr_main_visible(dormant_optional))
    check("hr available tasks has dormant", lc._hr_available_task(dormant_optional))
    check(
        "hr main shows required pending",
        lc._hr_main_visible({**doc, "status": "pending", "required": True}),
    )
    hr_proj = lc.build_hr_web_projection(
        [
            {**doc, "status": "accepted", "file_id": "c1", "required": True},
            dormant_optional,
            {**bank, "status": "pending"},
        ],
        company_code="WATHEFNI",
        employee_key="WATHEFNI-96599338566",
        file_index={"civil_id": "c1"},
        versions_by_type={},
        can_mutate=True,
        can_upload=True,
        lifecycle_on=True,
    )
    check("hr projection has available_tasks", hr_proj.get("available_tasks_count", 0) >= 1)
    check("hr projection hides dormant from items", all(i.get("item_id") != "passport_copy" for i in hr_proj.get("items") or []))
    check("hr actions include mark_complete", "mark_complete" in lc.actions_for_hr_item(doc, can_mutate=True, can_upload=True, lifecycle_on=True))
