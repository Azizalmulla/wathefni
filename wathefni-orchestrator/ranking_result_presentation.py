...
def _display_missing_codes(item: dict[str, Any], codes: list[str] | None = None) -> list[str]:
    ...
    # Also fold required_missing (includes historical platform reader failures).
    for extra in list(item.get("required_missing") or []):
        text = str(extra or "").strip()
        if text and text not in raw:
            raw.append(text)

    def _is_platform_noise(code: str) -> bool:
        lowered = str(code or "").strip().lower()
        return (
            lowered.startswith("ck_ranking_reader_failed")
            or "ck_ranking_reader_failed" in lowered
            or "undefinedfunction" in lowered
        )

    raw = [code for code in raw if not _is_platform_noise(code)]
...
def _numeric_score_presentation(
    ...
    *,
    ...
    required_missing = {
        code
        for code in required_missing
        if not code.startswith("ck_ranking_reader_failed")
        and "ck_ranking_reader_failed" not in code
        and "undefinedfunction" not in code
    }
...
def build_ranking_decision(
    ...
    *,
    ...
    """Single backend decision contract for Ranking HR surfaces."""
    lang = normalize_locale(locale)
    run_data = run if isinstance(run, dict) else {}
    score = presentation.get("score") if isinstance(presentation.get("score"), dict) else {}
    state = presentation.get("state") if isinstance(presentation.get("state"), dict) else {}
    fit = presentation.get("fit") if isinstance(presentation.get("fit"), dict) else {}
    components_raw = item.get("component_scores") if isinstance(item.get("component_scores"), dict) else {}
    ...
    for key in (
        ...
        if key not in components_raw:
            components[key] = None
            continue
        try:
            components[key] = float(components_raw.get(key))
        except (TypeError, ValueError):
            components[key] = None
    strengths = []
    for entry in list(presentation.get("strengths") or [])[:4]:
        if isinstance(entry, dict):
            label = str(entry.get("label") or "").strip()
            value = str(entry.get("value") or "").strip()
            strengths.append(f"{label}: {value}" if label and value else (label or value))
        elif entry:
            strengths.append(str(entry))
    gaps = [str(x) for x in list(presentation.get("gaps") or presentation.get("missing") or [])[:5] if str(x).strip()]
    evidence_refs = []
    for entry in list(presentation.get("evidence_highlights") or presentation.get("evidence_sections") or [])[:5]:
        if isinstance(entry, dict):
            label = str(entry.get("label") or "").strip()
            if label:
    ...
    if stale:
        freshness = "stale"
    elif run_data.get("run_id") or item.get("run_id"):
        freshness = "current"
    else:
        freshness = "missing"
    show_numeric = bool(score.get("show_numeric"))
    advisory = score.get("value") if show_numeric else None
    ranking_status = str(presentation.get("card_mode") or state.get("code") or "unavailable")
    return {
        "version": "ranking_decision_v1",
        "locale": lang,
        "ranking_status": ranking_status,
        "eligibility": {
            "bucket": item.get("eligibility_bucket") or state.get("code"),
            "label": state.get("label"),
            "fit_code": fit.get("code"),
        ...
        "fit_summary": presentation.get("verdict") or presentation.get("explanation"),
        "strengths": strengths,
        "gaps": gaps,
        ...
        "application_stage": (item.get("display") or {}).get("status") if isinstance(item.get("display"), dict) else item.get("status"),
        "job": {
            "position_code": item.get("position_code") or run_data.get("position_code"),
            "position_title": presentation.get("job_title") or item.get("position_title") or run_data.get("position_title"),
        },
        "candidate_name": presentation.get("candidate_name") or item.get("name"),
    }
...
def attach_presentations(
    ...
    *,
    ...
    for candidate in list(result.get("candidates") or []):
        ...
        candidate["ranking_decision"] = build_ranking_decision(
            source,
            presentation=candidate["presentation"],
            run=result,
            locale=lang,
        )
        # Keep legacy score field null when presentation hides numeric score.
        score_block = candidate["presentation"].get("score") if isinstance(candidate["presentation"], dict) else {}
        if isinstance(score_block, dict) and not score_block.get("show_numeric"):
            candidate["score"] = None
            if isinstance(candidate.get("gpt_evaluation"), dict) and not score_block.get("show_numeric"):
                # Do not keep a positive narrative beside an unavailable score.
                if candidate["gpt_evaluation"].get("fit_summary") and score_block.get("reason") in {
                    "required_evidence_incomplete",
                    "score_unavailable",
                    "thin_evidence_not_meaningful",
                    "job_criteria_not_configured",
                    "stale_result",
                }:
                    candidate["gpt_evaluation"]["fit_summary"] = candidate["presentation"].get("explanation") or score_block.get("label")
    presentations = [
        c.get("presentation")
