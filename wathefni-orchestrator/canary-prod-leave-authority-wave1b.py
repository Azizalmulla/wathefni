    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if leave_ids:
                ...
                cur.execute("DELETE FROM leave_ledger WHERE leave_id = ANY(%s::uuid[])", (leave_ids,))
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            if label == "terminated":
                ...
                cur.execute(
                    "UPDATE employees SET employment_status='active', start_date=%s, updated_at=now() WHERE employee_key=%s",
                    (date.today() - timedelta(days=90), emp_key),
                )
            life_map = {
...
def main() -> int:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE employees SET employment_status='active', start_date=%s, updated_at=now() WHERE employee_key=%s",
                (today - timedelta(days=90), key_b),
            )
            try:
                cur.execute(
                    """
                    UPDATE employee_employments
                    SET lifecycle_state='active', start_date=%s, updated_at=now()
                    WHERE company_code=%s AND legacy_employee_key=%s
                    """,
                    (today - timedelta(days=90), COMPANY, key_b),
                )
            except Exception:
                pass
        conn.commit()
