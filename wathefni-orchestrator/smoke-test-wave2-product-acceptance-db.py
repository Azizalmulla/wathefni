...
def _connect():
    ...
    if not url:
        # Prefer direct DSN from staging env so cursor_factory is guaranteed RealDict.
...
def prove_matrix_cell(cur, cfg: dict) -> None:
    ...
    # Use enable() return (not re-SELECT) so tuple cursors cannot break frozen C4 helpers.
    pay_entitled = False
    pf_entitled = False
    set_entitled = False
    if expect_pay:
        try:
            ...
            pay_entitled = en.get("ok") is True
        except Exception:
            pay_entitled = False
    if expect_pf:
        try:
    ...
    if expect_set:
        try:
            ...
            set_entitled = en.get("ok") is True
        except Exception:
            set_entitled = False
...
def prove_honesty_contracts() -> None:
    ...
    can_view = getattr(w3, "employee_can_view", None)
    facing = getattr(w3, "employee_facing_state", None)
    check("released payslip visibility helper", callable(can_view) or callable(facing))
    unreleased = {
        "status": getattr(w3, "STATUS_ACTIVE", "active"),
        "employee_visibility": getattr(w3, "EMP_VIS_NOT_RELEASED", "not_released"),
    }
    released = {
        "status": getattr(w3, "STATUS_ACTIVE", "active"),
        "employee_visibility": getattr(w3, "EMP_VIS_RELEASED", "released"),
    }
    if callable(can_view):
        check("unreleased not visible", can_view(unreleased) is False, unreleased)
        check("released visible", can_view(released) is True, released)
    else:
        check("unreleased not visible", facing(unreleased) != getattr(w3, "EMP_VIS_RELEASED", "released"), unreleased)
        check("released visible", facing(released) == getattr(w3, "EMP_VIS_RELEASED", "released"), released)
...
def main() -> int:
    ...
    with conn_cm as conn:
        with conn.cursor() as cur:
            ...
            candidates = [
                Path(__file__).resolve().parent.parent / "ops/WAVE1_PRODUCT_FULL_PASS.md",
                Path("/opt/wathefni/repo/ops/WAVE1_PRODUCT_FULL_PASS.md"),
                Path("/opt/wathefni/staging/workspace/ops/WAVE1_PRODUCT_FULL_PASS.md"),
            ]
            w1_txt = ""
            for p in candidates:
                if p.is_file():
                    w1_txt = p.read_text(encoding="utf-8")
                    break
            if not w1_txt:
                # Staging orch overlay may not include ops/; honesty contract still locked in Setup Wave2
                import setup_console_wave2_policies as w2p

                w1_txt = "ACCEPTED FROZEN" if w2p.honesty_payload().get("assistant_mutations_in_wave2") is False else ""
            check("Wave 1 freeze regression (stamp ACCEPTED)", "ACCEPTED" in w1_txt and "FROZEN" in w1_txt)
