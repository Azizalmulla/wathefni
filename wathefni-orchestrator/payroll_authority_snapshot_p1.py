...
def honesty_payload() -> dict[str, Any]:
    ...
    try:
        ...
        mode_a_unlocked = bool(p5.payroll_authority_p5_enabled())
    except Exception:
        mode_a_unlocked = False
    return {
        ...
        "mode_a_wathefni_seal_unlocked": mode_a_unlocked,
...
def insert_wathefni_sealed_snapshot(
    cur: Any,
    *,
    company_code: str,
    employee_key: str,
    ...
    replaces_snapshot_id: str | None = None,
    approval_actor_chain: list[dict[str, Any]] | None = None,
    statutory_provenance: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """P5 Mode A: insert money_authority=wathefni sealed snapshot (never mutates preview calc rows)."""
    company = (company_code or "").upper()
    content = built["content"]
    lines = built["lines"]
    totals = built["totals"]
    ...
    provenance = {
        ...
        "input_snapshot_id": content.get("input_snapshot_id"),
        "policy_version_id": content.get("policy_version_id"),
        "catalog_fingerprint": content.get("catalog_fingerprint"),
        "compensation_fingerprint": content.get("compensation_fingerprint"),
        "policy_fingerprint": content.get("policy_fingerprint"),
        "input_fingerprint": content.get("input_fingerprint"),
        "calc_content_fingerprint": content.get("calc_content_fingerprint"),
        "statutory_provenance": statutory_provenance or content.get("statutory_provenance") or {},
        "source_fingerprint": built["source_fingerprint"],
        "money_authority": MONEY_WATHEFNI,
        "sealed_from": "seal_wathefni_from_calc_employee",
        "period_close_required": False,
        "wathefni_calculated": True,
        "payment_processing": "disabled",
    }
    cur.execute(
        """
        ...
        (
            ...
            built["source_fingerprint"],
            built["content_fingerprint"],
            str(content.get("policy_version_id") or ""),
            "p3_compensation_fingerprint",
            content.get("compensation_fingerprint"),
            totals["earnings"],
            totals["deductions"],
            totals["gross"],
            totals["net"],
    ...
    for ln in lines:
        cur.execute(
            ...
            (
                ...
                ln["amount"],
                ln.get("currency") or "KWD",
                ln.get("source") or SOURCE_NATIVE_AUTH,
                ln.get("policy_version"),
                ln.get("taxable_treatment"),
                ln.get("statutory_treatment"),
                int(ln.get("sort_order") or 0),
    ...
    _record_event(
        ...
        company_code=company,
        authority_snapshot_id=snap_id,
        event_type="authority_snapshot_sealed_wathefni",
        payload={
            "money_authority": MONEY_WATHEFNI,
            "calc_run_id": calc_run_id,
            "finalize_run_id": finalize_run_id,
...
def seal_from_native_preview(
    ...
    *,
    ...
    """Wave 2B native preview still cannot seal. Mode A authority uses P5 finalize from P3 calc runs."""
    denied = require_audit_reason(reason)
    if denied:
        return denied
    ensure_payroll_authority_snapshot_schema(cur)
    _record_event(
        ...
        payload={
            ...
            "refused_because": "native_preview_non_authoritative_use_p5_finalize_from_calc",
        },
        actor_phone=actor_phone,
    )
    return {
        "ok": False,
        "error": "mode_a_requires_p5_finalize_from_calc",
        "message": (
            "Native Wave 2B preview cannot become money_authority=wathefni. "
            "Use Payroll Authority P5 finalize from a locked P2 + calculated P3 calc run."
        ),
...
def generate_wathefni_payslip_from_authority_snapshot(
    ...
    *,
    ...
    """Mode A: payslip projection from sealed money_authority=wathefni snapshot (exact monetary match)."""
    denied = require_audit_reason(reason)
    if denied:
        return denied
