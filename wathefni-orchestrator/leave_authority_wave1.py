...
LEAVE_PRIMARY_STATUSES: frozenset[str] = frozenset(
    {
        ...
        "expired_stale",
        "needs_review",
        "needs_info",  # Wave 3 return-for-information
        "withdrawn",  # Wave 3 pre-decision withdrawal
        "declined_lifecycle",
    }
)

LEAVE_DECIDABLE_STATUSES: frozenset[str] = frozenset({"requested", "needs_review"})
LEAVE_REJECTABLE_STATUSES: frozenset[str] = frozenset({"requested", "needs_review", "needs_info"})
LEAVE_CANCELABLE_STATUSES: frozenset[str] = frozenset(
    {"requested", "approved", "needs_review", "needs_info"}
)
LEAVE_WITHDRAWABLE_STATUSES: frozenset[str] = frozenset({"requested", "needs_review", "needs_info"})
...
def resolve_employee_lifecycle_label(
    ...
    *,
    ...
    # Hub pending_start must never resolve as active eligibility (P2/P4).
    if hub in {"pending_start", "joining", "future_start"}:
        return "future_start"
