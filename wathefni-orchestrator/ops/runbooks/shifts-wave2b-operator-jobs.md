# Shifts Wave 2B/2C — operator jobs runbook (synthetic-only)
...
| Leave job timers **disabled** | Default Wave 2C posture — units installed, timers not enabled |

## systemd units (Wave 2C — disabled by default)

| Unit | Role |
|---|---|
| `wathefni-shifts-reminder-drain.service` / `.timer` | Reminder drain oneshot + timer |
| `wathefni-shifts-lifecycle-recon.service` / `.timer` | Lifecycle recon oneshot + timer |
| `wathefni-shifts-leave-recon.service` / `.timer` | Leave recon oneshot + timer |

Install (does **not** enable timers):

```bash
bash /opt/wathefni/orchestrator/ops/install-shifts-wave2-job-timers.sh
```

Manual oneshot (synthetic-only; respects kill switch + advisory locks):

```bash
systemctl start --wait wathefni-shifts-reminder-drain.service
systemctl start --wait wathefni-shifts-lifecycle-recon.service
systemctl start --wait wathefni-shifts-leave-recon.service
```

Do **not** `systemctl enable --now` these timers for real records. Qualification requires timers remain disabled.

## Job schedule (operator-driven / optional timer)

| Job | Lock key | Suggested cadence | Bound |
|---|---|---|---|
| Reminder drain | `820260201` | every 5–15 min (manual or timer if explicitly enabled) | `limit≤200` |
...
## Shared synthetic cleanup (Wave 2C)

Module: `shifts_synthetic_cleanup.py` (contract `1.0.0`).  
Wave 1B + Wave 2B canaries must clean to residual **0** via this utility — operator mop-up is not a passing result.
...
6. Timers stay disabled unless a later authorized wave enables them for synthetic-only.
