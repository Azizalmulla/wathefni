#!/usr/bin/env python3
"""Production qualification — Kuwait pilot document journey.

Exercises reachable FastAPI handlers (same endpoints as UI) against wathefni
...
MARKER = "kuwait-pilot-document-journey-production-v1"
COMPANY = "KWDOCPRD1"
OTHER = "KWDOCPRD2"
EMP_NAT = "kwdocp-nat-1"
EMP_EXP = "kwdocp-exp-1"
EMP_OTHER = "kwdocp-other-1"
PHONE_NAT = "96555559101"
PHONE_EXP = "96555559102"
PHONE_OTHER = "96555559201"
...
def hr_ctx(company: str, *, manage: bool = True) -> dict:
    ...
    return {
        ...
        "hr_user": {
            ...
            "company_code": company,
            "phone": "96555558000",
            "name": "Production Doc Journey HR",
        },
        "actor": {
            "status": "active",
            "role": "owner" if manage else "viewer",
...
def main() -> int:
    ...
    with orch.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            gate.check("db_is_staging", cur.fetchone()["d"] == "wathefni")

    # Artifact identity on disk
    orch_path = Path("/opt/wathefni/orchestrator")
    gate.check(
    ...
    pin = Path("/opt/wathefni/production/kuwait-pilot-document-journey-artifact.sha256").read_text().strip()
    ...
    dash_js = list(Path("/var/www/wathefni-dashboard/assets").glob("dashboard-*.js"))
    ...
    emp_art = Path("/opt/wathefni/production/employee-mobile-artifact")
    ...
    cleanup()
    try:
        ensure_company(COMPANY, "KW Doc Journey Production A")
        ensure_company(OTHER, "KW Doc Journey Production B")
    ...
    out_dir = Path("/opt/wathefni/production/evidence")
    out_dir.mkdir(parents=True, exist_ok=True)
    ts = __import__("datetime").datetime.now(__import__("datetime").timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    out = out_dir / f"kuwait-pilot-document-journey-production-{ts}.json"
