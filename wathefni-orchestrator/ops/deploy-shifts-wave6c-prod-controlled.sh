...
# Freeze artifacts must live on the host too: the freeze regression runs here and asserts
# the doc, the cursor rule and this deploy template are present and unweakened.
mkdir -p /opt/wathefni/ops
for f in SHIFTS_CONTROLLED_ROLLOUT_COMPLETION_AND_FREEZE.md shifts-freeze.mdc; do
  [[ -f "$STAGE/freeze/$f" ]] && cp -a "$STAGE/freeze/$f" "/opt/wathefni/ops/$f"
done
[[ -f "$STAGE/deploy-shifts-wave6c-prod-controlled.sh" ]] && \
  cp -a "$STAGE/deploy-shifts-wave6c-prod-controlled.sh" "$ORCH/ops/deploy-shifts-wave6c-prod-controlled.sh"
