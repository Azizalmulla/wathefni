#!/usr/bin/env bash
# Platform Assistant Wave 2-B — production WATHEFNI synthetic Safe Ops Queue Reads deploy.
# Leave + Attendance read-only. Mutations off. Ingest off. No Wave 3 / Payroll / Shifts / WhatsApp.
set -euo pipefail

STAMP="${STAMP:?}"
ORCH=/opt/wathefni/orchestrator
BACKUP="/opt/wathefni/backups/production-pre-platform-assistant-wave2b-${STAMP}"
REMOTE_EVID="/opt/wathefni/production-evidence/platform-assistant-wave2b-prod-canary/${STAMP}"
STAGE="${STAGE_DIR:-/tmp/platform-assistant-w2b-prod-stage}"
...

{
  ...
  ls -la "$ORCH/platform_assistant_spine_wave1.py" "$ORCH/platform_assistant_wave2_safe_ops_reads.py" 2>&1 || true
  echo "=== flags before ==="
...
# Wave 1 must already be on for Wave 2.
WAVE1=$(tr '\0' '\n' < /proc/"$(systemctl show -p MainPID --value wathefni-orchestrator)"/environ | grep '^WATHEFNI_PLATFORM_ASSISTANT_WAVE1=' || echo "unset")
echo "$WAVE1" | tee "$REMOTE_EVID/preflight/wave1-before.txt"
if ! echo "$WAVE1" | grep -q '=1$'; then
  echo "REFUSE: Wave 1 must be enabled before Wave 2-B" >&2
  exit 3
fi
...
for f in app.py action_registry.py assistant_capability_catalog.py tool_call_orchestrator.py \
         ...
         setup_console_wave_a_launch_readiness.py; do
...
log "deploying platform assistant wave2 modules"
for f in app.py action_registry.py assistant_capability_catalog.py tool_call_orchestrator.py \
         platform_assistant_spine_wave1.py platform_assistant_wave2_safe_ops_reads.py \
...
mkdir -p "$ORCH/ops"
[[ -f "$STAGE/migrate-platform-assistant-wave2-prod.sh" ]] && cp -a "$STAGE/migrate-platform-assistant-wave2-prod.sh" "$ORCH/ops/"
[[ -f "$STAGE/deploy-platform-assistant-wave2b-prod-synthetic.sh" ]] && cp -a "$STAGE/deploy-platform-assistant-wave2b-prod-synthetic.sh" "$ORCH/ops/"

log "writing Wave 2-B production drop-in (mutations off; ingest off; WATHEFNI only)"
cat > "$DROPIN" <<'EOF'
[Service]
...
log "migrate ACK"
chmod +x "$ORCH/ops/migrate-platform-assistant-wave2-prod.sh"
...
ACK_PRODUCTION_PLATFORM_ASSISTANT_W2B=YES ORCH_PYTHON="$PYBIN" bash "$ORCH/ops/migrate-platform-assistant-wave2-prod.sh" \
...
{
  echo "=== SHAs after ==="
  sha256sum "$ORCH/app.py" "$ORCH/action_registry.py" "$ORCH/tool_call_orchestrator.py" \
    "$ORCH/platform_assistant_spine_wave1.py" "$ORCH/platform_assistant_wave2_safe_ops_reads.py"
  echo "=== flags after ==="
...
"$PYBIN" - <<'PY' | tee "$REMOTE_EVID/flags/platform-assistant-wave2b-flags.txt"
import os
import platform_assistant_spine_wave1 as w1
import platform_assistant_wave2_safe_ops_reads as w2
print("WAVE1", os.environ.get("WATHEFNI_PLATFORM_ASSISTANT_WAVE1"))
print("WAVE2", os.environ.get("WATHEFNI_PLATFORM_ASSISTANT_WAVE2"))
print("COMPANIES_W1", os.environ.get("WATHEFNI_PLATFORM_ASSISTANT_WAVE1_COMPANIES"))
print("COMPANIES_W2", os.environ.get("WATHEFNI_PLATFORM_ASSISTANT_WAVE2_COMPANIES"))
...
print("wave1_enabled", w1.platform_assistant_wave1_enabled())
print("wave2_enabled", w2.platform_assistant_wave2_enabled())
print("company", w2.wave2_enabled_for_company("WATHEFNI"))
print("company_external", w2.wave2_enabled_for_company("EXTERNALCO"))
print("mutations_allowed", w1.assistant_mutations_allowed())
print("kill_engaged", w1.assistant_kill_engaged())
h = w2.honesty_payload()
print("honesty", {k: h.get(k) for k in (
    "wathefni_only","hr_dashboard_only","mutations","payroll","shifts",
    "attendance_ingest","capture_ingest","attendance_records_basis","whatsapp_widening",
)})
assert w1.platform_assistant_wave1_enabled()
assert w2.platform_assistant_wave2_enabled()
assert w2.wave2_enabled_for_company("WATHEFNI")
assert not w2.wave2_enabled_for_company("EXTERNALCO")
assert w1.assistant_mutations_allowed() is False
assert w1.assistant_kill_engaged() is False
assert h.get("payroll") is False
assert h.get("attendance_ingest") is False
assert h.get("capture_ingest") == "off"
assert h.get("attendance_records_basis") == "existing_records_only"
print("PAW2B_MARKER=PAW2B")
print("flags_ok_wave2b=true")
PY
