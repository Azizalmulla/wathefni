...
CODE_FILES=(app.py tool_call_orchestrator.py action_registry.py outbound_delivery.py attendance_import.py delivery-sweep-worker.py video-interview-worker.py leave-accrual-worker.py)
...
deploy_staging() {
  ...
  "${SSH[@]}" "set -e; cd $STAGING_ORCH; /opt/wathefni/orchestrator/.venv/bin/python -m py_compile app.py tool_call_orchestrator.py action_registry.py outbound_delivery.py attendance_import.py; \
...
deploy_production() {
  ...
  if "${SSH[@]}" "set -e; cd $PROD_ORCH; /opt/wathefni/orchestrator/.venv/bin/python -m py_compile app.py tool_call_orchestrator.py action_registry.py outbound_delivery.py attendance_import.py; \
