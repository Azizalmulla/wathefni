...
CODE_FILES=(app.py runtime_environment.py company_setup.py module_catalog.py tool_call_orchestrator.py action_registry.py outbound_delivery.py attendance_import.py channel_account_routing.py cv_extraction.py cv_docx.py recruiting_lifecycle.py offer_lifecycle.py offer_service.py offer_routes.py assessment_lifecycle.py assessment_service.py assessment_ai_contracts.py assessment_ai_service.py assessment_ai_routes.py assessment_ai_qualification.py assessment-ai-worker.py assessment_ai_eval.py delivery-sweep-worker.py document-storage-reconcile-worker.py video-interview-worker.py leave-accrual-worker.py operator_mobile.py operator_mobile_data.py)
RUNTIME_OPS_FILES=(ops/provision-environment-identity.py ops/provision-assessment-ai-db-role.py ops/document-storage-reconciliation.py ops/migrate-assessments-cleanup1.py ops/assessment-product2-staging-pilot.py ops/assessment-product2-staging-validation.py ops/assessment-product2-live-staging-eval.py ops/staging-smoke.sh ops/wathefni-orchestrator-staging.service ops/wathefni-orchestrator-production-environment.conf ops/wathefni-document-storage-reconcile-staging.service ops/wathefni-document-storage-reconcile-staging.timer ops/wathefni-document-storage-reconcile.service ops/wathefni-document-storage-reconcile.timer ops/wathefni-delivery-sweep-staging.service ops/wathefni-delivery-sweep.service ops/wathefni-leave-accrual.service)
...
preflight() {
  ...
  ( cd "$ORCH_SRC" && python3 smoke-test-tenant-read-hardening.py && python3 smoke-test-toolcall-orchestrator.py && python3 smoke-test-whatsapp-identity.py && python3 smoke-test-channel-account-routing.py && python3 smoke-test-cv-extraction-ocr.py && python3 smoke-test-cv-docx.py && python3 smoke-test-canonical-recruiting-lifecycle.py && python3 smoke-test-offer-lifecycle.py && python3 smoke-test-offer-hire-override.py ) || fail "local source smokes failed"
}
...
deploy_production() {
  ...
  if "${SSH[@]}" "set -e; cd $PROD_ORCH; /opt/wathefni/orchestrator/.venv/bin/python -m py_compile app.py runtime_environment.py company_setup.py module_catalog.py tool_call_orchestrator.py action_registry.py outbound_delivery.py attendance_import.py channel_account_routing.py cv_extraction.py cv_docx.py recruiting_lifecycle.py offer_lifecycle.py offer_service.py offer_routes.py assessment_lifecycle.py assessment_service.py assessment_ai_contracts.py assessment_ai_service.py assessment_ai_routes.py assessment_ai_qualification.py assessment-ai-worker.py assessment_ai_eval.py operator_mobile.py operator_mobile_data.py; \
