#!/usr/bin/env python3
"""Candidates C0/C1 guarded production matrix.

Production-safe. Uses isolated disposable company fixtures (C01PRD/C01PXO), forces
dry_run delivery, and deterministic cleanup. Never uses WATHEFNI tenant data.
"""
...
REPORT_PATH = Path(
    os.environ.get(
        "C01_PRODUCTION_REPORT",
        "/opt/wathefni/orchestrator/ops/reports/candidates-c01-production-matrix.json",
    )
)

COMPANY = "C01PRD"
OTHER = "C01PXO"
MARKER = "temporary_candidates_c01_production"
ACTOR = "00000000-0000-4000-8000-c01c01c01prd"
PHONE = "965580020011"
PHONE_B = "965580020012"
PHONE_INTERVIEW = "965580020013"
PHONE_INTERVIEW_FAIL = "965580020014"
PHONE_HIRE_ROLLBACK = "965580020015"
        ...
        with conn.cursor() as cur:
            for company in (COMPANY, OTHER):
                cur.execute(
                    ...
                    (
                        company,
                        f"Candidates C0/C1 Production {company}",
                ...
                cur.execute(
                    ...
                    VALUES (%s,'pre_hiring',true,'c01_production_matrix',now())
                ...
                if company == COMPANY:
                    cur.execute(
                        ...
                        INSERT INTO company_modules
                          (company_code, module_key, enabled, source, updated_at)
                        VALUES (%s,'interviews',true,'c01_production_matrix_fixture',now())
                        ON CONFLICT (company_code, module_key) DO UPDATE
                          SET enabled=true, source=EXCLUDED.source, updated_at=now()
                        """,
                        (company,),
    ...
    with orch.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                VALUES (%s,%s,%s,%s,%s,'c01_production',%s::jsonb)
            ...
            cur.execute(
                ...
                ) VALUES (%s,%s,%s,%s,%s,%s,%s,true,'c01_production',now(),now(),%s::jsonb,0)
...
def main() -> int:
    ...
    matrix.check("production_db_identity", report["identity"]["db"] == "wathefni", report["identity"])
