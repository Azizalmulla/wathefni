    ...
    try:
        ...
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                inner = denied.get("result") if isinstance(denied.get("result"), dict) else {}
                comp_err = str(denied.get("error") or inner.get("error") or "")
                check(
                    "C Comp without JA governed",
                    denied.get("ok") is not True
                    and comp_err in {"ja_must_be_enabled", "ja_hard_dependency_unmet", "ja_hard_unmet"},
                    denied,
                )
        ...
        try:
            ...
            dep_http = client.patch(
                "/dashboard/setup/company/module-policies/wave6_comp_planning",
                json={"reason": "r6 http dep", "required": {"enabled": True}},
            )
            check("C HTTP Comp without JA is 409", dep_http.status_code == 409, dep_http.status_code)
