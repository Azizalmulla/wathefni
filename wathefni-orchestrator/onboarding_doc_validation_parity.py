...
Decision contract (soft canary — Aziz/Talal allowlist):
  * Obvious non-document → block with wrong-document message (wins over capture-quality copy)
  * Credible document + clearly bad capture (blur/glare/resolution) → block + retake
    (high OCR confidence must not convert those rejects into allow)
  * Crop/frame-fill is a weak signal only — block cut-off only with supporting
    OCR/side/page/partial/geometry evidence
  * Credible document + borderline capture → allow_uncertain → HR
  * Clear high-confidence complete documents → allow (including frame-filling)
...
CANARY_VALIDATION_ALIASES: dict[str, str] = {
    ...
    "civil_id_dual_side_canary": "civil_id",
    # Disposable multi-type qualification canaries (isolated governed lanes).
    "docs_qual_passport": "passport",
    "docs_qual_residence": "residence",
    "docs_qual_work_permit": "work_permit",
    "docs_qual_employment_contract": "employment_contract",
    "docs_qual_personal_photo": "personal_photo",
}
...
CORRECTION_COPY: dict[str, dict[str, str]] = {
    ...
    "wrong_side": {
        "en": "This looks like the other side of your Civil ID. Please upload it under the matching Front or Back slot.",
        "ar": "يبدو أن هذه الصورة للوجه الآخر من البطاقة المدنية. يرجى رفعها في خانة الوجه الأمامي أو الخلفي المطابقة.",
    },
    "duplicate_sides": {
        "en": "Front and back must be different photos of your Civil ID.",
        "ar": "يجب أن تكون صورة الوجه الأمامي مختلفة عن صورة الوجه الخلفي.",
    },
    "parts_incomplete": {
        "en": "Upload both the front and back of your Civil ID before this item can be reviewed.",
        "ar": "يرجى رفع الوجه الأمامي والخلفي للبطاقة المدنية قبل إرسالها للمراجعة.",
    },
    "side_mismatch": {
        "en": "The front and back do not look like a matching Civil ID pair. Please re-upload both sides.",
        "ar": "لا يبدو الوجهان الأمامي والخلفي كزوج متطابق من البطاقة المدنية. يرجى إعادة رفع الوجهين.",
    },
    "missing_page": {
...
def infer_capture_quality_reason(
    ...
    *,
    ...
    # Dual-side uploads validate the requested part separately. Never treat a
    # correctly labeled Civil ID back as legacy single-file "missing_side".
    part = str(expected_part or "").strip().lower()
    dual_part_mode = part in {"front", "back"}
    if (not dual_part_mode) and side == "back" and item == "civil_id":
        return "missing_side"
    if (not dual_part_mode) and any(
        tok in text for tok in ("missing_side", "missing_back", "back_only", "front_missing", "missing front")
    ):
        return "missing_side"
...
def _has_reliable_document_geometry(*sources: Any) -> bool:
    """True when a caller/engine supplied usable document bounds (future scanner)."""
    for src in _nested_dicts(*sources):
        for key in ("document_geometry", "document_bounds", "card_bounds", "quad", "corners"):
            geom = src.get(key)
            if isinstance(geom, dict) and (
                geom.get("truncated") is True
                or geom.get("incomplete") is True
                or geom.get("cut_off") is True
            ):
                return True
        issues = src.get("geometry_issues") or src.get("crop_issues")
        if isinstance(issues, list) and any(
            str(x).lower() in {"truncated", "cut_off", "incomplete", "partial"} for x in issues
        ):
            return True
    return False


def crop_concern_has_supporting_evidence(
    *,
    capture_metrics: dict[str, Any] | None,
    verification: dict[str, Any] | None,
    extraction: dict[str, Any] | None = None,
    expected_item: str | None = None,
) -> bool:
    """Weak crop/frame-fill signal may block only with independent support."""
    try:
        import onboarding_capture_quality as _capq

        weak_min = int(getattr(_capq, "CROP_WEAK_SIDES", 2))
    except Exception:
        weak_min = 2
    metrics = capture_metrics if isinstance(capture_metrics, dict) else {}
    try:
        crop_sides = int(metrics.get("crop_hot_sides") or 0)
    except Exception:
        crop_sides = 0
    crop_signal = str(metrics.get("crop_signal") or "").lower()
    if crop_sides < weak_min and crop_signal != "weak":
        return False

    item = canonical_validation_item(expected_item or "")
    try:
        conf = float((verification or {}).get("confidence") or 0)
    except Exception:
        conf = 0.0
    status = str((verification or {}).get("extraction_status") or "").strip().lower()
    matches = (verification or {}).get("matches_expected_item") is True

    if conf < VERIFY_CONFIDENCE_FLOOR or not matches or status in {"needs_review", "low_confidence"}:
        return True
    if _side_issue(item, extraction, verification):
        return True
    if _required_fields_issue(item, extraction):
        return True
    mapped = infer_capture_quality_reason(verification, extraction, expected_item=item)
    if mapped in {"document_not_fully_visible", "missing_side", "missing_page", "unreadable"}:
        return True
    if _has_reliable_document_geometry(verification, extraction, metrics):
        return True
    return False
...
def evaluate_employee_upload(
    *,
    ...
    # Capture metrics early for audit; decisions apply only after document-likeness.
    capture_borderline = False
    capture_assessment: dict[str, Any] | None = None
    if item != "personal_photo" and (
        ...
        try:
            ...
            capture_assessment = _capq.assess_capture_quality(media=media, extension=ext)
            out["capture_quality"] = {
                "status": capture_assessment.get("status"),
                "reason": capture_assessment.get("reason"),
                "signal_reason": capture_assessment.get("signal_reason"),
                "metrics": capture_assessment.get("metrics") or {},
                "engine": capture_assessment.get("engine"),
            }
        except Exception:
            logger.exception("capture quality assess failed; continuing without local metrics")
    ...
    # Capture-quality decisions only after document-likeness (wrong-doc message wins).
    # Blur/glare/resolution hard-reject still win even with high OCR confidence.
    # Crop/frame-fill is deferred: weak signal, needs supporting OCR/side/page evidence.
    if isinstance(capture_assessment, dict):
        cap_status = str(capture_assessment.get("status") or "")
        cap_reason = str(capture_assessment.get("reason") or "")
        if cap_status == "reject" and cap_reason != "document_not_fully_visible":
            out["quality_issues"] = list(out.get("quality_issues") or []) + [cap_reason or "too_blurry"]
            return _block(out, cap_reason or "too_blurry", locale=locale)
        if cap_status == "borderline" and str(capture_assessment.get("signal_reason") or "") != "document_not_fully_visible":
            capture_borderline = True
            out["quality_issues"] = list(out.get("quality_issues") or []) + [
                str(
                    capture_assessment.get("signal_reason")
                    or capture_assessment.get("reason")
                    or "capture_quality_borderline"
                )
            ]

    capture = infer_capture_quality_reason(
        verification,
        expected_item=item,
    ...
    if low_readability:
        if not credible:
            return _block(out, obvious_non_document_reason(verification), locale=locale)
        # Prefer explicit cut-off when weak crop + low readability support it.
        cap_metrics = (out.get("capture_quality") or {}).get("metrics") if isinstance(out.get("capture_quality"), dict) else None
        if crop_concern_has_supporting_evidence(
            capture_metrics=cap_metrics if isinstance(cap_metrics, dict) else None,
            verification=verification,
            expected_item=item,
        ):
            out["quality_issues"] = list(out.get("quality_issues") or []) + ["document_not_fully_visible"]
            return _block(out, "document_not_fully_visible", locale=locale)
    ...
    if item in IDENTITY_ITEMS or item in {"education_cert", "employment_contract", "medical", "residence", "work_permit"}:
        ...
        capture_ex = infer_capture_quality_reason(
            verification,
        ...
        # Dual-side path: validate requested part instead of single-file missing_side.
        if expected_part:
            try:
                import onboarding_civil_id_dual_side as _dual

                side_gate = _dual.validate_part_side(
                    expected_part=expected_part,
                ...
                out["expected_part"] = side_gate.get("expected_part")
                out["detected_side"] = side_gate.get("detected_side")
                out["side_decision"] = side_gate.get("side_decision")
                out["side_evidence"] = side_gate.get("side_evidence")
                if side_gate.get("hr_warning"):
                    out["hr_review_recommended"] = True
                    out["side_uncertain"] = True
            except Exception as exc:
                if getattr(exc, "code", None) == "wrong_side":
                    out["quality_issues"] = list(out.get("quality_issues") or []) + ["wrong_side"]
                    return _block(out, "wrong_side", locale=locale)
                raise
        else:
        ...
        # Back slot: do not require front-only identity fields; pair gate + HR cover completeness.
        if str(expected_part or "").strip().lower() == "back":
            req = None
        else:
    ...
    # Weak crop/frame-fill only blocks with supporting OCR/side/page/geometry evidence.
    # Clear high-confidence complete documents pass even when they fill the frame.
    cap_metrics = (
        (out.get("capture_quality") or {}).get("metrics")
        if isinstance(out.get("capture_quality"), dict)
        else None
    )
    if crop_concern_has_supporting_evidence(
        capture_metrics=cap_metrics if isinstance(cap_metrics, dict) else None,
...
def http_error_detail(result: dict[str, Any], *, locale: str = "en") -> dict[str, Any]:
    ...
    if isinstance(result.get("capture_quality"), dict):
        detail["capture_quality"] = result.get("capture_quality")
    if result.get("quality_issues"):
        detail["quality_issues"] = list(result.get("quality_issues") or [])
    return detail
