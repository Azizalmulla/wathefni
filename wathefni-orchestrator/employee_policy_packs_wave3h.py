...
-- Dual-controlled explicit HR classification (no lifecycle execution).
CREATE TABLE IF NOT EXISTS employee_policy_pack_classification_requests (
  ...
  jurisdiction_code text NOT NULL,
  worker_category text NOT NULL,
  proposed_pack_code text NOT NULL,
  contract_type text,
  pay_frequency text,
...
CREATE INDEX IF NOT EXISTS employee_policy_pack_classification_pending_idx
  ON employee_policy_pack_classification_requests (company_code, status)
...
def list_remediation_queue(legacy: Any, *, company_code: str) -> dict[str, Any]:
    """Read-only remediation queue: missing jurisdiction/category/classification fields."""
    company = str(company_code).upper()
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT e.employment_id, e.person_id, e.employment_status,
                       e.jurisdiction_code, e.worker_category,
                       e.policy_pack_code, e.policy_pack_version, e.policy_pack_status,
                       e.contract_type, e.pay_frequency, e.probation_status,
                       e.probation_start_date, e.probation_end_date,
                       m.employee_key
                FROM employee_employments e
                LEFT JOIN employee_key_authority_map m
                  ON m.company_code=e.company_code AND m.employment_id=e.employment_id
            ...
            for r in cur.fetchall() or []:
                row = dict(r)
                missing = []
                if not row.get("jurisdiction_code"):
                    missing.append("jurisdiction_code")
                if not row.get("worker_category"):
                    missing.append("worker_category")
                if not row.get("contract_type"):
                    missing.append("contract_type")
                if not row.get("pay_frequency"):
                    missing.append("pay_frequency")
                if not row.get("probation_status"):
                    missing.append("probation_status")
                status = str(row.get("policy_pack_status") or "")
                # Quarantined / archived synthetic leftovers are preserved as history
                # but excluded from the active remediation work queue.
                if status in {"quarantined_synthetic_test", "archived_synthetic_test", "synthetic_history"}:
                    continue
                needs = (
                    status in {"", "remediation", "unsupported", "reserved"}
                    or bool(missing)
                    or not row.get("policy_pack_code")
                )
                if not needs:
                    continue
                rows.append(
                    {
                        ...
                        "worker_category": row.get("worker_category"),
                        "policy_pack_code": row.get("policy_pack_code"),
                        "policy_pack_version": row.get("policy_pack_version"),
                        "policy_pack_status": row.get("policy_pack_status") or "unset",
    ...
    return {
        ...
        "count": len(rows),
        "rows": rows,
        "note": "Read-only. Explicit dual-controlled classification required. No auto-classify. No lifecycle execution.",
    }
...
def create_classification_request(
    ...
    *,
    ...
    probation_start_date: str | None = None,
    probation_end_date: str | None = None,
    reason: str | None = None,
) -> dict[str, Any]:
    """Propose explicit HR classification — dual control required; no lifecycle mutation."""
    ...
    resolution = resolve_pack_code(jurisdiction_code=jurisdiction_code, worker_category=worker_category)
    if not resolution.get("ok"):
        raise legacy.HTTPException(
            status_code=422,
            detail={
                "error": resolution.get("error") or "policy_pack_unresolved",
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                LEFT JOIN employee_key_authority_map m
                  ON m.company_code=e.company_code AND m.employment_id=e.employment_id
                 AND m.mapping_status='active'
                WHERE e.company_code=%s AND e.employment_id=%s
                """,
                (company, employment_id),
            )
            emp = cur.fetchone()
            if not emp:
                raise legacy.HTTPException(status_code=404, detail={"error": "employment_not_found"})
            emp = dict(emp)
            cur.execute(
                """
                INSERT INTO employee_policy_pack_classification_requests (
                  company_code, employment_id, employee_key, person_id, status,
                  jurisdiction_code, worker_category, proposed_pack_code,
                  contract_type, pay_frequency, probation_status,
                  probation_start_date, probation_end_date,
                  requester_user_id, designated_approver_user_id, payload
                ) VALUES (%s,%s,%s,%s,'pending',%s,%s,%s,%s,%s,%s,%s::date,%s::date,%s,%s,%s::jsonb)
                RETURNING *
                """,
                (
                    company,
                    employment_id,
                    emp.get("employee_key"),
                    emp.get("person_id"),
                    normalize_jurisdiction(jurisdiction_code),
                    normalize_worker_category(worker_category),
                    pack_code,
                    contract_type,
                    pay_frequency,
                    probation_status,
                    ...
                    actor,
                    approver,
                    json.dumps({"reason": reason or "explicit_hr_classification", "no_lifecycle": True}, default=str),
                ),
            )
            row = dict(cur.fetchone())
            conn.commit()
    return {"ok": True, "request": row, "lifecycle_executed": False}


def decide_classification_request(
    legacy: Any,
    context: dict[str, Any],
    *,
    request_id: str,
    ...
    """Approve/reject classification. On approve: bind pack + optional employment fields only."""
    company = str(context.get("company_code") or "").upper()
    actor = str(context.get("actor_user_id") or "").strip()
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                SELECT * FROM employee_policy_pack_classification_requests
                WHERE company_code=%s AND request_id=%s
                """,
                (company, request_id),
            )
            ...
            if action == "reject":
                cur.execute(
                    """
                    UPDATE employee_policy_pack_classification_requests SET
                      status='rejected', decided_by_user_id=%s, decided_at=now(),
                      decision_reason=%s, updated_at=now()
                    WHERE request_id=%s RETURNING *
                    """,
                    (actor, decision_reason, request_id),
                )
                row = dict(cur.fetchone())
                conn.commit()
                return {"ok": True, "request": row, "bound": False, "lifecycle_executed": False}

            pack = get_pack(str(req["proposed_pack_code"]))
            if not pack or not pack.get("enabled"):
                raise legacy.HTTPException(status_code=422, detail={"error": "pack_not_implemented"})
            bind_employment_pack(
                cur,
                company_code=company,
                ...
                resolution={
                    ...
                    "worker_category": req.get("worker_category"),
                },
            )
            # Optional classification fields — never invent; only write provided values.
            cur.execute(
                """
                UPDATE employee_employments SET
                ...
                (
                    ...
                    req.get("pay_frequency"),
                    req.get("probation_status"),
                    req.get("probation_start_date"),
                    req.get("probation_end_date"),
                    company,
                    req["employment_id"],
                ),
            )
            cur.execute(
                ...
                """,
                (actor, company, req["employment_id"]),
            )
            cur.execute(
                """
                UPDATE employee_policy_pack_classification_requests SET
                  status='approved', decided_by_user_id=%s, decided_at=now(),
                  decision_reason=%s, updated_at=now()
                WHERE request_id=%s RETURNING *
                """,
                (actor, decision_reason or "approved_explicit_classification", request_id),
            )
            row = dict(cur.fetchone())
            conn.commit()
    return {
...
def confirm_employment_kw_private_sector(
    ...
    *,
    ...
    """Deprecated direct bind — prefer create_classification_request + decide (dual control)."""
    company = str(company_code).upper()
    pack = get_pack("KW_PRIVATE_SECTOR")
    assert pack
    with legacy.db_connect() as conn:
    ...
    return {
        ...
        "policy_version": pack["policy_version"],
        "content_hash": pack["content_hash"],
        "warning": "direct_confirm_bypasses_dual_control_use_classification_request",
    }
