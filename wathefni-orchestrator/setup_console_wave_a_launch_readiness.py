...
def _dashboard_page(page: str) -> str:
    """Phase 1 deep link into the HR dashboard operational page."""
    return f"/dashboard?page={str(page).strip()}"
    ...
    *,
    ...
    if key == "attendance":
        ...
        if not auth_on:
            return _item(
                ...
                deep_link=_dashboard_page("attendance"),
    ...
    if key == "shifts":
        ...
        if not wave:
            return _item(
                ...
                deep_link=_dashboard_page("shifts"),
    ...
    if key == "leave":
        ...
        return _item(
            ...
            next_action_en="Review leave day-to-day in Leave. Company module on/off stays in Setup.",
            next_action_ar="راجع عمليات الإجازات اليومية في الإجازات. تشغيل الوحدة يبقى في الإعداد.",
            deep_link=_dashboard_page("leave"),
    ...
    if key == "employee_app":
        ...
        return _item(
            ...
            next_action_en="Set who can use the Employee App in Setup, then manage people in Employees.",
            next_action_ar="حدّد من يستخدم تطبيق الموظف في الإعداد، ثم أدِر الأفراد من الموظفين.",
            deep_link="/setup-console#classic-app-access",
    ...
    return _item(
        ...
        next_action_en=f"Open {label} for day-to-day work, or Setup Console for company entitlements.",
        next_action_ar=f"افتح {label} للعمل اليومي، أو وحدة التحكم لاستحقاقات الشركة.",
        deep_link=_dashboard_page(_MODULE_OPS_PAGE.get(key, "overview")),
        evidence={"module_key": key},
    )
...
def evaluate_launch_readiness(cur: Any, *, company_code: str = ALLOWED_COMPANY) -> dict[str, Any]:
    ...
    basics_items.append(
        _item(
            ...
            deep_link="/setup-console#classic-profile",
    ...
    people_items.append(
        _item(
            ...
            deep_link="/setup-console#classic-owner",
    ...
    people_items.append(
        _item(
            ...
            deep_link=_dashboard_page("settings"),
    ...
    channel_items.append(
        _item(
            ...
            deep_link="/setup-console#classic-channels",
