...
def redaction_contracts() -> None:
    ...
    check(
        "R8 redacts password= assignments in free text",
        "hunter2" not in observability.redact_text("password=hunter2 leftover"),
    )
    ...
    recorded = observability.record_error_event(
        _NoDb(),
        surface="hr_web",
        message="unit password=hunter2 ceo@acme.test",
        detail={"token": "abcd"},
    )
    check("record_error_event does not collide on level", recorded.get("level") == "error")
    check("record_error_event redacts before persist attempt", "[redacted-email]" in recorded["message"])
    check("record_error_event marks persist failure", recorded.get("stored") is False)
    check("event_id is not treated as a phone number", "[redacted-phone]" not in recorded["event_id"])
