...
def main() -> int:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            conn.commit()
            try:
                cur.execute("SAVEPOINT pb_onb_seed")
                cur.execute(
                    """
                    INSERT INTO onboarding_items (
                      employee_key, item_id, label, category, item_type,
                      required, owner, status
                    ) VALUES (%s,'civil_id','Civil ID','identity_legal','document',true,'employee','received')
                    ON CONFLICT DO NOTHING
                    """,
                    (emp3,),
                )
                cur.execute("RELEASE SAVEPOINT pb_onb_seed")
                conn.commit()
            except Exception as exc:
                try:
                    cur.execute("ROLLBACK TO SAVEPOINT pb_onb_seed")
                except Exception:
                    conn.rollback()
