...
Compatibility:
  - Hub `employees.employment_status`: active | left | pending_start.
  - Rich lifecycle lives on `employee_employments.lifecycle_state`.
  - terminated ⇒ hub left; pending_start ⇒ hub pending_start; other live states ⇒ hub active.
  - pending_start must never be interpreted as active for headcount/pay/time/leave/shifts/ESS.
...
def lifecycle_runtime_env() -> str:
    return (os.environ.get("WATHEFNI_ENV") or os.environ.get("ENV") or "local").strip().lower() or "local"


def lifecycle_synthetic_only_enabled() -> bool:
    """Production-safe default: synthetic-only gate ON in production unless explicitly disabled.

    When ON, Wave 3 refuses lifecycle mutations against non-synthetic employees.
    Real-employee production use remains NO-GO until this gate is deliberately lifted.
    """
    raw = str(os.environ.get("WATHEFNI_EMPLOYEE_LIFECYCLE_V3_SYNTHETIC_ONLY") or "").strip().lower()
    if raw in {"0", "false", "no", "off"}:
        return False
    if raw in {"1", "true", "yes", "on"}:
        return True
    return lifecycle_runtime_env() in {"production", "prod"}


def lifecycle_synthetic_phone_prefixes() -> tuple[str, ...]:
    raw = str(os.environ.get("WATHEFNI_EMPLOYEE_LIFECYCLE_V3_SYNTHETIC_PHONE_PREFIXES") or "965522")
    return tuple(p.strip() for p in raw.split(",") if p.strip())


def lifecycle_synthetic_name_prefix() -> str:
    return str(os.environ.get("WATHEFNI_EMPLOYEE_LIFECYCLE_V3_SYNTHETIC_NAME_PREFIX") or "W3D-SYNTH|").strip() or "W3D-SYNTH|"


def is_lifecycle_synthetic_employee_row(*, phone: str | None, name: str | None, provenance: Any = None, raw_json: Any = None) -> bool:
    digits = phone_digits(phone or "")
    for prefix in lifecycle_synthetic_phone_prefixes():
        if digits.startswith(prefix):
            return True
    name_s = str(name or "")
    if name_s.startswith(lifecycle_synthetic_name_prefix()):
        return True
    for blob in (provenance, raw_json):
        ...
        if isinstance(blob, dict) and (
            blob.get("lifecycle_synthetic_canary") is True
            or blob.get("lifecycle_synthetic") is True
            or str(blob.get("source") or "") == "wave3d_synthetic_canary"
        ):
            return True
    return False


def load_lifecycle_synthetic_markers(cur: Any, *, company: str, employee_key: str) -> dict[str, Any]:
    cur.execute(
        """
        SELECT emp.phone, emp.name, emp.raw_json,
               e.provenance AS employment_provenance
        FROM employees emp
        ...
        LEFT JOIN employee_employments e
...
def lifecycle_real_allowlist() -> set[str]:
    """Explicit real employee_key allowlist for controlled lifecycle readiness.

    When SYNTHETIC_ONLY=on, synthetics remain allowed and only these real keys
    may pass. Empty means no real employees. Exceptional/high-risk cases remain
    manual regardless of allowlist.
    """
    raw = str(os.environ.get("WATHEFNI_EMPLOYEE_LIFECYCLE_V3_REAL_ALLOWLIST") or "")
    return {k.strip() for k in raw.split(",") if k.strip()}


def assert_lifecycle_synthetic_target(legacy: Any, cur: Any, *, company: str, employee_key: str) -> None:
    if not lifecycle_synthetic_only_enabled():
        return
    markers = load_lifecycle_synthetic_markers(cur, company=company, employee_key=employee_key)
    if not markers:
        raise legacy.HTTPException(status_code=404, detail={"error": "employee_not_found"})
    if is_lifecycle_synthetic_employee_row(
        ...
        return
    if str(employee_key) in lifecycle_real_allowlist():
        return
    raise legacy.HTTPException(
        ...
        detail={
            ...
            "message": (
                "Wave 3 lifecycle is restricted to synthetic canary employees "
                "or explicitly allowlisted classified real employees. "
                "Broad real terminations remain NO-GO."
            ),
...
def stamp_lifecycle_synthetic_markers(cur: Any, *, company: str, employee_key: str) -> None:
    """Mark hub + employment as synthetic canary (idempotent)."""
    stamp = {
        "lifecycle_synthetic_canary": True,
        "source": "wave3d_synthetic_canary",
    ...
    cur.execute(
        ...
        UPDATE employee_employments e SET
          provenance = coalesce(e.provenance,'{}'::jsonb) || %s::jsonb,
          updated_at=now()
        FROM employee_key_authority_map m
        WHERE m.employment_id=e.employment_id AND m.company_code=e.company_code
          AND m.company_code=%s AND m.employee_key=%s AND m.mapping_status='active'
        """,
        (json.dumps(stamp), company, employee_key),
    )
...
-- Allow provisional joining employment on the authority row (P1/P2).
ALTER TABLE IF EXISTS employee_employments
  DROP CONSTRAINT IF EXISTS employee_employments_employment_status_check;
ALTER TABLE IF EXISTS employee_employments
  ADD CONSTRAINT employee_employments_employment_status_check
  CHECK (employment_status IN ('active', 'left', 'pending_start'));

UPDATE employee_employments
SET lifecycle_state = CASE
  WHEN lower(coalesce(employment_status,'active')) = 'left' THEN 'terminated'
  WHEN lower(coalesce(employment_status,'')) = 'pending_start' THEN 'pending_start'
  ELSE 'active'
END
...
def repair_pending_start_hub_projection(cur, company_code: str | None = None) -> dict[str, Any]:
    """Align hub employees.employment_status with lifecycle pending_start (projection fix).

    Not an offer→employment truth-sync writer. Idempotent. Company-scoped when provided.
    """
    params: list[Any] = []
    company_clause = ""
    if company_code:
        company_clause = "AND e.company_code=%s"
        params.append(str(company_code).strip().upper())
    # Prefer authority map; also match legacy_employee_key.
    cur.execute(
        f"""
        WITH targets AS (
          SELECT DISTINCT e.company_code, e.employee_key
            FROM employees e
            JOIN employee_employments ee
              ON ee.company_code = e.company_code
             AND (
               ee.legacy_employee_key = e.employee_key
               OR EXISTS (
                 SELECT 1 FROM employee_key_authority_map m
                  WHERE m.company_code = e.company_code
                    AND m.employee_key = e.employee_key
                    AND m.employment_id = ee.employment_id
                    AND m.mapping_status = 'active'
               )
             )
           WHERE ee.lifecycle_state = 'pending_start'
             AND lower(coalesce(nullif(btrim(e.employment_status), ''), 'active')) = 'active'
             {company_clause}
        )
        UPDATE employees e
          ...
          FROM targets t
         WHERE e.company_code = t.company_code
           AND e.employee_key = t.employee_key
        RETURNING e.employee_key
        """,
        tuple(params),
    )
    rows = cur.fetchall() or []
    # Keep authority employment_status in sync with lifecycle when CHECK allows.
    cur.execute(
        f"""
        UPDATE employee_employments
           SET employment_status = 'pending_start',
               updated_at = now()
         WHERE lifecycle_state = 'pending_start'
           AND employment_status = 'active'
           {"AND company_code=%s" if company_code else ""}
    ...
    return {
        "ok": True,
        "hub_rows_repaired": len(rows),
        "authority_rows_repaired": len(auth_rows),
    }
...
def ensure_lifecycle_schema(cur) -> None:
    ...
    # Projection repair: pending_start must not remain hub-active after schema ensure.
    try:
        repair_pending_start_hub_projection(cur, company_code=None)
    except Exception:
        pass
    cur.execute(
        """
...
# Hub / people-list projection of lifecycle. pending_start must NEVER collapse to
# "active" (P2/P3/P4/P5): visible as joining/future, excluded from active headcount
# and normal post-hire authority.
HUB_STATUS_LEFT = "left"
HUB_STATUS_ACTIVE = "active"
HUB_STATUS_PENDING_START = "pending_start"

# Authority employment_status on employee_employments (expanded beyond Wave2 active|left).
AUTHORITY_EMPLOYMENT_STATUSES = frozenset({"active", "left", "pending_start"})


def hub_status_for_lifecycle(lifecycle_state: str) -> str:
    state = str(lifecycle_state or "").strip().lower()
    if state == "terminated":
        return HUB_STATUS_LEFT
    if state == "pending_start":
        return HUB_STATUS_PENDING_START
    return HUB_STATUS_ACTIVE


def is_active_hub_status(status: str | None) -> bool:
    """True only for truly active employment. Empty/null = legacy active default."""
    s = str(status or "").strip().lower()
    if not s:
        return True
    return s == HUB_STATUS_ACTIVE


def is_pending_start_hub_status(status: str | None) -> bool:
    return str(status or "").strip().lower() in {HUB_STATUS_PENDING_START, "joining", "future_start"}


def sql_active_employment_predicate(alias: str = "e") -> str:
    """SQL fragment: countable/active workforce only (excludes pending_start and left)."""
    col = f"{alias}.employment_status" if alias else "employment_status"
    return (
        f"lower(coalesce(nullif(btrim({col}), ''), 'active')) = 'active'"
    )
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            assert_lifecycle_synthetic_target(
                legacy, cur, company=company, employee_key=str(req.get("employee_key") or "")
            )
            if str(req.get("requester_user_id")) == actor:
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            for employment in rows:
                emp_key = str(employment.get("employee_key") or "")
                if lifecycle_synthetic_only_enabled():
                    markers = load_lifecycle_synthetic_markers(cur, company=company, employee_key=emp_key)
            ...
            conn.commit()
    return {"ok": True, "executed": executed, "count": len(executed), "skipped": skipped}
