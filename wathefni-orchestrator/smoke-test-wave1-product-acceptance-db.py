...
def prove_task_sync_and_surfaces(cur, company: str) -> None:
    ...
    # Disabled module fail-closed (company_modules AND requisition_settings)
...
def prove_product_surface_wiring() -> None:
    ...
    sentinel = web / "lib/workspaceCapability.ts"
    if not sentinel.is_file():
        check("surface wiring (monorepo apps absent here — covered by unit smoke)", True)
        return
    checks = [
        ...
        ("nav capability requisitions", sentinel),
    ]
    for label, path in checks:
        check(label, path.is_file(), path)
    cap = sentinel.read_text(encoding="utf-8")
...
def main() -> int:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            sections = [
                ("setup console policies", lambda: prove_setup_policies(cur, company)),
                ("modularity matrix", lambda: (prove_modularity_matrix(cur, allow), _flags_for(*allow))),
                ("task sync + fail-closed", lambda: prove_task_sync_and_surfaces(cur, company)),
                ("visual canary WATHEFNI", lambda: prove_visual_canary(cur, allow)),
            ]
            for label, fn in sections:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    print(f"FAIL exception in {label}: {exc}")
                    import traceback
