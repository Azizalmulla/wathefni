...
def is_wave6_synthetic_employee(
    *,
    ...
    if isinstance(raw, dict) and (raw.get("shw6") or raw.get("shw6a") or raw.get("shw6b")):
        return True
    if isinstance(raw, str):
        low = raw.lower()
        if '"shw6"' in low or '"shw6a"' in low or '"shw6b"' in low:
            return True
    return False
...
def upsert_compliance_profile(
    ...
    *,
    ...
    # Phase 3B: refuse calendar/rest/holiday forks — working calendar is Setup Console (Payroll).
    calendar_keys = {
        "rest_weekdays",
        "weekend_days",
        "rest_days",
    ...
    for rule in rules:
        ...
        for k, v in rule.items():
            if k in {"money", "amount", "allowance", "payroll"}:
                continue
            if k in calendar_keys or str(k).endswith("_holidays"):
                calendar_stripped += 1
                continue
            r[k] = v
        cleaned.append(r)
    if calendar_stripped:
        # Soft-strip rather than hard-fail so existing clients keep working; Setup owns calendar.
        pass
    mode = str(payload.get("default_enforcement") or "warn").strip().lower()
    if mode not in {"warn", "block"}:
        mode = "warn"
    # Also refuse top-level calendar fields on the profile payload
    for banned in calendar_keys:
    ...
    out = {"ok": True, "profile": _row(cur), **honesty_payload()}
    if calendar_stripped:
        out["calendar_fields_stripped"] = calendar_stripped
        out["calendar_owner"] = "setup_console_payroll_working_calendar"
        out["setup_href"] = "/setup-console#classic-payroll-setup-calendar"
    return out
