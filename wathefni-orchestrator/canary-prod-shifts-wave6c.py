...
def canary_real_rows(cur, today: date) -> list[str]:
    """Active future rows on the real subject that carry this canary's provenance.

    Provenance is the only selector: a row must be future-dated, owned by the approved
    HR actor and carry a wave6c audit reason. Genuine business rows can never match.
    """
    cur.execute(
        """
        ...
        WHERE company_code=%s AND employee_key=%s AND shift_date >= %s
          ...
          AND created_by_phone = %s
          AND metadata::text ILIKE %s
        ORDER BY shift_date, start_time
        """,
        (COMPANY, TALAL_KEY, today.isoformat(), HR_ACTOR, "%wave6c%"),
    )
    return [dict(r)["shift_id"] for r in cur.fetchall()]


def soft_cancel_real(shift_ids, note: str) -> tuple[int, list]:
    ok = 0
    errors: list = []
    for sid in shift_ids:
        try:
            res = app.cancel_shift_assignment(
                {
                    ...
                    "shift_id": sid,
                    "reason": f"wave6c controlled canary: {note}",
            ...
            if res.get("ok"):
                ...
                errors.append({sid: res.get("error")})
        except Exception as exc:  # noqa: BLE001
            errors.append({sid: str(exc)[:200]})
    return ok, errors
...
def main() -> int:
    ...
    # A previous canary run may have left future-dated rows on the real subject. Retire
    # them before the baseline so the baseline is the true pre-canary production state.
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            stale_rows = canary_real_rows(cur, today)
        conn.commit()
    if stale_rows:
        swept, sweep_errors = soft_cancel_real(stale_rows, "retire prior-run residue")
        RESULTS["prior_run_sweep"] = {"found": stale_rows, "cancelled": swept, "errors": sweep_errors}
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                DELETE FROM shift_notification_consent
                WHERE company_code=%s AND employee_key=%s AND channel='email'
                  AND granted_by LIKE %s
                """,
                (COMPANY, TALAL_KEY, "owner:%"),
            )
        conn.commit()
    check("prior_run_residue_retired", not stale_rows or int(RESULTS["prior_run_sweep"]["cancelled"]) == len(stale_rows), RESULTS.get("prior_run_sweep"))
    talal_history_baseline = len([
        r for r in baseline["assignments"]
        if r["employee_key"] == TALAL_KEY and str(r["shift_date"]) < today.isoformat()
    ])
    talal_active_future_baseline = len([
        r for r in baseline["assignments"]
        if r["employee_key"] == TALAL_KEY and str(r["shift_date"]) >= today.isoformat() and r["status"] != "cancelled"
    ])
    check("real_subject_has_no_live_future_schedule", talal_active_future_baseline == 0, talal_active_future_baseline)
    ...
    with app.db_connect() as conn:
        ...
        conn.commit()
    check("created_rows_visible_in_db", len(created) == 4, [len(created), created])
    ...
    check("no_unexpected_active_real_rows", {r["shift_date"] for r in created} == {d1.isoformat(), d2.isoformat(), d3.isoformat()}, sorted({r["shift_date"] for r in created}))
    ...
    # The live orchestrator can hold DDL locks while serving traffic; a seed that loses a
    # deadlock is a scheduling accident, not a Shifts failure, so retry a bounded number.
    seed_error = None
    for attempt in range(4):
        try:
            ...
            seed_error = str(exc)[:200]
            time.sleep(2 * (attempt + 1))
    check("synthetic_manager_subjects_seeded", seed_error is None, seed_error)
    ...
    # Overlapping execution: hold the advisory lock on one session, then prove a second
    # session refuses to run the same job instead of processing concurrently.
    overlap_lock_id = w6c.JOB_LOCK_CONTROLLED_BASE + 42
    ran = {"called": False}
    with app.db_connect() as holder:
        with holder.cursor() as hcur:
            hcur.execute("SELECT pg_advisory_lock(%s)", (overlap_lock_id,))
            hcur.fetchone()
        holder.commit()
        try:
            with app.db_connect() as conn:
                with conn.cursor() as cur:
                    def should_not_run(_: int) -> dict:
                        ran["called"] = True
                        return {}

                    out = w6c.run_controlled_job(
                        cur, company_code=COMPANY, job_key=f"w6c-overlap-{TAG}",
                        handler=should_not_run, batch_size=10, lock_offset=42,
                    )
                conn.commit()
        finally:
            with holder.cursor() as hcur:
                hcur.execute("SELECT pg_advisory_unlock(%s)", (overlap_lock_id,))
                hcur.fetchone()
            holder.commit()
    check("no_overlapping_execution", out.get("reason") == "advisory_lock_held", out.get("reason"))
    check("locked_out_job_did_not_process", ran["called"] is False)
    check("locked_out_run_recorded", str((out.get("run") or {}).get("status")) == "skipped_locked", out.get("run"))

    with app.db_connect() as conn:
        with conn.cursor() as cur:
            after = w6c.run_controlled_job(
                cur, company_code=COMPANY, job_key=f"w6c-overlap-{TAG}",
                handler=lambda n: {"claimed": 0, "processed": 0, "failed": 0},
                batch_size=10, lock_offset=42,
            )
        conn.commit()
    check("job_runs_again_once_lock_released", after.get("skipped") is False, after.get("reason"))
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            live = canary_real_rows(cur, today)
        conn.commit()
    targets = sorted(set(live))
    created_ids = sorted({s for s in real_shift_ids if s and s != "None"})
    cancelled, cancel_errors = soft_cancel_real(targets, "soft-cancel after proof")
    if cancel_errors:
        RESULTS["cancel_errors"] = cancel_errors
    check("real_canary_rows_soft_cancelled", cancelled == len(targets) and len(targets) > 0, {"cancelled": cancelled, "targets": targets, "errors": cancel_errors})

    keep = sorted(set(targets) | set(created_ids))
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT count(*) AS c FROM shift_assignments WHERE shift_id::text = ANY(%s)",
                (keep or ["00000000-0000-0000-0000-000000000000"],),
            )
            still_present = int(dict(cur.fetchone())["c"])
            cur.execute(
                "SELECT count(*) AS c FROM shift_assignments WHERE shift_id::text = ANY(%s) AND status <> 'cancelled'",
                (created_ids or ["00000000-0000-0000-0000-000000000000"],),
            )
            created_still_active = int(dict(cur.fetchone())["c"])
        conn.commit()
    check("soft_cancel_not_hard_delete", still_present == len(keep), {"present": still_present, "expected": len(keep)})
    check("every_created_row_retired", created_still_active == 0, created_still_active)
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            check("historical_rows_untouched_count", hist == talal_history_baseline, {"now": hist, "baseline": talal_history_baseline})
        conn.commit()
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            residual = count_synthetic_residuals(cur, scope)
        conn.commit()
