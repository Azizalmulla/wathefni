...
DEFAULT_SYNTHETIC_KEY_MARKERS = ("SHW1B", "SHW1B-SYNTH|", "SHW1", "SHW1-SYNTH|")
DEFAULT_SYNTHETIC_PHONE_PREFIXES = ("965529", "965528")
...
# Governed schedule category for healthy-tile visual rhythm (EN/AR labels share these keys).
# Do not derive from free-text role/location. Legacy / unset → general.
ASSIGNMENT_TYPES = frozenset({"guest", "operations", "night", "event", "general"})
DEFAULT_ASSIGNMENT_TYPE = "general"
...
ALTER TABLE shift_assignments ADD COLUMN IF NOT EXISTS assignment_type text NOT NULL DEFAULT 'general';
...
def ensure_shifts_authority_wave1_schema(cur: Any) -> None:
    ...
    # CHECK added separately so ADD COLUMN IF NOT EXISTS stays idempotent on older rows.
    cur.execute(
        """
        DO $$
        BEGIN
          IF NOT EXISTS (
            SELECT 1 FROM pg_constraint WHERE conname = 'shift_assignment_type_chk'
          ) THEN
            ALTER TABLE shift_assignments
              ADD CONSTRAINT shift_assignment_type_chk
              CHECK (assignment_type IN ('guest','operations','night','event','general'));
          END IF;
        END $$;
        """
    )
...
def normalize_assignment_type(value: Any, *, reject_unknown: bool = False) -> str | None:
    """Return a governed assignment_type key, or None when reject_unknown and invalid."""
    raw = str(value or "").strip().lower()
    if not raw:
        return DEFAULT_ASSIGNMENT_TYPE
    if raw in ASSIGNMENT_TYPES:
        return raw
    if reject_unknown:
        return None
    return DEFAULT_ASSIGNMENT_TYPE
    ...
    *,
    ...
    if key and company:
        try:
            ...
            pass
        # Hub employees row fallback (swap/reassign paths often pass key-only stubs)
        if not facts.get("hub_status") or not facts.get("end_date"):
            try:
                cur.execute(
                    """
                    SELECT employment_status, raw_json
                    FROM employees
                ...
                if hub_row:
                    hr = dict(hub_row)
                    if not facts.get("hub_status"):
                        facts["hub_status"] = str(hr.get("employment_status") or "").strip().lower()
                    raw = hr.get("raw_json") if isinstance(hr.get("raw_json"), dict) else {}
                    facts["end_date"] = (
                        facts["end_date"]
                        or _as_date(raw.get("end_date"))
                        or _as_date(raw.get("termination_date"))
                        or _as_date(raw.get("employment_end_date"))
                    )
                    facts["start_date"] = (
                        facts["start_date"]
                        or _as_date(raw.get("start_date"))
                        or _as_date(raw.get("hire_date"))
                    )
                    if not facts.get("lifecycle_state") and facts["hub_status"] in {"terminated", "left"}:
                        facts["lifecycle_state"] = facts["hub_status"]
            except Exception:
                pass
    return facts
...
def evaluate_shift_lifecycle(
    *,
    ...
    # Hub/lifecycle pending_start is never schedulable until activation (P4).
    if settings.get("block_future_start", True) and (
        state == "pending_start" or hub in {"pending_start", "joining"}
    ):
        return {
            "ok": False,
            ...
            "operation": op,
            "employment_start": start_d.isoformat() if start_d else None,
            "shift_start": shift_start.isoformat(),
            "message": "Shifts are not allowed for pending-start / joining employees until activation.",
        }
