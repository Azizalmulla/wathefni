...
def main() -> int:
    ...
    check("version", w2.LEAVE_POLICY_WAVE2_VERSION == "2.1.0")
    check("carryover disabled", w2.carryover_enabled(w2.KUWAIT_PRIVATE_PACK) is False)
    elig = w2.resolved_eligibility_months()
    check("eligibility resolved to 6", elig.get("months") == 6 and elig.get("public_source_verified") is True, elig)
    check(
        "eligibility 9 obsolete in resolution",
        (elig.get("resolution") or {}).get("obsolete_figure_months") == 9,
        elig,
    )
    check(
        "eligibility statutory not unresolved",
        any(r["field"] == "annual_eligibility_months" and r["classification"] == "statutory" for r in w2.OFFICIAL_SOURCE_MATRIX),
    )
    check("law85 gazette hash present", len(w2.LAW_85_2017_GAZETTE_SHA256) == 64)
    check("carryover art72 statutory", any(r["field"] == "carryover_rules" and r["classification"] == "statutory" for r in w2.OFFICIAL_SOURCE_MATRIX))
    check("enforced false pack", w2.KUWAIT_PRIVATE_PACK["enforced"] is False and w2.KUWAIT_PRIVATE_PACK["legal_reviewed"] is False)
    # Pure chargeable
        ...
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                gate = w2.ensure_holiday_calendar_for_enforced(
                    cur, start_date=date(2026, 2, 22), end_date=date(2026, 2, 28)
                )
        check(
            "enforced holiday fail-closed while year pending_review",
            gate.get("ok") is False and gate.get("fail_closed") is True,
            gate,
        )
