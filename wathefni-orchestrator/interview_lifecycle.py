...
def ensure_interview_notes_concurrency_schema(cur: Any) -> None:
    """Wave 5 additive columns for interview notes optimistic concurrency."""
    cur.execute(
        """
        ALTER TABLE IF EXISTS candidate_interviews
          ADD COLUMN IF NOT EXISTS notes_version bigint NOT NULL DEFAULT 0,
...
MEETING_TYPES = frozenset({"in_person", "phone", "manual_link", "google_meet", "microsoft_teams", "async_video", "none"})
...
def ensure_interview_schema(cur: Any) -> None:
    ...
    ensure_interview_notes_concurrency_schema(cur)
    cur.execute(
        """
        ALTER TABLE IF EXISTS candidate_interviews
