...
def execute_exit_close(
    ...
    *,
    ...
    # Stale check against caller version BEFORE readiness re-eval (which bumps row_version)
    ...
    already_left = status_now in ("left", "terminated", "inactive")
    ...
    # Close case first (use post-evaluate row_version), then employment — avoids partial left without closed
    cur.execute(
        """
        UPDATE exit_close_cases
           SET status='closed',
    ...
    if not already_left:
        cur.execute(
            ...
            UPDATE employees
               SET employment_status='left',
                   ...
                   updated_at=now()
             WHERE company_code=%s AND employee_key=%s
               AND employment_status NOT IN ('left','terminated','inactive')
        ...
        # Repair path: employment already left; finish close/alumni without reopen
        cur.execute(
            """
            UPDATE employees
