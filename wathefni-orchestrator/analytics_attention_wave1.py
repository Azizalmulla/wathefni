...
Wave 1-B production synthetic posture is gated by env flags and an ACK table.
"""
...
ANALYTICS_WAVE1_VERSION = "1.0.0"
ANALYTICS_WAVE1_CONTRACT = "analytics_attention_wave1"
ANALYTICS_WAVE1_SYNTHETIC_MARKERS_DEFAULT = ("ANW1", "ANW1-SYNTH|")
ANALYTICS_WAVE1_SYNTHETIC_PHONE_PREFIX_DEFAULT = ("965540",)
...
def analytics_wave1_enabled() -> bool:
    return _truthy(os.environ.get("WATHEFNI_ANALYTICS_WAVE1"))


def analytics_wave1_synthetic_only() -> bool:
    # Fail closed: production synthetic posture requires explicit SYNTHETIC_ONLY.
    raw = os.environ.get("WATHEFNI_ANALYTICS_WAVE1_SYNTHETIC_ONLY")
    if raw is None or str(raw).strip() == "":
        return True
    return _truthy(raw)


def analytics_wave1_companies() -> set[str]:
    raw = str(os.environ.get("WATHEFNI_ANALYTICS_WAVE1_COMPANIES") or "WATHEFNI").strip()
    return {part.strip().upper() for part in raw.split(",") if part.strip()}


def analytics_wave1_enabled_for_company(company_code: str | None) -> bool:
    if not analytics_wave1_enabled():
        return False
    company = (company_code or "WATHEFNI").upper()
    allowed = analytics_wave1_companies()
    return not allowed or company in allowed


def synthetic_key_markers() -> tuple[str, ...]:
    raw = str(os.environ.get("WATHEFNI_ANALYTICS_WAVE1_SYNTHETIC_KEY_MARKERS") or ",".join(ANALYTICS_WAVE1_SYNTHETIC_MARKERS_DEFAULT))
    parts = tuple(part.strip() for part in raw.split(",") if part.strip())
    return parts or ANALYTICS_WAVE1_SYNTHETIC_MARKERS_DEFAULT


def synthetic_phone_prefixes() -> tuple[str, ...]:
    raw = str(os.environ.get("WATHEFNI_ANALYTICS_WAVE1_SYNTHETIC_PHONE_PREFIXES") or ",".join(ANALYTICS_WAVE1_SYNTHETIC_PHONE_PREFIX_DEFAULT))
    parts = tuple(part.strip() for part in raw.split(",") if part.strip())
    return parts or ANALYTICS_WAVE1_SYNTHETIC_PHONE_PREFIX_DEFAULT
...
def honesty_payload() -> dict[str, Any]:
    return {
        "wave": "analytics_wave1",
        "version": ANALYTICS_WAVE1_VERSION,
        "contract": ANALYTICS_WAVE1_CONTRACT,
        "read_only": True,
        "money_authority": False,
        "payroll_cost_analytics": False,
        ...
        "synthetic_only": analytics_wave1_synthetic_only(),
        "enabled": analytics_wave1_enabled(),
        "companies": sorted(analytics_wave1_companies()),
        "synthetic_key_markers": list(synthetic_key_markers()),
        "synthetic_phone_prefixes": list(synthetic_phone_prefixes()),
    }


def ensure_analytics_wave1_schema(cur: Any, *, force: bool = False) -> None:
    """Additive ACK / wave-audit table only. No frozen-module schema changes."""
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS analytics_wave_acks (
    ...
    cur.execute(
        """
        CREATE INDEX IF NOT EXISTS analytics_wave_acks_company_wave_idx
          ON analytics_wave_acks (company_code, wave, acknowledged_at DESC)
        """
    )
    if force:
        ...
        if not row.get("reg"):
            raise RuntimeError("analytics_wave_acks missing after ensure")


def record_analytics_wave_ack(
    cur: Any,
    *,
    company_code: str,
    environment: str,
    ...
    cur.execute(
        """
        INSERT INTO analytics_wave_acks (
        ...
        (
            ...
            "analytics_wave1",
            ANALYTICS_WAVE1_CONTRACT,
            environment,
            analytics_wave1_synthetic_only(),
            __import__("json").dumps(details or honesty_payload()),
            canary_tag,
        ),
    )
...
def residual_synthetic_acks(cur: Any, *, company_code: str, tag: str) -> int:
    cur.execute(
        """
        SELECT COUNT(*) AS n
        FROM analytics_wave_acks
        WHERE company_code=%s
          AND canary_tag=%s
          AND wave='analytics_wave1'
        """,
...
def cleanup_canary_acks(cur: Any, *, company_code: str, tag: str) -> int:
    cur.execute(
        """
        DELETE FROM analytics_wave_acks
        WHERE company_code=%s
          AND canary_tag=%s
          AND wave='analytics_wave1'
        """,
