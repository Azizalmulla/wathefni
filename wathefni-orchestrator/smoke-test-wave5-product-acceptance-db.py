...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Draft payroll exists but sealed money must cite sealed period authority
            ...
            check(
                "money uses sealed authority",
                money.get("status") == "ok"
                and (money.get("explain") or {}).get("sealed") is True
                and (money.get("explain") or {}).get("period_key") == "PAY-C7"
                and (money.get("explain") or {}).get("period_key") != "PAY-DRAFT",
                money,
            )
            # Pre-seal path already frozen in C4; C7 reproves draft period cannot be the money basis
            check(
                "draft is not money basis",
                (money.get("explain") or {}).get("period_key") != "PAY-DRAFT",
                money,
            )
            ...
            check(
                "genuine zero distinct",
                zero_hires.get("status") == "ok"
                and zero_hires.get("value") is not None
                and float(zero_hires["value"]) == 0.0,
                zero_hires,
            )
