...
# Rate-limit buckets are durable and lockouts last up to 30 minutes by design,
# so every run needs its own source address and operator identity. Reusing them
# would make a rerun inside the lock window fail on the previous run's lockout.
SOURCE = "10.%d.%d.%d" % (random.randrange(256), random.randrange(256), random.randrange(1, 255))
...
def login_throttle(client, app) -> None:
    ...
    app._rate_limit.reset(app, "dashboard_login", dimensions={"source": SOURCE, "identity": f"{CO_A}|{USER_A_EMAIL.lower()}"})
    ok = client.post(
        "/dashboard/auth/login",
...
def main() -> int:
    ...
    print(f"    tenants: {CO_A} / {CO_B}    source: {SOURCE}")
    _configure_env()
    ...
    cleanup(app)
    seed(app)
    client = TestClient(app.app, client=(SOURCE, 44100))
    ...
    print("\n    R2_SECURITY_FULL_PASS" if not FAIL else "\n    R2_SECURITY_FAIL")
