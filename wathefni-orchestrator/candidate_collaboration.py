...
# Wave 6: include hr_admin so authorized HR admins are eligible assignees.
RECRUITING_ROLES = frozenset({"owner", "hr_admin", "hr_manager", "recruiter", "hiring_manager"})
    ...
    cur.execute(
        ...
        BEGIN
          ...
          IF (
            ...
            OR COALESCE(NEW.role,'viewer') NOT IN ('owner','hr_admin','hr_manager','recruiter','hiring_manager')
          ) AND EXISTS (
            SELECT 1 FROM applications a
            WHERE a.owner_user_id=OLD.user_id
              AND a.company_code=OLD.company_code
            ...
            if int(note.get("version") or 0) != version:
                ...
                raise CollaborationError(
                    ...
                    details=_cs.conflict_envelope(
                        code="stale_note_version",
                        current_version=int(note.get("version") or 0),
                        expected_version=version,
                        current_updated_at=note.get("updated_at"),
                        last_changed_by={"user_id": note.get("updated_by_user_id") or note.get("created_by_user_id")},
                        entity_type="application_note",
                        entity_id=str(note_uuid),
                    ),
                )
            ...
            if not updated:
                ...
                raise CollaborationError(
                    ...
                    details={
                        ...
                        "entity_type": "application_note",
                        "entity_id": str(note_uuid),
                        "safe_next_action": _cs.SAFE_NEXT_ACTION,
                    },
                )
            cur.execute(
                """
            ...
            if int(task.get("version") or 0) != version:
                ...
                raise CollaborationError(
                    ...
                    details=_cs.conflict_envelope(
                        ...
                        expected_version=version,
                        current_updated_at=task.get("updated_at"),
                        last_changed_by={"user_id": task.get("updated_by_user_id") or task.get("assigned_to_user_id")},
                        entity_type="application_task",
                        entity_id=str(task_uuid),
                    ),
                )
