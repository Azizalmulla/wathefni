...
def rate_limit_contracts() -> None:
    ...
    _clear("WATHEFNI_TRUSTED_PROXY_HOSTS")
    check(
        "a direct caller's X-Forwarded-For is ignored",
        rl.client_source(_Req("203.0.113.9", "1.2.3.4")) == "203.0.113.9",
    )
    check(
        "behind the edge proxy the appended hop wins, not the caller's claim",
        rl.client_source(_Req("127.0.0.1", "1.2.3.4, 198.51.100.7")) == "198.51.100.7",
    )
    check(
        "a single trusted hop is still honoured",
        rl.client_source(_Req("127.0.0.1", "198.51.100.7")) == "198.51.100.7",
    )
    check("a peer with no header is used as-is", rl.client_source(_Req("127.0.0.1")) == "127.0.0.1")
    check("an unknown peer is labelled", rl.client_source(_Req("")) == "unknown")
    os.environ["WATHEFNI_TRUSTED_PROXY_HOSTS"] = "10.0.0.5"
    check(
        "the trusted proxy set is configurable for other topologies",
        rl.client_source(_Req("10.0.0.5", "1.2.3.4, 198.51.100.7")) == "198.51.100.7"
        and rl.client_source(_Req("127.0.0.1", "1.2.3.4")) == "127.0.0.1",
    )
    _clear("WATHEFNI_TRUSTED_PROXY_HOSTS")
