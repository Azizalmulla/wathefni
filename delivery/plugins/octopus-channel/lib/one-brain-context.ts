...
export type OneBrainNextRequiredAction = {
  action: import("./directive-reply-registry").DirectiveAction;
  field: string | null;
...
export function formatOneBrainLiveChannelContext(params: {
  ...
  lines.push("  10. Server-composed directive replies — for collection-flow directives (`ASK_SENDER_NAME_AND_PHONE_DECISION`, `ASK_SENDER_PHONE`, `ASK_RECIPIENT_NAME_AND_PHONE`, `ASK_PICKUP_ADDRESS`, `ASK_DELIVERY_ADDRESS`, `CONFIRM_SLOT_CONFLICT`) the SERVER composes your outbound reply from state. Your draft reply on these turns is discarded — what matters is the OP layer: call `apply_booking_field` for any values the customer just provided, call the right tools for any other state transitions, and be concise. Do not agonise over phrasing the ask: the server will render it, and it will name the correct field in the correct language. Your tool calls and intent routing still drive the state machine.");
