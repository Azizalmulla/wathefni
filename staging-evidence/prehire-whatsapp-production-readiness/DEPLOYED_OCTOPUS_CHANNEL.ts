...
async function handleInboundMessage(params: {
  ...
  if (replyTarget) {
    ...
    if (orchestratorReply?.authoritative && orchestratorReply.reply_text) {
      await sendOctopusTextReply({
        api,
        account,
        conversationId,
      ...
      api.logger.info(
        `[octopus] timing stage=hr_orchestrator_authoritative conversation=${conversationId} source=${orchestratorReply.final_reply_source || "-"} elapsedMs=${Date.now() - receivedAtMs}`,
      );
      return;
    }
    if (account.agentId === "wathefni-hr" && senderRole === "hr_admin") {
      await sendOctopusTextReply({
        api,
        account,
        conversationId,
      ...
      api.logger.warn(
        `[octopus] hr orchestrator fail-closed conversation=${conversationId} authoritative=${orchestratorReply?.authoritative ?? "null"} source=${orchestratorReply?.final_reply_source || "-"} elapsedMs=${Date.now() - receivedAtMs}`,
      );
      return;
    }
  }
