/**
 * Payroll Wave 2A-C/D — HR external payroll operations workspace.
 * Wave 2A-D: setup status, run checklist, package honesty, rollback concurrency,
 * quarantine acknowledge, CSV guide. Never claims money authority in Wathefni.
 */
...
import {
  ...
  postExternalPayrollQuarantineAcknowledge,
...
import {
  ...
  payrollExternalCopy,
  statusTone,
  type PayrollExternalLocale,
} from '@/posthire/payrollExternalUx'
...
function friendlyError(error: unknown, fallback: string, locale: PayrollExternalLocale): string {
  ...
  if (error instanceof DashboardApiError) {
    ...
    if (k === 'concurrency_token_required' || k === 'stale_row_version_or_bad_status') return c.rollback
...
export function ExternalPayrollWorkspace({
  ...
  const [tab, setTab] = useState<'overview' | 'exports' | 'quarantine' | 'history' | 'help'>('overview')
  ...
  const setup = (data?.setup as Record<string, unknown> | undefined) || (readiness?.setup as Record<string, unknown> | undefined) || {}
  const packageContents =
    (data?.package_contents as { included?: string[]; not_included_yet?: string[]; export_csv_headers?: string[]; import_csv_headers?: string[] } | undefined) ||
    (readiness?.package_contents as { included?: string[]; not_included_yet?: string[]; export_csv_headers?: string[]; import_csv_headers?: string[] } | undefined) ||
    {}
  ...
  const onRollback = async (exportRunId: string) => {
    ...
    const rowVersion = Number(selectedExport?.row_version)
    if (!Number.isFinite(rowVersion)) {
      onNotice(c.retry, 'error')
      return
    }
    setBusy(true)
    try {
      await postExternalPayrollRollback(access, exportRunId, {
        reason: reason.trim(),
  ...
  const onAcknowledge = async (quarantineId: string) => {
    if (!canManage || !requireReason()) return
    setBusy(true)
    try {
      await postExternalPayrollQuarantineAcknowledge(access, quarantineId, { reason: reason.trim() })
      onNotice(c.acknowledged, 'success')
      await reload()
    } catch (err) {
      onNotice(friendlyError(err, c.retry, locale), 'error')
    } finally {
  ...
  const openQuarantineCount = Number(data?.counts?.quarantine_open || quarantine.filter((q) => !q.acknowledged_at).length)

  const checklistDone = {
    setup: Boolean(setup.mode_ok) && Boolean(setup.payment_processing_ok),
    ready,
    export: exports.some((e) => String(e.status) === 'exported' || String(e.status) === 'superseded'),
    upload: (data?.imports || []).length > 0,
    reconcile: events.some((e) => String(e.event_type || '').includes('reconcil')),
  }
    ...
    <div className="space-y-5" dir={isAr ? 'rtl' : 'ltr'} data-testid="external-payroll-workspace">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" variant="ghost" onClick={() => void reload()} disabled={refreshing} aria-label={c.refresh}>
      ...
      {honestyOpen ? (
        <div className="space-y-2 rounded-2xl border border-dashed border-line/70 bg-[#fffaf0] px-4 py-3 text-sm text-muted" data-testid="external-payroll-honesty">
          <p>{c.honesty}</p>
      ...
      {tab === 'overview' ? (
        <div className="space-y-4">
          <div className="rounded-xl border border-line/50 bg-white/70 p-4" data-testid="external-payroll-setup">
            <h3 className="mb-3 text-[14px] font-semibold text-text">{c.setupTitle}</h3>
            <div className="grid gap-2 sm:grid-cols-2">
              {(
                [
                  [c.setupMode, setup.mode_label || setup.payroll_mode || '—', Boolean(setup.mode_ok)],
                  [c.setupContracts, String(setup.approved_contract_count ?? '—'), Boolean(setup.contracts_ok)],
                  [c.setupPeriods, String(setup.period_count ?? data?.periods?.length ?? '—'), Boolean(setup.periods_ok ?? (data?.periods || []).length)],
                  [c.setupPayment, 'disabled', Boolean(setup.payment_processing_ok)],
                ] as const
              ).map(([label, value, ok]) => (
                <div key={label} className="flex items-center justify-between gap-2 rounded-lg border border-line/40 px-3 py-2 text-[13px]">
                  <span>
                    <span className="text-subtle">{label}</span>
                    <span className="ms-2 font-medium text-text">{String(value)}</span>
                  </span>
                  <StatusPill tone={ok ? 'success' : 'warning'}>{ok ? c.setupOk : c.setupBlocked}</StatusPill>
                </div>
              ))}
            </div>
            <p className="mt-2 text-[12px] text-subtle">{c.setupModeHint}</p>
          </div>

          <div className="rounded-xl border border-line/50 bg-white/60 p-4" data-testid="external-payroll-checklist">
            <h3 className="mb-2 text-[14px] font-semibold">{c.checklistTitle}</h3>
            <ol className="space-y-1.5 text-[13px]">
              {(
                [
                  [c.checklistSetup, checklistDone.setup],
                  [c.checklistReady, checklistDone.ready],
                  [c.checklistExport, checklistDone.export],
                  [c.checklistUpload, checklistDone.upload],
                  [c.checklistReconcile, checklistDone.reconcile],
                  [c.checklistPayslips, false],
                  [c.checklistClose, false],
                ] as const
              ).map(([label, done]) => (
                <li key={label} className="flex items-center gap-2">
                  <StatusPill tone={done ? 'success' : 'neutral'}>{done ? '✓' : '·'}</StatusPill>
                  <span>{label}</span>
                </li>
              ))}
            </ol>
          ...
          <div className="rounded-xl border border-dashed border-line/60 bg-[#fffdf8] p-4" data-testid="external-payroll-package">
            <h3 className="mb-2 text-[14px] font-semibold">{c.packageTitle}</h3>
            <div className="grid gap-3 sm:grid-cols-2 text-[13px]">
              <div>
                <p className="mb-1 font-medium text-emerald-800">{c.packageIncluded}</p>
                <ul className="list-disc ps-4 text-subtle">
                  {(packageContents.included || ['employee_key', 'approved_compensation_components', 'contract_amounts', 'pay_period_dates']).map((x) => (
                    <li key={x}>{x}</li>
                  ))}
                </ul>
              </div>
              <div>
                <p className="mb-1 font-medium text-amber-900">{c.packageExcluded}</p>
                <ul className="list-disc ps-4 text-subtle">
                  {(packageContents.not_included_yet || []).map((x) => (
                    <li key={x}>{x}</li>
                  ))}
                </ul>
              </div>
            </div>
            <p className="mt-2 text-[12px] text-amber-900">{c.packageAttendanceNote}</p>
          </div>
                ...
                <ul className="space-y-1 text-[13px] text-rose-800">
                  {blockers.map((b) => (
                    <li key={b.code || b.message}>
                      <BlockedReason
                        locale={locale}
                        reason={b.message || (locale === 'ar' ? 'راجع متطلبات جاهزية الرواتب.' : 'Review payroll readiness requirements.')}
                      />
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {openQuarantineCount ? (
              <Button size="sm" variant="secondary" onClick={() => setTab('quarantine')}>
                {c.openQuarantine} ({openQuarantineCount})
              </Button>
            ) : null}
          </div>
              ...
              {selectedExport ? (
                <div className="space-y-3 rounded-xl border border-line/50 bg-white/70 p-4">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-[11px] text-subtle" title={String(selectedExport.input_fingerprint || '')}>
                      {c.inputSnapshot}: {String(selectedExport.input_fingerprint || '').slice(0, 10)}…
                    </span>
                  </div>
                  <p className="text-[12px] text-subtle">{c.inputSnapshotHint}</p>
      ...
      {tab === 'quarantine' ? (
        !quarantine.length ? (
          ...
          <ul className="divide-y divide-line/40 rounded-xl border border-rose-200/60 bg-rose-50/30">
            {quarantine.map((q) => {
              const acked = Boolean(q.acknowledged_at)
              return (
                <li key={String(q.quarantine_id)} className="space-y-2 px-3 py-2.5 text-[13px]">
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusPill tone={acked ? 'success' : 'danger'}>
                      {acked ? c.acknowledged : String(q.source_kind || c.quarantine)}
                    </StatusPill>
                  ...
                  {!acked && canManage ? (
                    <Button size="sm" variant="secondary" disabled={busy} onClick={() => void onAcknowledge(String(q.quarantine_id))}>
                      {c.acknowledge}
                    </Button>
                  ) : null}
                  {acked ? (
                    <p className="text-[11px] text-subtle">
                      {String(q.acknowledgement_reason || '')} · {String(q.acknowledged_at || '')}
                    </p>
                  ) : null}
                </li>
              )
      ...
      {tab === 'help' ? (
        <div className="space-y-4 rounded-xl border border-line/50 bg-white/70 p-4 text-[13px]" data-testid="external-payroll-csv-help">
          <h3 className="text-[14px] font-semibold">{c.csvHelpTitle}</h3>
          <p className="text-subtle">{c.csvHelpBody}</p>
          <div>
            <p className="mb-1 font-medium">{c.csvHelpExport}</p>
            <code className="block overflow-x-auto rounded bg-[#fffdf8] p-2 text-[12px]">
              {(packageContents.export_csv_headers || []).join(', ')}
            </code>
          </div>
          <div>
            <p className="mb-1 font-medium">{c.csvHelpImport}</p>
            <code className="block overflow-x-auto rounded bg-[#fffdf8] p-2 text-[12px]">
              {(packageContents.import_csv_headers || []).join(', ')}
            </code>
          </div>
          <p className="text-[12px] text-amber-900">{c.packageAttendanceNote}</p>
        </div>
      ) : null}
    </div>
  )
