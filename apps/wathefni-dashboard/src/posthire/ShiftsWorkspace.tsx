...
import {
  ...
  shiftsFiltersKey,
  shiftsWeekCacheKey,
  type ShiftsBoardCommit,
} from '@/posthire/shiftsBoardRange'
...
function parseIsoDateLocal(raw: string): Date {
  const [y, m, d] = raw.slice(0, 10).split('-').map(Number)
  return new Date(y, m - 1, d)
}

/**
 * Soft-keep board navigation:
 * - paint the last committed week/day until the latest request finishes
 * - ignore slower older responses
 * - cache + prefetch adjacent weeks
 */
function useShiftsData(
  ...
  const filtersKey = useMemo(() => shiftsFiltersKey(filters), [filters])
  const anchorIso = useMemo(() => isoDateLocal(startOfDay(anchor)), [anchor])
  const [committed, setCommitted] = useState<ShiftsBoardCommit | null>(null)
  const [refreshing, setRefreshing] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const requestIdRef = useRef(0)
  const cacheRef = useRef(new Map<string, PosthireShiftsResponse>())
  const boardScrollRef = useRef<HTMLDivElement | null>(null)
  const savedScrollRef = useRef<{ left: number; top: number } | null>(null)
  const targetRef = useRef({ week, view, anchorIso, filtersKey, filters })
  targetRef.current = { week, view, anchorIso, filtersKey, filters }

  const rememberScroll = useCallback(() => {
    const node = boardScrollRef.current?.querySelector('[data-shifts-roster-grid], [data-shifts-mobile-agenda]') as HTMLElement | null
    if (!node) return
    savedScrollRef.current = { left: node.scrollLeft, top: node.scrollTop }
  }, [])
  ...
  useEffect(() => {
    restoreScroll()
  }, [committed, restoreScroll])

  const fetchWeek = useCallback(
    async (targetWeek: number, targetFilters: Record<string, string>, targetFiltersKey: string) => {
      const cacheKey = shiftsWeekCacheKey(targetWeek, targetFiltersKey)
      const cached = cacheRef.current.get(cacheKey)
      if (cached) return cached
      // Always load the week window; day view filters client-side so intra-week day
      // steps stay soft and do not blank the board.
      const next = await getPosthireShifts(access, targetWeek, {
        view: 'week',
        limit: 200,
        ...targetFilters,
      })
  ...
  const prefetchAdjacent = useCallback(
    (centerWeek: number, targetFilters: Record<string, string>, targetFiltersKey: string) => {
      for (const neighbor of adjacentWeeks(centerWeek, 1)) {
        const key = shiftsWeekCacheKey(neighbor, targetFiltersKey)
        if (cacheRef.current.has(key)) continue
        void fetchWeek(neighbor, targetFilters, targetFiltersKey).catch(() => {
  ...
  const reload = useCallback(async () => {
    ...
    const cacheKey = shiftsWeekCacheKey(requestedWeek, requestedFiltersKey)
    const cached = cacheRef.current.get(cacheKey)
    if (cached) {
      const live = targetRef.current
      const immediate = commitIfLatest({
        requestId,
        latestRequestId: requestIdRef.current,
        targetWeek: live.week,
        targetView: live.view,
        targetAnchorIso: live.anchorIso,
        targetFiltersKey: live.filtersKey,
        requestedWeek,
        requestedFiltersKey,
        data: cached,
      })
      if (immediate) {
        applyCommit(immediate)
        setRefreshing(false)
        setError(null)
        prefetchAdjacent(requestedWeek, requestedFilters, requestedFiltersKey)
        return
      }
      // A newer navigation owns the board; do not refetch the abandoned range here.
    ...
    setError(null)
    try {
      const next = await fetchWeek(requestedWeek, requestedFilters, requestedFiltersKey)
      const live = targetRef.current
      const commit = commitIfLatest({
        requestId,
        latestRequestId: requestIdRef.current,
        targetWeek: live.week,
        targetView: live.view,
  ...
  // Day/week chrome may move before the board commits — board paints committed only.
  const displayData = useMemo(() => {
    if (!committed) return null
    if (committed.view === 'day') return filterShiftsForDay(committed.data, committed.anchorIso)
    return committed.data
  }, [committed])

  const displayAnchor = committed ? parseIsoDateLocal(committed.anchorIso) : anchor
  const displayView = committed?.view || view
  const rangePending = Boolean(
    committed &&
      (committed.week !== week || committed.view !== view || committed.anchorIso !== anchorIso || committed.filtersKey !== filtersKey),
  )

  return {
    data: displayData,
    rawData: committed?.data || null,
    refreshing,
    error,
    reload,
    coldLoading: !committed,
    rangePending,
    displayAnchor,
    displayView,
    committedWeek: committed?.week ?? null,
    committedAnchorIso: committed?.anchorIso ?? null,
    boardScrollRef,
    /** Invalidate week cache entries after mutations so next paint is fresh. */
    invalidateCache: () => {
...
export function ShiftsWorkspace({ access, permissions, role: _role, onNotice, onAccessIssue }: ShiftsWorkspaceProps) {
  ...
  const {
    ...
    boardScrollRef,
    invalidateCache,
  } = useShiftsData(access, week, view, anchor, filterKey, onAccessIssue)

  const committedWeekMarker = committedWeek == null ? '' : String(committedWeek)
  const committedAnchorMarker = committedAnchorIso || ''
  ...
  // Keep Branch/Site/Team/Location visible when required, populated, or actively used in the org tree.
  // Otherwise tuck unused optional dimensions under Advanced Organization.
  const orgFieldPrimary = {
    branch: Boolean(form.branch_key.trim()) || branchUnits.length > 0,
    site: Boolean(form.site_key.trim()) || composerSiteOptions.length > 0,
    team: Boolean(form.team_key.trim()) || composerTeamOptions.length > 0,
    location: Boolean(form.location.trim()) || composerLocationOptions.length > 0,
  }
  const orgHasAdvanced = Object.values(orgFieldPrimary).some((v) => !v)
  ...
  // Board paints the committed range only — chrome may lead with the pending selection.
  const range = useMemo(() => {
    if (displayView === 'day') {
      const start = startOfDay(displayAnchor)
      return { days: [start], start, end: addDays(start, 1) }
    }
    const weekStart = startOfWeek(displayAnchor)
    const days = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i))
    return { days, start: weekStart, end: addDays(weekStart, 7) }
  }, [displayAnchor, displayView])

  const rangeLabel = useMemo(() => {
    const opts: Intl.DateTimeFormatOptions = { month: 'short', day: 'numeric', year: 'numeric' }
    const loc = isAr ? 'ar' : 'en'
    // Paint from the committed board range so chrome never advertises a week the
    // board has not atomically replaced yet (avoids “stale week” mismatch).
    if (displayView === 'day') return startOfDay(displayAnchor).toLocaleDateString(loc, opts)
    const weekStart = startOfWeek(displayAnchor)
    const weekEnd = addDays(weekStart, 6)
    return `${weekStart.toLocaleDateString(loc, opts)} – ${weekEnd.toLocaleDateString(loc, opts)}`
  }, [isAr, displayAnchor, displayView])
  ...
  const openCreate = (day?: Date, employeeName?: string) => {
    setComposerOpen(true)
    setSelectedId(null)
    setSelectedSnapshot(null)
    setEditMode(false)
    setOrgAdvancedOpen(false)
    setForm((f) => ({
      ...f,
      employee_name: employeeName?.trim() || f.employee_name,
  ...
  const runAction = async (actionType: string, args: Record<string, unknown>, key: string) => {
    ...
    try {
      ...
      if (
        ...
        onNotice(surface || String(result.result?.message || result.message || c.retry), 'error')
        return
      }
  ...
  const submitCreate = async () => {
    ...
    try {
      const res = await createShift(access, {
        ...
        assignment_type: form.assignment_type || 'general',
  ...
  return (
    ...
    >
      <ConfigureInSetupBanner
        title={isAr ? 'سياسة جدولة المناوبات' : 'Company shifts policy'}
        body={
          isAr
            ? 'تعارض الإجازة والنشر الافتراضي وأيام الراحة (من تقويم الرواتب) في وحدة الإعداد. هنا القوالب والجداول والنشر.'
            : 'Leave conflict, publish defaults, and rest days (from Payroll calendar) live in Setup Console. This workspace owns templates, rosters, and publishing.'
        }
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-wf-ink/[0.06] pb-2.5" data-shifts-header>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {surface === 'schedule' && requestCount > 0 ? (
            ...
            >
              {c.attentionWaiting(requestCount)}
            </button>
          ) : null}
        </div>

        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted hover:text-ink" onClick={() => void reloadFresh()} disabled={refreshing} aria-label={c.refresh}>
      ...
      {/* Schedule Range & View Controls */}
      {surface === 'schedule' ? (
        <div className="flex flex-wrap items-center gap-2 pt-0.5" data-shifts-date-chrome>
          ...
          <span
            className="px-1 text-[13px] font-semibold tracking-[-0.01em] text-ink"
            data-shifts-range-label
            data-shifts-range-pending={rangePending ? 'true' : 'false'}
          >
            {rangeLabel}
          </span>
        </div>
      ...
      {error ? (
        <div className="rounded-[1.1rem] bg-wf-accent-review-soft/70 px-4 py-3 text-sm text-wf-accent-review-ink">
          ...
          <button type="button" className="mt-2 text-xs font-semibold underline" onClick={() => void reloadFresh()}>
            {c.retry}
          </button>
        </div>
      ) : null}
      ...
      <div className="grid gap-4">
        ...
        <div>
          {queue === 'board' ? (
            <div
              ...
              data-shifts-range-pending={rangePending ? 'true' : 'false'}
              data-shifts-committed-week={committedWeekMarker}
              data-shifts-target-week={week}
              data-shifts-committed-anchor={committedAnchorMarker}
              ref={boardScrollRef}
            >
              {refreshing || rangePending ? (
                <div
              ...
              <ShiftsRosterBoard
                ...
                locale={locale}
                isMobile={isMobile || displayView === 'day'}
        ...
        {asideOpen ? (
          <>
            <button
              type="button"
              className="fixed inset-0 z-50 bg-wf-ink/10 backdrop-blur-[1px]"
            ...
            {composerOpen ? (
              <div className="space-y-0" data-testid="shifts-create-card" data-shifts-composer-org>
                <section className="border-b border-wf-ink/[0.07] py-4" data-shifts-drawer-section="shift">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2.5">
                    ...
                    <label className="col-span-2 block space-y-1 text-[10px] font-medium text-muted">
                      ...
                      <select
                        className="h-10 w-full rounded-[0.65rem] border border-wf-ink/10 bg-wf-surface px-3 text-[13px] text-ink"
                        value={form.assignment_type}
                        onChange={(e) => setForm((f) => ({ ...f, assignment_type: e.target.value }))}
                        data-shifts-assignment-type
                      >
                        <option value="general">{c.assignmentGeneral}</option>
                        <option value="guest">{c.assignmentGuest}</option>
                        <option value="operations">{c.assignmentOperations}</option>
                        <option value="night">{c.assignmentNight}</option>
                        <option value="event">{c.assignmentEvent}</option>
                      </select>
                    </label>
                    <label className="block space-y-1 text-[10px] font-medium text-muted">
                      <span>{c.start}</span>
                  ...
                  {formOvernight ? <p className="mt-2 text-[10px] text-muted">{c.nextDay}</p> : null}
                </section>
                <section className="border-b border-wf-ink/[0.07] py-4" data-shifts-drawer-section="organization">
                  ...
                  {orgHasAdvanced ? (
                    <div className="mt-3" data-shifts-org-advanced>
                      <button
                        type="button"
                        className="text-[11px] font-medium text-wf-accent-follow-ink underline-offset-2 hover:underline"
                        onClick={() => setOrgAdvancedOpen((v) => !v)}
                        aria-expanded={orgAdvancedOpen}
                      >
                        {orgAdvancedOpen ? c.hideAdvancedOrg : c.showAdvancedOrg}
                      </button>
                      {orgAdvancedOpen ? (
