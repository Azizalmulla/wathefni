...
const lazySrc = readFileSync(resolve(__dirname, '../pages/lazy.tsx'), 'utf8')
...
describe('Alerts & Delivery Page Refinement Wave 1 contract', () => {
  ...
  it('keeps one follow-up resolve path and never mounts silent fake CTAs', () => {
    expect(notifications).toContain("resolveHrTask(access, taskId, 'done', expectedStatus)")
  ...
  it('is bilingual RTL and owns delivery without specialist-module strips', () => {
    expect(notifications).toContain('canManageAlertsAndDelivery')
    expect(notifications).toContain('alerts-forbidden')
    ...
    expect(postHire).not.toContain('export function PostHireDeliveryCenter')
    expect(notifications).not.toContain('PostHireDeliveryCenterPlaceholder')
  })
  ...
  it('wires App props for access and post-hire enablement without nested deliveryCenter cards', () => {
    ...
    expect(lazySrc).not.toContain('LazyPostHireDeliveryCenter')
