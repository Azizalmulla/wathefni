...
describe('Shifts Visual & Interaction Redesign Wave 3B', () => {
  ...
  it('keeps status filtering client-only and preserves Wave 1 / IQ-12 contracts', () => {
    ...
    const filterEnd = workspaceSrc.indexOf('= useShiftsData(', filterStart)
    ...
    expect(workspaceSrc).toContain('data-shifts-visual-direction')
    expect(workspaceSrc).toContain('data-shifts-range-soft-keep')
    ...
    expect(workspaceSrc).toContain('assignment_type')
    expect(rosterSrc).toContain('data-shift-assignment-type')
    expect(rosterSrc).toContain('c.nextDay')
  })
})
