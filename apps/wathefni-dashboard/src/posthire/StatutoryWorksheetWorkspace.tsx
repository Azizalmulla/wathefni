...
export function StatutoryWorksheetWorkspace({
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setSelected((prev) => {
        if (!prev) return prev
        const list = prev.kind === 'pifss' ? ws.pifss_worksheets || [] : ws.eos_worksheets || []
        const next = list.find((r) => r.worksheet_id === prev.row.worksheet_id)
        return next ? { kind: prev.kind, row: next } : prev
      })
    } catch (err) {
      if (requestId !== requestIdRef.current) return
      if (err instanceof DashboardApiError && err.status === 404) {
        ...
        setError(
          err instanceof DashboardApiError
            ? err.message
            : isAr
              ? 'تعذر تحميل أوراق العمل النظامية.'
              : 'We couldn’t load the statutory worksheets.',
        )
      }
    } finally {
  ...
  if (loading && !data) return <p className="text-sm text-subtle">{c.loading}</p>
  if (error === 'disabled') {
    return <BlockedReason reason={`${c.disabled} — ${c.disabledHint}`} locale={isAr ? 'ar' : 'en'} />
  }
  if (error && !data) return <BlockedReason reason={error} locale={isAr ? 'ar' : 'en'} />
  ...
  return (
    <div
      className="space-y-4"
      data-testid="statutory-worksheet-workspace"
    ...
    >
      ...
      {honestyOpen ? (
        <div className="space-y-1 rounded-md border border-dashed border-line/70 bg-black/[0.02] px-3 py-2 text-[13px] text-text" data-payroll-honesty>
          ...
          <p className="text-[12px] text-subtle">{c.counselHint}</p>
        </div>
      ) : null}
      ...
      {tab === 'rules' ? (
        <div className="overflow-x-auto">
          {(data?.rule_tables || []).length === 0 ? (
            <WorkflowEmpty title={c.empty} hint={c.counselHint} />
          ) : (
