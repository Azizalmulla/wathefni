...
export const WORKSPACE_SURFACES: WorkspaceSurface[] = [
  ...
  },
  {
    ...
    group: 'posthire',
    module: 'engagement',
    permissionAnyOf: ['engagement.read', 'engagement.manager'],
    label: 'Engagement',
  },
  {
    id: 'nav.compensation-planning',
    kind: 'nav',
    page: 'compensation-planning',
    group: 'posthire',
    module: 'comp_planning',
    permissionAnyOf: ['comp_planning.read', 'comp_planning.manager'],
    label: 'Compensation Planning',
  },
  {
    id: 'nav.workforce-planning',
    kind: 'nav',
    page: 'workforce-planning',
    group: 'posthire',
    module: 'workforce_planning',
    permissionAnyOf: ['workforce_planning.read', 'workforce_planning.manager'],
    label: 'Workforce Planning',
  },
  {
    id: 'nav.job-architecture',
    kind: 'nav',
  ...
  // Workspace
  {
    ...
    page: 'notifications',
    group: 'settings',
    permissionAnyOf: [...ALERTS_DELIVERY_MANAGE_PERMISSIONS],
...
export function resolveWorkspaceAuthority(args: {
  ...
  urgentCount?: number
  hasRolePriority?: boolean
  /** Phase 0 Action Inbox — when false/undefined falsey from bootstrap, hide nav.inbox */
  actionInboxOfferable?: boolean | null
}): WorkspaceAuthority {
  ...
  // Action Inbox Phase 0: fail-closed viewer allowlist from bootstrap.
  if (surfaces['nav.inbox']) {
    if (!args.actionInboxOfferable) {
      surfaces['nav.inbox'] = {
        ...surfaces['nav.inbox'],
...
export const COMPOSITION_MATRIX: CompositionMatrixRow[] = [
  ...
  },
  {
    ...
    role: 'recruiter',
    expectNavGroups: ['prehire'],
    expectNavIncludes: ['jobs', 'candidates', 'interviews'],
    expectNavExcludes: ['overview', 'ai', 'ranking', 'reports', 'payroll', 'assessments', 'notifications', 'settings'],
    expectOverviewLayout: 'two',
  },
]
