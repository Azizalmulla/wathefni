...
import type { MobileMe, PrioritiesResponse } from '@hr/api/types'
import { destinationAvailable, enabledWorkspaces, workspaceRoutes } from '@hr/capabilities'
import { toHrPath } from '@hr/navigation'
import { useLocale } from '@hr/i18n'
import { formatDate, formatDateRange } from '@hr/i18n/date'
import {
  ActionableCard,
  EditorialHeading,
  FadeIn,
      ...
      <View style={styles.navigation}>
        {routes.map((route) => (
          <ActionableCard
            ...
            onPress={onOpen ? () => onOpen(toHrPath(route.path)) : undefined}
          />
        ))}
        <ActionableCard
          title={t('nav.settings')}
          subtitle={t('settings.subtitle')}
          onPress={onOpen ? () => onOpen(toHrPath('/settings')) : undefined}
        />
      </View>
                ...
                <View style={styles.section}>
                  ...
                  {section.items.map((item) => (
                    <PriorityCard
                      ...
                      onPress={onOpen ? () => onOpen(toHrPath(item.destination)) : undefined}
                    />
                  ))}
                </View>
              </FadeIn>
