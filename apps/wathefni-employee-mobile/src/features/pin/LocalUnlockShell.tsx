...
export function LocalUnlockShell({ children }: Props) {
  ...
  const dismissUnlock = useCallback(() => {
    ...
    // Foreground refresh was skipped while the overlay was up — catch up now.
    // Does not touch sealed-session / AuthGate / overlay mount behavior.
  ...
  useEffect(() => {
    const onChange = (next: AppStateStatus) => {
      try {
        ...
        // Privacy cover only when unlock overlay is NOT up — Face ID makes AppState
        // inactive and must not add a second layer that fights the overlay host.
