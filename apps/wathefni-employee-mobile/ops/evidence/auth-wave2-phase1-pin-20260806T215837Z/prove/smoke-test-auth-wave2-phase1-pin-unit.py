...
def main() -> int:
    ...
    check("5-fail resets to OTP", "resetToOtpAfterPinFailure" in auth)
    check("no background re-lock", "Phase 3 owns idle/background lock" in auth)
    ...
    check("AuthGate unlock PIN", "UnlockPinFlow" in layout)
    check("change-pin route registered", "change-pin" in layout)

    pkg = (MOBILE / "package.json").read_text()
    check("no expo-local-authentication", "expo-local-authentication" not in pkg)
