#!/usr/bin/env bash
# Wave 6 C4 — Employee Relations qualify (company-scoped; global OFF).
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/employee-relations-c4-$STAMP"
REMOTE_STAGE="/tmp/employee-relations-c4-stage"
...
assert er.runtime_gate_for_company("WATHEFNI").get("ok") is not True
assert "employee_relations" in w6.WAVE6_MODULE_KEYS
assert w6.honesty_payload().get("er_case_level_need_to_know") is True
h = er.honesty_payload()
assert h["ordinary_hr_not_er"] is True
assert h["manager_not_er"] is True
assert h["outcome_not_employment_mutation"] is True
assert h["wave5_excludes_sensitive_free_text"] is True
assert h["assistant_mutations"] is False
src = open("employee_relations_c4.py").read()
assert "er_cases" in src and "er_access_grants" in src
assert "employment_mutated = false" in src
...
card = (root / "Wave6EmployeeRelationsPoliciesCard.tsx").read_text()
...
assert "wave6_employee_relations" in card
assert "need-to-know" in card.lower() or "الحاجة" in card
...
cp -a "$REPO_ROOT/ops/EMPLOYEE_RELATIONS_FULL_PASS.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
cp -a "$REPO_ROOT/ops/EMPLOYEE_RELATIONS_C4_FREEZE_AMENDMENT.md" "$LOCAL_EVID/docs/" 2>/dev/null || true

"${SSH[@]}" "rm -rf '$REMOTE_STAGE' && mkdir -p '$REMOTE_STAGE'"
"${SCP[@]}" \
  "$ORCH_SRC/employee_relations_c4.py" \
  "$ORCH_SRC/setup_console_wave6_policies.py" \
  "$ORCH_SRC/smoke-test-employee-relations-c4.py" \
...
if grep -Rls "WATHEFNI_EMPLOYEE_RELATIONS_C4=on" /etc/systemd/system/wathefni-orchestrator-staging.service.d 2>/dev/null; then
  echo "UNEXPECTED_SYSTEMD_ER_ON"; exit 1
fi
echo STAGING_COPY_OK_GLOBAL_ER_OFF
REMOTE
...
"\$PYBIN" smoke-test-employee-relations-c4.py
echo STAGING_RC=\$?
REMOTE

log "C1–C3 Wave6 + Waves 1–5 freeze unit regressions"
REG_OK=YES
set +e
...
for f in ja-c1-unit.out ld-c2-unit.out bn-c3-unit.out; do
...
DB_OK=NO
if grep -q 'EMPLOYEE_RELATIONS_FULL_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
if [[ "$GATES_OK" == YES && "$FE_OK" == YES && "$DB_OK" == YES && "$REG_OK" == YES ]]; then
  VERDICT='EMPLOYEE_RELATIONS_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Employee Relations C4 — Staging Prove

- Stamp: $STAMP
- Gates: $GATES_OK
- Frontend Setup card: $FE_OK
- Staging DB: $DB_OK
- C1–C3 Wave6 + Wave5/Wave4/Wave1 unit freezes: $REG_OK
- Global ER flag: remains **off**
- Verdict: **$VERDICT**

Stop before C5 Engagement until owner accepts.
EOF
