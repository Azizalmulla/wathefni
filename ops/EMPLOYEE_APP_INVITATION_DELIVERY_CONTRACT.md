...
```text
Employee eligible (company Employee App ON + employee access enabled)
  → auto invitation → deliver (email / WhatsApp) → employee activates → PIN → Face ID
HR does NOT normally copy or hand codes
```

| Moment | Behavior |
|---|---|
| Employee created / migrated / onboarding started | **No invite** unless access is explicitly enabled |
| Employee App access enabled (HR) | Auto-issue one pending invite + deliver (flag-gated) |
...
Eligibility model: `ops/EMPLOYEE_APP_ACCESS_ELIGIBILITY.md`
