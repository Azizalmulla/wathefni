#!/usr/bin/env bash
# Production Readiness R5D — Job Architecture product surface qualification.
#
# Proves Wave 6 C1 Job Architecture is a usable platform capability:
# HTTP adapter → HR Web authoring/workspace.
# Learning / Comp Planning / Workforce Planning stay unreleased.
# Does not begin R5E.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5d-job-architecture-$STAMP"
REMOTE_STAGE="/tmp/r5d-job-architecture-surface-stage"
...
log "2/8 local R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
test -f '$STG/job_architecture_http.py'
test -f '$STG/job_architecture_surfaces.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/job_architecture_http.py' '$STG/job_architecture_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5d-job-architecture-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live JA customer enableable", ready.customer_enableable("job_architecture") is True)
...
ck("live learning still hidden", ready.customer_enableable("learning") is False)
...
for path in ("/dashboard/job-architecture/workspace", "/dashboard/job-architecture/catalog"):
    ...
    ck(f"live {path} not public", status in (401, 403), status)
...
log "6/8 regressions (Waves 1–6 + R2–R5C)"
REG_OK=YES
set +e
for t in smoke-test-wave6-product-acceptance smoke-test-job-architecture-c1 smoke-test-learning-development-c2 \
         ...
         smoke-test-r5d-job-architecture-surface smoke-test-talent-profile-c5 smoke-test-talent-succession-c6; do
...
http = Path("wathefni-orchestrator/job_architecture_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/job-architecture" in line]
(evid / "api-inventory.md").write_text(
    "# R5D API inventory\\n\\n" + "\\n".join(f"- {r}" for r in routes[:80]) + "\\n",
    encoding="utf-8",
)
...
grep -q 'R5D_JOB_ARCHITECTURE_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5d-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5d-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5D_JOB_ARCHITECTURE_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5D_JOB_ARCHITECTURE_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5D — Job Architecture Product Surface

- Stamp: $STAMP
- Dashboard vitest: $DASH_OK
- Employee composition (R5C regression): $MOBILE_OK
...
- Waves 1–6 + R2–R5C regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5D Job Architecture product surface over frozen Wave 6 C1 authority.
Learning, Compensation Planning, and Workforce Planning remain unreleased.
FULL_PASS is not authorisation for production rollout.
Do not begin R5E until owner review.
EOF
