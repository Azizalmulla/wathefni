#!/usr/bin/env bash
# Production Readiness R5B — Performance product surface qualification.
#
# Proves Wave 4 Performance authorities are a usable product surface:
# HTTP adapter → HR Web → Manager → Employee App → thin HR Mobile.
# Talent stays unreleased. Does not begin R5C.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5b-performance-$STAMP"
REMOTE_STAGE="/tmp/r5b-performance-surface-stage"
...
FILES=(
  ...
  setup_console_employee_app_phase5.py
...
log "2/8 local R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
test -f '$STG/performance_http.py'
test -f '$STG/performance_surfaces.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/performance_http.py' '$STG/performance_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5b-performance-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live performance customer enableable", ready.customer_enableable("performance") is True)
ck("live talent enableable after R5C", ready.customer_enableable("talent") is True)

status, body = fetch("/health")
ck("live health", status == 200, status)

for path in ("/dashboard/performance/workspace", "/app/performance"):
...
log "6/8 regressions (Waves 1–6 + R2–R5A)"
REG_OK=YES
set +e
for t in smoke-test-wave6-product-acceptance smoke-test-job-architecture-c1 smoke-test-learning-development-c2 \
         ...
         smoke-test-r5a-capability-honesty smoke-test-r5b-performance-surface; do
...
routes = [line.strip() for line in http.splitlines() if "@app.get" in line or "@app.post" in line]
...
MOBILE_OK=NO
grep -qE 'passed' "$LOCAL_EVID/mobile/composition.out" && ! grep -qE 'FAIL' "$LOCAL_EVID/mobile/composition.out" && MOBILE_OK=YES
...
grep -q 'R5B_PERFORMANCE_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5b-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5b-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5B_PERFORMANCE_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5B_PERFORMANCE_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5B — Performance Product Surface
...
- Waves 1–6 + R2–R5A regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5B Performance product surface over frozen Wave 4 authorities.
Talent remains unreleased. FULL_PASS is not authorisation for production rollout.
Do not begin R5C until owner review.
EOF
