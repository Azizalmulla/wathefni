#!/usr/bin/env bash
# Controlled WATHEFNI WhatsApp-unsolicited + manual channel-authority cutover canary.
# Email stays authoritative. Job Stage B stays authoritative. ENFORCE stays OFF.
set -euo pipefail
...
REMOTE_STAGE="/tmp/unified-inbound-cv-wa-manual-cutover-${STAMP}"
BACKUP_ROOT="/opt/wathefni/backups/production-pre-wa-manual-cutover-${STAMP}"
EVIDENCE_ROOT="/opt/wathefni/production-evidence/unified-inbound-cv-wa-manual-cutover"
...
scp -o BatchMode=yes \
  ...
  "$ORCH_SRC/candidate_messages.py" \
  "$LOCAL_ROOT/ops/patch-production-app-unified-inbound-cv-wa-manual-cutover.py" \
  "$LOCAL_ROOT/ops/unified-inbound-cv-wa-manual-cutover-canary.py" \
  "$VPS:$REMOTE_STAGE/"

log "backup + patch + enable flags + canary"
"${SSH[@]}" bash -s <<REMOTE
set -euo pipefail
STAMP='$STAMP'
...
log "backup app + messages + flags"
cp -a "\$PROD_ORCH/app.py" "\$BACKUP_ROOT/app.py"
cp -a "\$PROD_ORCH/candidate_messages.py" "\$BACKUP_ROOT/candidate_messages.py"
cp -a "\$FLAGS_FILE" "\$BACKUP_ROOT/unified-inbound-cv.production.env"
sha256sum "\$BACKUP_ROOT/app.py" "\$BACKUP_ROOT/candidate_messages.py" | tee "\$EV/backup.sha256"
...
cp -a "\$ROOT/candidate_messages.py" /opt/wathefni/orchestrator/candidate_messages.py
cp -a "\$ROOT/unified-inbound-cv.production.env" /opt/wathefni/var/unified-inbound-cv.production.env
rm -f /opt/wathefni/orchestrator/inbound_cv_channel_cutover.py
...
log "install cutover module + messages"
cp -a "\$REMOTE_STAGE/inbound_cv_channel_cutover.py" "\$PROD_ORCH/inbound_cv_channel_cutover.py"
cp -a "\$REMOTE_STAGE/candidate_messages.py" "\$PROD_ORCH/candidate_messages.py"

log "surgical app patch"
/opt/wathefni/orchestrator/.venv/bin/python "\$REMOTE_STAGE/patch-production-app-unified-inbound-cv-wa-manual-cutover.py" "\$PROD_ORCH/app.py" | tee "\$EV/patch.json"
python3 -c "import json; d=json.load(open('\$EV/patch.json')); assert d.get('ok') is True, d; print(d)"

log "enable cutover flags"
cat > "\$FLAGS_FILE" <<'EOF'
# Unified Inbound CV — WATHEFNI production-dark + WA/manual controlled cutover
...
/opt/wathefni/orchestrator/.venv/bin/python "\$REMOTE_STAGE/unified-inbound-cv-wa-manual-cutover-canary.py" | tee "\$EV/canary.stdout.json"
...
for i in \$(seq 1 40); do curl -sf http://127.0.0.1:8010/health >/dev/null && break; sleep 1; done
PID=\$(systemctl show -p MainPID --value wathefni-orchestrator.service)
tr '\\0' '\\n' < /proc/\$PID/environ | grep -E 'INTAKE_AUTHORITY_WHATSAPP|INTAKE_AUTHORITY_MANUAL' | tee "\$EV/kill-switch-flags.txt" || true
! grep -q 'WATHEFNI_UNIFIED_INTAKE_AUTHORITY_WHATSAPP_UNSOLICITED=on' "\$EV/kill-switch-flags.txt"
! grep -q 'WATHEFNI_UNIFIED_INTAKE_AUTHORITY_MANUAL=on' "\$EV/kill-switch-flags.txt"
/opt/wathefni/orchestrator/.venv/bin/python - <<'PY' | tee "\$EV/kill-switch-skip.json"
import json, sys
sys.path.insert(0, "/opt/wathefni/orchestrator")
import inbound_cv_channel_cutover as c
assert not c.wa_unsolicited_authority_enabled("WATHEFNI")
assert not c.manual_authority_enabled("WATHEFNI")
print(json.dumps({"ok": True, "wa": False, "manual": False}))
PY
...
log "rollback proof then re-apply cutover posture"
bash "\$BACKUP_ROOT/ROLLBACK.sh" | tee "\$EV/rollback-run.txt"
curl -sf http://127.0.0.1:8010/health >/dev/null
cp -a "\$REMOTE_STAGE/inbound_cv_channel_cutover.py" "\$PROD_ORCH/inbound_cv_channel_cutover.py"
cp -a "\$REMOTE_STAGE/candidate_messages.py" "\$PROD_ORCH/candidate_messages.py"
cp -a "\$BACKUP_ROOT/app.py" "\$PROD_ORCH/app.py"
/opt/wathefni/orchestrator/.venv/bin/python "\$REMOTE_STAGE/patch-production-app-unified-inbound-cv-wa-manual-cutover.py" "\$PROD_ORCH/app.py" | tee "\$EV/patch-reapply.json"
cat > "\$FLAGS_FILE" <<'EOF'
# Unified Inbound CV — WATHEFNI production-dark + WA/manual controlled cutover
WATHEFNI_UNIFIED_INTAKE_ENVELOPE_DUAL_WRITE=on
WATHEFNI_UNIFIED_CV_VERSION_DUAL_WRITE=on
...
log "fetch evidence"
mkdir -p "$LOCAL_ROOT/ops/screenshots/unified-inbound-cv-wa-manual-cutover"
scp -o BatchMode=yes -r \
  "$VPS:$EVIDENCE_ROOT/$STAMP/" \
  "$LOCAL_ROOT/ops/screenshots/unified-inbound-cv-wa-manual-cutover/"
echo "$STAMP" > "$LOCAL_ROOT/ops/screenshots/unified-inbound-cv-wa-manual-cutover/LATEST_STAMP.txt"
log "local evidence ops/screenshots/unified-inbound-cv-wa-manual-cutover/$STAMP"
