#!/usr/bin/env bash
# Production Readiness R5G — Employee Relations product surface qualification.
#
# Proves Wave 6 C4 Employee Relations is a usable product:
# HTTP adapter → sealed HR Web workspace → thin HR Mobile alerts/actions.
# Engagement / Comp Planning / Workforce Planning stay unreleased.
# Does not begin R5H.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5g-employee-relations-$STAMP"
REMOTE_STAGE="/tmp/r5g-employee-relations-surface-stage"
...
log "2/8 local R5G + R5F + R5E + R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
cp -a "$DASH_SRC/src/posthire/EmployeeRelationsWorkspace.tsx" "$LOCAL_EVID/dashboard/" 2>/dev/null || true
cp -a "$MOBILE_SRC/src/hr/features/employee-relations/HREmployeeRelationsQueueView.tsx" "$LOCAL_EVID/mobile/" 2>/dev/null || true
...
test -f '$STG/employee_relations_http.py'
test -f '$STG/employee_relations_surfaces.py'
test -f '$STG/employee_relations_c4.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/employee_relations_http.py' '$STG/employee_relations_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5g-employee-relations-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live ER customer enableable", ready.customer_enableable("employee_relations") is True)
...
for path in ("/dashboard/employee-relations/workspace", "/dashboard/employee-relations/cases", "/dashboard/mobile/employee-relations"):
...
log "6/8 regressions (Waves 1–6 + R2–R5F)"
REG_OK=YES
set +e
...
http = Path("wathefni-orchestrator/employee_relations_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/employee-relations" in line or "/dashboard/mobile/employee-relations" in line]
...
grep -q 'R5G_EMPLOYEE_RELATIONS_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5g-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5g-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5G_EMPLOYEE_RELATIONS_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5F_DB_OK" == YES && "$R5E_DB_OK" == YES && "$R5D_DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5G_EMPLOYEE_RELATIONS_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5G — Employee Relations Product Surface
...
- Waves 1–6 + R2–R5F regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5G Employee Relations product surface over frozen Wave 6 C4 authority.
Engagement, Compensation Planning, and Workforce Planning remain unreleased.
FULL_PASS is not authorisation for production rollout.
Do not begin R5H until owner review.
EOF
