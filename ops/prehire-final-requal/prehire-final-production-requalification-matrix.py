#!/usr/bin/env python3
"""Final production requalification of the complete pre-hiring lifecycle.

Read-only with respect to product code and genuine tenant data. Creates isolated
synthetic tenants against current production-green module boundaries, exercises
Jobs → Candidates → Ranking → Assessments → Live Interviews → Video Interviews →
Offers → Hire together, then removes every row and asserts zero residue.

Synthetic tenants (required combinations under independent live/video modules):
  FQAVLO  assessments ON  / live ON  / video ON  / offers ON
  FQALXO  assessments ON  / live ON  / video OFF / offers ON
  FQAXVO  assessments ON  / live OFF / video ON  / offers ON
  FQAXXO  assessments ON  / live OFF / video OFF / offers ON
  FQXVLO  assessments OFF / live ON  / video ON  / offers ON
  FQXXXO  assessments OFF / live OFF / video OFF / offers ON
  FQNOOF  assessments OFF / live OFF / video OFF / offers OFF
  FQPEER  all ON (cross-tenant peer + historical-preservation subject)

Requires WATHEFNI_BOUNDARY_ALLOW_PRODUCTION=1 and dry_run delivery.
Does not modify, deploy, or reopen any frozen module.
"""
...
MARKER = "prehire-final-production-requal-v1"
POSITION = "FQ_ROLE"
PHONE_PREFIX = "+9658863"
ACTOR_A = "f0a10000-0000-4000-8000-00000000000a"
ACTOR_B = "f0a10000-0000-4000-8000-00000000000b"
...
TENANTS: list[dict[str, Any]] = [
    {"code": "FQAVLO", "combo": "asm_ON__live_ON__video_ON__offers_ON", "assessments": True, "live": True, "video": True, "offers": True},
    {"code": "FQALXO", "combo": "asm_ON__live_ON__video_OFF__offers_ON", "assessments": True, "live": True, "video": False, "offers": True},
    {"code": "FQAXVO", "combo": "asm_ON__live_OFF__video_ON__offers_ON", "assessments": True, "live": False, "video": True, "offers": True},
    {"code": "FQAXXO", "combo": "asm_ON__live_OFF__video_OFF__offers_ON", "assessments": True, "live": False, "video": False, "offers": True},
    {"code": "FQXVLO", "combo": "asm_OFF__live_ON__video_ON__offers_ON", "assessments": False, "live": True, "video": True, "offers": True},
    {"code": "FQXXXO", "combo": "asm_OFF__live_OFF__video_OFF__offers_ON", "assessments": False, "live": False, "video": False, "offers": True},
    {"code": "FQNOOF", "combo": "all_optional_OFF", "assessments": False, "live": False, "video": False, "offers": False},
    {"code": "FQPEER", "combo": "peer_isolation_and_history", "assessments": True, "live": True, "video": True, "offers": True},
]
...
def ensure_company(cur: Any, app: Any, tenant: dict[str, Any]) -> None:
    ...
    cur.execute(
        ...
        (
            company,
            f"Final Qual {company}",
            app.Json({"final_qual_marker": MARKER}),
            app.Json({"final_qual_marker": MARKER}),
        ),
    )
...
def insert_application(cur: Any, app: Any, *, company: str, key: str, status: str = "awaiting_cv") -> dict[str, Any]:
    ...
    app_key = f"{company}-FQ-{key}"
    raw = {
        "final_qual_marker": MARKER,
        "candidate_name": f"FQ {company} {key}",
        "locale": "en",
    ...
    cur.execute(
        ...
        """,
        (phone, f"FQ {company} {key}", f"{key}@{company.lower()}.invalid", app.Json({"final_qual_marker": MARKER})),
    )
    cur.execute(
        """
        ...
        RETURNING *
        """,
        (app_key, company, phone, POSITION, "FQ Operations Engineer", status, app.Json(raw)),
    )
...
def cleanup(app: Any) -> dict[str, Any]:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    "DELETE FROM companies WHERE company_code = ANY(%s) AND coalesce(metadata->>'final_qual_marker','')=%s",
                    (COMPANIES, MARKER),
                )
...
def main() -> int:  # noqa: C901
    ...
    allow_prod = os.environ.get("WATHEFNI_BOUNDARY_ALLOW_PRODUCTION") == "1"
    if expected_db != "wathefni" or env_name != "production":
        print(
            f"REFUSING: final production requalification requires production/wathefni "
            f"(env={env_name!r} db={expected_db!r})",
            file=sys.stderr,
        )
        return 2
    if not allow_prod:
        print("REFUSING: set WATHEFNI_BOUNDARY_ALLOW_PRODUCTION=1", file=sys.stderr)
        return 2
    if (os.environ.get("WATHEFNI_DELIVERY_MODE") or "") != "dry_run":
        print("REFUSING: production requalification requires WATHEFNI_DELIVERY_MODE=dry_run", file=sys.stderr)
        return 2
    if not os.environ.get("WATHEFNI_APPLY_WHATSAPP_NUMBER"):
        ...
        return 2
    os.environ.setdefault("WATHEFNI_OFFER_TOKEN_SECRET", "production-final-requal-secret-v1")
    ...
    def terra_stub(*, envelope, locale, model, prompt_version):
        ...
        return {"narrative": f"FinalQual committed evidence only ({locale}). Advisory {committed.get('advisory_score')}. HR decides."}
        ...
        for tenant in TENANTS:
            ...
            job = jobs.create_job(
                ...
                payload={
                    ...
                    "title_en": "FQ Operations Engineer",
            ...
            if live_on:
                ...
                try:
                    with mock.patch.object(ilife, "google_calendar_configured", return_value=True), mock.patch.object(
                        isvc, "_google_create", return_value={"event_id": f"fq-evt-{uuid.uuid4().hex[:8]}", "html_link": "https://calendar.invalid/fq", "meet_url": None}
                    ):
            ...
            if offers_on:
                draft = osvc.create_draft(
                    ...
                    actor_user_id=ACTOR_A,
                    permissions=FULL_PERMISSIONS,
                    position_title="FQ Operations Engineer",
                    ...
                    authorized_signatory="FQ Authorized Signatory",
                ...
                if ar_id:
                    try:
                        osvc.withdraw_offer(
                            ...
                            actor_user_id=ACTOR_A,
                            permissions=FULL_PERMISSIONS,
                            reason="FQ arabic document control probe",
                        )
                    except Exception:
                        pass
                ...
                try:
                    osvc.withdraw_offer(
                        ...
                        actor_user_id=ACTOR_A,
                        permissions=FULL_PERMISSIONS,
                        reason="FQ withdraw of a live sent offer",
                    )
                except Exception as exc:
            ...
            rejected = canonical_move(
                ...
                action="reject",
                trigger="hr_reject",
                payload={"reason_code": "not_a_fit", "reason_text": "FQ rejection"},
            )
            gate.check(
            ...
            withdrawn_app = canonical_move(
                ...
                payload={"reason_code": "candidate_withdrew", "source": "candidate", "reason_text": "FQ withdrawal"},
            )
            gate.check(
        ...
        if state[t1].get("offer_id"):
            ...
            if op:
                ...
                try:
                    ...
                    post_detail = json.dumps(post_preview.get("link_policy") or {}, default=str)
                except Exception as exc:
                ...
                gate.check(
                    ...
                    bool(post_preview.get("ok")),
                    post_detail,
                    scope=scope,
                )
                gate.check(
                    "issued_public_offer_link_policy_reason",
                    (post_preview.get("link_policy") or {}).get("reason") == "issued_before_module_disabled",
                    post_detail,
                    scope=scope,
                )
    ...
    stamped = out_dir / f"prehire-final-production-requalification-{_now()}.json"
    stamped.write_text(json.dumps(evidence, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (out_dir / "prehire-final-production-requalification.json").write_text(
