#!/usr/bin/env bash
# Resume Attendance final prod qualify after local freezes + dashboard build.
# Uses STAMP/LOCAL_EVID from /tmp/attwfinal.*
set -euo pipefail
...
STAMP="$(cat /tmp/attwfinal.stamp)"
LOCAL_EVID="$(cat /tmp/attwfinal.evid)"
...
test -f "$REPO_ROOT/apps/wathefni-dashboard/dist/index.html"
test -f "$LOCAL_EVID/tests/wave4b-local.out"

log "copy sources"
mkdir -p "$LOCAL_EVID/sources"
...
CHUNK=$(ls "$REPO_ROOT/apps/wathefni-dashboard/dist/assets/PostHire-"*.js | head -1)
...
log "rollback proof then redeploy"
...
log "production UI screenshots + cleanup"
...
cp -a "$REPO_ROOT/ops/ATTENDANCE_CONTROLLED_ROLLOUT_COMPLETION_AND_FREEZE.md" "$LOCAL_EVID/freeze/"
cp -a "$REPO_ROOT/.cursor/rules/attendance-freeze.mdc" "$LOCAL_EVID/freeze/" 2>/dev/null || true
