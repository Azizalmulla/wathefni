#!/usr/bin/env bash
# Wave 6 C7 — Workforce Planning qualify (company-scoped; global OFF).
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/workforce-planning-c7-$STAMP"
REMOTE_STAGE="/tmp/workforce-planning-c7-stage"
...
assert wfp.PASS_STAMP == "WORKFORCE_PLANNING_FULL_PASS"
...
assert wfp.runtime_gate_for_company("WATHEFNI").get("ok") is not True
assert "workforce_planning" in w6.WAVE6_MODULE_KEYS
assert w6.honesty_payload().get("workforce_planning_ja_hard") is True
assert w6.honesty_payload().get("workforce_planning_not_actual_headcount") is True
h = wfp.honesty_payload()
assert h["ja_is_hard"] is True
assert h["planned_headcount_never_enters_actual_wave5"] is True
assert h["recruiting_optional"] is True
assert h["approved_ne_execution"] is True
assert h["no_ai_forecast_authority"] is True
assert h["assistant_mutations"] is False
assert wfp.no_duplicate_planning_job_catalog_check()["no_duplicate_planning_job_catalog"] is True
src = open("workforce_planning_c7.py").read()
assert "wfp_plans" in src and "wfp_execution_handoffs" in src
assert "CREATE TABLE IF NOT EXISTS wfp_grade" not in src
assert "CHECK (employment_mutated_by_wfp = false)" in src
assert "CHECK (hire_created = false)" in src
...
card = (root / "Wave6WorkforcePlanningPoliciesCard.tsx").read_text()
app = (root / "SetupConsoleApp.tsx").read_text()
assert "Wave6WorkforcePlanningPoliciesCard" in app
assert "Wave6CompensationPlanningPoliciesCard" in app
assert "wave6_workforce_planning" in card
...
cp -a "$REPO_ROOT/ops/WORKFORCE_PLANNING_FULL_PASS.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
cp -a "$REPO_ROOT/ops/WORKFORCE_PLANNING_C7_FREEZE_AMENDMENT.md" "$LOCAL_EVID/docs/" 2>/dev/null || true

"${SSH[@]}" "rm -rf '$REMOTE_STAGE' && mkdir -p '$REMOTE_STAGE'"
"${SCP[@]}" \
  "$ORCH_SRC/workforce_planning_c7.py" \
  "$ORCH_SRC/setup_console_wave6_policies.py" \
  "$ORCH_SRC/smoke-test-workforce-planning-c7.py" \
...
if grep -Rls "WATHEFNI_WORKFORCE_PLANNING_C7=on" /etc/systemd/system/wathefni-orchestrator-staging.service.d 2>/dev/null; then
  echo "UNEXPECTED_SYSTEMD_WORKFORCE_PLANNING_ON"; exit 1
fi
echo STAGING_COPY_OK_GLOBAL_WORKFORCE_PLANNING_OFF
REMOTE
...
"\$PYBIN" smoke-test-workforce-planning-c7.py
echo STAGING_RC=\$?
REMOTE

log "C1–C6 Wave6 + Waves 1–5 freeze unit regressions"
REG_OK=YES
set +e
...
for f in ja-c1-unit.out ld-c2-unit.out bn-c3-unit.out er-c4-unit.out eg-c5-unit.out cp-c6-unit.out; do
...
DB_OK=NO
if grep -q 'WORKFORCE_PLANNING_FULL_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
if [[ "$GATES_OK" == YES && "$FE_OK" == YES && "$DB_OK" == YES && "$REG_OK" == YES ]]; then
  VERDICT='WORKFORCE_PLANNING_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Workforce Planning C7 — Staging Prove

- Stamp: $STAMP
- Gates: $GATES_OK
- Frontend Setup card: $FE_OK
- Staging DB: $DB_OK
- C1–C6 Wave6 + Wave5/Wave4/Wave1 unit freezes: $REG_OK
- Global Workforce Planning flag: remains **off**
- Verdict: **$VERDICT**

Stop before C8 Wave 6 Product Acceptance until owner accepts.
EOF
