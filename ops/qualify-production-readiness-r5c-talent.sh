#!/usr/bin/env bash
# Production Readiness R5C — Talent product surface qualification.
#
# Proves Wave 4 Talent authorities are a usable product surface:
# HTTP adapter → HR Web → scoped Manager → limited Employee App.
# Job Architecture stays unreleased. Does not begin R5D.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5c-talent-$STAMP"
REMOTE_STAGE="/tmp/r5c-talent-surface-stage"
...
log "2/8 local R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
test -f '$STG/talent_http.py'
test -f '$STG/talent_surfaces.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/talent_http.py' '$STG/talent_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5c-talent-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live talent customer enableable", ready.customer_enableable("talent") is True)
ck("live performance still enableable", ready.customer_enableable("performance") is True)
ck("live JA still hidden", ready.customer_enableable("job_architecture") is False)

status, body = fetch("/health")
ck("live health", status == 200, status)

for path in ("/dashboard/posthire/talent/workspace", "/app/talent"):
...
log "6/8 regressions (Waves 1–6 + R2–R5B)"
REG_OK=YES
set +e
...
http = Path("wathefni-orchestrator/talent_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/" in line or "/app/talent" in line]
...
grep -q 'R5C_TALENT_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5c-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5c-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5C_TALENT_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5C_TALENT_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5C — Talent Product Surface
...
- Waves 1–6 + R2–R5B regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5C Talent product surface over frozen Wave 4 C5–C6 authorities.
Job Architecture remains unreleased. FULL_PASS is not authorisation for production rollout.
Do not begin R5D until owner review.
EOF
