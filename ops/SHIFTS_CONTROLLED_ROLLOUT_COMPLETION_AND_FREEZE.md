...
**Authority evidence:** `ops/evidence/shifts-wave6c-20260803T033336Z/REPORT.md` (`PROD_CONTROLLED_WAVE6C_SHIFTS_GO`)  
**Production stamp:** `20260803T033336Z`  
...
| Scoped manager production scheduling | **NO-GO globally** — Wave 2 C3 company-scoped MSS only via `ops/SHIFTS_MSS_C3_FREEZE_AMENDMENT.md` (`WATHEFNI_SHIFTS_MSS_*`; global `MANAGER_ALLOWLIST` stays empty) |
...
Because environment variables can be edited without review, the boundary is also pinned in code as `APPROVED_HR_OPERATORS` in `shifts_controlled_wave6c.py`. Every prior-wave canary asserts `allowlists_within_approved_boundary()` (Wave 1B asserts `authority_scope_within_approved_boundary()`), so adding any manager, or an HR phone nobody approved, fails Waves 1B and 3B–6B immediately.

### Deployed production SHA-256 (first 16 hex, stamp `20260803T033336Z`)

```
cc774b5ad9a128b1  app.py
7dc2486e7300edb8  shifts_controlled_wave6c.py
33ab0fb9b86bff3f  shifts_notifications_wave6b.py
088dd8a1dff8d107  shifts_wave3_controlled.py
0f26fa3c65e4a819  shifts_authority_wave1.py
```
...
10. Do not populate `WATHEFNI_SHIFTS_MANAGER_ALLOWLIST`, and do not widen `APPROVED_HR_OPERATORS`, without a new owner-approved wave. Wave 2 C3 uses separate `WATHEFNI_SHIFTS_MSS_*` flags — see `ops/SHIFTS_MSS_C3_FREEZE_AMENDMENT.md`.
