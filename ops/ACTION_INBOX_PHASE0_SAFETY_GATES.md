...
**Rollout gate (not final product behavior):** empty or narrow allowlists hide other employees by hard exclusion, not by ranking below a result limit. Broad visibility requires a separate widen change-control. See `ops/NEEDS_ATTENTION_WAVE1_SURFACE_AND_GROUPING_FREEZE.md`.
