...
**Stamp:** `20260805T134124Z` (HR web canary: projection + drawer + manage gates)  
**Prior stamps:** `20260805T114246Z` (final cleanup), `20260805T045200Z` (mobile projection), `20260805T042655Z` (mobile build), `20260805T040948Z` (lifecycle canary)  
...
**Canary deploy:** production orchestrator + dashboard Onboarding page; flags still Aziz + Talal only for lifecycle V2A.
...
## Employee visibility (cleanup)

- Required employee items always visible.
- Optional items only when submitted / under review / replacement / complete **or** file/value evidence **or** `lifecycle_meta.explicitly_assigned|applicable`.
- Seed `due_date` alone does not surface dormant optionals.
- HR-owned docs only with evidence or active review.
- Bank always visible; Wave 2A actions empty (informational).
...
| Mobile grouped UX + preview/resubmit + global RTL | `OnboardingView`, `i18n`, root/tab layouts |
...
2. Restore module from `/opt/wathefni/backups/production-pre-onboarding-wave2a-cleanup-20260805T114246Z`.
3. Mobile falls back to prior internal IPA (build 6).
4. Optional: leave `processing` rows as-is (legacy map still understands them as non-complete when flag re-enabled).
...
- `smoke-test-onboarding-lifecycle-wave2a.py` (local + prod)
- Canary smoke Aziz: visible set = civil_id, employment_contract, personal_photo, offer_letter, bank_details
