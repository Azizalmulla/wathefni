#!/usr/bin/env python3
"""Pre-Hiring end-to-end integration qualification matrix.

Read-only with respect to product code and genuine tenant data. Creates isolated
synthetic tenants, exercises the frozen modules together, then removes every row
it created and asserts zero residue.

Synthetic tenants (module combinations):
  E2EAVO  assessments ON  / video_interviews ON  / employment_offers ON
  E2EAXO  assessments ON  / video_interviews OFF / employment_offers ON
  E2EXVO  assessments OFF / video_interviews ON  / employment_offers ON
  E2EXXO  assessments OFF / video_interviews OFF / employment_offers ON
  E2ENOOF assessments OFF / video_interviews OFF / employment_offers OFF
  E2EPEER all ON (cross-tenant peer + historical-preservation subject)

Does not touch production. Does not modify any frozen module.
"""
...
MARKER = "prehire-e2e-qualification-v1"
POSITION = "E2E_ROLE"
PHONE_PREFIX = "+9658860"
ACTOR_A = "e2e00000-0000-4000-8000-00000000000a"
ACTOR_B = "e2e00000-0000-4000-8000-00000000000b"
...
TENANTS: list[dict[str, Any]] = [
    {"code": "E2EAVO", "combo": "assessments_ON__interviews_ON", "assessments": True, "video": True, "offers": True},
    {"code": "E2EAXO", "combo": "assessments_ON__interviews_OFF", "assessments": True, "video": False, "offers": True},
    {"code": "E2EXVO", "combo": "assessments_OFF__interviews_ON", "assessments": False, "video": True, "offers": True},
    {"code": "E2EXXO", "combo": "assessments_OFF__interviews_OFF", "assessments": False, "video": False, "offers": True},
    {"code": "E2ENOOF", "combo": "offers_OFF_tenant_contract", "assessments": False, "video": False, "offers": False},
    {"code": "E2EPEER", "combo": "peer_isolation_and_history", "assessments": True, "video": True, "offers": True},
]
COMPANIES = [t["code"] for t in TENANTS]

ASSESSMENT_WORDS = ("assessment", "assessments", "تقييم", "التقييم")
INTERVIEW_WORDS = ("interview", "interviews", "مقابلة", "المقابلة", "video_interview")
...
def ensure_company(cur: Any, app: Any, tenant: dict[str, Any]) -> None:
    ...
    cur.execute(
        ...
        (
            company,
            f"E2E {company} Holding",
            app.Json({"e2e_marker": MARKER}),
            app.Json({"e2e_marker": MARKER}),
        ),
    )
    cur.execute("DELETE FROM company_modules WHERE company_code=%s", (company,))
    modules = ["pre_hiring"]
    if tenant["assessments"]:
        modules.append("assessments")
    if tenant["video"]:
        modules.append("video_interviews")
    if tenant["offers"]:
        modules.append("employment_offers")
    for module in modules:
        cur.execute(
            "INSERT INTO company_modules(company_code,module_key,enabled,source,updated_at) VALUES (%s,%s,true,%s,now())",
...
def insert_application(cur: Any, app: Any, *, company: str, key: str, status: str = "awaiting_cv") -> dict[str, Any]:
    ...
    app_key = f"{company}-E2E-{key}"
    raw = {
        "e2e_marker": MARKER,
        "candidate_name": f"E2E {company} {key}",
        "locale": "en",
    ...
    cur.execute(
        ...
        """,
        (phone, f"E2E {company} {key}", f"{key}@{company.lower()}.invalid", app.Json({"e2e_marker": MARKER})),
    )
    cur.execute(
        """
        ...
        RETURNING *
        """,
        (app_key, company, phone, POSITION, "E2E Operations Engineer", status, app.Json(raw)),
    )
...
def seed_cv_evidence(cur: Any, app: Any, cve: Any, cvf: Any, *, company: str, app_key: str, content: str) -> None:
    ...
    cur.execute(
        ...
        (
            ...
            source_hash,
            app.Json({"latest": True, "e2e_marker": MARKER}),
            app.Json({"e2e_marker": MARKER}),
        ),
    )
    cur.execute(
        ...
        """,
        (finalization_id, company, f"e2e-doc-{app_key}", app_key, source_hash, text_hash, app.Json({"e2e_marker": MARKER})),
    )
    materialized = cve.materialize_ready(
        cur,
        company_code=company,
        app_key=app_key,
        document_id=f"e2e-doc-{app_key}",
        file_id=file_id,
...
def seed_semantic_doc(cur: Any, app: Any, cr: Any, *, company: str, app_key: str, phone: str, content: str) -> None:
    ...
    cur.execute(
        ...
        (
            ...
            app.Json({"ranking_projection_version": cr.RANKING_PROJECTION_VERSION, "e2e_marker": MARKER}),
        ),
    )
...
def cleanup(app: Any) -> dict[str, Any]:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    "DELETE FROM companies WHERE company_code = ANY(%s) AND coalesce(metadata->>'e2e_marker','')=%s",
                    (COMPANIES, MARKER),
                )
...
def main() -> int:  # noqa: C901
    ...
    os.environ.setdefault("WATHEFNI_OFFER_TOKEN_SECRET", "staging-e2e-qualification-secret-v1")
    ...
    def terra_stub(*, envelope, locale, model, prompt_version):
        ...
        return {"narrative": f"E2E committed evidence only ({locale}). Advisory {committed.get('advisory_score')}. HR decides."}
    ...
    try:
        ...
        gate.check(
            "catalog_has_dedicated_interviews_module",
            any(k in catalog_keys for k in ("interviews", "interviewing", "live_interviews")),
            f"interview-related keys present={[k for k in catalog_keys if 'interview' in k]}; full={catalog_keys}",
            scope="contract",
        )
        gate.check(
            "toolcall_gated_covers_optional_prehire_modules",
            {"assessments", "video_interviews", "employment_offers"} <= set(mc.TOOLCALL_GATED_MODULES),
        ...
        for tenant in TENANTS:
            ...
            job = jobs.create_job(
                ...
                payload={
                    ...
                    "title_en": "E2E Operations Engineer",
            ...
            for key, row in apps.items():
                ...
                ready = rl.mark_cv_ready_for_review(
                    ...
                    cv_version="e2e-1",
                    document_id=f"e2e-doc-{row['app_key']}",
                )
            ...
            if video_on:
                ...
                # contract expectation: interviews OFF must block scheduling actions
                gate.check("interviews_off_blocks_live_scheduling", not live_ok, live_detail, scope=scope)

            provider_status = str((live.get("interview") or {}).get("provider_sync_status") or "not_configured")
            gate.check(
                "google_calendar_disconnected_still_works",
                (not video_on) or (live_ok and provider_status in {"not_configured", "skipped", "none", "None", "disabled"}),
            ...
            if video_on:
                ...
                try:
                    with mock.patch.object(ilife, "google_calendar_configured", return_value=True), mock.patch.object(
                        isvc, "_google_create", return_value={"event_id": f"e2e-evt-{uuid.uuid4().hex[:8]}", "html_link": "https://calendar.invalid/e2e", "meet_url": None}
                    ):
                ...
                gate.check("google_calendar_connected_path_works", gconnected_ok, gdetail, scope=scope)
                ...
                try:
                    req = app.DashboardVideoInterviewRequest(account_id="default", response_mode="single_video", send_invite=False)
                    createdv = app.create_or_resume_async_video_interview(apps["main"], req, actor_context=actor(ACTOR_A))
                    vdetail = f"created={(createdv.get('interview') or {}).get('interview_id')}"
                    if (createdv.get("interview") or {}).get("interview_id"):
                        tstate["video_interview_id"] = str((createdv.get("interview") or {}).get("interview_id"))
                except Exception as exc:
                    video_blocked = True
                    vdetail = f"{type(exc).__name__}:{str(getattr(exc, 'detail', exc))[:280]}"
                gate.check("async_video_blocked_when_module_off", video_blocked, vdetail, scope=scope)

            # unassigned interview state: bare candidate for OFF tenants must carry no interview debt
            queue = overview.compute_work_queue(
                company=company,
                db_connect=app.db_connect,
                assessments_enabled=assessments_on,
                interviews_enabled=("interviews" in app.configured_company_modules(company)) or True,
            )
            ...
            if video_on:
                gate.note("interview_queue_items_when_on", json.dumps([i.get("reason") or i.get("kind") for i in debt_items], default=str)[:400], scope=scope)
            else:
                gate.check(
                    "interviews_off_no_interview_queue_or_warning",
                    not debt_items,
            ...
            if offers_on:
                draft = osvc.create_draft(
                    ...
                    actor_user_id=ACTOR_A,
                    permissions=FULL_PERMISSIONS,
                    position_title="E2E Operations Engineer",
                    country_of_employment="KW",
                    employing_legal_entity=f"{company} K.S.C.C.",
                    ...
                    template_id="KW-OFFER-E2E",
                    template_version="2026.7",
                    template_approval_ref="E2E-BOARD-2026-07",
                    employer_policy_version="HR-POLICY-E2E-1",
                    authorized_signatory="E2E Authorized Signatory",
                ...
                sent_offer = osvc.send_offer(
                    ...
                    permissions=FULL_PERMISSIONS,
                    channel="whatsapp",
                    idempotency_key=f"send:{company}:{offer_id}",
                )
                ...
                resent = osvc.send_offer(
                    ...
                    resend=True,
                    idempotency_key=f"resend:{company}:{offer_id}",
                )
                gate.check("offer_resend_ok", bool(resent.get("ok") or resent.get("delivery")), json.dumps(resent, default=str)[:300], scope=scope)
                ...
                with app.db_connect() as conn:
                    with conn.cursor() as cur:
                        cur.execute(
                            """
                            SELECT operation_id FROM employment_offer_delivery_operations
                            WHERE company_code=%s AND offer_id=%s ORDER BY created_at DESC LIMIT 1
                            """,
                            (company, offer_id),
                        )
                        op_row = cur.fetchone()
                raw_token = ""
                if op_row:
                    raw_token, _ = osvc._token_for_delivery_operation(app, str(op_row["operation_id"]))
                preview = osvc.public_offer_preview(app, raw_token=raw_token) if raw_token else {}
                gate.check(
                    "public_offer_preview_available_after_send",
                    bool(preview.get("offer") or preview.get("position_title") or preview.get("company")),
                    json.dumps(sorted(preview.keys()), default=str)[:300],
                    scope=scope,
                )
                accepted = osvc.respond_via_token(app, raw_token=raw_token, decision="accepted") if raw_token else {}
                gate.check("offer_accepted_by_candidate", bool(accepted.get("ok") or accepted.get("status") == "accepted"), json.dumps(accepted, default=str)[:300], scope=scope)
                ...
                if ar_id:
                    try:
                        osvc.withdraw_offer(
                            ...
                            actor_user_id=ACTOR_A,
                            permissions=FULL_PERMISSIONS,
                            reason="E2E arabic document control probe",
                        )
                    except Exception:
                        pass
                gate.check("arabic_offer_document_control_enforced_at_approval", ar_guard, ar_detail, scope=scope)
                ...
                osvc.send_offer(
                    ...
                    permissions=FULL_PERMISSIONS,
                    channel="whatsapp",
                    idempotency_key=f"send-bare:{company}:{bare_offer_id}",
                )
                with app.db_connect() as conn:
                    with conn.cursor() as cur:
                        cur.execute(
                            "UPDATE employment_offers SET expires_at=now() - interval '1 hour' WHERE offer_id=%s",
                            (bare_offer_id,),
                        )
                    conn.commit()
                expired = osvc.expire_offer_if_due(app, company_code=company, offer_id=bare_offer_id)
                expired_twice = osvc.expire_offer_if_due(app, company_code=company, offer_id=bare_offer_id)
                with app.db_connect() as conn:
                    with conn.cursor() as cur:
                        cur.execute("SELECT status FROM employment_offers WHERE offer_id=%s", (bare_offer_id,))
                        bare_status = str((cur.fetchone() or {}).get("status") or "")
                gate.check(
                    "offer_expiry_canonical_and_idempotent",
                    bare_status == "expired" and bool(expired) and not bool((expired_twice or {}).get("expired") if isinstance(expired_twice, dict) else expired_twice),
                    json.dumps({"status": bare_status, "first": expired, "second": expired_twice}, default=str)[:300],
                    scope=scope,
                )
                ...
                with app.db_connect() as conn:
                    with conn.cursor() as cur:
                        cur.execute(
                            ...
                            WHERE company_code=%s AND offer_id=%s ORDER BY created_at DESC LIMIT 1
                            """,
                            (company, bare_offer_id),
                        )
                        bare_op = cur.fetchone()
                if bare_op:
                    bare_token, _ = osvc._token_for_delivery_operation(app, str(bare_op["operation_id"]))
                    try:
                        ...
                        stale_blocked = True
                        stale_detail = f"{type(exc).__name__}:{str(getattr(exc, 'detail', exc))[:250]}"
                ...
                gate.check(
                    "expired_offer_status_is_terminal_and_not_rewritable",
                    withdraw_blocked and wstatus == "expired",
                    f"status={wstatus} {withdraw_detail}",
                    scope=scope,
                )
                ...
                osvc.send_offer(
                    ...
                    permissions=FULL_PERMISSIONS,
                    channel="whatsapp",
                    idempotency_key=f"send-withdraw:{company}:{w_id}",
                )
                w_err = ""
                try:
                    osvc.withdraw_offer(
                        ...
                        actor_user_id=ACTOR_A,
                        permissions=FULL_PERMISSIONS,
                        reason="E2E withdraw of a live sent offer",
                    )
                except Exception as exc:
            ...
            gate.check(
                ...
                json.dumps({"hire_ok": hire_result.get("ok"), "status": hired_status, "emp": emp, "hop": hop}, default=str)[:500],
                scope=scope,
            )
            ...
            rejected = canonical_move(
                ...
                action="reject",
                trigger="hr_reject",
                payload={"reason_code": "not_a_fit", "reason_text": "E2E rejection"},
            )
            gate.check(
            ...
            withdrawn_app = canonical_move(
                ...
                payload={"reason_code": "candidate_withdrew", "source": "candidate", "reason_text": "E2E withdrawal"},
            )
            gate.check(
            ...
            if not video_on:
                ...
                gate.check("reports_zero_interview_wording_when_off", not ihits, json.dumps(ihits, ensure_ascii=False)[:600], scope=scope)
            if not offers_on:
                ohits = word_hits(
                    {"summary": summary, "exports": payload.get("exports"), "breakdowns": payload.get("breakdowns"), "funnel": funnel},
                    OFFER_WORDS,
            ...
            if video_on:
                gate.note("assistant_interview_tools_when_on", str(sorted(n for n in names if "interview" in n)), scope=scope)
            else:
                gate.check(
                    "assistant_hides_interview_tools_when_off",
                    not any("interview" in n for n in names),
                    str(sorted(n for n in names if "interview" in n)),
                    scope=scope,
                )
            # Which module each candidate-facing tool is gated by, and whether execution fails closed.
            for tool_name, expected_module in (("send_video_interview", "video_interviews"), ("send_assessment", "assessments"), ("schedule_interview", "pre_hiring")):
            ...
            if not video_on:
                mob_hits = word_hits(caps, INTERVIEW_WORDS)
                gate.check("mobile_zero_interview_payload_when_off", not mob_hits, json.dumps(mob_hits, ensure_ascii=False)[:600], scope=scope)
            if not offers_on:
        ...
        # public offer link with the offers module disabled mid-flight
        offers_off_public = "not_evaluated"
        if state[t1].get("offer_id"):
            with app.db_connect() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        ...
                        WHERE company_code=%s AND offer_id=%s ORDER BY created_at DESC LIMIT 1
                        """,
                        (t1, state[t1]["offer_id"]),
                    )
                    op = cur.fetchone()
            if op:
                tok, _ = osvc._token_for_delivery_operation(app, str(op["operation_id"]))
                set_module(app, t1, "employment_offers", False)
                try:
                    prev = osvc.public_offer_preview(app, raw_token=tok)
                    offers_off_public = f"served: keys={sorted(prev.keys())}"
                    public_blocked = False
                except Exception as exc:
                    public_blocked = True
                    offers_off_public = f"{type(exc).__name__}:{getattr(exc, 'code', '')}"
                finally:
                    set_module(app, t1, "employment_offers", True)
                gate.check(
                    "public_offer_link_respects_disabled_offers_module",
                    public_blocked,
                    offers_off_public,
                    scope=scope,
                )
        ...
        for key in ("assessments", "video_interviews", "employment_offers"):
    ...
    out_dir = ORCH / "ops" / "prehire-e2e"
    out_dir.mkdir(parents=True, exist_ok=True)
    stamped = out_dir / f"prehire-e2e-qualification-{_now()}.json"
    stamped.write_text(json.dumps(evidence, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (out_dir / "prehire-e2e-qualification.json").write_text(
