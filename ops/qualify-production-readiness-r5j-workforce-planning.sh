#!/usr/bin/env bash
# Production Readiness R5J — Workforce Planning product surface qualification.
#
# Proves Wave 6 C7 Workforce Planning is a usable product:
# HTTP adapter → HR Web planning workspace.
# No Employee App surface. No HR Mobile surface.
# Job Architecture is a hard dependency.
# Recruiting, Compensation Planning, and Talent remain optional.
# Does not begin R6.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5j-workforce-planning-$STAMP"
REMOTE_STAGE="/tmp/r5j-workforce-planning-surface-stage"
...
FILES=(
  ...
  workforce_planning_surfaces.py
  workforce_planning_http.py
  compensation_planning_c6.py
  compensation_surfaces.py
  compensation_http.py
  engagement_c5.py
  engagement_surfaces.py
  engagement_http.py
  employee_relations_c4.py
  employee_relations_surfaces.py
  employee_relations_http.py
  operator_mobile.py
  ...
  smoke-test-workforce-planning-c7.py
  smoke-test-compensation-planning-c6.py
  smoke-test-engagement-c5.py
  smoke-test-employee-relations-c4.py
...
log "2/8 local R5J + R5I + R5H + R5G + R5F + R5E + R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
cp -a "$DASH_SRC/src/posthire/WorkforcePlanningWorkspace.tsx" "$LOCAL_EVID/dashboard/" 2>/dev/null || true
...
test -f '$STG/workforce_planning_http.py'
test -f '$STG/workforce_planning_surfaces.py'
test -f '$STG/workforce_planning_c7.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/workforce_planning_http.py' '$STG/workforce_planning_surfaces.py' '$STG/capability_readiness.py'
...
log "4/8 staging DB journeys A–I + HTTP security"
staging_py smoke-test-r5j-workforce-planning-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live workforce planning customer enableable", ready.customer_enableable("workforce_planning") is True)
ck("live comp planning still enableable", ready.customer_enableable("comp_planning") is True)
ck("live engagement still enableable", ready.customer_enableable("engagement") is True)
ck("live ER still enableable", ready.customer_enableable("employee_relations") is True)
ck("live benefits still enableable", ready.customer_enableable("benefits") is True)
...
for path in ("/dashboard/workforce-planning/workspace", "/dashboard/workforce-planning/plans"):
...
log "6/8 regressions (Waves 1–6 + R2–R5I)"
REG_OK=YES
set +e
for t in smoke-test-wave6-product-acceptance smoke-test-job-architecture-c1 smoke-test-learning-development-c2 \
         ...
         smoke-test-r5i-compensation-planning-surface \
...
http = Path("wathefni-orchestrator/workforce_planning_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/workforce-planning" in line]
...
grep -q 'R5J_WORKFORCE_PLANNING_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5j-unit.out" \
  && grep -q 'R5I_COMPENSATION_PLANNING_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5i-unit.out" \
  && grep -q 'R5H_ENGAGEMENT_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5h-unit.out" \
  && grep -q 'R5G_EMPLOYEE_RELATIONS_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5g-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5j-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5J_WORKFORCE_PLANNING_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
R5I_DB_OK=NO
grep -q 'R5I_COMPENSATION_PLANNING_SURFACE_DB_PASS' "$LOCAL_EVID/tests/r5i-staging-db.out" \
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5i-staging-db.out" && R5I_DB_OK=YES

R5H_DB_OK=NO
grep -q 'R5H_ENGAGEMENT_SURFACE_DB_PASS' "$LOCAL_EVID/tests/r5h-staging-db.out" \
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5h-staging-db.out" && R5H_DB_OK=YES

R5G_DB_OK=NO
grep -q 'R5G_EMPLOYEE_RELATIONS_SURFACE_DB_PASS' "$LOCAL_EVID/tests/r5g-staging-db.out" \
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5g-staging-db.out" && R5G_DB_OK=YES

R5F_DB_OK=NO
grep -q 'R5F_BENEFITS_SURFACE_DB_PASS' "$LOCAL_EVID/tests/r5f-staging-db.out" \
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5f-staging-db.out" && R5F_DB_OK=YES
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5I_DB_OK" == YES && "$R5H_DB_OK" == YES && "$R5G_DB_OK" == YES && "$R5F_DB_OK" == YES && "$R5E_DB_OK" == YES && "$R5D_DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5J_WORKFORCE_PLANNING_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5J — Workforce Planning Product Surface
...
- R5H staging DB regression: $R5H_DB_OK
- R5G staging DB regression: $R5G_DB_OK
- R5F staging DB regression: $R5F_DB_OK
...
- Waves 1–6 + R2–R5I regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5J Workforce Planning product surface over frozen Wave 6 C7 authority.
Job Architecture remains a hard dependency. Recruiting, Compensation Planning, and Talent remain optional.
FULL_PASS is not authorisation for production rollout.
Do not begin R6 until owner review.
EOF
