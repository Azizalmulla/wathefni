...
def main() -> None:
    ...
    check(all(not str(e["title"]).startswith("Preview") for e in events), "titles omit Preview prefix")
    check(all(e.get("preview_only") and (e.get("metadata") or {}).get("preview_only") for e in events), "preview disclosure via flags")
