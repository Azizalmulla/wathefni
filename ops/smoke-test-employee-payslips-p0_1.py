...
os.environ.setdefault(
    ...
    "PYW3,PYW3-SYNTH|,PYW2B,PYW2B-SYNTH|,PYW2A,PYW2ACB,PYW1,PYW1-SYNTH|,W2BB,W3B,EP0,EP01,PYAUTH,PYP1",
)
...
def unique_period(app: Any) -> tuple[date, date]:
    """First far-future month with no payroll period for this company.

    Hashing the tag into a fixed 120-month window collided with periods left by
    earlier runs, which aborted the transaction on the unique constraint. Ask the
    database which month is actually free instead.
    """
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT period_start FROM payroll_periods WHERE company_code=%s AND period_start >= %s",
                (COMPANY, date(2032, 1, 1)),
            )
            taken = {dict(row)["period_start"] for row in cur.fetchall()}
    ...
    raise RuntimeError("no free synthetic payroll period in 2032-2081; clean up residual periods")
...
def main() -> int:
    ...
    # P5 Mode A added a sealed wathefni money authority beside the external mirror, so
    # eligibility is now stated as an explicit two-source list. What must stay true is
    # that both sources require a seal and Mode A stays synthetic-only until P6.
    check(
        "both sealed authority modes are documented",
        [
            "external_import + sealed money_authority=external",
            "native_authoritative + sealed money_authority=wathefni (P5 Mode A)",
        ]
        == list(blockers.get("official_pdf_eligible_sources") or []),
        str(blockers.get("official_pdf_eligible_sources")),
    )
    check("mode A stays synthetic-only", blockers.get("mode_a_synthetic_only") is True)
    check("period close is never a money seal", blockers.get("period_close_is_money_seal") is False)
    check("payment date is never invented", blockers.get("payment_date_invented") is False)
    check("remaining gate present", bool(blockers.get("remaining_production_authority_gate")))
    check(
        "native_authoritative without a seal is ineligible",
        opdf.is_official_pdf_eligible({"source_kind": "native_authoritative", "money_authority": "wathefni"})
        is False,
    )
    check(
        "native_authoritative sealed is eligible (Mode A)",
        opdf.is_official_pdf_eligible(
            {
                "source_kind": "native_authoritative",
                "money_authority": "wathefni",
                "authority_snapshot_id": "00000000-0000-0000-0000-000000000098",
            }
        )
        is True,
    )
    ...
    p_start, p_end = unique_period(app)
    ext_id = ""
    native_id = ""
    replaced_id = ""
    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                # A failing create_period aborts the surrounding transaction, so the
                # reuse lookup below needs its own savepoint to stay queryable.
                cur.execute("SAVEPOINT ep01_period")
                try:
                ...
                except Exception as exc:  # noqa: BLE001 — reported by the check below
                    period = {"ok": False, "error": f"{type(exc).__name__}: {exc}"}
                if not period.get("ok"):
                    cur.execute("ROLLBACK TO SAVEPOINT ep01_period")
                    cur.execute(
                ...
                cur.execute("RELEASE SAVEPOINT ep01_period")
                ...
                ext = p1.seal_and_generate_external_payslip(
                    cur,
                    company_code=COMPANY,
                    import_run_id=iid,
                    employee_key=EMP_A,
                ...
                check("linked sealed authority", bool(eslip.get("authority_snapshot_id")), eslip)
                check(
                    "external official eligible",
                    opdf.is_official_pdf_eligible(eslip) is True,
                    eslip,
                )
