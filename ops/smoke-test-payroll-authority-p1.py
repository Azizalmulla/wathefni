...
def main() -> int:
    ...
    check("native preview never authoritative", h.get("native_preview_authoritative") is False)
    check("mode a honesty flag present", "mode_a_wathefni_seal_unlocked" in h)
    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                check(
                    "mode a seal refused",
                    refuse_a.get("error") == "mode_a_requires_p5_finalize_from_calc",
                    refuse_a,
                )
    ...
    report = {
        ...
        "authority_transitions": {
            ...
            "mode_a_p1": "native_preview → seal_from_native_preview → REFUSED (use P5 finalize from P3 calc)",
