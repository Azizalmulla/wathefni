#!/usr/bin/env bash
# Production Readiness R5H — Engagement product surface qualification.
#
# Proves Wave 6 C5 Engagement is a usable product:
# HTTP adapter → HR Web survey administration → scoped Manager aggregates →
# Employee App participation.
# Compensation Planning / Workforce Planning stay unreleased.
# Does not begin R5I.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5h-engagement-$STAMP"
REMOTE_STAGE="/tmp/r5h-engagement-surface-stage"
...
log "2/8 local R5H + R5G + R5F + R5E + R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
cp -a "$DASH_SRC/src/posthire/EngagementWorkspace.tsx" "$LOCAL_EVID/dashboard/" 2>/dev/null || true
cp -a "$MOBILE_SRC/src/features/engagement/EngagementHubView.tsx" "$LOCAL_EVID/mobile/" 2>/dev/null || true
cp -a "$MOBILE_SRC/app/engagement/index.tsx" "$LOCAL_EVID/mobile/" 2>/dev/null || true
...
test -f '$STG/engagement_http.py'
test -f '$STG/engagement_surfaces.py'
test -f '$STG/engagement_c5.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/engagement_http.py' '$STG/engagement_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5h-engagement-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live engagement customer enableable", ready.customer_enableable("engagement") is True)
...
for path in ("/dashboard/engagement/workspace", "/dashboard/engagement/campaigns", "/app/engagement"):
...
log "6/8 regressions (Waves 1–6 + R2–R5G)"
REG_OK=YES
set +e
for t in smoke-test-wave6-product-acceptance smoke-test-job-architecture-c1 smoke-test-learning-development-c2 \
         ...
         smoke-test-r5g-employee-relations-surface smoke-test-talent-profile-c5 smoke-test-talent-succession-c6; do
...
http = Path("wathefni-orchestrator/engagement_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/engagement" in line or "/app/engagement" in line]
...
grep -q 'R5H_ENGAGEMENT_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5h-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5h-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5H_ENGAGEMENT_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5G_DB_OK" == YES && "$R5F_DB_OK" == YES && "$R5E_DB_OK" == YES && "$R5D_DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5H_ENGAGEMENT_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5H — Engagement Product Surface
...
- Waves 1–6 + R2–R5G regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5H Engagement product surface over frozen Wave 6 C5 authority.
Compensation Planning and Workforce Planning remain unreleased.
FULL_PASS is not authorisation for production rollout.
Do not begin R5I until owner review.
EOF
