...
ATTW3_RESULTS_PATH="\$REMOTE_EVID/tests/qualification-staging.json" \
  WATHEFNI_SKIP_FREEZE=1 \
  "\$PYBIN" smoke-test-attendance-ops-wave3.py 2>&1 | tee "\$REMOTE_EVID/tests/ops-smoke.out" | tee /tmp/attw4-ops-smoke.tail | tail -30

set +e
"\$PYBIN" smoke-test-employees360-freeze-regression.py 2>&1 | tee "\$REMOTE_EVID/tests/freeze-employees360.out" | tail -5
E360_RC=\$?
"\$PYBIN" smoke-test-onboarding-freeze-regression.py 2>&1 | tee "\$REMOTE_EVID/tests/freeze-onboarding.out" | tail -8
ONB_RC=\$?
set -e
echo "freeze_rc e360=\$E360_RC onboarding=\$ONB_RC" | tee "\$REMOTE_EVID/tests/freeze-rc.txt"
if [[ "\$E360_RC" -ne 0 ]]; then
  echo "REFUSE: Employees 360 freeze failed on staging" >&2
  exit 2
fi
if [[ "\$ONB_RC" -ne 0 ]]; then
  echo "NOTE: staging onboarding freeze drift (pre-existing; local freeze green) — continuing UX shots" | tee "\$REMOTE_EVID/docs/STAGING_ONBOARDING_FREEZE_DRIFT.md"
fi
