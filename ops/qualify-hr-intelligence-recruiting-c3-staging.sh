#!/usr/bin/env bash
# Wave 5 C3 — Recruiting + Hire→Ready Intelligence qualify (company-scoped; global OFF).
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/hr-intelligence-recruiting-c3-$STAMP"
REMOTE_STAGE="/tmp/hr-intelligence-recruiting-c3-stage"
...
import os, sys
sys.path.insert(0, ".")
os.environ["WATHEFNI_HR_INTELLIGENCE_RECRUITING_C3"]="off"
os.environ["WATHEFNI_HR_INTELLIGENCE_REGISTRY_C1"]="off"
import hr_intelligence_recruiting_c3 as t
...
assert t.PASS_STAMP == "HR_INTELLIGENCE_RECRUITING_FULL_PASS"
assert t.honesty_payload().get("uses_c1_registry_evaluator") is True
assert t.honesty_payload().get("ttf_distinct_from_tth") is True
assert t.honesty_payload().get("no_universal_source_effectiveness_score") is True
assert t.honesty_payload().get("canonical_hire_bridge") is True
...
cp -a \
  ...
  "$LOCAL_EVID/sources/"
cp -a "$REPO_ROOT/ops/HR_INTELLIGENCE_RECRUITING_FULL_PASS.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
cp -a "$REPO_ROOT/ops/HR_INTELLIGENCE_RECRUITING_C3_FREEZE_AMENDMENT.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
...
for flag in HR_INTELLIGENCE_RECRUITING_C3 HR_INTELLIGENCE_WORKFORCE_C2 HR_INTELLIGENCE_REGISTRY_C1; do
...
staging_py smoke-test-hr-intelligence-recruiting-c3.py "$LOCAL_EVID/tests/staging-db.out"
...
DB_OK=NO
if grep -q 'HR_INTELLIGENCE_RECRUITING_FULL_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$GATES_OK" == YES && "$DB_OK" == YES && "$C2_OK" == YES && "$C1_OK" == YES ]]; then
  VERDICT='HR_INTELLIGENCE_RECRUITING_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# HR Intelligence Recruiting C3 — Staging Prove
...
if [[ "$VERDICT" != HR_INTELLIGENCE_RECRUITING_FULL_PASS ]]; then
  exit 1
fi
exit 0
