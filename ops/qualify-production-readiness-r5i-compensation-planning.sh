#!/usr/bin/env bash
# Production Readiness R5I — Compensation Planning product surface qualification.
#
# Proves Wave 6 C6 Compensation Planning is a usable product:
# HTTP adapter → HR Web planning workspace.
# No Employee App surface. No HR Mobile surface.
# Job Architecture is a hard dependency.
# Workforce Planning stays unreleased.
# Does not begin R5J.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5i-compensation-planning-$STAMP"
REMOTE_STAGE="/tmp/r5i-compensation-planning-surface-stage"
...
log "2/8 local R5I + R5H + R5G + R5F + R5E + R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
cp -a "$DASH_SRC/src/posthire/CompensationPlanningWorkspace.tsx" "$LOCAL_EVID/dashboard/" 2>/dev/null || true
...
test -f '$STG/compensation_http.py'
test -f '$STG/compensation_surfaces.py'
test -f '$STG/compensation_planning_c6.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/compensation_http.py' '$STG/compensation_surfaces.py' '$STG/capability_readiness.py'
...
log "4/8 staging DB journeys A–G + HTTP security"
staging_py smoke-test-r5i-compensation-planning-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live comp planning customer enableable", ready.customer_enableable("comp_planning") is True)
...
for path in ("/dashboard/compensation-planning/workspace", "/dashboard/compensation-planning/cycles"):
...
log "6/8 regressions (Waves 1–6 + R2–R5H)"
REG_OK=YES
set +e
...
http = Path("wathefni-orchestrator/compensation_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/compensation-planning" in line]
...
grep -q 'R5I_COMPENSATION_PLANNING_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5i-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5i-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5I_COMPENSATION_PLANNING_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5H_DB_OK" == YES && "$R5G_DB_OK" == YES && "$R5F_DB_OK" == YES && "$R5E_DB_OK" == YES && "$R5D_DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5I_COMPENSATION_PLANNING_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5I — Compensation Planning Product Surface
...
- Waves 1–6 + R2–R5H regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5I Compensation Planning product surface over frozen Wave 6 C6 authority.
Workforce Planning remains unreleased.
FULL_PASS is not authorisation for production rollout.
Do not begin R5J until owner review.
EOF
