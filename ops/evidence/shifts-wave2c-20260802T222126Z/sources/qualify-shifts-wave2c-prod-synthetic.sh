...
cp -a "\$STAGE/run-shifts-wave2b-jobs.sh" "\$ORCH/ops/"
chmod +x "\$ORCH/ops/run-shifts-wave2b-jobs.sh"
cp -a "\$STAGE/shifts-wave2b-operator-jobs.md" "\$ORCH/ops/runbooks/"
cp -a "\$STAGE/install-shifts-wave2-job-timers.sh" "\$ORCH/ops/"
chmod +x "\$ORCH/ops/install-shifts-wave2-job-timers.sh"
# unit files staged beside install script
cp -a "\$STAGE"/wathefni-shifts-*.service "\$STAGE"/wathefni-shifts-*.timer "\$ORCH/ops/"
# preflight flags
{
  echo "stamp=$STAMP"
