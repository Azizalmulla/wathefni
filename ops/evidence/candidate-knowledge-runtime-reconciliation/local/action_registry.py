...
def _send_email_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="send_email")
    if denied:
        return denied
...
def _notify_candidate_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="notify_candidate")
    if denied:
        return denied
    account_id = getattr(ctx.request, "account_id", None)
...
def _send_assessment_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="send_assessment")
    if denied:
        return denied
    account_id = getattr(ctx.request, "account_id", None)
...
def _send_screening_questions_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="send_screening_questions")
    if denied:
        return denied
    company = str(app.get("company_code") or "").strip().upper()
    if not company:
...
def _send_video_interview_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="send_video_interview")
    if denied:
        return denied
...
def _execute_candidate_batch_preflight(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    preview = {
        ...
        "excluded_held_count": len(excluded_held),
    }
    preview["confirmation_text"] = "\n".join([
        ...
        *(
            [
                "",
                f"Excluded held/restricted rows: {len(excluded_held)}",
            ]
            if excluded_held
            else []
        ),
    ...
    return {
        ...
        "message": (
            ...
            f"I can {action_label} for: {', '.join(candidate_lines)}{channel_text}."
            + (f" Excluded {len(excluded_held)} held/restricted record(s)." if excluded_held else "")
            + " Confirm to continue?"
        ),
...
def _execute_candidate_batch_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    # Record excluded held rows as failed items without outbound work
    for denied in excluded_held:
        failed_count += 1
        results.append(
            {
                "app_key": denied.get("app_key"),
                "candidate_name": denied.get("candidate_name"),
                "status": "failed",
                "result": legacy.json_safe(
                    {
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE batch_id=%s
                """,
                (batch_status, success_count, failed_count, legacy.Json(legacy.json_safe({"items": results, "excluded_held": excluded_held})), batch_id),
            )
        conn.commit()
    return {
        "action_type": "execute_candidate_batch",
        ...
        "total_count": len(apps) + len(excluded_held),
        "excluded_held": excluded_held,
        "items": results,
    }
...
def _schedule_interview_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="schedule_interview")
    if denied:
        return denied
    text = " ".join(
        str(value or "")
...
def _send_interview_invite_executor(ctx: ExecutionContext) -> dict[str, Any]:
    ...
    denied = _assert_app_communication(ctx, app, action_type="send_interview_invite")
    if denied:
        return denied
