...
def iter_export_rows(
    ...
    *,
    ...
    # Reuse legacy SQL builders via orch helpers when present, with filter post-check.
    # For V1 we implement filtered queries directly for the five types.
    ...
    with orch.db_connect() as conn:
        with conn.cursor() as cur:
            ensure_schema(cur)
            if report_type == "candidates":
                # Selected columns follow cols exactly, so a disabled module
                # contributes neither a header nor a value.
                select_parts = [
                ...
                cur.execute(
                    ...
                    WHERE ci.company_code=%s AND {reviewable} AND {ingest_filter}
            ...
            for row in cur.fetchall():
                ...
                yield [payload.get(c) for c in cols]
