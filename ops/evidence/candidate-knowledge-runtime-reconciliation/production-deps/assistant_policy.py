...
def reports_parity_unavailable_message(*, locale_hint: str = "en") -> dict[str, Any]:
    ...
    return {
        ...
        "success": False,
        "status": "reports_parity_deferred" if False else "ok",  # placeholder overwritten by caller
        "error": None,
        "reports_boundary": True,
        "message": (
            "هذا سؤال تقارير أداء التوظيف. افتح صفحة Reports في لوحة التحكم، "
            "أو استخدم أداة التقارير عندما تكون متاحة. أرقام Overview ليست Reports."
            if ar
            else "That is a hiring Reports question. Open the Reports page in the dashboard "
            "(or use the Reports Assistant tool when available). Overview work-queue counts are not Reports."
        ),
