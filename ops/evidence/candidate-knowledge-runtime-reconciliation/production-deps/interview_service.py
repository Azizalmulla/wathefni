...
def resolve_meeting_fields(
    *,
    ...
    if mtype == "manual_link" and not link:
        raise life.InterviewAuthorityError("meet_link_required", "Online interviews require a meeting link when Google Meet is not used.")
    if mtype == "phone":
        link = None
    provider_key = "google" if (mtype == "google_meet" and google_connected) else "none"
    provider_sync = "pending" if provider_key == "google" else "not_configured"
    return {
        "meeting_type": mtype,
        "location": loc,
        "meet_link": link,
...
def sync_provider_for_interview(
    ...
    *,
    ...
    if provider_key == "none" or life.normalize_meeting_type(interview.get("meeting_type")) in {"in_person", "phone", "async_video", "none"}:
        if life.normalize_meeting_type(interview.get("meeting_type")) == "google_meet" and provider_key == "google":
            pass
        else:
            cur.execute(
                """
...
def schedule_interview(
    *,
    ...
    if life.normalize_meeting_type(meeting_type) == "google_meet" and google_connected:
        ...
        # meet_link may be filled by sync
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            conflicts = life.find_schedule_conflicts(
                cur,
                company_code=company,
                app_key=app_key,
                start=start_dt,
            ...
            if conflicts:
                life.mark_operation(cur, str(op["operation_id"]), lifecycle_status="failed", result_payload={"error": "schedule_conflict", "conflicts": conflicts})
                conn.commit()
                raise life.InterviewAuthorityError(
                    "schedule_conflict",
                    "Interview conflicts with an existing candidate or panel booking.",
                    status_code=409,
                    conflicts=legacy.json_safe(conflicts),
                )
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if conflicts:
                raise life.InterviewAuthorityError(
                    "schedule_conflict",
                    "Reschedule conflicts with an existing candidate or panel booking.",
                    status_code=409,
                    conflicts=legacy.json_safe(conflicts),
                )
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING *
                """,
                (next_status, notes or transcript, transcript, _json(summary), actor.get("actor_phone"), interview_id, company),
            )
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            for row in rows:
                ...
                items.append(enrich_interview_row(row, assignments=assignments))
