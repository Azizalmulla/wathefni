...
def main() -> int:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cov_warn = w5.evaluate_coverage_for_version(cur, company_code=company, version_id=version_id, period=period_res.get("period"))
            ...
            cov_block = w5.evaluate_coverage_for_version(cur, company_code=company, version_id=version_id, period=period_res.get("period"))
            ...
            cov_open = w5.evaluate_coverage_for_version(cur, company_code=company, version_id=version_id, period=period_res.get("period"))
            check(
            ...
            open_night = w5.create_open_shift(
                ...
                payload={
                    "period_id": period_id,
                    "shift_date": (start + timedelta(days=3)).isoformat(),
                    "start_time": "22:00",
                    "end_time": "06:00",
            ...
            open_direct = w5.create_open_shift(
                ...
                payload={
                    "period_id": period_id,
                    "shift_date": (start + timedelta(days=4)).isoformat(),
                    "start_time": "09:00",
                    "end_time": "17:00",
            ...
            if overlap_src:
                ...
                if claim_ov.get("ok"):
                    ...
                    check("gate_conflict without ack", gate.get("error") == "gate_conflict", gate)
                else:
            ...
            manual = app.create_shift_assignment(
                {
                    ...
                    "shift_date": (start + timedelta(days=5)).isoformat(),
                    "start_time": "10:00",
                    "end_time": "14:00",
