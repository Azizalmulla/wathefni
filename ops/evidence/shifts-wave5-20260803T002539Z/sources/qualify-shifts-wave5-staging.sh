...
W5_OK=NO
if grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/wave5-local.out" 2>/dev/null; then W5_OK=YES; fi
STG_W5_OK=NO
grep -A200 '=== W5 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE '[0-9]+ passed, 0 failed' && STG_W5_OK=YES || true
if grep -A200 '=== W5 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE 'FAIL  |Traceback'; then STG_W5_OK=NO; fi
if [[ "$W5_OK" != YES ]] && grep -q 'LOCAL_DB_UNAVAILABLE' "$LOCAL_EVID/tests/wave5-local.out" 2>/dev/null && [[ "$STG_W5_OK" == YES ]]; then
  W5_OK=YES
fi
W4_OK=NO
grep -A120 '=== W4 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE '[0-9]+ passed, 0 failed' && W4_OK=YES || true
if grep -A120 '=== W4 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE 'FAIL  |Traceback'; then W4_OK=NO; fi
W3_OK=NO
grep -A120 '=== W3 UX ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE '[0-9]+ passed, 0 failed' && W3_OK=YES || true
W1_OK=NO
grep -A20 '=== W1 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE 'passed, 0 failed' && W1_OK=YES || true
W2_OK=NO
grep -A30 '=== W2 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE 'passed, 0 failed' && W2_OK=YES || true
if grep -A40 '=== W2 ===' "$LOCAL_EVID/tests/staging-regressions.out" 2>/dev/null | grep -qE 'FAIL  |Traceback|IndexError'; then W2_OK=NO; fi
