...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        first_assign.click()
        page.wait_for_selector(f"[data-testid='held-row-assign-panel-{first_key}']", timeout=5000)
        ...
        open_panels = page.locator("[data-testid^='held-row-assign-panel-']")
        assert open_panels.count() == 1, f"expected only one row assign panel, got {open_panels.count()}"
        assert open_panels.first.get_attribute("data-testid") == f"held-row-assign-panel-{first_key}"
        ...
        page.get_by_role("button", name="Admit", exact=True).click()
