...
import { isDeviceScreenLocked } from './deviceLock'
import { classifyAuthFailure, markRefreshFailed, type AuthFailurePhase } from './authFailure'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const deviceLockedWhileAwayRef = useRef(false)
  const unlockEmployeeKeyRef = useRef('')
  const employeeKeyRef = useRef('')
  const appStateRef = useRef<AppStateStatus>(AppState.currentState)
  const awayLockTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const refreshMeRef = useRef<() => Promise<boolean>>(async () => false)
  ...
  employeeKeyRef.current =
    employeeKeyOf(me) || String(profile?.employee_key || '').trim() || unlockEmployeeKey || unlockEmployeeKeyRef.current
  ...
  }, [applyMeSignedIn, blockForError, refreshBiometricState, resetToOtpAfterPinFailure])
  ...
  const unlockWithPin = useCallback(
    ...
    [resetToOtpAfterPinFailure, unlockSealedSession],
  )
