...
export function shiftsCopy(locale: ShiftsLocale) {
  ...
  return {
    ...
    honesty: isAr
      ? 'الورديات تملك الفترات المخططة فقط. الحضور يملك الوقت الفعلي. الإجازات تملك القرارات. الرواتب تملك الحساب المالي. القوالب والجداول المتكررة تعليمات تخطيط؛ سلطة التشغيل تبقى في التعيينات L0.'
      : 'Shifts owns planned intervals only. Attendance owns worked time. Leave owns leave decisions. Payroll owns money. Templates and recurrences are planning instructions; L0 assignments remain operational authority.',
