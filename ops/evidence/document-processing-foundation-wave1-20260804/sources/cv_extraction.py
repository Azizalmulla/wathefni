...
def gpt_vision_rescue_enabled() -> bool:
    """Bounded GPT-5.4 vision rescue after Mistral OCR quality failure.

    Foundation / Production Authority Wave 1: GPT is NOT an automatic OCR fallback.
    Only explicit force flags may enable it.
    """
    try:
        from document_processing_foundation import gpt_auto_ocr_fallback_forbidden

        if gpt_auto_ocr_fallback_forbidden():
    ...
    *,
    ...
    if is_image:
        ...
        if mistral_ocr_enabled():
            ...
            # Rescue after OCR failure — GPT only if explicitly forced (foundation forbids auto).
        ...
        # Flag off: do not invoke GPT automatically under foundation policy.
        if vision_rescue is not None and gpt_vision_rescue_enabled():
