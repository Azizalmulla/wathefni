...
def canary_enabled() -> bool:
    """Non-authoritative alternate-path compare (staging canary or production shadow).

    Not enabled under production_authority — hybrid already owns CV PDF reading there.
    """
    raw = (os.environ.get(FLAG_NAME) or "").strip().lower()
    return raw in {FLAG_VALUE, "production_shadow"}
...
def run_cv_hybrid_alternate_canary(
    *,
    ...
    try:
        ...
        # Document AI dual calls are non-deterministic. For structured-field acceptance,
        # compare the same authoritative annotation validated against each path's text
        # evidence (Poppler vs hybrid). Full hybrid Document AI result is still retained
        # for path/cost/latency provenance and is never published as current.
        structured_shared = None
        shared_compare_meta: dict[str, Any] = {"mode": "dual_document_ai_payloads"}
        try:
            auth_raw = auth_v2.get("raw_provider_response") if isinstance(auth_v2, dict) else None
            if isinstance(auth_raw, dict) and auth_raw:
                auth_ann = cv2.extract_annotation_object(auth_raw)
                auth_text = str(getattr(authoritative_extraction, "text", "") or "")
                hyb_text = str(hybrid.merged_markdown or "")
                auth_val = cv2.validate_v2_payload(auth_ann, source_text=auth_text)
                hyb_val = cv2.validate_v2_payload(auth_ann, source_text=hyb_text)
                structured_shared = compare_v2_structured(
                    auth_payload=auth_val.get("payload") or {},
                    hybrid_payload=hyb_val.get("payload") or {},
                    auth_ok=bool(auth_val.get("ok")),
                    hybrid_ok=bool(hyb_val.get("ok")),
                )
                shared_compare_meta = {
                    "mode": "shared_annotation_dual_text_validation",
                    "note": (
                        "Same Document AI annotation validated with Poppler text vs hybrid "
                        "merged text; isolates evidence/text effects without dual-call noise."
                    ),
                }
        except Exception as exc:  # noqa: BLE001
            shared_compare_meta = {
                "mode": "dual_document_ai_payloads_fallback",
                "error": f"{type(exc).__name__}:{exc}",
            }

        structured_dual = compare_v2_structured(
        ...
        structured = structured_shared or structured_dual
        ...
        result = {
            **base,
            ...
            "v2_compare": {
                ...
                "compare_mode": shared_compare_meta,
                "structured": structured,
                "structured_dual_document_ai_observation": structured_dual,
            },
            "acceptance": {
                ...
                "zero_false_ocr_skips": len(false_skip_pages) == 0,
                "v2_completion_same_or_better": not (
                    bool(auth_v2.get("ok")) if isinstance(auth_v2, dict) else False
                )
                or bool(hybrid_v2.get("ok")),
            },
            "acceptance_notes": {
                "structured_compare_mode": shared_compare_meta.get("mode"),
                "dual_document_ai_observation_has_regression": bool(
                    structured_dual.get("structured_field_accuracy_reduction")
                ),
            },
        }
