...
def main() -> int:
    ...
    check("ER still hidden", ready.customer_enableable("employee_relations") is False)
    ...
    check("fail-closed keeps ER", ("employee_relations", "/dashboard/employee-relations") in ready.all_http_namespaces())
    ...
    check("Setup still omits ER", "Wave6EmployeeRelationsPoliciesCard" not in setup_app)
