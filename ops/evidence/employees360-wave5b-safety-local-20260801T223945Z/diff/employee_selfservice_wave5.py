...
def _apply_bank(legacy: Any, cur: Any, *, company: str, req: dict[str, Any], proposed: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"bank_profile_version": version, "bank_fingerprint": fp, "key_id": cipher.get("key_id"), "overlay": "employee_ess_bank_profiles"}
...
def revoke_employee_sessions(
    ...
    *,
    ...
    """Bump session_epoch (invalidate stale sessions) and optionally revoke binding."""
    company = _require_ess(legacy, context)
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            conn.commit()
    return {"ok": True, "binding": _jsonable(updated), "sessions_revoked": True}
