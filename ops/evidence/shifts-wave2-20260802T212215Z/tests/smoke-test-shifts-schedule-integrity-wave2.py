...
def main() -> int:
    ...
    midday_day = shift_day + timedelta(days=11)
    with app.db_connect() as conn:
        with conn.cursor() as cur:
    ...
    check("midday restriction site hit warns", bool(mid_hit.get("warnings")), mid_hit)
    check("midday other site not matched", not mid_miss_site.get("warnings"), mid_miss_site)
    ...
    check("wave2 module no leave_balances", "leave_balances" not in w2_src)
