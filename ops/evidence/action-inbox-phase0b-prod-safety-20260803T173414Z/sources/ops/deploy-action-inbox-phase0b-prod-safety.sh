#!/usr/bin/env bash
# Action Inbox Phase 0-B — production safety-gate deploy.
# Deploys Phase 0 viewer/subject gates + dashboard dist.
# Allowlists remain EMPTY. No real Aziz/Talal canary enablement.
set -euo pipefail

STAMP="${STAMP:?}"
ORCH=/opt/wathefni/orchestrator
BACKUP="/opt/wathefni/backups/production-pre-action-inbox-phase0b-${STAMP}"
REMOTE_EVID="/opt/wathefni/production-evidence/action-inbox-phase0b-prod-safety/${STAMP}"
STAGE="${STAGE_DIR:-/tmp/action-inbox-p0b-prod-stage}"
# Sorts after wave1b drop-in so empty allowlists / payroll exclude win.
DROPIN=/etc/systemd/system/wathefni-orchestrator.service.d/zzzzzzzzzzzzzzz-action-inbox-phase0b-safety.conf
...
mkdir -p "$ORCH/ops"
[[ -f "$STAGE/deploy-action-inbox-phase0b-prod-safety.sh" ]] && cp -a "$STAGE/deploy-action-inbox-phase0b-prod-safety.sh" "$ORCH/ops/"
...
[[ -f "$STAGE/App.tsx" ]] && cp -a "$STAGE/App.tsx" /opt/wathefni/apps/wathefni-dashboard/src/ || true
...
for f in api.ts moduleWorkspace.ts workspaceCapability.ts; do
  [[ -f "$STAGE/$f" ]] && cp -a "$STAGE/$f" /opt/wathefni/apps/wathefni-dashboard/src/lib/ || true
done

log "writing phase0b safety drop-in (EMPTY allowlists)"
cat > "$DROPIN" <<'EOF'
[Service]
...
"$PYBIN" - <<'PY' | tee "$REMOTE_EVID/flags/phase0b-flags.txt"
...
assert not w.real_viewer_allowlist()
assert not w.real_subject_allowlist()
assert w.honesty_payload().get("real_canary_enabled") is False
assert "def real_viewer_allowlist" in open("action_inbox_wave1.py", encoding="utf-8").read()
print("flags_ok_empty_allowlists=true")
PY
...
echo "DEPLOY_OK stamp=$STAMP backup=$BACKUP evid=$REMOTE_EVID allowlists=EMPTY"
