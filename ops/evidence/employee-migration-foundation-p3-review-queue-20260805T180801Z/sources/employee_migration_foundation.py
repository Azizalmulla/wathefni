...
def _row_entry(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "name_change_approved": bool(detail.get("name_change_approved")) if isinstance(detail, dict) else False,
        "detail": detail,
    }
...
def _is_resolved_identity_row(row: dict[str, Any], detail: dict[str, Any] | None = None) -> bool:
    """Approved / applied identity outcomes that clear the active queue for that person."""
    status = str(row.get("status") or "")
    detail = detail if detail is not None else _parse_row_detail(row)
    if detail.get("name_change_approved") and status in {"will_update", "updated", "skipped", "created"}:
        return True
    if status == "updated" and detail.get("name_identity_review"):
        return True
    return False
...
def list_review_items(
    ...
    *,
    ...
    """Active Needs review queue — one unresolved identity conflict per person/source.
            ...
            cur.execute(
                ...
                WHERE r.company_code=%s
                  ...
                  AND (
                    r.status = 'conflict'
                    OR (
                      r.status IN ('will_update', 'updated', 'skipped')
    ...
    # Newest row per identity decides: conflict → active; approved/resolved → suppress.
    ...
    for row in rows:
        ...
        if status == "conflict" and not _is_actionable_identity_conflict(row, detail):
            continue
        if status != "conflict" and not _is_resolved_identity_row(row, detail):
            continue
        key = _review_identity_key(row, detail)
        prior = newest_by_identity.get(key)
        if prior is None or _row_recency_tuple(row) > _row_recency_tuple(prior):
            newest_by_identity[key] = row

    items: list[dict[str, Any]] = []
    for row in sorted(newest_by_identity.values(), key=_row_recency_tuple, reverse=True):
        detail = _parse_row_detail(row)
        if str(row.get("status") or "") != "conflict":
            continue
        if not _is_actionable_identity_conflict(row, detail):
            continue
        ...
        # Calm queue copy — keep technical ids out of the primary reason when possible.
        if detail.get("name_identity_review"):
...
def approve_identity_name_change(
    ...
    *,
    ...
    Does not change employee_key or deactivate source mappings. Moves the row to
    will_update (preview) so Confirm import can apply the proposed field changes,
    including name.
    """
    company = legacy.require_employee_roster_admin(context)
    if not foundation_enabled(company):
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if not batch_row:
                raise legacy.HTTPException(status_code=404, detail={"error": "batch_not_found"})
            batch = dict(batch_row)
            if batch.get("status") not in {"previewed", "committing"}:
                raise legacy.HTTPException(
                    status_code=409,
                    detail={
                        "error": "batch_not_approvable",
                        "message": "Approve name changes before confirming the import.",
                        "status": batch.get("status"),
                    },
                )
            cur.execute(
            ...
            next_detail["name_change_approved_by"] = str(
                context.get("user_id") or context.get("email") or ""
            ).strip() or None
            next_detail["name_change_approved_at"] = datetime.now(timezone.utc).isoformat()
            next_detail["changes"] = changes
            next_detail["approvable"] = False
            # Keep mapping / employee_key intact.
            employee_key = row.get("employee_key") or (detail.get("match") or {}).get("employee_key")
            change_labels = ", ".join(sorted(changes.keys()))
            reason = (
                f"Will be updated ({change_labels}) — name change approved by HR."
            )
            cur.execute(
                """
                UPDATE employee_import_rows
            ...
            cur.execute(
                """
                UPDATE employee_import_batches
                SET totals=%s::jsonb, updated_at=now()
                WHERE batch_id=%s AND company_code=%s
                RETURNING *
                """,
                (json.dumps(totals), batch_id, company),
            )
            batch = dict(cur.fetchone())
            conn.commit()
    ...
    legacy.record_admin_audit(
        ...
        target=str(batch_id),
        details={"batch_id": str(batch_id), "row_id": str(row_id), "employee_key": employee_key},
    )
    out = _batch_payload(batch, rows)
    out["ok"] = True
    ...
    out["message"] = (
        "Name change approved. Confirm import is still required to apply the update."
    )
    out["dry_run"] = True
    return out
