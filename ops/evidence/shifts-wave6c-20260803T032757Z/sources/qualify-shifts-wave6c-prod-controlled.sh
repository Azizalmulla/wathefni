...
log "push stage"
"${SSH[@]}" "rm -rf '$REMOTE_STAGE' && mkdir -p '$REMOTE_STAGE/ops/sql' '$REMOTE_STAGE/dashboard-dist' '$REMOTE_STAGE/freeze' '$REMOTE_EVID'"
for f in "${MODULES[@]}"; do
...
"${SCP[@]}" "$REPO_ROOT/ops/SHIFTS_CONTROLLED_ROLLOUT_COMPLETION_AND_FREEZE.md" "$VPS_HOST:$REMOTE_STAGE/freeze/" >/dev/null
"${SCP[@]}" "$REPO_ROOT/.cursor/rules/shifts-freeze.mdc" "$VPS_HOST:$REMOTE_STAGE/freeze/" >/dev/null
...
for w in 1b 2b 3b 4b 5b 6b; do
  ...
  if [[ -f "\$f" ]]; then
    ...
    VAR=SHW\${w^^}_EVID
    if [[ "\$w" == "2b" ]]; then
      env "\$VAR=/tmp/shw6c-coexist/\$w" WATHEFNI_SHIFTS_INTEGRITY_JOBS=1 \$PY -u "\$f" | tee /tmp/shw6c-coexist/\$w.out | tail -6
    else
      env "\$VAR=/tmp/shw6c-coexist/\$w" \$PY -u "\$f" | tee /tmp/shw6c-coexist/\$w.out | tail -6
    fi
    echo "W\${w^^}_RC=\${PIPESTATUS[0]}"
    grep -E '^(FAIL|\[FAIL\])' /tmp/shw6c-coexist/\$w.out || true
  fi
done
export WATHEFNI_SHIFTS_INTEGRITY_JOBS=0
echo '=== W3 UX ==='; \$PY smoke-test-shifts-wave3-ux.py | tail -3; echo W3UX_RC=\${PIPESTATUS[0]}
...
echo '=== SHIFTS FREEZE ==='; \$PY smoke-test-shifts-freeze-regression.py | tee /tmp/shw6c-shfrz.out | tail -3; echo SHFRZ_RC=\${PIPESTATUS[0]}
grep -E '^FAIL' /tmp/shw6c-shfrz.out || true
echo '=== E360 ==='; \$PY smoke-test-employees360-freeze-regression.py | tail -3; echo E360_RC=\${PIPESTATUS[0]}
echo '=== ONB ==='; \$PY smoke-test-onboarding-freeze-regression.py | tail -3; echo ONB_RC=\${PIPESTATUS[0]}
echo '=== ATT ==='; \$PY smoke-test-attendance-freeze-regression.py | tail -3; echo ATT_RC=\${PIPESTATUS[0]}
echo '=== LEAVE ==='; \$PY smoke-test-leave-freeze-regression.py | tail -3; echo LEAVE_RC=\${PIPESTATUS[0]}
