...
def manager_real_rollout_enabled() -> bool:
    """Scoped-manager real scheduling. Hard NO-GO in Wave 6C.

    Production has no dashboard user with role `manager`, no `manager_scopes` rows and no
    branch/team structure, so there is no identity or scope to qualify against. Manager
    scope is proven on synthetic subjects only.
    """
    return False
...
def allowlists_within_approved_boundary() -> bool:
    ...
    Two postures are legal: the pre-Wave-6C synthetic-only posture (both allowlists empty)
    and the Wave 6C controlled posture (HR limited to the named approved operators, no
    manager at all). Anything else — any manager, or an HR phone nobody approved — is out
    of boundary and the prior-wave canaries must fail on it.
    """
...
def ensure_shifts_wave6c_schema(cur: Any) -> None:
