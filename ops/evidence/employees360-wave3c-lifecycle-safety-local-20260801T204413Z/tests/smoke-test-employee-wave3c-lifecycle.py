...
def main() -> int:
    ...
    try:
        ...
        # Reinstate after effective — same employment, no silent session restore
        hub_updated = proj_t["hub_updated_at"]
        version = int(proj_t["lifecycle_version"])
        # refresh hub token after downstream updates
        proj_t = w3c.get_lifecycle_projection(app, company_code=company, employee_key=key)
        hub_updated = proj_t["hub_updated_at"]
        version = int(proj_t["lifecycle_version"])
        ...
        check("reinstate committed", dec_re.get("committed") and dec_re.get("case_type") == "reinstate", dec_re)
