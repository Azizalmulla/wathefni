"""Employees 360 Wave 3C — production-safety checklist on Wave 3 lifecycle.

Implements Wave 3B P0-1..P0-10 for local/staging. No legal/payroll math.
Requires Wave 3 + Wave 2 flags. WATHEFNI allowlist by default.
"""
...
SCHEMA_VERSION = "employees360-wave3c-lifecycle-safety-v1"
...
DEFAULT_POLICY = {
    ...
    "disclaimer": "Policy hints only. Not legal advice. No EOSB or pay-in-lieu calculations.",
}
            ...
            if not row:
                cur.execute(
                    ...
                    (
                        ...
                        json.dumps({"disclaimer": DEFAULT_POLICY["disclaimer"], "wave": "wave3c"}),
                    ),
                )
                row = cur.fetchone()
            conn.commit()
            policy = _jsonable(dict(row))
            policy["disclaimer"] = DEFAULT_POLICY["disclaimer"]
            return policy
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (
                    ...
                    json.dumps(_jsonable(merged.get("policy_json") or {"disclaimer": DEFAULT_POLICY["disclaimer"]})),
...
def create_lifecycle_request(
    ...
    *,
    ...
    body["policy_snapshot"] = {
        ...
        "disclaimer": DEFAULT_POLICY["disclaimer"],
    }
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            packet = {
                ...
                "disclaimer": "Inputs only. No legal or payroll amounts calculated in Employees 360.",
            }
