...
def _configure_env() -> None:
    ...
    os.environ["WATHEFNI_ORG_HIERARCHY"] = "on"
...
def seed(app) -> dict[str, str]:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            for uid in (ids["owner_a"], ids["hr_a"], ids["mgr_a"], ids["hr_b"], ids["hr_c"]):
                cur.execute(
                    """
                    ...
                    VALUES (%s,%s,'employees.read','active','r9-attack',%s,'r9 directory probe')
                    ON CONFLICT (company_code, user_id, permission) DO UPDATE SET status='active'
                    """,
                    (
                        CO_A if uid in {ids["owner_a"], ids["hr_a"], ids["mgr_a"]} else (CO_B if uid == ids["hr_b"] else CO_C),
                        uid,
                        ids["owner_a"] if uid in {ids["owner_a"], ids["hr_a"], ids["mgr_a"]} else uid,
                    ),
                )
...
def attack(client, app, ids: dict[str, str]) -> None:
    ...
    check("HR A directory includes A1", r.status_code == 200 and EMP_A1 in keys, (r.status_code, keys, r.text[:200]))
    ...
    check("manager directory includes scoped A1", r.status_code == 200 and EMP_A1 in mgr_keys, (r.status_code, mgr_keys, r.text[:200]))
