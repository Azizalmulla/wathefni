...
def _refuse_nonsynthetic(
    *,
    ...
    """Fail closed under SYNTHETIC_ONLY for non-synthetic employee keys.

    Actor phone alone must never authorize a real employee_key.
    """
    ...
    if allowed_employee_keys is not None:
        ...
        if key not in allowed:
            return {"ok": False, "error": "payroll_wave5_synthetic_only", "employee_key": key}
    # Key gate is authoritative: markers must appear on employee_key.
    if not is_wave5_synthetic_employee(employee_key=key, phone=None):
        return {"ok": False, "error": "payroll_wave5_synthetic_only", "employee_key": key}
    # Optional actor-phone hygiene when provided.
    if phone and not is_wave5_synthetic_employee(employee_key=None, phone=phone):
        return {"ok": False, "error": "payroll_wave5_synthetic_only", "employee_key": key, "phone": digits_phone(phone)}
    return None
