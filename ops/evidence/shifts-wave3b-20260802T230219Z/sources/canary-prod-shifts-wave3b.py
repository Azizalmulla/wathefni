...
def main() -> int:
    ...
    # List wave3 enrich
    listing = app.dashboard_posthire_shifts(week=0, limit=200, context=ctx)
    ...
    check("list_contains_synth", len(found) >= 1, len(found))
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO shift_swap_requests (
                      company_code, requester_employee_key, target_employee_key, shift_id, status, reason, created_at
                    ) VALUES (%s,%s,%s,%s::uuid,'requested',%s, now())
                    RETURNING swap_id::text AS swap_id
                    """,
                    (COMPANY, KEY_A, KEY_B, sid_on, f"w3b swap {TAG}"),
                )
