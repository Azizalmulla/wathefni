...
| Scope | Verdict |
|---|---|
| Production synthetic Wave 3 UX | see canary results below |
| Controlled HR scheduling | **NO-GO** (allowlists empty) |
| Scoped manager scheduling | **NO-GO** (allowlists empty) |
| Talal read-only scheduling view | **NO-GO** for mutations (read-only by design; not separately qualified) |
| Real reminders | **NO-GO** |
| Broad employee-app rollout | **NO-GO** |

## Canary results
- Deploy #1 / rollback / redeploy: see \`tests/deploy*.out\`, \`tests/rollback.out\`
- API canary #1: $API1_PASS
- API canary #2: $API2_PASS
