...
class PostgresCaptureStore:
    ...
    def cleanup_synthetic(self, marker: str) -> dict[str, Any]:
        ...
        with self._conn() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT set_config('wathefni.allow_capture_cleanup','1', true)")
                # delete by connector/site name markers
                cur.execute(
                    """
                    DELETE FROM attendance_capture_replay_ledger
                    WHERE item_id IN (
                      SELECT item_id FROM attendance_capture_remediation
                      WHERE source_event_id LIKE %s OR device_user_id LIKE %s OR employee_key LIKE %s
                    )
                    """,
                    (f"%{marker}%", f"%{marker}%", f"%{marker}%"),
                )
                cur.execute("DELETE FROM attendance_capture_idempotency WHERE idempotency_key LIKE %s", (f"%{marker}%",))
                cur.execute(
                    "DELETE FROM attendance_capture_audit_events WHERE payload::text LIKE %s OR connector_id LIKE %s",
                    (f"%{marker}%", f"%{marker}%"),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_remediation WHERE source_event_id LIKE %s OR device_user_id LIKE %s OR employee_key LIKE %s",
                    (f"%{marker}%", f"%{marker}%", f"%{marker}%"),
                )
                cur.execute("DELETE FROM attendance_capture_mappings WHERE device_user_id LIKE %s OR employee_key LIKE %s", (f"%{marker}%", f"%{marker}%"))
                cur.execute(
                    """
                    DELETE FROM attendance_capture_health_events WHERE connector_id IN (
                      SELECT connector_id FROM attendance_capture_connectors WHERE connector_id LIKE %s OR connector_version LIKE %s
                    )
                    """,
                    (f"%{marker}%", f"%{marker}%"),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_health WHERE connector_id IN (SELECT connector_id FROM attendance_capture_connectors WHERE connector_id LIKE %s)",
                    (f"%{marker}%",),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_checkpoints WHERE connector_id IN (SELECT connector_id FROM attendance_capture_connectors WHERE connector_id LIKE %s)",
                    (f"%{marker}%",),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_credentials WHERE connector_id IN (SELECT connector_id FROM attendance_capture_connectors WHERE connector_id LIKE %s)",
                    (f"%{marker}%",),
                )
                cur.execute("DELETE FROM attendance_capture_connectors WHERE connector_id LIKE %s OR connector_version LIKE %s", (f"%{marker}%", f"%{marker}%"))
                cur.execute("DELETE FROM attendance_capture_devices WHERE terminal_sn LIKE %s", (f"%{marker}%",))
                cur.execute("DELETE FROM attendance_capture_sites WHERE name LIKE %s", (f"%{marker}%",))
