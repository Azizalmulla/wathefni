...
/** Inline-start rail carries state without turning every healthy tile into a status badge. */
export function shiftStateRailClass(shift: PosthireShiftRow): string {
  const ui = String(shift.ui_state || shift.status || 'scheduled').toLowerCase()
  if (ui === 'cancelled' || String(shift.status).toLowerCase() === 'cancelled') {
    return 'bg-wf-accent-paused'
  }
  if (ui === 'reconciliation_required') return 'bg-wf-accent-assess-ink'
  if (ui === 'conflicted') return 'bg-wf-accent-review-ink'
  if (shift.ends_next_day || shift.is_overnight) return 'bg-wf-accent-follow'
  return 'bg-wf-accent-priority'
}

export function shiftStateTextClass(shift: PosthireShiftRow): string {
  const ui = String(shift.ui_state || shift.status || 'scheduled').toLowerCase()
  if (ui === 'cancelled' || String(shift.status).toLowerCase() === 'cancelled') {
    return 'text-wf-accent-paused-ink'
  }
  if (ui === 'reconciliation_required') return 'text-wf-accent-assess-ink'
  if (ui === 'conflicted') return 'text-wf-accent-review-ink'
  if (shift.ends_next_day || shift.is_overnight) return 'text-wf-accent-follow-ink'
  return 'text-wf-accent-priority-ink'
}
