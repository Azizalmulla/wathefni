...
function roleLine(shift: PosthireShiftRow) {
  return String(shift.role || shift.location || shift.site_key || shift.team_key || '—')
}

function locationLine(shift: PosthireShiftRow) {
  const values = [shift.site_key, shift.team_key, shift.location].filter(
    (value, index, all) => value && all.indexOf(value) === index && value !== shift.role,
  )
  return values.join(' · ')
}
...
function WeekShiftTile({
  ...
  const status = isExceptional(shift) ? shiftStatusLabel(state, locale) : overnight ? c.overnight : ''
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 items-center justify-between gap-1.5">
        <span
          className={cn(
            'truncate text-[11px] font-bold leading-none tracking-[-0.02em] [font-variant-numeric:tabular-nums]',
            overlap && 'text-[10px]',
          )}
        >
          {continuation ? `← ${end}` : overnight ? `${start} →` : `${start}–${end}`}
        </span>
        {status ? (
          <span
            className={cn(
      ...
      <span className={cn('mt-1 block truncate text-[10px] font-semibold leading-tight', overlap && 'text-[9px]')}>
        {continuation ? c.overnightHint : roleLine(shift)}
      </span>
      {!overlap && locationLine(shift) ? (
        <span className="mt-0.5 block truncate text-[8px] leading-tight opacity-65">{locationLine(shift)}</span>
      ) : null}
    </button>
  )
}
...
function RosterIdentity({ row, compact = false }: { row: RosterRow; compact?: boolean }) {
  const palette = ['bg-wf-accent-priority-soft text-wf-accent-priority-ink', 'bg-wf-accent-follow-soft text-wf-accent-follow-ink', 'bg-wf-accent-assess-soft text-wf-accent-assess-ink']
  const tone = palette[Math.abs(row.key.length) % palette.length]
  return (
    <div
      className={cn(
        'grid min-w-0 items-center gap-2',
        compact ? 'grid-cols-[2.25rem_minmax(0,1fr)]' : 'grid-cols-[2rem_minmax(0,1fr)_auto]',
      )}
      data-shifts-identity
    >
      <span className={cn('grid h-8 w-8 place-items-center rounded-[0.7rem] text-[10px] font-bold', tone)}>
        {initials(row.label)}
      </span>
      <span className="min-w-0">
        <span className="block truncate text-[12px] font-semibold tracking-[-0.015em] text-ink">{row.label}</span>
        {row.site || row.team ? (
      ...
      {!compact ? (
        <span className="text-[9px] text-muted [font-variant-numeric:tabular-nums]">
          {(row.weeklyMinutes / 60).toFixed(row.weeklyMinutes % 60 ? 1 : 0)}h
        </span>
      ) : null}
    </div>
  )
...
function DayAgendaShift({
  ...
  return (
    <WeekShiftTile shift={shift} locale={locale} selected={selected} overlap={false} onSelect={onSelect} />
  )
}
  ...
  return (
    <div className="min-w-[1080px]" data-shifts-roster-loading aria-label={c.loading}>
      {Array.from({ length: 4 }, (_, row) => (
        <div
          key={row}
          className="grid grid-cols-[190px_repeat(7,minmax(124px,1fr))] gap-[3px] border-t border-wf-ink/[0.05] px-3"
        >
  ...
  return (
    <div className="overflow-x-auto" data-shifts-roster-grid>
      <div className="min-w-[1080px]">
        <div className="sticky top-0 z-[4] grid grid-cols-[190px_repeat(7,minmax(124px,1fr))] gap-[3px] bg-wf-surface-raised/95 px-3 pb-2 pt-3 backdrop-blur-sm">
        ...
        {coldLoading ? (
          ...
          <div
            ...
            data-shifts-empty
          >
            <div className="sticky start-3 z-[2] flex min-h-28 items-center gap-2 bg-wf-surface-raised py-3 pe-3">
            ...
            <div className="col-span-7 flex min-h-28 items-center bg-wf-frame/25 px-4">
              {canManage ? (
                <Button size="sm" onClick={() => onCreate(days[0])}>
                  <Plus className="h-4 w-4" /> {c.schedule}
                </Button>
              ) : null}
            </div>
                  ...
                  >
                    <div className={cn('grid gap-1.5', overlap && 'grid-cols-2')} data-shifts-overlap={overlap ? 'true' : 'false'}>
                      ...
                      {canManage && items.length === 0 ? (
                        <button
                          type="button"
                          className="min-h-7 rounded-lg border border-dashed border-wf-ink/10 text-[9px] text-muted opacity-0 transition hover:bg-wf-surface hover:opacity-100 focus:opacity-100"
        ...
        <div className="flex items-center gap-3 px-4 py-3 text-[9px] text-muted" data-shifts-legend>
          {[
            ['bg-wf-accent-priority', isAr ? 'مجدولة' : 'Scheduled'],
            ['bg-wf-accent-review', isAr ? 'تعارض' : 'Conflict'],
            ['bg-wf-accent-assess', isAr ? 'مراجعة' : 'Review'],
            ['bg-wf-accent-follow', isAr ? 'ليلية' : 'Overnight'],
          ].map(([tone, label]) => (
