...
def runtime_gate_for_company(company_code: str | None) -> dict[str, Any]:
    ...
    if not c1.slice_allowlist_admits(company, "WATHEFNI_HR_INTELLIGENCE_WORKFORCE_COMPANIES"):
        return {
            "ok": False,
            "enabled": False,
...
def _handler_turnover(cur, *, company_code, settings, formula_contract, publication, filters, time_window, actor_phone, actor_role):
    ...
    if not hc_start.get("ok") or not hc_end.get("ok"):
        return {
            "status": "unavailable",
            "value": None,
            "population_ids": [],
            "error": "workforce_population_unavailable",
            "explain": {"hc_start": hc_start, "hc_end": hc_end},
        }
