...
def main() -> int:
    ...
    print("\n    readiness contract")
    check("zero unreleased capabilities after R5J", len(ready.UNRELEASED_CAPABILITY_KEYS) == 0)
    ...
    check("workforce planning left unreleased set", "workforce_planning" not in ready.UNRELEASED_CAPABILITY_KEYS)
    ...
    check("workforce planning is a catalog SKU", "workforce_planning" in catalog.MODULE_BY_KEY)
    ...
    wfp_ready = ready.readiness_payload("workforce_planning")
    check("workforce planning http_ready", wfp_ready.get("http_ready") is True)
    check("workforce planning hr_web_ready", wfp_ready.get("hr_web_ready") is True)
    check("workforce planning employee surface not required", wfp_ready.get("employee_surface_required") is False)
    check("workforce planning mobile not required", wfp_ready.get("mobile_required") is False)
    check("workforce planning people_surface off", catalog.MODULE_BY_KEY["workforce_planning"].people_surface is False)
    check("workforce planning depends on JA", "job_architecture" in (wfp_ready.get("depends_on") or ()))
    ...
    check("wave6 workforce planning write unblocked", ready.customer_setup_write_block("wave6_workforce_planning") is None)
    ...
    check("catalog enable workforce planning unblocked", ready.unreleased_catalog_enable_block("workforce_planning") is None)
    ...
    check("Wave6 Workforce Planning card remounted", "Wave6WorkforcePlanningPoliciesCard" in setup_app)
    check("classic-wave6-workforce-planning mounted", "classic-wave6-workforce-planning" in setup_app or "Wave6WorkforcePlanningPoliciesCard" in setup_app)
    check(
    ...
    check(
        "Wave6 Workforce Planning card file retained",
        (DASH / "setup-console" / "Wave6WorkforcePlanningPoliciesCard.tsx").is_file(),
    )
    ...
    check("no remaining unreleased fail-closed namespaces", ready.all_http_namespaces() == [])
    ...
    wfp_hit = client.get("/dashboard/workforce-planning")
    check(
        "fail-closed does not steal Workforce Planning namespace",
        not (
            wfp_hit.status_code == 404
            and isinstance(wfp_hit.json().get("detail"), dict)
            and wfp_hit.json()["detail"].get("error") == "capability_not_released"
        ),
        wfp_hit.json() if wfp_hit.headers.get("content-type", "").startswith("application/json") else wfp_hit.status_code,
    )
