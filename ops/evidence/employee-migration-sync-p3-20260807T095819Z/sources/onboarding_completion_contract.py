...
def recompute(
    ...
    *,
    ...
    # P3: settled migration dispositions win until native Wathefni activity appears.
    try:
        import employee_migration_onboarding as _omo

        guarded = _omo.recompute_guard(cur, company=company, employee_key=employee_key)
...
def reminder_targets(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    ...
    """
    migration = snapshot.get("migration") if isinstance(snapshot.get("migration"), dict) else {}
    if migration.get("suppress_reminders") or migration.get("disposition") in {
