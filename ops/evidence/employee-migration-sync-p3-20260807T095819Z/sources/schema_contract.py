    ...
    *,
    ...
    try:
        try:
            ...
            # If the caller already aborted, clear before DDL.
...
def _record_ledger(cur: Any, *, module: str, mode: str) -> None:
    """Best-effort migration ledger write.

    Must never abort the caller's transaction. PostgreSQL ``now()`` is stable
    within a transaction, so repeated apply of the same module in one txn can
    hit the ledger primary key; swallow that with a savepoint + DO NOTHING.
    """
    try:
        cur.execute("SAVEPOINT wathefni_schema_ledger")
        ...
        cur.execute("RELEASE SAVEPOINT wathefni_schema_ledger")
    except Exception:
        try:
            cur.execute("ROLLBACK TO SAVEPOINT wathefni_schema_ledger")
        except Exception:
            pass
