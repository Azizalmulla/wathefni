...
def main() -> int:
    ...
    check("engagement still hidden", ready.customer_enableable("engagement") is False)
    check("comp planning still hidden", ready.customer_enableable("comp_planning") is False)
    ...
    check(
        "fail-closed keeps engagement",
        ("engagement", "/dashboard/engagement") in ready.all_http_namespaces(),
    )
    ...
    check("Setup still omits Engagement", "Wave6EngagementPoliciesCard" not in setup_app)
    ...
    engagement_detail = engagement.json().get("detail") if engagement.headers.get("content-type", "").startswith("application/json") else None
    check(
        "engagement still fail-closed",
        engagement.status_code == 404
        and isinstance(engagement_detail, dict)
        and engagement_detail.get("error") == "capability_not_released",
        engagement_detail,
    )
