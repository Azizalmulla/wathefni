    ...
    *,
    ...
    if idem:
        cur.execute(
            ...
            SELECT task_id, company_code, task_type, status, subject_type, subject_id,
                   title, due_at, sla_clock_id, metadata, row_version
              FROM hr_tasks
             WHERE company_code=%s
    ...
    cur.execute(
        ...
        (
            ...
            assignee_uuid,
            str(subject_type or "").strip() or None,
            str(subject_id or "").strip() or None,
            due_at,
            sla_clock_id,
            json.dumps(meta),
        ),
    ...
    _insert_event(
        ...
        actor_user_id=actor_user_id,
        actor_phone=actor_phone,
        payload={"task_type": tt, "subject_type": subject_type, "subject_id": subject_id},
    )
...
def resolve_workflow_task(
    ...
    *,
    ...
    _insert_event(
        ...
        actor_user_id=actor_user_id,
        actor_phone=actor_phone,
        payload={"status": status_n},
    )
    return {"ok": True, "task": _task_row(dict(updated))}
...
def start_sla_clock(
    ...
    *,
    ...
    when = now or utc_now()
    due_at = when + timedelta(seconds=int(policy["due_in_seconds"]))
    remind_at = None
    if policy.get("remind_before_seconds") is not None:
        remind_at = due_at - timedelta(seconds=int(policy["remind_before_seconds"]))
...
def satisfy_sla_clock(
    ...
    *,
    ...
    return {"ok": True, "clock": _clock_row(dict(row))}
...
def tick_sla_clock(
    ...
    *,
    ...
    if clock["status"] != "running":
        return {"ok": True, "clock": clock, "action": "noop"}
