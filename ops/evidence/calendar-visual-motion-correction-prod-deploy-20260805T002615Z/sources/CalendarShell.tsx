...
export function CalendarShell({
  ...
  const previewMeta = eventsQuery.data?.populated_preview
  const previewMode = Boolean(previewMeta?.enabled)
  const refreshing = eventsQuery.isFetching && Boolean(eventsQuery.data)
  ...
  useEffect(() => {
    ...
    // Keep the prior complete surface painted until the requested query is ready,
    // then swap it once. This avoids empty flashes across scope/filter/view changes.
    startTransition(() => setPaintedCalendar(next))
  }, [anchor, eventsQuery.data, eventsQuery.isPlaceholderData, range, view])
    ...
    <section className="relative space-y-2" dir={isAr ? 'rtl' : 'ltr'} data-calendar-wave1c data-calendar-composition>
      ...
      >
        ...
        <div className="ms-auto inline-flex items-center gap-1.5">
          <div className="inline-flex rounded-full bg-panel-muted p-0.5" role="group" aria-label={isAr ? 'نطاق التقويم' : 'Calendar scope'}>
            ...
            {canCompany ? (
              <button
                ...
                onClick={() => {
                  if (scope !== 'company') closeSelected()
                  setScope('company')
                }}
              >
                {isAr ? 'الشركة' : 'Company'}
          ...
          {effectiveScope !== 'mine' ? (
            <label className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-line/55 bg-wf-surface-raised/90 px-2 text-[12px] text-ink">
              <input
                type="checkbox"
                checked={mineOnly}
                onChange={(e) => {
        ...
        {selectedId && selected && !composerOpen
          ...
          >
            ...
            <div className="overflow-y-auto px-3.5 py-3">
              ...
              {selected.detail_level === 'full' && selected.status !== 'cancelled' && !isCalendarPreviewEvent(selected) ? (
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {canManage && selected.detail_level === 'full' && !selected.interview_managed && !isCalendarPreviewEvent(selected) ? (
                  <>
