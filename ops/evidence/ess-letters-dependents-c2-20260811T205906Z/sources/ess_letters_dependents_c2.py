...
def _sync_ess_letter_order(
    ...
    *,
    ...
    sp = f"c2_ess_order_{uuid.uuid4().hex[:10]}"
    try:
        cur.execute(f"SAVEPOINT {sp}")
        cur.execute("SELECT to_regclass('public.employee_ess_letter_orders') AS t")
        if not dict(cur.fetchone()).get("t"):
            cur.execute(f"RELEASE SAVEPOINT {sp}")
            return
        # letter_type CHECK may reject salary_certificate on older schemas — skip then
        payload = {
...
def _sync_lifecycle_service_certificate(
    ...
    *,
    ...
    """Best-effort Art. 54 alignment — only when employment_id is available."""
    sp = f"c2_svc_cert_{uuid.uuid4().hex[:10]}"
    try:
        cur.execute(f"SAVEPOINT {sp}")
        ...
        cur.execute(
            """
            SELECT employment_id::text AS employment_id, person_id::text AS person_id
              FROM employee_lifecycle_employments
        ...
        emp = cur.fetchone()
        if not emp or not dict(emp).get("employment_id"):
            # No lifecycle employment — skip; C2 version remains SoT
            cur.execute(f"RELEASE SAVEPOINT {sp}")
            return
        ed = dict(emp)
        cur.execute(
            ...
            INSERT INTO employee_lifecycle_service_certificates (
              ...
              issued_at, payload, created_at, updated_at
            ) VALUES (
              %s,%s,%s::uuid,%s,'issued',now(),%s::jsonb,now(),now()
            )
            """,
            (
                company,
                employee_key,
                ed.get("employment_id"),
                ed.get("person_id"),
                json.dumps(
                    {
                        "c2_version_id": str(version.get("version_id")),
