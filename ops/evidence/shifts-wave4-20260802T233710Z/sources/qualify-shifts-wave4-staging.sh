...
mkdir -p /etc/systemd/system/wathefni-staging-orchestrator.service.d
cat > /etc/systemd/system/wathefni-staging-orchestrator.service.d/zzzz-shifts-wave4.conf <<'EOF'
[Service]
...
systemctl restart wathefni-staging-orchestrator || systemctl restart wathefni-orchestrator-staging || true
sleep 2
...
set +e
STG=/opt/wathefni/staging/orchestrator
PY=\$STG/.venv/bin/python
cd \$STG
set -a
if [[ -f /root/.openclaw/secrets/postgres.staging.env ]]; then
  source /root/.openclaw/secrets/postgres.staging.env
elif [[ -f /opt/wathefni/staging/secrets/postgres.env ]]; then
  source /opt/wathefni/staging/secrets/postgres.env
fi
set +a
# Prefer staging service env
for unit in wathefni-staging-orchestrator wathefni-orchestrator-staging; do
...
echo '=== W4 ==='; \$PY smoke-test-shifts-templates-wave4.py; echo W4_RC=\$?
echo '=== W3 UX ==='; \$PY smoke-test-shifts-wave3-ux.py; echo W3_RC=\$?
echo '=== W1 ==='; \$PY smoke-test-shifts-authority-wave1.py | tail -5; echo W1_RC=\${PIPESTATUS[0]}
echo '=== W2 ==='; \$PY smoke-test-shifts-schedule-integrity-wave2.py | tail -8; echo W2_RC=\${PIPESTATUS[0]}
