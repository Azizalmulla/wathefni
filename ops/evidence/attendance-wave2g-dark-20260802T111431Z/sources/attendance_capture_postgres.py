...
class PostgresCaptureStore:
    ...
    def cleanup_synthetic(self, marker: str) -> dict[str, Any]:
        """Canary cleanup — deletes by marker including connectors linked via site/device."""
        like = f"%{marker}%"
        with self._conn() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT set_config('wathefni.allow_capture_cleanup','1', true)")
                cur.execute(
                    ...
                    WHERE connector_id ILIKE %s
                       OR connector_version ILIKE %s
                       OR company_code ILIKE %s
                       OR device_id IN (
                            SELECT device_id FROM attendance_capture_devices
                            WHERE terminal_sn ILIKE %s OR company_code ILIKE %s
                       )
                       OR site_id IN (
                            SELECT site_id FROM attendance_capture_sites
                            WHERE name ILIKE %s OR company_code ILIKE %s
                       )
                    """,
                ...
                cur.execute(
                    ...
                    WHERE item_id IN (
                      ...
                      WHERE source_event_id ILIKE %s OR device_user_id ILIKE %s OR employee_key ILIKE %s
                         OR company_code ILIKE %s
                         OR (cardinality(%s::text[]) > 0 AND connector_id = ANY(%s))
                    )
                    OR company_code ILIKE %s
                    """,
                    (like, like, like, like, connector_ids, connector_ids, like),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_idempotency WHERE idempotency_key ILIKE %s OR company_code ILIKE %s",
                    (like, like),
                )
                cur.execute(
                ...
                if connector_ids:
                    cur.execute("DELETE FROM attendance_capture_audit_events WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_health_events WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_health WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_checkpoints WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_credentials WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_remediation WHERE connector_id = ANY(%s)", (connector_ids,))
                    cur.execute("DELETE FROM attendance_capture_connectors WHERE connector_id = ANY(%s)", (connector_ids,))
                cur.execute(
                ...
                cur.execute(
                    "DELETE FROM attendance_capture_quarantine WHERE company_code ILIKE %s OR source_event_id ILIKE %s",
                    (like, like),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_devices WHERE terminal_sn ILIKE %s OR company_code ILIKE %s",
                    (like, like),
                )
                cur.execute(
                    "DELETE FROM attendance_capture_sites WHERE name ILIKE %s OR company_code ILIKE %s",
                    (like, like),
                )
        return {"ok": True, "marker": marker, "connectors_removed": len(connector_ids)}
