    ...
    *,
    ...
    with orch.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if not include_terminal:
                count_where.append("a.status NOT IN ('hired','rejected','withdrawn')")
    ...
    return {
        ...
        "applications": rows,
        "authority": "sql_job_pool",
        "denylist_version": DENYLIST_VERSION,
    }


def load_complete_job_pool(orch: Any, *, company_code: str, position_code: str) -> list[dict[str, Any]]:
    """Load the full authoritative pool (no semantic truncation)."""
...
def rank_job_applications(
    ...
    *,
    ...
    pool_rows = load_complete_job_pool(orch, company_code=company, position_code=position)
    ...
    # Soft rank only reorders; never drops.
    # Advisory (not_applicable) compares alongside eligible when no hard criteria exist.
    ordered = sorted(
        items,
        key=lambda item: (
            0
            if item["eligibility_bucket"] in {"eligible", "not_applicable"}
            and item.get("required_evidence_complete", True)
            else 1,
            -(float(item.get("advisory_score") or 0)),
    ...
    visible_rank = 0
    for item in ordered:
        if not item.get("required_evidence_complete"):
            item["soft_rank"] = None
            continue
        visible_rank += 1
    ...
    eligible_count = sum(1 for i in ordered if i["eligibility_bucket"] == "eligible")
    not_met_count = sum(1 for i in ordered if i["eligibility_bucket"] == "requirement_not_met")
    unknown_count = sum(1 for i in ordered if i["eligibility_bucket"] == "insufficient_information")
...
def to_legacy_rank_candidates_shape(result: dict[str, Any], *, top_n: int | None = None) -> dict[str, Any]:
    ...
    for item in selected:
        ...
        # Legacy gpt-5.4 evaluations must not be presented as canonical R0–R3 narratives.
        ...
        candidates.append(
            {
                ...
                "score": item.get("advisory_score"),
                "score_breakdown": item.get("component_scores"),
                "confidence": item.get("confidence"),
                "evidence": [e.get("field") for e in (item.get("evidence") or []) if isinstance(e, dict)],
                "reasons": [backend_explanation],
                "eligibility_bucket": item.get("eligibility_bucket"),
                "requirement_results": item.get("requirement_results"),
                "missing_data": item.get("missing_data"),
                ...
                "gpt_evaluation": {
                    "source": gpt_source,
                    "fit_summary": display_explanation,
                    "backend_explanation": backend_explanation,
                    ...
                    "legacy": False,
                    "canonical": True,
                    "strengths": [str(e.get("value")) for e in (item.get("evidence") or [])[:3] if isinstance(e, dict)],
                    "gaps": list(item.get("missing_data") or [])[:3],
    ...
    return {
        ...
        "total_matching": result.get("pool_total"),
        "pool_scanned": result.get("pool_total"),
        "shown_top_n": len(candidates),
        "capped": len(items) > len(candidates),
