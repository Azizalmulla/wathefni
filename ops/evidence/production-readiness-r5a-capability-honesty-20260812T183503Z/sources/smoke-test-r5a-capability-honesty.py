...
def main() -> int:
    ...
    print("\n    readiness contract")
    check("nine unreleased capabilities", len(ready.UNRELEASED_CAPABILITY_KEYS) == 9)
    ...
    check("usable false despite stored enabled", annotated.get("usable") is False)
    check("customer_facing_state not_released", annotated.get("customer_facing_state") == "not_released")
    ...
    check("catalog enable performance blocked", (catalog_block or {}).get("error") == "capability_not_customer_enableable")
    ...
    check("Wave4 card omitted", "Wave4PerformanceTalentPoliciesCard" not in setup_app)
    check(
    ...
    check("no classic-wave4 mount", "classic-wave4" not in setup_app)
    ...
    for cap, path in (
        ...
        ("performance", "/dashboard/performance/goals"),
