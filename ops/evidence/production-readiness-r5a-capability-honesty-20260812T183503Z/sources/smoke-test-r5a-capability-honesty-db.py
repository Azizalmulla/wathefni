    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                check("GET usable false", annotated.get("usable") is False, annotated)
                check("GET customer_enableable false", annotated.get("customer_enableable") is False, annotated)

                all_wave4 = ready.annotate_wave4_setup(w4p.get_all_wave4_policies(cur, COMPANY))
                check("wave4 payload not customer visible", all_wave4.get("customer_visible") is False)
                ...
                check("wave4 goals not usable", goals.get("usable") is False, goals)

                blocked = ready.customer_setup_write_block("wave4_performance_goals")
                check("customer cannot enable performance", (blocked or {}).get("error") == "capability_not_customer_enableable")
        ...
        check(
            "env on still not customer usable",
            ready.customer_usable(capability_key="performance", stored_enabled=True, runtime_available=True) is False,
        )
