...
def frontend_setup_cards_scan(dashboard_setup_dir: Path | None = None) -> dict[str, Any]:
    ...
    # R5A: card files stay for later unhide; customer Setup must omit enable toggles.
    mounted = [name for name in component_names if name in app_text]
    return {
        "ok": not missing and not mounted and app.exists(),
        "missing_cards": missing,
        "unmounted": [name for name in component_names if name not in app_text],
        "mounted_customer_cards": mounted,
        "setup_owns_seven_cards": True,
        "customer_setup_omits_unreleased_cards": not mounted,
    }
