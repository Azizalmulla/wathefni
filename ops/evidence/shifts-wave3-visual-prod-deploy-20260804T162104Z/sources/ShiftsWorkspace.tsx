/**
 * Shifts workspace — Visual Design Wave 3 shared post-hiring baseline.
...
import { shiftStateChipClass, shiftStateSurfaceClass } from '@/posthire/visualBaseline'
import type {
  DashboardAccess,
  OrgUnitRow,
...
function ShiftBlock({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        'w-full cursor-pointer rounded-[0.95rem] border-0 px-2.5 py-2 text-start shadow-[0_1px_0_rgba(255,255,255,0.35)_inset,0_6px_16px_rgba(35,33,29,0.06)] transition hover:-translate-y-px hover:brightness-[0.99]',
        shiftSurfaceClass(shift),
        selected && 'ring-2 ring-wf-ink/35',
        compact && 'px-2 py-1.5',
      )}
      data-testid="shift-block"
      data-shift-state={String(ui).toLowerCase()}
    >
      <div className="flex items-start justify-between gap-2">
        <p className={cn('font-semibold leading-snug tracking-[-0.01em]', compact ? 'text-[12px]' : 'text-[13px]')}>
          {shift.employee_name || shift.employee_key || '—'}
        </p>
        <span className={cn('shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-medium', shiftStateChipClass(shift))}>
          {shiftStatusLabel(ui, locale)}
        </span>
      </div>
      <p className={cn('mt-0.5 opacity-80', compact ? 'text-[11px]' : 'text-[12px]')}>
        {start} – {end}
      ...
      {overnight && !compact ? <p className="mt-1 text-[10px] font-medium opacity-75">{c.overnightHint}</p> : null}
    </button>
  )
}
...
export function ShiftsWorkspace({ access, permissions, role: _role, onNotice, onAccessIssue }: ShiftsWorkspaceProps) {
  ...
  const filterKey = useMemo(
    ...
    [debouncedEmployee, filters.branch_key, filters.site_key, filters.team_key, filters.role, filters.status],
  )
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2" data-shifts-surfaces role="tablist" aria-label={c.title}>
        {(
          [
          ...
          <button
            ...
            className={cn(
              ...
              surface === id ? 'bg-wf-ink text-white' : 'bg-wf-surface text-muted hover:text-ink',
            )}
      ...
      {surface === 'schedule' ? (
        <div className="flex flex-wrap items-center gap-2" data-shifts-date-chrome>
          <div className="inline-flex rounded-full bg-wf-surface p-0.5 shadow-[inset_0_0_0_1px_rgba(35,33,29,0.06)]">
          ...
          <div className="inline-flex items-center gap-0.5 rounded-full bg-wf-surface px-1 py-0.5 shadow-[inset_0_0_0_1px_rgba(35,33,29,0.06)]">
            <button type="button" className="rounded-full p-1.5 text-muted hover:bg-wf-canvas/80 hover:text-ink" onClick={() => navigate(-1)} aria-label="prev">
              {isAr ? <ChevronRight className="h-4 w-4" /> : <ChevronLeft className="h-4 w-4" />}
            </button>
            <button type="button" className="rounded-full px-2.5 py-1 text-[13px] font-semibold text-ink hover:bg-wf-canvas/80" onClick={() => setAnchor(startOfDay(new Date()))}>
              {isAr ? 'اليوم' : 'Today'}
            </button>
            <button type="button" className="rounded-full p-1.5 text-muted hover:bg-wf-canvas/80 hover:text-ink" onClick={() => navigate(1)} aria-label="next">
      ...
      {surface === 'schedule' ? (
        <div
          className={cn(
            'flex gap-3 rounded-[1.1rem] px-3.5 py-2.5',
            requestCount
              ? 'bg-wf-accent-review-soft/55 text-wf-accent-review-ink'
              : 'bg-wf-surface-raised/80 text-muted',
          )}
          data-shifts-attention
        >
          <CalendarClock className={cn('mt-0.5 h-4 w-4 shrink-0', requestCount ? 'text-wf-accent-review-ink' : 'text-subtle')} />
          <div className="min-w-0 flex-1">
            <p className={cn('text-[13px] font-semibold', requestCount ? 'text-wf-accent-review-ink' : 'text-text')}>
              {requestCount ? c.attentionWaiting(requestCount) : c.attentionClear}
            </p>
            {requestCount ? (
              <button
                type="button"
      ...
      {/* Compact filter chrome — Schedule only */}
      {surface === 'schedule' ? (
        <div
          className="flex flex-wrap items-center gap-1.5 rounded-full bg-wf-surface px-2 py-1.5 shadow-[inset_0_0_0_1px_rgba(35,33,29,0.06)] sm:gap-2"
          data-testid="shifts-filters"
          data-shifts-filters
        >
          <input
            className="h-8 min-w-[7.5rem] flex-1 rounded-full border-0 bg-transparent px-3 text-[13px] text-ink outline-none placeholder:text-muted sm:min-w-[9rem]"
            placeholder={c.employee}
            value={filters.employee}
            onChange={(e) => setFilters((f) => ({ ...f, employee: e.target.value }))}
          />
          <span className="hidden h-5 w-px bg-wf-ink/10 sm:block" aria-hidden />
          <select
            className="h-8 max-w-[9rem] rounded-full border-0 bg-transparent px-2 text-[13px] text-ink outline-none"
      ...
      <div className={`grid gap-4 ${asideOpen && queue === 'board' ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : ''}`}>
        ...
        <div>
          {queue === 'board' ? (
            <div
              className={cn(
                'relative -mt-1 overflow-hidden rounded-[1.55rem] bg-wf-surface-raised shadow-[0_18px_40px_rgba(35,33,29,0.07)] ring-1 ring-wf-ink/[0.06]',
            ...
            >
              {refreshing && !coldLoading ? (
                <div
                  className="pointer-events-none absolute inset-x-0 top-0 z-10 flex h-7 items-center gap-2 bg-wf-surface-raised/92 px-4 text-xs text-muted backdrop-blur-[1px]"
                  data-shifts-updating
                  aria-live="polite"
                >
                  <Loader2 className="h-3.5 w-3.5 animate-spin" /> {c.updating}
                </div>
              ) : null}
              ...
              {coldLoading ? (
                ...
                <div className="divide-y divide-wf-ink/[0.06]">
                  {range.days.map((day) => {
                    ...
                    const isToday = isoDate(startOfDay(new Date())) === dayKey
                    return (
                      <div key={dayKey} className="p-4">
                        <div className="mb-3 flex items-center justify-between gap-2">
                          <h3
                            className={cn(
                              'inline-flex items-center rounded-full px-2.5 py-1 text-sm font-semibold tracking-[-0.01em]',
                              isToday ? 'bg-wf-ink text-white' : 'text-ink',
                            )}
                          >
                        ...
                        {all.length === 0 ? (
                          <ShiftsBoardEmpty
                            title={c.emptyBoardTitle}
                            hint={c.emptyBoardHint}
                            action={
                              canManage ? (
                                <Button className="mt-3" size="sm" onClick={() => openCreate(day)}>
                                  <Plus className="h-4 w-4" />
                                  {c.schedule}
                                </Button>
                              ) : null
                          ...
                          <ul className="space-y-2">
                            {all.map((shift, idx) => (
                              <li key={`${shift.shift_id || idx}-${shift.spanMarker ? 'span' : 'row'}`}>
                                ...
                                />
                                {shift.spanMarker ? (
                                  <p className="mt-1 text-[11px] text-wf-accent-follow-ink/80">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[880px] border-collapse text-sm">
                    <thead>
                      <tr className="border-b border-wf-ink/[0.06] bg-wf-canvas/40">
                        <th className="sticky start-0 z-[1] bg-wf-canvas/95 px-3 py-3 text-start text-[11px] font-semibold uppercase tracking-[0.1em] text-muted backdrop-blur-sm">
                          {c.employee}
                        </th>
                        {range.days.map((day) => {
                          ...
                          return (
                            <th
                              key={isoDate(day)}
                              className="min-w-[118px] px-2 py-2.5 text-center"
                            >
                              <div
                                className={cn(
                                  'mx-auto inline-flex min-w-[3.25rem] flex-col items-center rounded-full px-2.5 py-1.5',
                              ...
                              >
                                ...
                                <span className={cn('text-[13px] font-semibold', today ? 'text-white' : 'text-muted')}>
                                  {day.getDate()}
                                </span>
                              </div>
                            </th>
                    ...
                    <tbody>
                      {rosterRows.length === 0 ? (
                        ...
                        rosterRows.map((row) => (
                          <tr key={row.key} className="border-b border-wf-ink/[0.05] align-top">
                            <td className="sticky start-0 z-[1] bg-wf-surface-raised/95 px-3 py-2.5 backdrop-blur-sm">
                              <p className="font-semibold tracking-[-0.01em] text-ink">{row.label}</p>
                            ...
                            {range.days.map((day) => {
                              ...
                              return (
                                <td key={dayKey} className={cn('px-1.5 py-1.5', today && 'bg-wf-ink/[0.03]')}>
                                  <div className="space-y-1.5">
                                    {split ? <p className="text-[10px] font-medium text-wf-accent-review-ink/70">{c.splitHint}</p> : null}
                                    {cell.map((shift, idx) => (
                                      <ShiftBlock
                                    ...
                                    {canManage && cell.length === 0 ? (
                                      <button
                                        type="button"
                                        className="w-full rounded-[0.7rem] bg-wf-canvas/50 px-1 py-2 text-[10px] text-muted transition hover:bg-wf-accent-priority-soft/60 hover:text-wf-accent-priority-ink"
        ...
        {asideOpen && queue === 'board' ? (
          <aside className="rounded-[1.55rem] bg-wf-surface-raised p-4 shadow-[0_18px_40px_rgba(35,33,29,0.07)] ring-1 ring-wf-ink/[0.06]" data-testid="shifts-aside">
            <div className="mb-3 flex items-center justify-between gap-2">
            ...
            {composerOpen ? (
              <div className="space-y-3" data-testid="shifts-create-card" data-shifts-composer-org>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 rounded-xl border border-[#e8dfd0] bg-[#fffdf8]/80 p-3" data-shifts-edit-org>
                  {(
                    [
                    ...
                    return (
                      <label key={field} className="block space-y-1 text-[11px] font-medium text-muted">
                        <span>{label}</span>
                        <select
                          className="h-9 w-full rounded-xl border border-[#e8dfd0] bg-white px-3 text-sm text-ink disabled:opacity-80"
...
/** Designed empty state — quiet rhythm, not a dashed box. */
function ShiftsBoardEmpty({ title, hint, action }: { title: string; hint: string; action?: ReactNode }) {
  return (
    <div
      className="relative overflow-hidden rounded-[1.25rem] bg-gradient-to-br from-wf-accent-priority-soft/35 via-wf-surface to-wf-accent-follow-soft/25 px-5 py-8 text-center"
      data-shifts-empty
    >
      <div className="pointer-events-none absolute inset-x-6 top-4 flex justify-center gap-2 opacity-50" aria-hidden>
        <span className="h-2 w-14 rounded-full bg-wf-accent-priority/45" />
        <span className="h-2 w-10 rounded-full bg-wf-accent-follow/40" />
        <span className="h-2 w-8 rounded-full bg-wf-accent-review/35" />
      </div>
      <CalendarClock className="mx-auto mt-2 h-5 w-5 text-muted" aria-hidden />
      <p className="mt-3 text-[15px] font-semibold tracking-[-0.01em] text-ink">{title}</p>
      <p className="mx-auto mt-1.5 max-w-sm text-[12px] leading-relaxed text-muted">{hint}</p>
      {action}
    </div>
  )
}
