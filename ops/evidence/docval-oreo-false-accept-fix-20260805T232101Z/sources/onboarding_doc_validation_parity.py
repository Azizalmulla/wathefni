...
def evaluate_employee_upload(
    *,
    ...
    # Soft uncertain only when there is credible document evidence but readability is poor.
    if (status == "needs_review" and confidence < VERIFY_CONFIDENCE_FLOOR) or (not matches or confidence < VERIFY_CONFIDENCE_FLOOR):
        if not credible:
            return _block(out, obvious_non_document_reason(verification), locale=locale)
        if mode == "hard":
            reason = "wrong_media_item" if detected and detected != item and not matches else "needs_review"
