"""Setup Console Wave A — Launch Readiness authority (staging).
...
def _load_company(cur: Any, company_code: str) -> dict[str, Any] | None:
    cur.execute(
        """
        SELECT company_code, name, country, timezone, currency, status,
...
def _settings_map(cur: Any, company_code: str) -> dict[str, str]:
    cur.execute(
        """
        SELECT key, value
    ...
    for row in cur.fetchall():
        d = dict(row)
        out[str(d.get("key") or "")] = str(d.get("value") or "")
    return out
...
def evaluate_launch_readiness(cur: Any, *, company_code: str = ALLOWED_COMPANY) -> dict[str, Any]:
    ...
    locale_default = settings.get("default_locale") or settings.get("locale") or ""

    stages: list[dict[str, Any]] = []

    # --- Company basics ---
    basics_items: list[dict[str, Any]] = []
    profile_ok = bool(row.get("name") and row.get("country") and row.get("timezone") and row.get("currency"))
    basics_items.append(
        _item(
            key="company_profile",
