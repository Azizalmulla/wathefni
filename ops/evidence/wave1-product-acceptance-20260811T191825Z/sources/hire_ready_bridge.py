...
def on_hire_employee_tx(
    ...
    *,
    ...
    # OPTIONAL: probation auto-plan (dark unless probation module entitled).
    probation = {"ok": True, "skipped": True, "reason": "not_attempted"}
    try:
        import probation as _prob

        # Only auto-plan when employment is/was activated (not future pending_start).
        if str((employment or {}).get("lifecycle_state") or refreshed.get("employment_status") or "") == "active":
            probation = _prob.maybe_create_case_on_hire(
                cur,
                company_code=company,
                employee_key=key,
                probation_start=joining or as_date(refreshed.get("start_date")),
                actor_user_id=actor_user_id,
            )
        else:
            probation = {"ok": True, "skipped": True, "reason": "pending_start_defers_probation"}
    except Exception as exc:
        probation = {"ok": True, "skipped": True, "reason": f"probation_unavailable:{exc.__class__.__name__}"}
    ...
    _audit(
        ...
        payload={
            ...
            "probation": {
                "skipped": probation.get("skipped"),
                "reason": probation.get("reason") or probation.get("error"),
                "ok": probation.get("ok"),
            },
        },
    )
