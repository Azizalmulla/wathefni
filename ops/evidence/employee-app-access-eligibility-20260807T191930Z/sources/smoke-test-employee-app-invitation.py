...
def main() -> int:
    # Additive invitation columns are applied by the invitation module itself
    # (runtime ensure_schema is migrate-gated and does not run DDL here).
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE table_schema = current_schema()
                  AND table_name='employee_app_invites'
                  AND column_name IN (
            ...
            cols = {str(dict(r).get("column_name") or r[0]) for r in cur.fetchall()}
            conn.commit()
    needed = {
        "delivery_status",
        "delivery_channel",
    ...
    try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO employees (
                      ...
                      employment_status, position_title, raw_json
                    )
                    VALUES (%s,%s,%s,%s,%s,'active','Invite Smoke',%s::jsonb)
        ...
        # Access eligibility is required before any invite (create alone never invites).
        ...
        access.set_company_app_access_policy(
            legacy,
            {
                ...
                "user_id": "invitation-smoke",
                "actor_user_id": "invitation-smoke",
        ...
        access.set_employee_app_access(
            ...
            enabled=True,
            reason="invitation-smoke-enable",
