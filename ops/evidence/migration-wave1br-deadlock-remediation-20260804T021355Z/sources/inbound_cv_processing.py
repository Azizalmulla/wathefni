...
def plan_extraction_providers(
    *,
    ...
    """Describe local → Mistral OCR → GPT rescue without calling providers."""
    ...
    rescue_raw = env.get("WATHEFNI_CV_GPT_VISION_RESCUE")
    rescue_on = _truthy(rescue_raw) if rescue_raw is not None and str(rescue_raw).strip() != "" else ocr_on
    ...
    if needs_ocr or is_image:
        ...
        if not ocr_on:
            ...
            return ExtractionProviderPlan(
                local_first=True,
                mistral_ocr_eligible=False,
                gpt_vision_rescue_eligible=bool(rescue_on and is_image),
                reason_codes=tuple(reasons),
            )
        reasons.append("mistral_ocr_eligible")
        if rescue_on:
            reasons.append("gpt_vision_rescue_eligible_on_ocr_fail")
        return ExtractionProviderPlan(
            local_first=True,
            mistral_ocr_eligible=True,
            gpt_vision_rescue_eligible=rescue_on,
            reason_codes=tuple(reasons),
        )
