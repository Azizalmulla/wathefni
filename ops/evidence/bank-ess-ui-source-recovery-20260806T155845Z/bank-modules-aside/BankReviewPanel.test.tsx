...
describe('bank review helpers', () => {
  ...
  test('tones separate approved, awaiting review and returned', () => {
    expect(bankStateTone('approved')).toBe('success')
  ...
  test('an unrecognized state degrades to readable text instead of a wrong label', () => {
    expect(bankStateLabel('pending_review', false)).toBe('Awaiting your review')
    expect(bankStateLabel('pending_review', true)).toBe('بانتظار مراجعتك')
...
describe('BankReviewPanel', () => {
  ...
  test('approving tells the host to refetch so onboarding cannot show stale', async () => {
    ...
    expect(screen.getByText(/Apply the change to make it payroll-effective/)).toBeTruthy()
