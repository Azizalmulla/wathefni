...
def main() -> int:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            # Staging may predate P4A column order; pre-add before ensure SQL body references it.
            for stmt in (
                "ALTER TABLE IF EXISTS payroll_statutory_policy_versions ADD COLUMN IF NOT EXISTS source_classification text",
                "ALTER TABLE IF EXISTS payroll_statutory_rules ADD COLUMN IF NOT EXISTS source_classification text",
            ):
                try:
                    cur.execute(stmt)
                except Exception:
            ...
            try:
                ...
                if "source_classification" not in str(exc):
                    raise
            ...
            # SoD + stale approval on a fresh calculated period
            ps = date(p_start.year + 1, 1, 1)
            pe = date(p_start.year + 1, 1, 31)
            path = run_mode_a_path(
                cur,
                c4=c4,
                pyw1=pyw1,
