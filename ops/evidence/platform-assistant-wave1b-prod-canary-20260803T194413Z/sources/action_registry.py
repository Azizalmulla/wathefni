...
def _bind_spine_flag(legacy: Any) -> None:
    """Ensure legacy exposes the Wave 1 tool gate checker used by build_tool_schemas."""
    if legacy is None:
        return
...
def build_tool_schemas(legacy: Any, request: Any) -> list[dict[str, Any]]:  # type: ignore[misc]
    ...
    for tool in tools:
        ...
        if spine.is_wave1_read_tool(name):
            if not dashboard or not spine.wave1_enabled_for_company(company or spine.ALLOWED_COMPANY):
                continue
        # Mutation kill: hide confirming/sensitive tools from the catalog when Wave 1 denies mutations.
