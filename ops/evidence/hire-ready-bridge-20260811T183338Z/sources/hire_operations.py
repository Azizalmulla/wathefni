...
def _employee_transaction(
    ...
    def apply(cur: Any, before: dict[str, Any], hired: dict[str, Any]) -> dict[str, Any]:
        ...
        phone = legacy.canonical_employee_phone(hired.get("phone"))
        if not company or not phone:
            return {"ok": False, "error": "candidate_identity_incomplete"}
        aliases = sorted(legacy.phone_identity_candidates(phone))
        alias_keys = [f"{company}-{alias}" for alias in aliases]
        cur.execute("SELECT * FROM candidates WHERE phone=%s LIMIT 1", (phone,))
        candidate_row = cur.fetchone()
        if not candidate_row and aliases:
            # Alias-aware candidate lookup for local/965 form mismatches.
            cur.execute("SELECT * FROM candidates WHERE phone = ANY(%s) LIMIT 1", (aliases,))
            candidate_row = cur.fetchone()
        ...
        cur.execute(
            ...
            FOR UPDATE
            """,
            (company, hired["app_key"], alias_keys, aliases, hired["app_key"]),
        )
        ...
        if employee:
            ...
            cur.execute(
                ...
                RETURNING *
                """,
                (hired["app_key"], name, email, position, phone, employee["employee_key"], company),
            )
            ...
            employee = dict(cur.fetchone())
        # Wave 1 Hire→Ready bridge: advance same pending_start employment (P7) before
        # authority upsert so future joiners are not coerced to active.
        bridge_handled = False
        try:
            import hire_ready_bridge as _hrb

            bridge = _hrb.on_hire_employee_tx(
                cur,
            ...
            bridge_handled = bool(bridge.get("handled_employment"))
            if bridge.get("employee"):
                employee = dict(bridge["employee"])
        except Exception:
            bridge_handled = False
        # Wave 2: shadow person/employment/assignment (opt-in). Same TX as hire hub write.
        # Skip when bridge already advanced the canonical pending_start employment.
        if not bridge_handled:
            try:
                import employee_authority_wave2 as _e360_authority

                if _e360_authority.authority_v2_enabled(company):
                    _e360_authority.ensure_authority_schema(cur)
                    _e360_authority.upsert_person_employment_assignment(
                        cur,
                        employee=dict(employee),
                        hire_source="canonical_hire",
                        provenance={"operation_id": str(operation_id)},
                    )
                    cur.execute(
                        "SELECT * FROM employees WHERE employee_key=%s AND company_code=%s",
                        (employee["employee_key"], company),
                    )
                    employee = dict(cur.fetchone() or employee)
            except Exception:
                pass
