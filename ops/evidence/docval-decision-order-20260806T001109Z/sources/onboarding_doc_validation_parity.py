...
Decision contract (soft canary — Aziz/Talal allowlist):
  ...
  * Credible document + clearly bad capture → block + retake guidance
    (high OCR confidence must not convert reject-capture into allow)
    ...
    *,
    ...
    if isinstance(capture_assessment, dict):
        ...
        if cap_status == "reject":
            reason = str(capture_assessment.get("reason") or "too_blurry")
            out["quality_issues"] = list(out.get("quality_issues") or []) + [reason]
            # High OCR confidence must not convert clearly poor capture into allow.
            return _block(out, reason, locale=locale)
        if cap_status == "borderline":
