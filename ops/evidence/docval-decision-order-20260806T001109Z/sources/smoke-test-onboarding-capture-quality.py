...
def test_gate_wiring(tmpdir: Path) -> None:
    ...
    check("severe blur blocks despite high OCR confidence", v.should_block(blur_r), blur_r)
    check("severe blur reason too_blurry", blur_r.get("reason") == "too_blurry", blur_r.get("reason"))
    check("severe blur still ran verify", (blur_r.get("verification") or {}).get("confidence") == 0.95, blur_r.get("verification"))

    # Live defect regression: glare-heavy non-document must NOT use capture-quality copy.
    glare_arr = _doc_card().astype(np.float32)
    glare_arr[:, :, :] = 255
    ...
    check("nondoc blocks", v.should_block(nondoc_r), nondoc_r)
    check(
        "nondoc wrong-document message wins over glare",
        nondoc_r.get("reason") in {"looks_like_photo_not_document", "not_a_document"},
        nondoc_r.get("reason"),
    )
    check(
        "nondoc not capture-quality reason",
        nondoc_r.get("reason") not in {"glare_or_shadow", "too_blurry", "document_not_fully_visible"},
        nondoc_r.get("reason"),
    )
    # Metrics may still record glare reject status for audit.
    cap = nondoc_r.get("capture_quality") or {}
    check("nondoc capture metrics present", isinstance(cap.get("metrics"), dict), cap)
    check("nondoc capture would have rejected glare", cap.get("status") == "reject", cap)
    check("nondoc capture signal glare", cap.get("reason") == "glare_or_shadow", cap)
