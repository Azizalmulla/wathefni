...
def list_assignments(
    ...
    *,
    ...
    """Company-scoped assignment queue (read helper for surfaces — no SM change)."""
    company = company_code_norm(company_code)
    ...
    if status:
        status_filter.append(str(status).strip().lower())
    status_filter = [s for s in status_filter if s in ASSIGNMENT_STATUSES]
    if status_filter:
        where.append("status = ANY(%s)")
        params.append(status_filter)
    elif not include_terminal:
        where.append("status NOT IN ('converted','cancelled')")
    if employee_key:
        where.append("employee_key=%s")
        params.append(str(employee_key).strip())
    if manager_user_id:
        where.append("manager_user_id=%s")
        params.append(str(manager_user_id).strip())
    q = str(search or "").strip()
    if q:
        where.append("(employee_key ILIKE %s OR COALESCE(metadata->>'name','') ILIKE %s)")
        like = f"%{q}%"
        params.extend([like, like])
    where_sql = " AND ".join(where)
    cur.execute(f"SELECT count(*) AS c FROM preboard_assignments WHERE {where_sql}", params)
        ...
        SELECT * FROM preboard_assignments
         ...
         ORDER BY
           CASE status
             ...
             ELSE 4
           END,
           joining_date ASC NULLS LAST,
           updated_at DESC
         LIMIT %s OFFSET %s
        """,
        (*params, lim, off),
    )
    rows = [_assignment_row(dict(r)) for r in (cur.fetchall() or [])]
    # Queue counts by status (open set)
    cur.execute(
        """
        SELECT status, count(*) AS c
          FROM preboard_assignments
         WHERE company_code=%s AND status NOT IN ('converted','cancelled')
         GROUP BY status
        """,
        (company,),
    )
    by_status = {str(r["status"]): int(r["c"]) for r in (cur.fetchall() or [])}
    return {
        "ok": True,
        "company_code": company,
        "assignments": rows,
...
def list_events(
    ...
    *,
    ...
    cur.execute(
        """
        SELECT event_id, company_code, assignment_id, item_id, event_type,
               actor_user_id, actor_phone, payload, created_at
          FROM preboard_events
         WHERE company_code=%s AND assignment_id=%s
         ORDER BY created_at DESC
         LIMIT %s
        """,
        (company, str(assignment_id), lim),
    )
        ...
        out.append(
            {
                ...
                "assignment_id": str(d["assignment_id"]) if d.get("assignment_id") else None,
                "item_id": str(d["item_id"]) if d.get("item_id") else None,
                "event_type": d.get("event_type"),
                "actor_user_id": d.get("actor_user_id"),
                "actor_phone": d.get("actor_phone"),
                "payload": payload or {},
...
def get_open_assignment_for_employee(
...
def list_template_items(cur: Any, *, company_code: str, template_id: str | None = None) -> list[dict[str, Any]]:
    company = company_code_norm(company_code)
        ...
        out.append(
            {
                ...
                "due_offset_days": d.get("due_offset_days"),
                "depends_on": _parse_deps(d.get("depends_on")),
...
def plan_onboarding_handoff_dedupe(
    ...
    # onboarding_items is keyed by employee_key; company_code may be absent on older schemas.
    try:
        cur.execute("SAVEPOINT pb_handoff_plan")
        try:
            cur.execute(
                """
            ...
            cur.execute("ROLLBACK TO SAVEPOINT pb_handoff_plan")
            cur.execute("RELEASE SAVEPOINT pb_handoff_plan")
            return {"ok": True, "satisfied": {}, "error_soft": "onboarding_items_unavailable"}
        cur.execute("RELEASE SAVEPOINT pb_handoff_plan")
        for row in rows:
            d = dict(row)
        ...
        try:
            ...
            pass
        return {"ok": True, "satisfied": {}, "error_soft": str(exc), "company_code": company}
    return {"ok": True, "satisfied": satisfied, "company_code": company}
