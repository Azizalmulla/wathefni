#!/usr/bin/env bash
# Payroll Wave 3-B — production WATHEFNI synthetic qualification.
# Deploy payslips → canary → rollback proof → redeploy → canary → freezes.
# Does NOT enable money/bank/PIFSS/EOS/payments/AI. Does NOT start next payroll wave.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/payroll-wave3b-prod-canary-$STAMP"
REMOTE_STAGE="/tmp/payroll-w3b-prod-stage"
...
echo "$LOCAL_EVID" > /tmp/pyw3b.evid
echo "$STAMP" > /tmp/pyw3b.stamp
...
"$PY_LOCAL" smoke-test-payroll-payslip-wave3-ux.py 2>&1 | tee "$LOCAL_EVID/tests/ux-en-ar-mobile-local.out" | tail -20

log "stage sources"
mkdir -p "$LOCAL_EVID/sources/ops/sql" "$LOCAL_EVID/migrate"
cp -a app.py payroll_payslip_wave3.py payroll_native_preview_wave2b.py payroll_authority_wave1.py payroll_external_adapter_wave2a.py \
  ...
  "$LOCAL_EVID/sources/"
cp -a ops/deploy-payroll-wave3b-prod-synthetic.sh ops/migrate-payroll-payslip-wave3-prod.sh "$LOCAL_EVID/sources/ops/"
cp -a ops/sql/payroll_payslip_wave3_v1.sql ops/sql/payroll_native_preview_wave2b_v1.sql \
  ops/sql/payroll_authority_wave1_v1.sql ops/sql/payroll_external_adapter_wave2a_v1.sql \
  "$LOCAL_EVID/sources/ops/sql/"
cp -a ops/migrate-payroll-payslip-wave3-prod.sh "$LOCAL_EVID/migrate/"
cp -a "$REPO_ROOT/ops/qualify-payroll-wave3b-prod-synthetic.sh" "$LOCAL_EVID/sources/" 2>/dev/null || true
...
cp -a "$DASH_SRC/src/posthire/PayslipWorkspace.tsx" \
...
(
  cd "$ORCH_SRC"
  "${SCP[@]}" app.py payroll_payslip_wave3.py payroll_native_preview_wave2b.py payroll_authority_wave1.py payroll_external_adapter_wave2a.py \
    canary-prod-payroll-payslip-wave3b.py \
    smoke-test-payroll-payslip-wave3.py \
    smoke-test-payroll-payslip-wave3-ux.py \
    smoke-test-payroll-native-preview-wave2b.py \
    ...
    smoke-test-leave-freeze-regression.py \
    smoke-test-shifts-freeze-regression.py \
    ops/deploy-payroll-wave3b-prod-synthetic.sh \
    ops/migrate-payroll-payslip-wave3-prod.sh \
    "$VPS_HOST:$REMOTE_STAGE/"
  "${SCP[@]}" ops/sql/payroll_payslip_wave3_v1.sql ops/sql/payroll_native_preview_wave2b_v1.sql \
    ops/sql/payroll_authority_wave1_v1.sql ops/sql/payroll_external_adapter_wave2a_v1.sql \
    "$VPS_HOST:$REMOTE_STAGE/"
)
"${SCP[@]}" \
...
chmod +x '$REMOTE_STAGE/deploy-payroll-wave3b-prod-synthetic.sh'
bash '$REMOTE_STAGE/deploy-payroll-wave3b-prod-synthetic.sh'
REMOTE
...
export PYW3B_EVID="\$OUTDIR"
mkdir -p "\$OUTDIR"
export PYTHONUNBUFFERED=1
\$PYBIN -u canary-prod-payroll-payslip-wave3b.py
REMOTE
}
...
set -euo pipefail
BACKUP=\$(cat /opt/wathefni/production-evidence/payroll-wave3b-prod-canary/$STAMP/backup/BACKUP_PATH.txt)
bash "\$BACKUP/ROLLBACK.sh" "\$BACKUP"
test ! -f /etc/systemd/system/wathefni-orchestrator.service.d/zzzzzzzzzzzzzzzzz-payroll-wave3b-synthetic.conf
PID=\$(systemctl show -p MainPID --value wathefni-orchestrator)
...
if echo "\$ENV_DUMP" | grep -q '^WATHEFNI_PAYROLL_WAVE3=1'; then
  echo WAVE3_STILL_ENABLED
  exit 4
fi
echo WAVE3_FLAGS_CLEARED
...
with app.db_connect() as conn:
  with conn.cursor() as cur:
    cur.execute("""
      ...
      WHERE company_code='WATHEFNI' AND employee_key LIKE 'WATHEFNI-PYW1-PYW3-W3B-%'
    """)
    n=int(dict(cur.fetchone())["n"])
    cur.execute("""
      SELECT COUNT(*) AS n FROM payroll_payslip_documents
      WHERE company_code='WATHEFNI' AND employee_key LIKE 'WATHEFNI-PYW1-PYW3-W3B-%'
    """)
    p=int(dict(cur.fetchone())["n"])
print("RESIDUAL_SYNTH_CONTRACTS", n)
print("RESIDUAL_SYNTH_PAYSLIPS", p)
assert n==0 and p==0
PY
...
set -euo pipefail
ORCH=/opt/wathefni/orchestrator
OUTDIR='/opt/wathefni/production-evidence/payroll-wave3b-prod-canary/${STAMP}/canary/after-redeploy'
PYBIN=\$ORCH/.venv/bin/python
cd \$ORCH
set -a; source /root/.openclaw/secrets/postgres.env; set +a
PID=\$(systemctl show -p MainPID --value wathefni-orchestrator)
...
log "Wave 1/2A/2B honesty + production freezes"
...
h=w3.honesty_payload(); b=w2b.honesty_payload(); a=w2a.honesty_payload(); inv=w2a.freeze_invariants()
...
assert b["authoritative"] is False and a["money_authority"]=="external" and a["vendor_claimed"] is False
assert inv["mirror_only_imports"] and inv["wave1_ddl_untouched"]
print("WAVE1_2A_2B_3_FREEZE_HONESTY_OK")
PY
...
"${SCP[@]}" -r "$VPS_HOST:/opt/wathefni/production-evidence/payroll-wave3b-prod-canary/${STAMP}-redeploy/." "$LOCAL_EVID/remote-redeploy/" 2>/dev/null || true
...
ack_flags = ""
for cand in [evid / "remote" / "flags" / "payroll-wave3b-flags.txt", evid / "remote-redeploy" / "flags" / "payroll-wave3b-flags.txt"]:
    if cand.exists():
...
prod_ok = "FREEZES_AND_PAYROLL_OK" in prod_txt and "WAVE1_2A_2B_3_FREEZE_HONESTY_OK" in prod_txt

rb_txt = (evid / "tests" / "rollback.out").read_text() if (evid / "tests" / "rollback.out").exists() else ""
rb_ok = (
    "ROLLBACK_VERIFIED" in rb_txt
    and "WAVE3_FLAGS_CLEARED" in rb_txt
    and "RESIDUAL_SYNTH_CONTRACTS 0" in rb_txt
    and "RESIDUAL_SYNTH_PAYSLIPS 0" in rb_txt
)
...
if not rb_ok:
    blockers.append("rollback not verified (Wave 1/2A/2B retained + Wave 3 cleared + residual 0)")
...
if "WATHEFNI_PAYROLL_WAVE3_SYNTHETIC_ONLY=1" not in flags:
    blockers.append("synthetic_only flag not confirmed")
if "MIGRATE_OK" not in mig or "ACK_PRODUCTION_PAYROLL_W3B=YES" not in mig:
    blockers.append("migrate/ACK markers missing")

freeze_go = "GO" if not blockers else "NO-GO"
report = f"""# Payroll Wave 3-B — production synthetic qualification

**Stamp:** `{stamp}`  
**Evidence:** `ops/evidence/payroll-wave3b-prod-canary-{stamp}/`  
...
| Scope | Verdict |
|---|---|
| Production synthetic Wave 3 payslips | **{freeze_go}** |
| Freeze Wave 3 | **{freeze_go}** |
...
ACK flags:
```
{ack_flags.strip() or '(see remote/flags/payroll-wave3b-flags.txt)'}
```
...
| Leave (local) | {flv[1]} |
| Shifts (local) | {fsh[1]} |
| Production freezes + Wave 1/2A/2B/3 honesty | {'PASS' if prod_ok else 'FAIL'} |

---

## Blockers

{chr(10).join('- ' + b for b in blockers) if blockers else '- None for Wave 3 freeze scope.'}
...
gate = "PROD_SYNTHETIC_PAYROLL_WAVE3_GO" if freeze_go == "GO" else "PROD_SYNTHETIC_PAYROLL_WAVE3_NO_GO"
