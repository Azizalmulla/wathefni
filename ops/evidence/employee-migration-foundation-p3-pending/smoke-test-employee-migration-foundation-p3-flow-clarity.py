...
def main() -> None:
    ...
    try:
        ...
        if not approved.get("confirm_still_required") and not approved.get("apply_still_required"):
            _fail("confirm/apply still required missing")
        msg = str(approved.get("message") or "")
        if "Apply approved change" not in msg and "Confirm import" not in msg:
        ...
        _ok("approve keeps batch, moves out of review, apply/confirm still required")
