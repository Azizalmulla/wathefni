...
export function ShiftsWorkspace({ access, permissions, role: _role, onNotice, onAccessIssue }: ShiftsWorkspaceProps) {
  ...
  const siteUnits = useMemo(
    () => orgUnits.filter((u) => ['site', 'location'].includes(String(u.unit_type || '').toLowerCase())),
    [orgUnits],
  )
  ...
  const unitSelectValue = (units: OrgUnitRow[], key: string) => {
    if (!key) return ''
    const hit = units.find((u) => u.unit_key === key || u.org_unit_id === key || u.name === key)
    return hit?.unit_key || hit?.org_unit_id || key
  }
