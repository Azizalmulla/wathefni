...
export function shiftsCopy(locale: ShiftsLocale) {
  ...
  return {
    ...
    showOperations: isAr ? 'إظهار تفاصيل الجاهزية' : 'Show readiness details',
    hideOperations: isAr ? 'إخفاء تفاصيل الجاهزية' : 'Hide readiness details',
    ...
    honestyEnterprise: isAr
      ? 'الطبقة المؤسسية: الدورات والروستر عن بُعد والامتثال وتصدير PAM للقراءة فقط. المسودة لا تكتب سلطة تشغيل. الرواتب تملك الحساب المالي. لا إرسال حكومي آلي.'
      : 'Enterprise layer: rotations, remote rosters, compliance, and read-only PAM export. Drafts never write operational authority. Payroll owns money. No automated government submission.',
    templates: isAr ? 'القوالب والتكرار' : 'Templates & recurrence',
    publish: isAr ? 'النشر والتغطية' : 'Publish & coverage',
    enterprise: isAr ? 'الدورات والامتثال' : 'Rotations & compliance',
    ...
    rotationPreview: isAr ? 'معاينة الدورة' : 'Rotation preview',
    generateFromRotation: isAr ? 'مسودة من الدورة' : 'Draft from rotation',
    pamExport: isAr ? 'تصدير PAM' : 'PAM export',
