...
export function I18nProvider({ initialLocale, children }: { initialLocale: AppLocale; children: ReactNode }) {
  ...
  const setLocale = useCallback(async (next: AppLocale) => {
    ...
    setLocaleState(next)
    const rtl = next === 'ar'
    const needsNativeRtlFlip = Platform.OS !== 'web' && I18nManager.isRTL !== rtl
    // Native RTL flips need a full process restart. Skip the native bridge on web —
    // RN-web's I18nManager is a no-op for isRTL.
    if (needsNativeRtlFlip) {
