...
def frontend_setup_cards_scan(dashboard_setup_dir: Path | None = None) -> dict[str, Any]:
    root = dashboard_setup_dir or (
        Path(__file__).resolve().parents[1] / "apps" / "wathefni-dashboard" / "src" / "setup-console"
    )
    required = [
    ...
    unmounted = [n.replace(".tsx", "") for n in required if n.replace(".tsx", "") not in app_text]
    return {
        "ok": not missing and not unmounted and app.exists(),
