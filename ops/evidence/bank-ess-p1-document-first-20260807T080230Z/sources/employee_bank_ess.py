...
def sanitize_bank_extraction(raw: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    for key in BANK_EXTRACT_FIELDS:
        cell = fields_in.get(key)
        value = _field_value(cell)
        if value in (None, ""):
            continue
    ...
    return {
        ...
        "document_type": data.get("document_type") or "bank_certificate",
    }
        ...
        for r in cur.fetchall() or []:
            ...
            if isinstance(raw, str):
                try:
                    ...
                    raw = {}
            sanitized = (raw.get("sanitized") if isinstance(raw, dict) else None) or sanitize_bank_extraction(raw)
            row["extraction"] = sanitized
            rows.append(row)
        return rows
    except Exception:
        return []
