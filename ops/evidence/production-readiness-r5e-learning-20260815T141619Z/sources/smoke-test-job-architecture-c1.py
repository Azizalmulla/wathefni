...
def main() -> int:
    ...
    empty_gate = ja.runtime_gate_for_company(COMPANY)
    if ready.customer_enableable("job_architecture"):
        check("empty allowlist admits after R5D", empty_gate.get("ok") is True, empty_gate)
    else:
        check("empty allowlist", empty_gate.get("ok") is not True, empty_gate)
