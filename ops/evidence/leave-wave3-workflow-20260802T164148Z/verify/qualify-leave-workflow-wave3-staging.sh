#!/usr/bin/env bash
# Leave Wave 3 — staging qualification (local sync + staging migrate/smoke + freeze regressions).
...
EVID="$ROOT/ops/evidence/leave-wave3-workflow-${STAMP}"
...
printf '%s\n' "$EVID" > /tmp/leave-w3.evid
printf '%s\n' "$STAMP" > /tmp/leave-w3.stamp
echo "evidence=$EVID"
...
cp -a "$ORCH/smoke-test-leave-workflow-wave3.py" "$EVID/tests/"
cp -a "$ORCH/ops/migrate-leave-workflow-wave3.sh" "$EVID/migrate/"
cp -a "$ORCH/ops/qualify-leave-workflow-wave3-staging.sh" "$EVID/verify/" 2>/dev/null || true
...
rsync -az -e "ssh -o BatchMode=yes" \
  ...
  "$ORCH/smoke-test-leave-workflow-wave3.py" \
  "$ORCH/app.py" \
  "root@$HOST:$REMOTE_ORCH/"
ssh -o BatchMode=yes "root@$HOST" "mkdir -p $REMOTE_ORCH/ops"
rsync -az -e "ssh -o BatchMode=yes" \
  "$ORCH/ops/migrate-leave-workflow-wave3.sh" \
...
chmod +x ops/migrate-leave-workflow-wave3.sh
ACK_DB="\$ACK_DB" WATHEFNI_ENV=staging ORCH_PYTHON="\$PY" bash ops/migrate-leave-workflow-wave3.sh | tee /tmp/leave-w3-migrate.out
"\$PY" smoke-test-leave-workflow-wave3.py | tee /tmp/leave-w3-smoke.out
if grep -E '[1-9][0-9]* failed' /tmp/leave-w3-smoke.out; then
  echo SMOKE_FAILED
  exit 1
fi
echo STAGING_LEAVE_W3_OK
REMOTE

scp -o BatchMode=yes "root@$HOST:/tmp/leave-w3-migrate.out" "$EVID/migrate/" 2>/dev/null || true
scp -o BatchMode=yes "root@$HOST:/tmp/leave-w3-smoke.out" "$EVID/tests/" 2>/dev/null || true
