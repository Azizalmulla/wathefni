...
type QueueTab = 'board' | 'swaps' | 'availability' | 'reconciliation' | 'reminders'
    ...
    <section className="space-y-4" dir={isAr ? 'rtl' : 'ltr'} lang={locale} data-testid="shifts-workspace">
      ...
      <div className={`grid gap-4 ${asideOpen && queue === 'board' ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : ''}`}>
        ...
        <div>
          ...
          {queue === 'reminders' ? (
            <QueueCanvas title={c.reminders} icon={<AlertTriangle className="h-4 w-4" />} empty={c.emptyReminders} refreshing={refreshing} hint={c.remindersSynthetic}>
              {reminders.map((r, idx) => (
                <div key={String(r.reminder_id || idx)} className="rounded-2xl border border-[#e8dfd0] bg-white/70 px-4 py-3 text-sm">
                  <p className="font-semibold text-ink">{String(r.employee_key || r.shift_id || '—')}</p>
                  <p className="mt-1 text-xs text-[#7a5a20]">{String(r.last_error || r.status || '')}</p>
                </div>
              ))}
            </QueueCanvas>
          ) : null}
          ...
          <aside className="rounded-[1.55rem] border border-line/55 bg-white p-4 shadow-[0_18px_40px_rgba(35,33,29,0.08)]" data-testid="shifts-aside">
            ...
            {composerOpen ? (
              <div className="space-y-3" data-testid="shifts-create-card">
                ...
                <Button className="w-full" disabled={actionBusy === 'create-shift'} onClick={() => void submitCreate()}>
