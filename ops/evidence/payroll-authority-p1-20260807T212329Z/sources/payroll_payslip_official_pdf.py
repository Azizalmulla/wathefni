...
def _sealed_authority_ok(doc: dict[str, Any] | None, sealed: dict[str, Any] | None = None) -> bool:
    ...
    return (
        ...
        and str(sealed.get("money_authority") or "") == MONEY_EXTERNAL
...
def is_official_pdf_eligible(
    ...
    *,
    ...
    """True only for sealed external money-authority payslip projections."""
    if not doc:
        return False
    source = str(doc.get("source_kind") or "")
    money = str(doc.get("money_authority") or "")
    if source != SOURCE_EXTERNAL or money != MONEY_EXTERNAL:
        return False
...
def official_pdf_refusal(
    ...
    *,
    ...
    return {
        ...
        "message": (
            "Official PDF requires a sealed external money-authority snapshot linked to the payslip. "
        ...
        "official_employee_pdf_requires": (
            "sealed authority_snapshot (money_authority=external) + "
            "source_kind=external_import + money_authority=external"
        ),
    }
...
def production_authority_blockers() -> dict[str, Any]:
    ...
    return {
        ...
        "official_pdf_eligible_source": "external_import + sealed authority_snapshot",
        ...
        "native_reason": (
            ...
            "P1 Mode A seal path refuses wathefni authority until authoritative-finalize."
        ),
        "external_is_mirror_not_wathefni_money_authority": True,
        "external_reason": (
            "External sealed snapshots mirror imported results; money_authority=external, "
