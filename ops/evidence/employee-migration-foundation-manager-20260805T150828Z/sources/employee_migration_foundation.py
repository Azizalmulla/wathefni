"""Employee Migration Foundation P0–P2 — roster import ledger.

Create-only · no messages · no auto-onboarding · no existing-employee updates.
Adds: batch ledger, row results, exception CSV, idempotent replay, import history,
preview totals (create/skip/conflict/invalid), optional external_employee_id /
payroll_id / source_system via reusable employee_source_mappings.
"""
...
CONTRACT = "employee_migration_foundation_p0p2"
...
# Row outcomes align with product buckets; legacy needs_review maps to conflict.
...
TOTAL_KEYS = ("create", "skip", "conflict", "invalid", "warnings")
...
def _classify_row(
    ...
    *,
    ...
    existing = legacy.find_employee_by_phone(phone_digits, company_code=company)
    if existing:
        existing_name = str(existing.get("name") or "").strip()
        if existing_name and existing_name.lower() != clean_name.lower():
            return {
                "status": "conflict",
                "reason": f"Already exists as '{existing_name}' with a different name.",
                "name": row_label,
                "phone": phone_digits,
                "employee_key": existing.get("employee_key"),
                "normalized": normalized,
                "detail": {**detail, "existing_employee_key": existing.get("employee_key")},
            }
        return {
            "status": "skipped",
            "reason": "Already in the workforce.",
            "name": row_label,
            "phone": phone_digits,
            "employee_key": existing.get("employee_key"),
            "normalized": normalized,
...
def _apply_manager_on_hub(
    ...
    *,
    ...
    """Deterministically set manager after create. Never invents a manager when unresolved."""
    cur.execute(
        """
        UPDATE employees
    ...
    try:
        cur.execute(
            ...
            WHERE table_schema='public' AND table_name='employee_assignments'
              AND column_name='manager_employee_key'
            LIMIT 1
            """
        )
        if not cur.fetchone():
        ...
        cur.execute(
            ...
            SET manager_employee_key=%s, updated_at=now()
            WHERE company_code=%s AND employee_key=%s
            """,
            (manager_employee_key, company, employee_key),
        )
    except Exception:
        # Assignment projection is best-effort; hub stamp above is authoritative for import.
        try:
            cur.connection.rollback()
        except Exception:
            pass
        # Re-stamp after rollback so hub provenance is not lost if caller shares the connection.
        cur.execute(
            """
            UPDATE employees
            SET raw_json = COALESCE(raw_json, '{}'::jsonb) || %s::jsonb,
...
def _batch_payload(batch: dict[str, Any], rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    legacy_counts = {
        ...
        "needs_review": int(totals.get("conflict") or 0),
        "failed": int(totals.get("invalid") or 0),
    }
    results = {"created": [], "skipped": [], "needs_review": [], "failed": []}
    if rows is not None:
        for row in rows:
            bucket = _legacy_bucket(str(row.get("status") or ""))
            results[bucket].append(_row_entry(row))
    return {
        "ok": True,
        "foundation": True,
        ...
        "totals": {
            ...
            "conflict": int(totals.get("conflict") or 0),
...
def commit_import_batch(
    ...
    *,
    ...
    seed = legacy.company_has_module(company, "compliance")
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ensure_schema(cur)
            cur.execute(
    ...
    failed = 0
    for row in rows:
        if str(row.get("status")) != "will_create":
            continue
        norm = row.get("normalized") or {}
        if isinstance(norm, str):
            norm = json.loads(norm)
    ...
    if created:
        legacy.record_admin_audit(
            context,
            "employees_imported",
            summary=f"Imported {created} employees via migration foundation batch {batch_id}.",
            target_type="company",
            target=company,
            details={"batch_id": str(batch_id), "created": created, "failed": failed, "totals": totals},
        )
    ...
    *,
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            created_rows = [dict(r) for r in (cur.fetchall() or [])]
            removed = 0
    ...
    legacy.record_admin_audit(
        ...
        summary=f"Rolled back employee import batch {batch_id} ({removed} removed).",
        target_type="company",
        target=company,
        details={"batch_id": str(batch_id), "removed": removed, "blocked": blocked, "idempotency_key": rollback_key},
    )
    return {
        "ok": True,
        "replayed": False,
