...
def build_mobile_priorities(app_mod: Any, context: dict[str, Any], *, limit: int = 12) -> dict[str, Any]:
    ...
    if (hr_features.get("performance_reviews") or {}).get("enabled"):
        try:
            ...
            pass
    if (hr_features.get("employee_relations_actions") or {}).get("enabled"):
        try:
            import employee_relations_surfaces as _ers

            company = str(context.get("company_code") or "").strip().upper()
            actor_key = _ers.actor_key_from_context(context)
            actor_role = _ers.map_actor_role(
                dashboard_role=str(context.get("actor_role") or "hr"),
                permissions=set(context.get("permissions") or []),
            )
            with app_mod.db_connect() as conn:
                with conn.cursor() as cur:
                    queue = _ers.mobile_queue(
                        cur,
                        company_code=company,
                        actor_key=actor_key,
            ...
            sections.append(
                {
                    "type": "employee_relations_actions",
                    "title": "Employee Relations",
                    "total": int(queue.get("total") or 0),
                    "items": [
                        {
                            "type": "employee_relations_action",
                            "target_id": item.get("case_id"),
                            "summary": item.get("summary_en") or "Employee Relations action requires your attention",
                            "status": item.get("status"),
                            "permitted_actions": item.get("allowed_actions") or ["read"],
                            "destination": item.get("destination") or f"/employee-relations/{item.get('case_id')}",
                        }
                        for item in (queue.get("items") or [])
                    ],
                }
