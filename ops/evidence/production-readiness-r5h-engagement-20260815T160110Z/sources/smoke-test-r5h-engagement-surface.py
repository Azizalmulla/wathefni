...
def main() -> int:
    ...
    check(
        "fail-closed keeps comp planning",
        ("comp_planning", "/dashboard/compensation-planning") in ready.all_http_namespaces(),
    )
    ...
    comp_detail = comp.json().get("detail") if comp.headers.get("content-type", "").startswith("application/json") else None
    check(
        "comp planning still fail-closed",
        comp.status_code == 404
        and isinstance(comp_detail, dict)
        and comp_detail.get("error") == "capability_not_released",
        comp_detail,
    )
