...
type LiveIssue = {
  ...
  when: string | null
  statusLabel: string
  primary: { kind: 'navigate'; page: Page; label: string } | { kind: 'resolve'; taskId: string; label: string } | { kind: 'status'; label: string }
...
function canManageDelivery(permissions: string[]) {
  if (permissions.includes('users.manage')) return true
  return ['leave', 'onboarding', 'compliance', 'attendance', 'shifts', 'payroll'].some((m) =>
    permissions.includes(`${m}.manage`),
  )
}
...
function issueFromHrTask(task: HrTask, isAr: boolean, resolved: boolean): LiveIssue {
  return {
    ...
    primary: resolved
      ...
      : { kind: 'resolve', taskId: task.task_id, label: isAr ? 'تعليم كمكتمل' : 'Mark done' },
    details: [
...
export function NotificationsPage({
  ...
  const canManage = canManageDelivery(permissions)
  ...
  }, [access, posthireEnabled, onAccessIssue, isAr])
  ...
  const resolveTask = async (taskId: string) => {
    ...
    try {
      await resolveHrTask(access, taskId, 'done')
  ...
  return (
    <div className="space-y-5" dir={isAr ? 'rtl' : 'ltr'} lang={locale} data-testid="alerts-delivery-workspace" data-alerts-delivery>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button variant="ghost" size="sm" onClick={() => void reload()} disabled={loading} aria-label={isAr ? 'تحديث' : 'Refresh'}>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    {issue.primary.kind === 'resolve' && canManage ? (
                      <Button size="sm" disabled={resolvingId === issue.primary.taskId} onClick={() => void resolveTask(issue.primary.kind === 'resolve' ? issue.primary.taskId : '')}>
...
/** Kept for lazy import compatibility — delivery UI now lives on NotificationsPage. */
export function PostHireDeliveryCenterPlaceholder() {
  return null
}
