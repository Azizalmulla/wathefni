#!/usr/bin/env python3
"""Shifts Wave 3B — staging create-path + browser composer closure.
...
KEY = f"WATHEFNI-SHW3B-STG-{TAG}"
PHONE = f"965531{TAG[:5].zfill(5)}"[:12]
NAME = f"SHW3B-SYNTH| Staging {TAG}"
...
ORCH = os.environ.get("STAGING_ORCH") or "/opt/wathefni/staging/orchestrator"
...
def main() -> int:
    print(f"SHW3B staging create-path tag={TAG}")
    token = mint()
    seed_employee()
    ...
    with sync_playwright() as p:
        ...
        shot(page, "w3b-staging-board-before")
        ...
        shot(page, "w3b-staging-after-same-day")
        ...
        page.locator("[data-testid='shifts-create-submit']").click()
        page.wait_for_timeout(2500)
        shot(page, "w3b-staging-after-split")
        ...
        page.locator("[data-testid='shifts-create-submit']").click()
        page.wait_for_timeout(2500)
        shot(page, "w3b-staging-after-overnight")
        ...
        page.wait_for_selector("[data-testid='shifts-workspace']", timeout=20000)
        shot(page, "w3b-staging-mobile-rtl")
        context.close()
        browser.close()
    ...
    (OUT / "w3b-staging-create-path.json").write_text(json.dumps(RESULTS, indent=2, default=str))
    print(f"\n{PASS} passed, {FAIL} failed")
    print("RESULTS", OUT / "w3b-staging-create-path.json")
