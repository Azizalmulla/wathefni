/**
 * Attendance Wave 4 — exception queue, corrections, disputes (customer UX).
 * Uses Wave 3 ops APIs. No real punch ingest / devices.
 */
...
function friendlyError(error: unknown, fallback: string): string {
  if (error instanceof DashboardApiError) {
    ...
    if (typeof detail === 'object' && detail?.error === 'stale_row_version') {
      return 'stale'
    }
...
const copy = {
  en: {
    ...
    subtitle: 'Exceptions, corrections, and disputes — ready for HR and managers.',
    refresh: 'Refresh',
    ...
    after: 'After',
    payrollWhy: 'Payroll',
    loading: 'Loading attendance operations…',
  ...
  ar: {
    title: 'عمليات الحضور',
    subtitle: 'الاستثناءات والتصحيحات والنزاعات — جاهزة للموارد البشرية والمديرين.',
    refresh: 'تحديث',
    queue: 'طابور الاستثناءات',
    ...
    after: 'بعد',
    payrollWhy: 'الرواتب',
    loading: 'جاري تحميل عمليات الحضور…',
...
export function AttendanceOpsPanel({
  ...
  const selected = exceptions.find((e) => e.exception_id === selectedId) || openish[0] || exceptions[0] || null
  ...
  return (
    <div className="space-y-3" data-testid="attendance-ops" dir={isAr ? 'rtl' : 'ltr'}>
      <Card className="border-[#e8dfd0] bg-[#fffdf8]">
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <CardTitle className="text-[17px] tracking-[-0.02em]">{t.title}</CardTitle>
          ...
          {loading ? (
            ...
            <>
              ...
              <WorkforceSectionRail
                ...
                onChange={setTab}
                sections={[
                  { id: 'queue', labelEn: t.queue, labelAr: copy.ar.queue },
                  { id: 'corrections', labelEn: t.corrections, labelAr: copy.ar.corrections },
                  { id: 'disputes', labelEn: t.disputes, labelAr: copy.ar.disputes },
                  { id: 'locked', labelEn: t.locked, labelAr: copy.ar.locked },
                ]}
              />
              ...
              {tab === 'queue' ? (
                openish.length === 0 ? (
                  ...
                  <div className="grid gap-3 lg:grid-cols-[1.1fr_0.9fr]">
                      ...
                      {openish.map((exc) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() => setSelectedId(exc.exception_id)}
                            className={cn(
                              'w-full rounded-[1.1rem] border px-3 py-3 text-start transition',
                              active ? 'border-[#c89445]/50 bg-white shadow-sm' : 'border-[#e8dfd0] bg-[#fffdf8]/60 hover:bg-white/80',
                            )}
                          >
                            <div className="flex flex-wrap items-center gap-2">
                              <StatusPill tone="review">{exceptionKindLabel(exc.kind, locale)}</StatusPill>
                              <Badge tone="muted" className="text-[11px]">{exc.priority || 'normal'}</Badge>
                            ...
                            <p className="mt-1 text-[12px] text-subtle/90">
                              {String(exc.work_date || '').slice(0, 10)} · {t.owner}: {exc.owner_phone || '—'} · {t.due}: {formatDue(exc.due_at, locale)}
                            </p>
                            <p className="mt-1 text-[12px] font-medium text-wf-ink">
                              {t.next}: {nextActionFor(exc, locale)}
                            </p>
                          </button>
                        )
                      })}
                    ...
                    {selected ? (
                      <div className="space-y-3 rounded-[1.2rem] border border-[#e8dfd0] bg-white p-4">
