...
MARKER_OUT="$LOCAL_EVID/tests/bundle-markers.txt"
{
  ...
  if [[ -n "${CHUNK:-}" ]]; then
    for needle in "Attendance operations" "عمليات الحضور" "Connector health" "صحة الموصلات" "Day detail" "تفاصيل اليوم" "Approve — ready to apply" "Payroll excluded"; do
      if grep -qF "$needle" "$CHUNK"; then echo "FOUND $needle"; else echo "MISSING $needle"; fi
    done
    for bad in "Wave 4" "wave 4" "authority mode" "dark mode (no real"; do
...
ATTW3_RESULTS_PATH="\$REMOTE_EVID/tests/qualification-staging.json" \
  .venv/bin/python smoke-test-attendance-ops-wave3.py 2>&1 | tee "\$REMOTE_EVID/tests/ops-smoke.out" | tail -30

.venv/bin/python smoke-test-employees360-freeze-regression.py 2>&1 | tee "\$REMOTE_EVID/tests/freeze-employees360.out" | tail -5
.venv/bin/python smoke-test-onboarding-freeze-regression.py 2>&1 | tee "\$REMOTE_EVID/tests/freeze-onboarding.out" | tail -5
...
# Install playwright chromium if needed is assumed on box; run shots
\$STAGING_ORCH/.venv/bin/python "\$REMOTE_STAGE/attendance-wave4-ui-staging-screenshots.py" 2>&1 | tee "\$REMOTE_EVID/tests/screenshots.out" | tail -40
