...
def run_core() -> dict:
    ...
    created = mig.create_staged_cv_batch(
        app, company_code=COMPANY_A, stage_dir=stage, dry_run=False, chunk_size=10, created_by="smoke"
    )
    assert_true(created.get("ok"), f"create: {created}")
    batch_id = created["batch_id"]

    # Resume: process one chunk, then finish
    w1 = mig.run_worker(app, company_code=COMPANY_A, limit=1, worker_id="smoke-resume-1")
    ...
    created_r = mig.create_staged_cv_batch(
        app, company_code=COMPANY_A, stage_dir=stage_r, dry_run=False, chunk_size=8, created_by="smoke-retry"
    )
    ...
    try:
        for _ in range(8):
            out = mig.run_worker(app, company_code=COMPANY_A, limit=1, worker_id="smoke-inject")
    ...
    b_created = mig.create_staged_cv_batch(
        app, company_code=COMPANY_B, stage_dir=stage_b, dry_run=False, chunk_size=5, created_by="smoke-b"
    )
