...
def main() -> int:
    ...
    check("job architecture enableable after R5D", ready.customer_enableable("job_architecture") is True)
    ...
    check("fail-closed keeps Learning namespaces", ("learning", "/dashboard/learning") in ready.all_http_namespaces() or ("learning", "/app/learning") in ready.all_http_namespaces())
    ...
    check("Setup still omits Learning", "Wave6LearningPoliciesCard" not in setup_app)
    ...
    check(
        ...
        detail or ja.status_code,
    )
    learning = client.get("/dashboard/learning")
    learning_detail = learning.json().get("detail") if learning.headers.get("content-type", "").startswith("application/json") else None
    check(
        "Learning still fail-closed",
        learning.status_code == 404
        and isinstance(learning_detail, dict)
        and learning_detail.get("error") == "capability_not_released",
        learning_detail,
    )
