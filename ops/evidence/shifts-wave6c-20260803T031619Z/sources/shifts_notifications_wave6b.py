    ...
    *,
    ...
    cur.execute(
        ...
        WHERE company_code=%s AND event_id=%s
          AND status IN ('failed','queued','unsupported')
    ...
    for channel, attempts in by_channel.items():
        ...
        # Unsupported channels never recover — escalate immediately.
        if last_status == "unsupported" or n >= max_attempts:
            cur.execute(
                """
                UPDATE shift_notification_deliveries SET
                  status='terminal_failed',
                  error_code=coalesce(error_code, CASE WHEN %s='unsupported' THEN 'channel_unsupported' ELSE 'max_attempts' END),
                  updated_at=now()
                WHERE delivery_id=%s
                RETURNING *
                """,
                (last_status, last["delivery_id"]),
            )
            terminal.append(_row(cur) or {})
            continue
        result = _mock_send(channel, dedupe_key=str(event.get("dedupe_key")), payload=event.get("payload") or {})
        status = result.get("status") or ("delivered" if result.get("ok") else "failed")
        if status == "unsupported" or (not result.get("ok") and (n + 1) >= max_attempts):
            status = "terminal_failed"
        cur.execute(
            """
            INSERT INTO shift_notification_deliveries (
