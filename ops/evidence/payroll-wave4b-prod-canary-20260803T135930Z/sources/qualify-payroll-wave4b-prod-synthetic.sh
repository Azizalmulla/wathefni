#!/usr/bin/env bash
# Payroll Wave 4-B — production WATHEFNI synthetic qualification.
# Deploy close/export → canary → rollback proof → redeploy → canary → freezes.
# Does NOT enable real bank/WPS/PIFSS/EOS/payments/AI. Does NOT start next payroll wave.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/payroll-wave4b-prod-canary-$STAMP"
REMOTE_STAGE="/tmp/payroll-w4b-prod-stage"
...
echo "$LOCAL_EVID" > /tmp/pyw4b.evid
echo "$STAMP" > /tmp/pyw4b.stamp
...
"$PY_LOCAL" smoke-test-payroll-close-export-wave4-ux.py 2>&1 | tee "$LOCAL_EVID/tests/ux-en-ar-mobile-local.out" | tail -20

log "stage sources"
mkdir -p "$LOCAL_EVID/sources/ops/sql" "$LOCAL_EVID/migrate"
cp -a app.py payroll_close_export_wave4.py payroll_payslip_wave3.py payroll_native_preview_wave2b.py payroll_authority_wave1.py payroll_external_adapter_wave2a.py \
  ...
  "$LOCAL_EVID/sources/"
cp -a ops/deploy-payroll-wave4b-prod-synthetic.sh ops/migrate-payroll-close-export-wave4-prod.sh "$LOCAL_EVID/sources/ops/"
cp -a ops/sql/payroll_close_export_wave4_v1.sql ops/sql/payroll_payslip_wave3_v1.sql \
  ops/sql/payroll_native_preview_wave2b_v1.sql ops/sql/payroll_authority_wave1_v1.sql \
  ops/sql/payroll_external_adapter_wave2a_v1.sql \
  "$LOCAL_EVID/sources/ops/sql/"
cp -a ops/migrate-payroll-close-export-wave4-prod.sh "$LOCAL_EVID/migrate/"
cp -a "$REPO_ROOT/ops/qualify-payroll-wave4b-prod-synthetic.sh" "$LOCAL_EVID/sources/" 2>/dev/null || true
...
cp -a "$DASH_SRC/src/posthire/CloseExportWorkspace.tsx" \
...
(
  cd "$ORCH_SRC"
  "${SCP[@]}" app.py payroll_close_export_wave4.py payroll_payslip_wave3.py payroll_native_preview_wave2b.py payroll_authority_wave1.py payroll_external_adapter_wave2a.py \
    canary-prod-payroll-close-export-wave4b.py \
    smoke-test-payroll-close-export-wave4.py \
    smoke-test-payroll-close-export-wave4-ux.py \
    smoke-test-payroll-payslip-wave3.py \
    ...
    smoke-test-leave-freeze-regression.py \
    smoke-test-shifts-freeze-regression.py \
    ops/deploy-payroll-wave4b-prod-synthetic.sh \
    ops/migrate-payroll-close-export-wave4-prod.sh \
    "$VPS_HOST:$REMOTE_STAGE/"
  "${SCP[@]}" ops/sql/payroll_close_export_wave4_v1.sql ops/sql/payroll_payslip_wave3_v1.sql \
    ops/sql/payroll_native_preview_wave2b_v1.sql ops/sql/payroll_authority_wave1_v1.sql \
    ops/sql/payroll_external_adapter_wave2a_v1.sql \
    "$VPS_HOST:$REMOTE_STAGE/"
)
...
chmod +x '$REMOTE_STAGE/deploy-payroll-wave4b-prod-synthetic.sh'
bash '$REMOTE_STAGE/deploy-payroll-wave4b-prod-synthetic.sh'
REMOTE
...
export PYW4B_EVID="\$OUTDIR"
mkdir -p "\$OUTDIR"
export PYTHONUNBUFFERED=1
\$PYBIN -u canary-prod-payroll-close-export-wave4b.py
REMOTE
}
...
set -euo pipefail
BACKUP=\$(cat /opt/wathefni/production-evidence/payroll-wave4b-prod-canary/$STAMP/backup/BACKUP_PATH.txt)
bash "\$BACKUP/ROLLBACK.sh" "\$BACKUP"
test ! -f /etc/systemd/system/wathefni-orchestrator.service.d/zzzzzzzzzzzzzzzzzz-payroll-wave4b-synthetic.conf
PID=\$(systemctl show -p MainPID --value wathefni-orchestrator)
...
with app.db_connect() as conn:
  with conn.cursor() as cur:
    cur.execute("""
      ...
      WHERE company_code='WATHEFNI' AND employee_key LIKE 'WATHEFNI-PYW1-PYW4-W4B-%'
    """)
    n=int(dict(cur.fetchone())["n"])
    cur.execute("""
      SELECT COUNT(*) AS n FROM payroll_close_runs
      WHERE company_code='WATHEFNI' AND decision_note LIKE '%%w4b_%%'
    """)
    c=int(dict(cur.fetchone())["n"])
print("RESIDUAL_SYNTH_CONTRACTS", n)
print("RESIDUAL_SYNTH_CLOSE_RUNS", c)
assert n==0 and c==0
PY
...
set -euo pipefail
ORCH=/opt/wathefni/orchestrator
OUTDIR='/opt/wathefni/production-evidence/payroll-wave4b-prod-canary/${STAMP}/canary/after-redeploy'
PYBIN=\$ORCH/.venv/bin/python
cd \$ORCH
set -a; source /root/.openclaw/secrets/postgres.env; set +a
PID=\$(systemctl show -p MainPID --value wathefni-orchestrator)
...
log "Wave 1/2A/2B/3/4 honesty + production freezes"
...
h=w4.honesty_payload(); b=w2b.honesty_payload(); a=w2a.honesty_payload(); inv=w2a.freeze_invariants()
p=w3.honesty_payload()
...
print("WAVE1_2A_2B_3_4_FREEZE_HONESTY_OK")
PY
...
"${SCP[@]}" -r "$VPS_HOST:/opt/wathefni/production-evidence/payroll-wave4b-prod-canary/${STAMP}-redeploy/." "$LOCAL_EVID/remote-redeploy/" 2>/dev/null || true
...
ack_flags = ""
for cand in [evid / "remote" / "flags" / "payroll-wave4b-flags.txt", evid / "remote-redeploy" / "flags" / "payroll-wave4b-flags.txt"]:
    if cand.exists():
...
prod_ok = "FREEZES_AND_PAYROLL_OK" in prod_txt and "WAVE1_2A_2B_3_4_FREEZE_HONESTY_OK" in prod_txt

rb_txt = (evid / "tests" / "rollback.out").read_text() if (evid / "tests" / "rollback.out").exists() else ""
rb_ok = (
    "ROLLBACK_VERIFIED" in rb_txt
    and "WAVE4_FLAGS_CLEARED" in rb_txt
    and "RESIDUAL_SYNTH_CONTRACTS 0" in rb_txt
    and "RESIDUAL_SYNTH_CLOSE_RUNS 0" in rb_txt
)
...
if not rb_ok:
    blockers.append("rollback not verified (Wave 1/2A/2B/3 retained + Wave 4 cleared + residual 0)")
...
if "WATHEFNI_PAYROLL_WAVE4_SYNTHETIC_ONLY=1" not in flags:
    blockers.append("synthetic_only flag not confirmed")
if "MIGRATE_OK" not in mig or "ACK_PRODUCTION_PAYROLL_W4B=YES" not in mig:
    blockers.append("migrate/ACK markers missing")

freeze_go = "GO" if not blockers else "NO-GO"
report = f"""# Payroll Wave 4-B — production synthetic qualification

**Stamp:** `{stamp}`  
**Evidence:** `ops/evidence/payroll-wave4b-prod-canary-{stamp}/`  
...
| Scope | Verdict |
|---|---|
| Production synthetic Wave 4 close/export | **{freeze_go}** |
| Freeze Wave 4 | **{freeze_go}** |
...
ACK flags:
```
{ack_flags.strip() or '(see remote/flags/payroll-wave4b-flags.txt)'}
```
...
| Leave (local) | {flv[1]} |
| Shifts (local) | {fsh[1]} |
| Production freezes + Wave 1/2A/2B/3/4 honesty | {'PASS' if prod_ok else 'FAIL'} |

---

## Blockers

{chr(10).join('- ' + b for b in blockers) if blockers else '- None for Wave 4 freeze scope.'}
...
gate = "PROD_SYNTHETIC_PAYROLL_WAVE4_GO" if freeze_go == "GO" else "PROD_SYNTHETIC_PAYROLL_WAVE4_NO_GO"
