...
(global kill switch, prod default off) AND company allowlist
WATHEFNI_ATTENDANCE_CAPTURE_INGEST_COMPANIES (empty = nobody).
See attendance_truth_c1.py / WAVE2 C1 charter.
"""
...
import attendance_truth_c1 as _truth_c1
...
def capture_ops_ingest_enabled() -> bool:
    """Global ingest kill switch (prod default off). Prefer company gate for writes."""
    return _truth_c1.capture_ingest_runtime_on()


def capture_ops_ingest_enabled_for_company(company_code: str | None) -> dict[str, Any]:
    """C1: company-entitled ingest. Empty allowlist = nobody even if global on."""
    return _truth_c1.capture_ingest_enabled_for_company(company_code)
...
def overview(company_code: str) -> dict[str, Any]:
    ...
    return {
        ...
        "ingest_companies": sorted(_truth_c1.capture_ingest_company_allowlist()),
        "ingest_gate": "company_allowlist_fail_closed",
