...
def register_attendance_capture_ops_routes(
    ...
    *,
    ...
    def capture_ops_approve_mapping(item_id: str, body: CaptureApproveMapping, context: dict[str, Any] = Depends(dashboard_context)):
        company = _guard(context, write=True)
        _bind_scope()
        # C1: replay only when company-entitled ingest is on
        ingest_gate = capture_ops.capture_ops_ingest_enabled_for_company(company)
        replay = bool(body.replay) and bool(ingest_gate.get("ok"))
    ...
    def capture_ops_replay(item_id: str, body: CaptureReplayItem, context: dict[str, Any] = Depends(dashboard_context)):
        company = _guard(context, write=True)
        ingest_gate = capture_ops.capture_ops_ingest_enabled_for_company(company)
        if not ingest_gate.get("ok"):
            raise HTTPException(
                status_code=403,
                detail={
                    "ok": False,
                    "error": ingest_gate.get("error") or "capture_ingest_off",
                    "message": ingest_gate.get("message")
                    or "Punch ingest is off globally or company is not allowlisted (Wave 2 C1).",
                    "gate": ingest_gate.get("gate"),
                },
            )
        _bind_scope()
