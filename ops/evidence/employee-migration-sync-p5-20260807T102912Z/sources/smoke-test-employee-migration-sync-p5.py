...
def main() -> None:
    ...
    if honesty.get("contract") != "employee_migration_sync_p5_connectors":
        _fail(f"contract {honesty.get('contract')}")
    if not honesty.get("connected_systems_p5"):
        _fail("p5 honesty missing")
    if "P6_leavers" not in (honesty.get("expansion_phases_deferred") or []):
        _fail("P6 should remain deferred")
    _ok("P5 honesty")
    ...
    try:
        ...
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        _ok("lifecycle signals captured only")
