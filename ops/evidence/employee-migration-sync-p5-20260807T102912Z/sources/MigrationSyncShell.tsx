  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-[13px] leading-6 text-subtle/90">
          Connect a source, map fields, run sync, then review accepted changes. Deactivation from sync stays off.
        </p>
      </div>
      ...
      <Card className="border-line/50 bg-white/55 shadow-none">
        <CardContent className="space-y-3 p-4">
          ...
          <p className="text-[11.5px] leading-5 text-subtle/75">
            Canary connector is safe for WATHEFNI qualification. API/SFTP vendor connectors use the same flow when enabled.
          </p>
        </CardContent>
      </Card>
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-[11.5px] text-subtle/70">
                      Status {c.status}
                      {c.last_success_at ? ` · last success ${c.last_success_at}` : ''}
