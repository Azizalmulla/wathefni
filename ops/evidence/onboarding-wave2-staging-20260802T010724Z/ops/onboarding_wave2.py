...
def item_is_satisfied(status: str | None) -> bool:
    return str(status or "").lower() in {
        "received", "complete", "completed", "verified", "waived",
    }
    ...
    *,
    ...
    for order, spec in enumerate(DEFAULT_KUWAIT_V2):
        ...
        cur.execute(
            ...
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,'pending',%s,%s::jsonb,%s,%s,%s,1,%s::jsonb, now())
            """,
            (
                employee_key,
                spec.item_id,
...
def activate_delayed_starts(cur: Any, *, company_code: str | None = None, today: date | None = None) -> dict[str, Any]:
    ...
    clause = ""
    if company:
        clause = " AND company_code=%s"
        params.append(company)
    cur.execute(
        f"""
        SELECT * FROM employee_onboarding_assignments
        WHERE status='delayed' AND planned_start_date IS NOT NULL AND planned_start_date <= %s
        {clause}
    ...
    for row in rows:
        ...
        activated.append(row["employee_key"])
    return {"ok": True, "activated": activated, "count": len(activated)}
        ...
        for spec in DEFAULT_KUWAIT_V2:
            ...
            cur.execute(
                ...
                WHERE employee_key=%s AND item_id=%s
                  AND lower(coalesce(status,'')) NOT IN ('received','complete','completed','verified','waived')
                """,
...
def cancel_onboarding(
    ...
    *,
    ...
    cur.execute(
        ...
        UPDATE onboarding_items
        SET status=CASE
              WHEN lower(coalesce(status,'')) IN ('received','complete','completed','verified','waived') THEN status
...
def transition_model() -> dict[str, Any]:
    return {
        ...
        "transitions": [
            ...
            {"from": "in_progress", "to": "abandoned", "action": "lifecycle employment end"},
