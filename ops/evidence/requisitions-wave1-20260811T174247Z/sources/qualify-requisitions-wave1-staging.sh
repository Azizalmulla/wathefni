...
text = p.read_text(encoding="utf-8")
needle = "import workflow_task_sla as _workflow_task_sla"
hook = '''
...
if "ensure_requisitions_schema" in text:
    ...
    # Insert after workflow_task_sla ensure block
    marker = "_workflow_task_sla.ensure_workflow_task_sla_schema(cur)"
    idx = text.find(marker)
    if idx < 0:
        raise SystemExit("APP_HOOK_ANCHOR_MISSING")
    # find end of that try/except
    ...
    raise SystemExit("APP_HOOK_NEEDLE_MISSING")
PY
else
