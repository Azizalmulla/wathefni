...
def main() -> int:
    ...
    try:
        ...
        mods_code, mods = http(
            ...
            token=op_token,
            body={"modules": ["leave", "attendance", "onboarding", "employee_app"]},
        )
        check(
            "Setup enables catalog modules",
            mods_code == 200 and (not isinstance(mods, dict) or mods.get("ok") is not False),
            (mods_code, _err(mods)),
        )
        ...
        perms = []
        if isinstance(hr_login, dict):
            user = hr_login.get("user") if isinstance(hr_login.get("user"), dict) else {}
            access = hr_login.get("access") if isinstance(hr_login.get("access"), dict) else {}
            perms = list(user.get("permissions") or access.get("permissions") or [])
        check("owner session includes employees.read from bootstrap", "employees.read" in perms, perms[:12])
        check("owner session includes employees.manage from bootstrap", "employees.manage" in perms, perms[:12])
        if not hr_token:
        ...
        # Grant-only employee scopes: first-owner bootstrap must write employees.manage.
        # This harness may not SQL-grant it.
        if emp_code in {200, 201} and isinstance(emp, dict) and emp.get("ok") is not False:
            ...
            emp_obj = emp.get("employee") if isinstance(emp.get("employee"), dict) else emp
            emp_key = str((emp_obj or {}).get("employee_key") or emp.get("employee_key") or "")
            roster_code, roster = http("GET", "/dashboard/posthire/employees", token=hr_token)
            roster_blob = json.dumps(roster) if not isinstance(roster, str) else roster
            check(
                "owner can read the new roster",
                roster_code == 200 and (not emp_key or emp_key in roster_blob or EMP_PHONE in roster_blob),
                (roster_code, emp_key or _err(roster)),
            )
            leave_code, leave = http("GET", "/dashboard/posthire/leave", token=hr_token)
            check(
                "HR leave surface is reachable for the new company",
                leave_code in {200, 403},
                (leave_code, _err(leave)),
            )
        else:
            err = _err(emp)
            grant_blocked = "permission_denied" in err or "employees.manage" in err
            module_blocked = "module_disabled" in err
            check(
                "clean owner roster create is possible through Setup/HR (no cutover CLI)",
                False,
                (emp_code, err, "grant-only employees.manage" if grant_blocked else ("module_disabled" if module_blocked else "unexpected")),
            )
            if grant_blocked:
                print("      NOTE  employees.* are grant-only; Setup owner seed does not grant them; no Setup write path exists for the first grant")
            elif module_blocked:
                print("      NOTE  catalog people modules were not enabled for the new company")
    finally:
        cleanup_sql()
