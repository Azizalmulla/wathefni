...
type QueueTab = 'board' | 'swaps' | 'availability' | 'reconciliation' | 'reminders' | 'templates' | 'publish' | 'enterprise'
type RequestPanel = 'swaps' | 'availability' | 'reconciliation'
type PlanningPanel = 'templates' | 'publish' | 'enterprise' | 'reminders'
...
export function ShiftsWorkspace({ access, permissions, role: _role, onNotice, onAccessIssue }: ShiftsWorkspaceProps) {
  ...
  const queue: QueueTab = useMemo(() => {
    ...
    if (planningPanel === 'templates' && !wave4?.enabled) {
      ...
      if (wave6?.enabled) return 'enterprise'
      return 'reminders'
    }
    ...
    if (planningPanel === 'enterprise' && !wave6?.enabled) {
      if (wave5?.enabled) return 'publish'
      if (wave4?.enabled) return 'templates'
      return 'reminders'
    }
    return planningPanel
  }, [surface, requestPanel, planningPanel, wave4?.enabled, wave5?.enabled, wave6?.enabled])
  ...
  const queueCounts: Record<QueueTab, number> = {
    ...
    enterprise: rotationPatterns.length || rotationAssignments.length || pamExports.length,
  }
  ...
  return (
    ...
    >
        ...
        {(
          ...
          <button
            ...
            className={cn(
              ...
              surface === id ? 'bg-wf-ink text-white' : 'bg-[#f3ebe0] text-muted',
            )}
      ...
      {/* Schedule chrome: day/week + date nav */}
      {surface === 'schedule' ? (
        <div className="flex flex-wrap items-center gap-2" data-shifts-date-chrome>
          ...
          <span className="px-1 text-sm font-semibold text-ink">{rangeLabel}</span>
        </div>
      ) : null}

      {/* Compact attention on Schedule */}
      {surface === 'schedule' ? (
        <div
          className={cn(
            'flex gap-3 rounded-[1.15rem] border px-3.5 py-3',
            requestCount ? 'border-[#e8c9a0] bg-wf-accent-review-soft/45' : 'border-[#e8dfd0] bg-[#fffdf8]/70',
          )}
          data-shifts-attention
        >
          <CalendarClock className="mt-0.5 h-4 w-4 shrink-0 text-subtle" />
          <div className="min-w-0 flex-1">
            ...
            {requestCount ? (
              <button
                type="button"
                className="mt-1 text-[12px] font-medium text-subtle underline"
                onClick={() => {
                  setSurface('requests')
          ...
          {(
            ...
            <button
              ...
              className={cn(
                ...
                requestPanel === id ? 'bg-wf-ink text-white' : 'bg-[#f3ebe0] text-muted',
              )}
      ...
      {surface === 'planning' ? (
        <div className="space-y-3" data-shifts-planning>
          <div className="flex flex-wrap gap-2" role="tablist">
            {(
              [
                ...
                ...(wave6?.enabled ? ([['enterprise', c.enterprise]] as [PlanningPanel, string][]) : []),
                ['reminders', c.reminders] as [PlanningPanel, string],
              ...
              <button
                ...
                className={cn(
                  'rounded-full px-3 py-1.5 text-sm',
                  planningPanel === id ? 'bg-wf-ink text-white' : 'bg-[#f3ebe0] text-muted',
                )}
                onClick={() => setPlanningPanel(id)}
              >
                {label}
          ...
          <div data-shifts-operations>
            <button
              type="button"
              className="text-[12px] font-medium text-subtle underline"
              onClick={() => setReadinessOpen((v) => !v)}
            >
              {readinessOpen ? c.hideOperations : c.showOperations}
            </button>
            {readinessOpen && wave3?.enabled ? (
              <div className="mt-2 space-y-2 rounded-[1rem] border border-dashed border-[#e8dfd0] bg-[#fffaf0]/70 px-3.5 py-3 text-[12px] text-muted" data-testid="shifts-honesty-banner">
                <p>
                  {wave6?.enabled
                    ? c.honestyEnterprise
                    : wave5?.enabled
      ...
      {/* Governed compact filters — Schedule only */}
      {surface === 'schedule' ? (
        <div className="flex flex-wrap gap-2" data-testid="shifts-filters" data-shifts-filters>
          <input
            className="h-9 min-w-[8rem] rounded-xl border border-[#e8dfd0] bg-white px-3 text-sm"
            placeholder={c.employee}
            value={filters.employee}
            onChange={(e) => setFilters((f) => ({ ...f, employee: e.target.value }))}
          />
          <select
            className="h-9 rounded-xl border border-[#e8dfd0] bg-white px-3 text-sm"
      ...
      <div className={`grid gap-4 ${asideOpen && queue === 'board' ? 'xl:grid-cols-[minmax(0,1fr)_360px]' : ''}`}>
        ...
        <div>
                  ...
                  {range.days.map((day) => {
                    ...
                    return (
                      <div key={dayKey} className="p-4">
                        ...
                        {all.length === 0 ? (
                          <div className="rounded-[1rem] border border-dashed border-[#e8dfd0] bg-white/40 px-4 py-6 text-center" data-shifts-empty>
                            <p className="text-[14px] font-semibold text-ink">{c.emptyBoardTitle}</p>
                            <p className="mt-1 text-[12px] text-muted">{c.emptyBoardHint}</p>
                            {canManage ? (
                  ...
                  onClick={() => {
                    void (async () => {
                      ...
                      try {
                        ...
                        setPublishInfo(`${c.publishAction}${res.idempotent ? ' (idempotent)' : ''}`)
                        const p = await listSchedulePeriods(access)
                        setPeriods(p.periods || [])
                      } catch (err) {
          ...
          {queue === 'enterprise' && wave6?.enabled ? (
            <div className="space-y-4 rounded-[1.55rem] border border-line/55 bg-white p-4 shadow-[0_18px_40px_rgba(35,33,29,0.08)]" data-testid="shifts-enterprise-panel">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h2 className="text-lg font-semibold text-ink">{c.enterprise}</h2>
                  ...
                  onClick={() => {
                    void (async () => {
                      ...
                      try {
                        ...
                        onNotice?.(err instanceof Error ? err.message : 'Failed to load enterprise surfaces')
                      } finally {
                        setWave6Busy(null)
                      }
                    })()
              ...
              <div className="grid gap-3 md:grid-cols-3">
                  ...
                  {rotationPatterns.length === 0 ? (
                    ...
                    rotationPatterns.slice(0, 8).map((p) => (
                      <div key={String(p.pattern_id)} className="rounded-xl border border-[#e8dfd0] px-3 py-2 text-sm">
                        <p className="font-medium text-ink">{String(p.name || '')}</p>
                        <p className="text-xs text-muted">{String(p.pattern_kind || '')}</p>
                      </div>
                    ))
                  )}
                </div>
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-wide text-muted">{isAr ? 'تعيينات الدورة' : 'Roster assignments'}</p>
                  {rotationAssignments.length === 0 ? (
                    ...
                    rotationAssignments.slice(0, 8).map((a) => (
                      <div key={String(a.assignment_id)} className="space-y-1 rounded-xl border border-[#e8dfd0] px-3 py-2 text-sm">
                        ...
                        <p className="text-xs text-muted">
                          offset={String(a.cycle_offset ?? 0)}
                          {a.remote_site_key ? ` · ${String(a.remote_site_key)}` : ''}
                        </p>
                            ...
                            onClick={() => {
                              void (async () => {
                                ...
                                try {
                                  ...
                                  setEnterpriseInfo(
                                    `${c.rotationPreview}: work=${prev.counts?.work ?? 0} rest=${prev.counts?.rest ?? 0} travel=${prev.counts?.travel ?? 0}`,
                                  )
                                } catch (err) {
                                  onNotice?.(err instanceof Error ? err.message : 'Preview failed')
                                } finally {
                            ...
                            onClick={() => {
                              void (async () => {
                                ...
                                try {
                                  ...
                                  setEnterpriseInfo(`${c.generateFromRotation}: ${res.draft_rows ?? 0} rows`)
                                } catch (err) {
                                  onNotice?.(err instanceof Error ? err.message : 'Draft from rotation failed')
                                } finally {
                                  setWave6Busy(null)
                                }
                              })()
                ...
                <div className="space-y-2">
                  <p className="text-xs uppercase tracking-wide text-muted">{c.pamExport}</p>
                  <Button
                    size="sm"
                    ...
                    onClick={() => {
                      void (async () => {
                        ...
                        try {
                          ...
                          onNotice?.(err instanceof Error ? err.message : 'PAM export failed')
                        } finally {
                          setWave6Busy(null)
                        }
                      })()
                  ...
                  {pamExports.length === 0 ? (
                    ...
                    pamExports.slice(0, 6).map((e) => (
                      <div key={String(e.export_id)} className="rounded-xl border border-[#e8dfd0] px-3 py-2 text-sm">
                        <p className="font-medium text-ink">{String(e.status || '')}</p>
                        <p className="text-xs text-muted">{String(e.fingerprint || '').slice(0, 20)}</p>
                      </div>
                    ))
                  )}
                </div>
              </div>
              {enterpriseInfo ? <p className="rounded-xl bg-[#fbf7ee] px-3 py-2 text-xs text-muted" data-testid="shifts-enterprise-info">{enterpriseInfo}</p> : null}
            </div>
          ) : null}
        </div>
