...
# Border content energy → likely cropped / cut off
CROP_REJECT_SIDES = 3
CROP_BORDERLINE_SIDES = 2
...
def _border_content_sides(gray, *, border_frac: float = CROP_BORDER_FRAC) -> int:
    """Count sides where border strip has high content energy (likely cut off)."""
    import numpy as np
    ...
    for strip in strips.values():
        std = float(strip.std())
        # Quiet desk/margin borders are low-std; text/photo cut at the edge is high-std.
...
def assess_capture_quality(
    *,
    ...
    if resolved is None:
        # Try infer extension from media path later; if unknown image-ish, skip softly.
    ...
    # Reject: clearly bad / fixable (order: resolution → glare → blur → crop)
    ...
    if crop_sides >= CROP_REJECT_SIDES:
        return {
            "status": "reject",
            "reason": "document_not_fully_visible",
            "metrics": metrics,
            "engine": "pil_numpy_v1",
        }

    # Borderline: imperfect but possibly readable → HR
    ...
    elif crop_sides >= CROP_BORDERLINE_SIDES:
        borderline_reason = "document_not_fully_visible"
