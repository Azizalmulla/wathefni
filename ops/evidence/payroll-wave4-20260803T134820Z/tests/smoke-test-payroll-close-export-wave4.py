...
def main() -> int:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            # Prior-wave synthetic cleanup for this EMP marker
            cur.execute("DELETE FROM payroll_preview_lines WHERE employee_key=%s", (EMP,))
            cur.execute("DELETE FROM payroll_preview_employee_results WHERE employee_key=%s", (EMP,))
            cur.execute("DELETE FROM payroll_adapter_import_lines WHERE employee_key=%s", (EMP,))
            cur.execute(
                """
            ...
            cur.execute("DELETE FROM payroll_compensation_contracts WHERE employee_key=%s", (EMP,))
