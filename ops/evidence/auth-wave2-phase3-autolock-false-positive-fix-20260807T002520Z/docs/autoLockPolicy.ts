...
/**
 * Decide whether returning from a true background should require local unlock.
 * Device screen-lock wins only when confirmed while AppState was `background`
 * (never from brief `inactive` / Control Center / notification shade).
 */
export type AutoLockDecisionReason = 'timeout' | 'confirmed_device_lock' | 'none'

export function decideAutoLockOnResume(opts: {
  elapsedMs: number
  timeoutMs: AutoLockTimeoutMs
  deviceWasLocked: boolean
  enteredBackground: boolean
}): AutoLockDecisionReason {
  if (!opts.enteredBackground) return 'none'
  if (opts.deviceWasLocked) return 'confirmed_device_lock'
  if (opts.timeoutMs == null) return 'none'
  if (opts.elapsedMs >= opts.timeoutMs) return 'timeout'
  return 'none'
}

/** @deprecated prefer decideAutoLockOnResume for reason logging */
