...
def setup_isolation_companies() -> None:
    ...
    # WATHEFNI auto-admit ON — Wave1 path must still force OFF
    app.set_company_setting(WATHEFNI, "intake_auto_admit_explicit", True)
...
def cleanup_wathefni_canary_batches() -> dict:
    """Rollback/delete only Wave 1 canary batches tagged for WATHEFNI — keep company."""
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE company_code=%s
                  AND (
                    coalesce(metadata->>'wave','') = 'migration_wave1'
                    OR coalesce(created_by,'') LIKE 'smoke-migw1%%'
                    OR coalesce(created_by,'') LIKE 'canary-migw1b%%'
                    OR coalesce(created_by,'') = 'smoke'
                    OR coalesce(created_by,'') LIKE 'smoke%%'
                  )
                """,
                (WATHEFNI,),
            )
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            for bid in batch_ids:
                ...
                deleted["batches"] += 1
            # Sweep any remaining canary apps by marker
            cur.execute(
                """
...
def residual() -> dict:
    ...
    total = wathefni_apps + wathefni_rows + iso_companies + iso_apps + wathefni_batches
    return {
...
def run_core() -> dict:
    ...
    batch_id = created["batch_id"]
    w1 = mig.run_worker(app, company_code=WATHEFNI, limit=1, worker_id="canary-resume-1")
    ...
    try:
        for _ in range(8):
            mig.run_worker(app, company_code=WATHEFNI, limit=1, worker_id="canary-inject")
    ...
    a_created = mig.create_staged_cv_batch(
        app, company_code=ISO_A, stage_dir=stage_a, dry_run=False, chunk_size=5, created_by="canary-iso-a"
    )
    ...
    b_created = mig.create_staged_cv_batch(
        app, company_code=ISO_B, stage_dir=stage_b, dry_run=False, chunk_size=5, created_by="canary-iso-b"
    )
...
def main() -> None:
    ...
    assert_production()
    # Pre-clean any leftover canary residue from a prior interrupted run
    cleanup_wathefni_canary_batches()
    for c in (ISO_A, ISO_B):
        try:
            cleanup_isolation_company(c)
        except Exception:
            pass
