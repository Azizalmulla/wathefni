...

{
  ...
  echo "=== SHAs before ==="
  sha256sum "$ORCH/migration_wave1_cv_foundation.py" "$ORCH/smoke-test-migration-wave1.py" 2>/dev/null || true
  echo "=== flags before ==="
...
for f in migration_wave1_cv_foundation.py smoke-test-migration-wave1.py canary-prod-migration-wave1b.py; do
...
mkdir -p "$ORCH/ops"
for f in migration_wave1_cv_foundation.py smoke-test-migration-wave1.py canary-prod-migration-wave1b.py \
...
{
  echo "=== SHAs after ==="
  sha256sum "$ORCH/migration_wave1_cv_foundation.py" "$ORCH/canary-prod-migration-wave1b.py"
  echo "=== flags after ==="
