        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (
                    ...
                    len(files),
                    chunk_size,
                    Json({**(options or {}), **honesty_payload(), "per_file_max_bytes": per_file_max_bytes}),
...
def _claim_chunk_job(cur: Any, *, company_code: str | None, worker_id: str, lease_seconds: int = 180) -> dict[str, Any] | None:
    ...
    cur.execute(
        ...
        RETURNING j.*
        """,
        (company, company, worker_id, str(int(lease_seconds))),
    )
    row = cur.fetchone()
    ...
    for row in rows:
        ...
        cur.execute(
            ...
            (
                ...
                item_id,
                err,
                Json({"import_status": status, "external_id": external_id}),
                row["row_id"],
            ),
        )
    ...
    *,
    ...
    for _ in range(max(1, int(limit))):
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                ensure_schema(cur)
                job = _claim_chunk_job(cur, company_code=company_code, worker_id=worker_id)
                if not job:
                    conn.commit()
                    break
                cur.execute(
                ...
                try:
                    ...
                    cur.execute(
                        """
                        UPDATE migration_chunk_jobs
                        SET status=%s, last_error=%s, available_at=now(),
                            leased_by=NULL, lease_expires_at=NULL, updated_at=now()
                        WHERE job_id=%s
                        """,
                        (status, str(exc)[:500], job["job_id"]),
                    )
                    record_event(
                        cur,
                        batch_id=str(job["batch_id"]),
                        ...
                        detail={"job_id": str(job["job_id"]), "error": str(exc)[:500], "attempts": attempts},
                    )
...
def run_until_idle(
    ...
    *,
    ...
    while loops < max_loops:
        loops += 1
        out = run_worker(legacy, company_code=company, limit=worker_burst)
...
def rollback_batch(legacy: Any, *, batch_id: str, company_code: str) -> dict[str, Any]:
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            deleted = {"applications": 0, "documents": 0, "files": 0, "candidates": 0}
            if app_keys:
            ...
            record_event(
                ...
                detail={"deleted": deleted, "app_keys": len(app_keys)},
            )
        conn.commit()
    return {"ok": True, "batch_id": batch_id, "deleted": deleted, "app_keys": len(app_keys)}
