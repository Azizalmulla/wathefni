...
def seed(cur) -> dict:
    ...
    try:
        cur.execute(
            """
            INSERT INTO employees (employee_key, company_code, name, phone, employment_status, start_date, metadata, updated_at)
            VALUES (%s,%s,%s,%s,'pending_start',%s,%s::jsonb, now())
            ON CONFLICT (employee_key) DO UPDATE SET employment_status='pending_start', start_date=EXCLUDED.start_date, updated_at=now()
            """,
            (emp, COMPANY, f"Visual Joiner {suffix}", phone, joining, meta),
        )
    except Exception:
        cur.execute(
            """
    ...
    try:
        cur.execute(
            """
            INSERT INTO employees (employee_key, company_code, name, phone, employment_status, start_date, metadata, updated_at)
            VALUES (%s,%s,%s,%s,'active',%s,%s::jsonb, now())
            ON CONFLICT (employee_key) DO UPDATE SET employment_status='active', updated_at=now()
            """,
            (emp2, COMPANY, f"Visual Probationer {suffix}", f"9655111{suffix[:4]}", start, meta),
        )
    except Exception:
        cur.execute(
            """
