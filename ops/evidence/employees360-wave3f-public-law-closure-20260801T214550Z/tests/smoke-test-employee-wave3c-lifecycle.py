...
def term_fields(**extra):
    """Wave 3F required classification — never inferred by product."""
    base = {
...
def main() -> int:
    ...
    try:
        ...
        check("wave3f schema", str(w3c.SCHEMA_VERSION).startswith("employees360-wave3f"), w3c.SCHEMA_VERSION)
        ...
        check("policy upsert wave3f", (policy.get("policy_json") or {}).get("wave") == "wave3f", policy)
