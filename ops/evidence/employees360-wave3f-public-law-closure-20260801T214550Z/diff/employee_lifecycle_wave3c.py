...
SCHEMA_VERSION = "employees360-wave3f-public-law-closure-v1"
...
# Wave 3F required classification vocabularies (Kuwait private-sector product contract).
CONTRACT_TYPES = frozenset({"unlimited", "fixed_term"})
PAY_FREQUENCIES = frozenset({"monthly", "other"})
PROBATION_STATUSES = frozenset({"none", "active", "completed"})
TERMINATION_CASE_CLASSES = frozenset({
    "resignation",
    "dismissal_ordinary",
    "summary_dismissal_41a",  # Art. 41(a) — no notice/compensation/benefit path (lawfulness is human)
    "summary_dismissal_41b",  # Art. 41(b) — indemnity preserved path (lawfulness is human)
    "end_of_fixed_term",
    "mutual",
    "abandonment_42",
...
COUNSEL_QUESTIONS = [
    {"id": "PL1", "text": "Confirm jurisdiction is Kuwait private sector (Law 6/2010 Arts. 2–5) and excluded categories are out of scope."},
...
def _default_policy_json() -> dict[str, Any]:
    return {k: DEFAULT_POLICY[k] for k in WAVE3D_POLICY_JSON_KEYS if k in DEFAULT_POLICY} | {"wave": "wave3f"}
...
def _normalize_policy(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(pj.get("wave") or "") in {"", "wave3d", "wave3c"}:
    ...
    elif str(merged_json.get("wave")) == "wave3f":
...
def create_lifecycle_request(
    ...
    *,
    ...
    if case_type == "termination":
        ...
        # Persist classification onto employment for future reads (authoritative HR entry).
