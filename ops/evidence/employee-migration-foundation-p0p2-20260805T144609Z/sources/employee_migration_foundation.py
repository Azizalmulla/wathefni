...
TOTAL_KEYS = ("create", "skip", "conflict", "invalid")
    ...
    if existing:
        ...
        if existing_name and existing_name.lower() != clean_name.lower():
            return {
                ...
                "normalized": normalized,
                "detail": {"existing_employee_key": existing.get("employee_key")},
            }
        return {
            "status": "skipped",
    ...
    *,
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if existing:
                batch = dict(existing)
                # Deterministic replay: return prior preview/commit without rewriting.
                if batch.get("status") in {"previewed", "committed", "partial", "rolled_back", "committing"}:
                    cur.execute(
            ...
            with legacy.db_connect() as conn:
                with conn.cursor() as cur:
                    if status == "created":
                        ...
                        # Stamp hub provenance for rollback.
                        cur.execute(
                            """
                            UPDATE employees
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                SELECT row_number, status, name, phone, email, external_employee_id, payroll_id,
                       source_system, employee_key, reason
                FROM employee_import_rows
                WHERE batch_id=%s AND company_code=%s
                  AND status IN ('skipped','conflict','invalid','failed','rolled_back')
                ORDER BY row_number
                """,
                (batch_id, company),
            )
