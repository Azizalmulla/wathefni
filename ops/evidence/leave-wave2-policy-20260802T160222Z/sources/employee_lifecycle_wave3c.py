"""Employees 360 Wave 3C–3H — production-safety + jurisdiction policy packs.

Wave 3C: P0 safety. Wave 3D/3F: KW public-law closure.
Wave 3H: reusable jurisdiction policy-pack architecture (KW_PRIVATE_SECTOR only verified).
...
from employee_lifecycle_wave3 import (
    ...
    _request_hash,
    _ts_equal,
    assert_lifecycle_synthetic_target,
    ...
    is_lifecycle_synthetic_employee_row,
    lifecycle_synthetic_only_enabled,
    lifecycle_v3_enabled,
    list_pending_lifecycle_for_actor,
    load_lifecycle_synthetic_markers,
    preview_downstream_impact,
    rollback_lifecycle_wave3,
    stamp_lifecycle_synthetic_markers,
)
import employee_policy_packs_wave3h as policy_packs
...
# Vocabularies sourced from the verified KW pack (Wave 3F-compatible). Soft defaults for imports/tests.
_KW_PACK = policy_packs.get_pack("KW_PRIVATE_SECTOR") or {}
CONTRACT_TYPES = frozenset(_KW_PACK.get("contract_types") or {"unlimited", "fixed_term"})
PAY_FREQUENCIES = frozenset(_KW_PACK.get("pay_frequencies") or {"monthly", "other"})
PROBATION_STATUSES = frozenset(_KW_PACK.get("probation_statuses") or {"none", "active", "completed"})
TERMINATION_CASE_CLASSES = frozenset(
    _KW_PACK.get("termination_case_classes")
    or {
        "resignation",
        "dismissal_ordinary",
...
EXCEPTIONAL_CASE_CLASSES = frozenset(
    _KW_PACK.get("exceptional_case_classes")
    or {
        "summary_dismissal_41a",
        "summary_dismissal_41b",
        "abandonment_42",
...
# Conservative Wave 3F defaults. Public-law closure: normal ops do not require private counsel gate.
# Exceptional/high-risk cases escalate to manual legal judgment outside software decision authority.
DEFAULT_POLICY = {
    "tier": "small",
    ...
    "document_retention_floor_days": 365,  # Art. 144 one-year lawsuit horizon — retain at least; no auto-purge
    ...
    # Public-law closure: normal Kuwait private-sector ops use archived public sources.
    # Private counsel is not a product gate for normal cases. Exceptional cases escalate manually.
...
WAVE3D_POLICY_JSON_KEYS = (
    ...
    "require_counsel_gate",
    "disclaimer",
    "ui_copy",
    "wave",
)

# Public-law closure checklist (replaces private-counsel dependency for normal ops).
COUNSEL_QUESTIONS = [
    {"id": "PL1", "text": "Confirm employment resolves to a verified policy pack (KW_PRIVATE_SECTOR today) and unsupported packs fail closed."},
    {"id": "PL2", "text": "Confirm notice guidance stays hidden unless unlimited + non-probation + pay frequency are complete (Art. 44)."},
    {"id": "PL3", "text": "Confirm Employees 360 never computes pay-in-lieu, garden-leave pay, damages, EOSB or leave encashment."},
    {"id": "PL4", "text": "Confirm EOSB / leave-encashment formulas live only in Payroll (Arts. 51–53, 73; amendments handled by Payroll)."},
    {"id": "PL5", "text": "Confirm summary-dismissal / Art. 41–42–48 paths are exceptional manual judgment — software does not decide lawfulness."},
    {"id": "PL6", "text": "Confirm post-effective reinstate remains disabled; true rehire is the default path."},
    {"id": "PL7", "text": "Confirm service-certificate (Art. 54) and personnel-file retain (Art. 80) workflows are enabled."},
    {"id": "PL8", "text": "Confirm document retention has no auto-purge and respects at least the Art. 144 one-year horizon."},
    {"id": "PL9", "text": "Confirm dual approval / no self-approval remains mandatory for irreversible lifecycle actions."},
    {"id": "PL10", "text": "Confirm missing classification fields fail closed into manual review — never inferred."},
]

APPROVED_DISCLAIMERS = {
    "general": DEFAULT_POLICY["disclaimer"],
    "notice": DEFAULT_POLICY["notice_hint_label"],
    **DEFAULT_POLICY["ui_copy"],
}
...
-- Wave 3F employment classification (fail-closed; never inferred)
ALTER TABLE IF EXISTS employee_employments
...
CREATE TABLE IF NOT EXISTS employee_lifecycle_service_certificates (
...
def ensure_wave3c_schema(cur) -> None:
    ...
    policy_packs.ensure_wave3h_schema(cur)
...
def _default_policy_json() -> dict[str, Any]:
    return {k: DEFAULT_POLICY[k] for k in WAVE3D_POLICY_JSON_KEYS if k in DEFAULT_POLICY} | {"wave": "wave3h"}
...
def notice_guidance_eligibility(
    *,
    policy: dict[str, Any],
    ...
    """Fail closed: hints appear only when authoritative fields fully qualify Art. 44 unlimited notice."""
    reasons: list[str] = []
    if not policy.get("show_notice_hints"):
        reasons.append("policy_show_notice_hints_false")
    if str(contract_type or "") != "unlimited":
        reasons.append("contract_not_unlimited")
    if str(probation_status or "") == "active":
        reasons.append("probation_active")
    if str(pay_frequency or "") not in PAY_FREQUENCIES:
        reasons.append("pay_frequency_unknown")
    if str(termination_case_class or "") in EXCEPTIONAL_CASE_CLASSES or exceptional_case:
        reasons.append("exceptional_or_summary_case")
    eligible = len(reasons) == 0
    hint_days = None
    if eligible:
        hint_days = (
            int(policy.get("notice_hint_monthly_days") or 90)
            if str(pay_frequency) == "monthly"
            else int(policy.get("notice_hint_other_days") or 30)
        )
    return {
        "eligible": eligible,
        "show": eligible,
        ...
        "ui_copy": (policy.get("ui_copy") or DEFAULT_POLICY["ui_copy"]).get("notice_hint_hidden"),
    }


def _assert_termination_classification(legacy: Any, *, policy: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    """Require complete authoritative fields; never infer. Missing → manual_review fail-closed."""
    if not policy.get("require_employment_classification", True):
        return {"ok": True, "skipped": True}
    missing = []
    contract_type = str(payload.get("contract_type") or "").strip().lower() or None
    pay_frequency = str(payload.get("pay_frequency") or "").strip().lower() or None
    probation_status = str(payload.get("probation_status") or "").strip().lower() or None
    case_class = str(payload.get("termination_case_class") or payload.get("case_class") or "").strip().lower() or None

    if policy.get("require_employment_classification", True):
        if contract_type not in CONTRACT_TYPES:
            missing.append("contract_type")
        if pay_frequency not in PAY_FREQUENCIES:
            missing.append("pay_frequency")
        if probation_status not in PROBATION_STATUSES:
            missing.append("probation_status")
        if probation_status == "active":
            if not payload.get("probation_start_date") or not payload.get("probation_end_date"):
                missing.append("probation_dates")
    if policy.get("require_termination_case_class", True):
        if case_class not in TERMINATION_CASE_CLASSES:
            missing.append("termination_case_class")
    ...
    exceptional = bool(payload.get("exceptional_case")) or (case_class in EXCEPTIONAL_CASE_CLASSES)
    if exceptional and policy.get("exceptional_cases_manual_only", True):
        if not str(payload.get("exceptional_escalation_note") or "").strip():
            raise legacy.HTTPException(
                status_code=422,
                detail={
    ...
    return {
        ...
        "termination_case_class": case_class,
        "exceptional_case": exceptional,
        "notice": notice_guidance_eligibility(
            policy=policy,
            contract_type=contract_type,
            pay_frequency=pay_frequency,
            probation_status=probation_status,
...
def _normalize_policy(row: dict[str, Any]) -> dict[str, Any]:
    """Merge Wave 3F conservative defaults; hide numeric notice hints unless explicitly enabled."""
    ...
    # Migrate prior wave3d rows to wave3f public-law closure defaults when unset in stored JSON
    if str(pj.get("wave") or "") in {"", "wave3d", "wave3c", "wave3f"}:
        merged_json.setdefault("require_counsel_gate", False)
        merged_json.setdefault("require_employment_classification", True)
        merged_json.setdefault("require_termination_case_class", True)
        merged_json.setdefault("jurisdiction_mode", "policy_pack")
        merged_json.setdefault("default_policy_pack", "KW_PRIVATE_SECTOR")
        merged_json["wave"] = "wave3h"
    ...
    policy["ui_copy"] = merged_json.get("ui_copy") if isinstance(merged_json.get("ui_copy"), dict) else DEFAULT_POLICY["ui_copy"]
    ...
    if show:
        policy["notice_hints_ui"] = {
            ...
            "eligibility_note": "Hints still require complete classification fields at request time.",
        }
    else:
    ...
    policy["require_employment_classification"] = bool(policy.get("require_employment_classification", True))
    policy["require_termination_case_class"] = bool(policy.get("require_termination_case_class", True))
    policy["auto_cancel_shifts"] = bool(policy.get("auto_cancel_shifts"))
    policy["auto_decline_leave"] = bool(policy.get("auto_decline_leave"))
    # Wave 3F: prefer policy_json counsel gate (default off for normal public-law ops)
    if "require_counsel_gate" in merged_json:
        policy["require_counsel_gate"] = bool(merged_json.get("require_counsel_gate"))
    elif str(merged_json.get("wave")) == "wave3h":
        policy["require_counsel_gate"] = False
    return policy


def _assert_kuwait_jurisdiction(legacy: Any, *, policy: dict[str, Any], payload: dict[str, Any]) -> None:
    """Deprecated Wave 3D/3F helper — jurisdiction is now resolved via policy packs (Wave 3H)."""
    _ = policy
    # Keep callable for older tests; no longer Kuwait-only hard fail when pack framework is active.
    if not policy_packs.wave3h_enabled():
        raw = (
        ...
        raise legacy.HTTPException(
            ...
            detail={
                ...
                "message": "Legacy Kuwait-only mode: non-Kuwait cases are excluded.",
                "work_country": code,
            },
        )


def _resolve_request_policy_pack(
    legacy: Any,
    *,
    company: str,
    employee_key: str,
    ...
    """Resolve exactly one pack for the employment; fail closed if missing/unsupported."""
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            try:
                ...
                employment = {}
        conn.commit()
    resolution = policy_packs.resolve_employment_pack(
        legacy,
    ...
    return policy_packs.assert_pack_resolved(legacy, resolution)
...
def upsert_company_policy(legacy: Any, context: dict[str, Any], *, patch: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for key in WAVE3D_POLICY_JSON_KEYS:
        if key in patch and patch[key] is not None:
            ...
            pj[key] = merged[key]
    pj["wave"] = "wave3h"
    # Prefer Wave 3F approved disclaimer/copy when upgrading from prior waves
    old_wave = str((current.get("policy_json") or {}).get("wave") if isinstance(current.get("policy_json"), dict) else "") or str(
        (merged.get("policy_json") or {}).get("wave") if isinstance(merged.get("policy_json"), dict) else ""
    )
    ...
    if not isinstance(pj.get("ui_copy"), dict) or old_wave in {"", "wave3c", "wave3d"}:
        pj["ui_copy"] = DEFAULT_POLICY["ui_copy"]
    # Persist counsel-gate preference into JSON for Wave 3F
    pj["require_counsel_gate"] = bool(merged.get("require_counsel_gate", DEFAULT_POLICY["require_counsel_gate"]))
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ensure_wave3c_schema(cur)
            cur.execute(
                ...
                (
                    ...
                    bool(merged.get("require_counsel_gate", DEFAULT_POLICY["require_counsel_gate"])),
...
def create_lifecycle_request(
    ...
    *,
    ...
    pack_resolution = _resolve_request_policy_pack(
        legacy, company=company, employee_key=employee_key, payload=body
    )
    pack = pack_resolution["pack"]
    pack_overlay = policy_packs.pack_as_company_policy_overlay(pack)
    # Overlay pack mandatory floors onto company policy for this request (tenant may only have tightened).
    for k, v in pack_overlay.items():
        if k in {
            "require_last_working_day",
            "require_employment_classification",
            "require_termination_case_class",
            ...
            "ui_copy",
            "notice_hint_label",
            "notice_hint_monthly_days",
            "notice_hint_other_days",
        }:
            # Prefer pack mandatory / overlay; company show_notice_hints remains company-controlled.
            if k.startswith("require_") or k in {
                "monetary_calculations_owner",
                "settlement_packet_mode",
                "exceptional_cases_manual_only",
                "document_retention_mode",
                ...
                policy[k] = v
            elif k == "allow_reinstate_after_effective":
                # Pack default is false; company may explicitly opt in (Wave 3F-compatible).
                # Do not overwrite an explicit company True with pack False.
                if not policy.get("allow_reinstate_after_effective"):
                    policy[k] = v
            elif k in {"disclaimer", "ui_copy", "notice_hint_label"} and not policy.get(k):
                policy[k] = v
            elif k in {"notice_hint_monthly_days", "notice_hint_other_days"}:
                policy[k] = v
    ...
    if case_type == "termination":
        ...
        # Fail closed: never infer contract/pay/probation/case class.
        classification = _assert_termination_classification(legacy, policy=policy, payload=body)
        # Prefer pack-aware notice eligibility (Wave 3F-compatible for KW_PRIVATE_SECTOR).
        classification["notice"] = policy_packs.notice_guidance_from_pack(
            pack=pack,
            company_show_notice_hints=bool(policy.get("show_notice_hints")),
            contract_type=classification.get("contract_type"),
            pay_frequency=classification.get("pay_frequency"),
            probation_status=classification.get("probation_status"),
            termination_case_class=classification.get("termination_case_class"),
            exceptional_case=bool(classification.get("exceptional_case")),
        )
        body["classification"] = {
            "contract_type": classification["contract_type"],
            "pay_frequency": classification["pay_frequency"],
            "probation_status": classification["probation_status"],
            "termination_case_class": classification["termination_case_class"],
            "exceptional_case": classification["exceptional_case"],
        }
        body["notice_guidance"] = classification["notice"]
        body["employment_classification_write"] = True
        if classification["exceptional_case"]:
            body["exceptional_escalation_state"] = "manual_legal_judgment_required"
            body["ui_warning"] = (policy.get("ui_copy") or DEFAULT_POLICY["ui_copy"]).get("exceptional_case_banner")
    body["policy_pack"] = {
        "pack_code": pack_resolution.get("pack_code") or pack.get("pack_code"),
        "policy_version": pack_resolution.get("policy_version") or pack.get("policy_version"),
        "content_hash": pack_resolution.get("content_hash") or pack.get("content_hash"),
        "jurisdiction_code": pack_resolution.get("jurisdiction_code") or pack.get("jurisdiction_code"),
        "worker_category": pack_resolution.get("worker_category") or pack.get("worker_category"),
    }
    body["policy_snapshot"] = {
        ...
        "require_employment_classification": bool(policy.get("require_employment_classification", True)),
        "require_termination_case_class": bool(policy.get("require_termination_case_class", True)),
        "jurisdiction_mode": "policy_pack",
        "document_retention_mode": policy.get("document_retention_mode"),
        "document_retention_floor_days": policy.get("document_retention_floor_days") or 365,
        "monetary_calculations_owner": policy.get("monetary_calculations_owner") or "payroll",
        "settlement_packet_mode": policy.get("settlement_packet_mode") or "inputs_only",
        "service_certificate_default": bool(policy.get("service_certificate_default", True)),
        "disclaimer": policy.get("disclaimer") or DEFAULT_POLICY["disclaimer"],
        "ui_copy": policy.get("ui_copy") or DEFAULT_POLICY["ui_copy"],
        "policy_pack_code": body["policy_pack"]["pack_code"],
        "policy_pack_version": body["policy_pack"]["policy_version"],
        "policy_pack_hash": body["policy_pack"]["content_hash"],
        "wave": "wave3h",
    }
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                UPDATE employee_lifecycle_requests SET
                  ...
                  impact_ack_by_user_id=%s, impact_ack_text=%s,
                  policy_pack_code=%s, policy_pack_version=%s, policy_pack_hash=%s,
                  updated_at=now()
                ...
                (
                    ...
                    snap_hash,
                    actor,
                    str(impact_ack_text or "").strip()[:500],
                    body["policy_pack"]["pack_code"],
                    body["policy_pack"]["policy_version"],
                    body["policy_pack"]["content_hash"],
                    request_id,
                ),
            )
            updated = dict(cur.fetchone())
            # Bind employment pack when resolved from payload (not silent — fields were explicit or stored).
            if req.get("employment_id") and pack_resolution.get("ok"):
                policy_packs.bind_employment_pack(
                    cur,
                    company_code=company,
    ...
    result["policy_pack"] = body["policy_pack"]
    result["wave3c"] = True
    result["wave3h"] = True
...
def decide_lifecycle_request(
    ...
    *,
    ...
    def _freeze_if_approved(out: dict[str, Any]) -> dict[str, Any]:
        if action != "approve" or not (out.get("committed") or out.get("ok")):
            return {**out, "wave3h": True}
        pack_meta = (payload.get("policy_pack") if isinstance(payload, dict) else None) or {}
        pack_code = pack_meta.get("pack_code") or req.get("policy_pack_code")
        pack_ver = pack_meta.get("policy_version") or req.get("policy_pack_version")
        pack = policy_packs.get_pack(str(pack_code or ""), version=str(pack_ver) if pack_ver else None)
        if not pack:
            # Fall back to KW overlay from snapshot for historical Wave 3F requests being approved.
            pack = policy_packs.get_pack("KW_PRIVATE_SECTOR")
        if pack:
            with legacy.db_connect() as conn:
                with conn.cursor() as cur:
                    ensure_wave3c_schema(cur)
                    freeze = policy_packs.freeze_pack_on_request(
                        cur,
                        company_code=company,
                        request_id=str(req["request_id"]),
                        case_id=str(req["case_id"]) if req.get("case_id") else None,
                        employee_key=str(req["employee_key"]),
                        employment_id=str(req["employment_id"]) if req.get("employment_id") else None,
                        pack=pack,
                        frozen_by_user_id=str(context.get("actor_user_id") or "") or None,
                    )
                    conn.commit()
            out = {**out, "policy_pack_freeze": _jsonable(freeze)}
        return {**out, "wave3h": True}

    if action == "approve" and case_type in {"cancel_scheduled", "reinstate"}:
        return _freeze_if_approved(
            _decide_cancel_or_reinstate(
    ...
    if action == "approve" and case_type == "rehire":
        return _freeze_if_approved(
            _decide_rehire_same_key(legacy, context, req=req, decision_reason=decision_reason, payload=payload)
        )
    if action == "approve" and case_type == "termination":
        ...
        return _freeze_if_approved({**out, "wave3c": True})
    out = wave3_decide_lifecycle_request(
        legacy, context, request_id=request_id, action=action, decision_reason=decision_reason
    )
    return _freeze_if_approved({**out, "wave3c": True})
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            packet = {
                ...
                "termination_case_class": classification.get("termination_case_class")
                or payload.get("termination_case_class"),
                "contract_type": classification.get("contract_type") or payload.get("contract_type"),
                "pay_frequency": classification.get("pay_frequency") or payload.get("pay_frequency"),
                "probation_status": classification.get("probation_status") or payload.get("probation_status"),
                "exceptional_case": bool(classification.get("exceptional_case")),
                "exceptional_escalation_state": payload.get("exceptional_escalation_state"),
                ...
                "inputs": {
                    ...
                    "probation_start_date": payload.get("probation_start_date"),
                    "probation_end_date": payload.get("probation_end_date"),
                    "service_certificate_status": "pending"
                    if policy.get("service_certificate_default", True)
                    else "not_requested",
                },
                "document_retention_mode": policy.get("document_retention_mode") or "retain",
                "document_retention_floor_days": int(policy.get("document_retention_floor_days") or 365),
                "monetary_calculations_owner": policy.get("monetary_calculations_owner") or "payroll",
                "settlement_packet_mode": policy.get("settlement_packet_mode") or "inputs_only",
                "ui_copy": (policy.get("ui_copy") or DEFAULT_POLICY["ui_copy"]).get("settlement_handoff"),
                "disclaimer": (
            ...
            if "amounts" in packet or any(
                k in (packet.get("inputs") or {}) for k in ("eosb", "notice_pay", "garden_leave_pay", "leave_encashment")
            ):
            ...
            # Persist employment classification + optional Art. 54 service-certificate workflow.
            cur.execute(
                """
                UPDATE employee_employments SET
                  contract_type=COALESCE(%s, contract_type),
                  pay_frequency=COALESCE(%s, pay_frequency),
                  probation_status=COALESCE(%s, probation_status),
                  probation_start_date=COALESCE(%s::date, probation_start_date),
                  probation_end_date=COALESCE(%s::date, probation_end_date),
                  updated_at=now()
                WHERE company_code=%s AND employment_id=%s
                """,
                (
                    classification.get("contract_type") or payload.get("contract_type"),
                    classification.get("pay_frequency") or payload.get("pay_frequency"),
                    classification.get("probation_status") or payload.get("probation_status"),
                    payload.get("probation_start_date"),
                    payload.get("probation_end_date"),
                    company,
                    req["employment_id"],
                ),
            )
            if policy.get("service_certificate_default", True):
                cur.execute(
                    ...
                    INSERT INTO employee_lifecycle_service_certificates (
                      company_code, employee_key, employment_id, person_id, status,
                            ...
                            _jsonable(
                                {
                                    ...
                                    "article": "54",
                                    "ui_copy": (policy.get("ui_copy") or DEFAULT_POLICY["ui_copy"]).get(
                                        "service_certificate"
                                    ),
                                    "wave": "wave3f",
                                }
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            for row in rows:
                ...
                if lifecycle_synthetic_only_enabled():
                    markers = load_lifecycle_synthetic_markers(cur, company=company, employee_key=key)
                    if not is_lifecycle_synthetic_employee_row(
                        phone=markers.get("phone"),
                        name=markers.get("name"),
                        provenance=markers.get("employment_provenance"),
                        raw_json=markers.get("raw_json"),
                    ):
                        skipped.append({"employee_key": key, "reason": "synthetic_only_gate"})
                        continue
                cur.execute(
                    """
            ...
            conn.commit()
    return {"ok": True, "revoked": revoked, "count": len(revoked), "skipped": skipped, "as_of": now_local.isoformat()}
...
def create_downstream_action_request(
    ...
    *,
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ensure_wave3c_schema(cur)
            assert_lifecycle_synthetic_target(legacy, cur, company=company, employee_key=employee_key)
            conn.commit()
    ...
    if action_type == "decline_open_leave":
        cur.execute(
            ...
            SET metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
                  'lifecycle_prev_status', lower(coalesce(status, 'requested'))
                ),
                status='declined_lifecycle',
                decision_note='lifecycle downstream action',
                decided_at=now(),
                updated_at=now()
            WHERE company_code=%s AND employee_key=%s
              AND lower(coalesce(status,'')) IN ('pending','requested','open','approved','needs_review')
              AND coalesce(end_date, start_date) >= %s
            RETURNING *
            """,
            (company, key, today),
        )
        declined_rows = [dict(r) for r in (cur.fetchall() or [])]
        try:
            import leave_policy_wave2 as leave_w2
            import leave_authority_wave1 as leave_a

            if leave_w2.leave_policy_wave2_enabled():
                for row in declined_rows:
                    canon = leave_a.canonicalize_leave_type(row.get("leave_type"))
                    cur.execute(
                    ...
                    leave_w2.release_leave_reservation(
                        cur,
                        company_code=company,
                        leave={**row, "leave_type": canon},
                        policy=policy,
                        actor_phone=None,
                        reason="declined_lifecycle",
                    )
        except Exception:
            pass
        return {"declined_leave_ids": [str(r["leave_id"]) for r in declined_rows], "reversible": True}
    if action_type == "reverse_decline_open_leave":
        # Restore into the primary leave state machine — never 'pending'.
        # Prefer stored lifecycle_prev_status; map legacy pending → requested.
        cur.execute(
            """
            UPDATE leave_requests
            SET status = CASE
                  WHEN lower(coalesce(metadata->>'lifecycle_prev_status','')) IN ('approved') THEN 'approved'
                  WHEN lower(coalesce(metadata->>'lifecycle_prev_status','')) IN ('needs_review') THEN 'needs_review'
                  ELSE 'requested'
                END,
                decision_note='lifecycle reverse',
                updated_at=now()
            WHERE company_code=%s AND employee_key=%s AND status='declined_lifecycle'
            RETURNING leave_id, status
            """,
            (company, key),
        )
        restored = [dict(r) for r in (cur.fetchall() or [])]
        return {
            "restored_leave_ids": [str(r["leave_id"]) for r in restored],
            "restored_statuses": [str(r.get("status")) for r in restored],
        }
