...
def actor_is_mss_c3_manager(actor_phone: str | None, company_code: str | None = None) -> bool:
    """Wave 2 C3 company-scoped MSS manager (not the global MANAGER_ALLOWLIST)."""
    try:
        import shifts_mss_c3 as _c3

        return _c3.mss_mutation_allowed_via_c3(actor_phone=actor_phone, company_code=company_code)
    except Exception:
        return False
...
def real_mutation_denied(
    *,
    ...
    if actor_allowlisted_for_real_mutation(actor_phone):
        return None
    # Wave 2 C3: company-scoped MSS unlock (does not fill global MANAGER_ALLOWLIST).
    if actor_is_mss_c3_manager(actor_phone, company_code):
        return None
    return {
        "ok": False,
...
def enrich_shift_row_for_ui(row: dict[str, Any], *, open_flags: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    # Governed category only — never inferred from free-text role. Missing/legacy → general.
    raw_type = str(out.get("assignment_type") or "").strip().lower()
    out["assignment_type"] = (
        raw_type
        if raw_type in {"guest", "operations", "night", "event", "general"}
        else "general"
    )
    out["display_window"] = {
