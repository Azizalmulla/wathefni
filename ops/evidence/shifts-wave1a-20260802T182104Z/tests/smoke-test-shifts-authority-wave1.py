...
def main() -> int:
    ...
    check(
        "future_start blocks before start date",
    ...
    check(
        "future_start allows on start date",
        sw1.evaluate_shift_lifecycle(facts=facts_future, shift_start=on_start[0], shift_end=on_start[1], settings=base_settings) is None,
    )
    ...
    scoped = app.approve_shift_swap(
        {"swap_id": swap_id, "company_code": company, "viewer_phone": actor},
        company_code=company,
        created_by_phone=actor,
    )
    check("swap scoped decision approved", scoped.get("ok") is True, scoped.get("error"))

    replay = app.approve_shift_swap(
        {"swap_id": swap_id, "company_code": company},
        company_code=company,
        created_by_phone=actor,
    )
    ...
    first = app.reject_shift_swap(
        {"swap_id": swap2_id, "company_code": company},
        company_code=company,
        created_by_phone=actor,
    )
    second = app.approve_shift_swap(
        {"swap_id": swap2_id, "company_code": company},
        company_code=company,
        created_by_phone=actor,
    )
