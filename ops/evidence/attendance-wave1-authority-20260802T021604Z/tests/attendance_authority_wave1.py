...
PUNCH_SOURCES = frozenset({"whatsapp", "import", "hr", "leave", "absence_scan", "correction", "system"})
...
class AttendanceAuthorityService:
    ...
    def __init__(self, store: InMemoryAuthorityStore | None = None) -> None:
    ...
    def mark_absent(
        ...
        *,
        ...
        # Idempotent absence marker via synthetic system punch-less projection + event id on metadata.
        # Use a punch ledger no-op marker in metadata only; force absent status.
        # Still record a system punch? Prefer no fake punch — force status projection with source_event idempotency via correction-like key in metadata.
        # Encode idempotency by attempting a reserved punch direction? Better: store absence as projection only,
        # but guard duplicates via existing current absent + same source_event_id in metadata.
    ...
    def approve_day(
        ...
        *,
        ...
        # New version marked approved + payroll snapshot.
        # Rebuild from punches to keep calc, but stamp approval.
        ...
        # Enforce payroll_eligible true only when approved + clean.
...
def get_authority_service(company_code: str | None = None) -> AttendanceAuthorityService:
    """Process-local services keyed by company (in-memory). Postgres adapter is future Wave."""
