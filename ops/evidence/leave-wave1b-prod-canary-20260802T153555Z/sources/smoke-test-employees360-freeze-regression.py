...
def main() -> int:
    ...
    # Sibling Onboarding freeze (controlled WATHEFNI) — must remain after E360 closure
    onb_freeze = ops / "ONBOARDING_CONTROLLED_ROLLOUT_COMPLETION_AND_FREEZE.md"
    check("onboarding freeze intact", onb_freeze.is_file())
    if onb_freeze.is_file():
        onb_txt = onb_freeze.read_text(encoding="utf-8")
        check("onboarding freeze mentions SEED off", "ONBOARDING_SEED" in onb_txt and "disabled" in onb_txt)
        check("onboarding freeze mentions default_kuwait@2.0.0", "default_kuwait@2.0.0" in onb_txt)
