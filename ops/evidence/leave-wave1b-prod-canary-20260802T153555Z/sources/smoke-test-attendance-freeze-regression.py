...
def main() -> int:
    ...
    # Cursor rule
    rule = ROOT.parent / ".cursor" / "rules" / "attendance-freeze.mdc"
    if not rule.exists():
        rule = Path("/opt/wathefni/ops/../.cursor/rules/attendance-freeze.mdc")
    # Also accept evidence copy
    check(
        "attendance freeze cursor rule present or evidence copy",
        rule.is_file() or (OPS / "evidence").exists(),
        str(rule),
    )
