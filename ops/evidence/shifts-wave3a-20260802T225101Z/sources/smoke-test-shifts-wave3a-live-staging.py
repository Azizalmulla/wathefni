...
def main() -> int:
    ...
    overnight_day = (date.today() + timedelta(days=4)).isoformat()
    ...
    # Dashboard GET enrich
    ctx = mint_owner_context()
    today = date.today()
    week = ((date.fromisoformat(day) - today).days) // 7
    ...
    check("list_contains_created", bool(found), {"count": len(listing.get("shifts") or [])})
    if found:
        check("ui_state_present", bool(found[0].get("ui_state")), found[0])
