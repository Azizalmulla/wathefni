...
_ON = {"1", "true", "yes", "on"}
DEFAULT_W2_SYNTHETIC_KEY_MARKERS = ("SHW2B", "SHW2B-SYNTH|")
DEFAULT_W2_SYNTHETIC_PHONE_PREFIXES = ("965530",)
# Advisory lock keys for operator jobs (pg_try_advisory_lock bigint)
JOB_LOCK_REMINDER_DRAIN = 820260201
JOB_LOCK_LIFECYCLE_RECON = 820260202
JOB_LOCK_LEAVE_RECON = 820260203
...
def shifts_wave2_companies() -> set[str]:
    raw = str(os.environ.get("WATHEFNI_SHIFTS_INTEGRITY_COMPANIES") or "").strip()
    if not raw:
        return set()
    return {p.strip().upper() for p in raw.split(",") if p.strip()}


def shifts_wave2_enabled_for_company(company_code: str | None) -> bool:
    if not shifts_wave2_enabled():
        return False
    allowed = shifts_wave2_companies()
    if not allowed:
        # Empty allowlist: non-production defaults open; production requires explicit companies.
        env = (os.environ.get("WATHEFNI_ENV") or "").strip().lower()
        return env != "production"
    return str(company_code or "").strip().upper() in allowed


def shifts_wave2_synthetic_only() -> bool:
    raw = os.environ.get("WATHEFNI_SHIFTS_INTEGRITY_SYNTHETIC_ONLY")
    env = (os.environ.get("WATHEFNI_ENV") or "").strip().lower()
...
def wave2_synthetic_key_markers() -> tuple[str, ...]:
    raw = str(os.environ.get("WATHEFNI_SHIFTS_INTEGRITY_SYNTHETIC_KEY_MARKERS") or "").strip()
    if not raw:
        return DEFAULT_W2_SYNTHETIC_KEY_MARKERS
    return tuple(p.strip() for p in raw.split(",") if p.strip()) or DEFAULT_W2_SYNTHETIC_KEY_MARKERS


def wave2_synthetic_phone_prefixes() -> tuple[str, ...]:
    raw = str(os.environ.get("WATHEFNI_SHIFTS_INTEGRITY_SYNTHETIC_PHONE_PREFIXES") or "").strip()
    if not raw:
        return DEFAULT_W2_SYNTHETIC_PHONE_PREFIXES
    return tuple(p.strip() for p in raw.split(",") if p.strip()) or DEFAULT_W2_SYNTHETIC_PHONE_PREFIXES
...
def is_wave2_synthetic_employee(
    ...
    *,
    ...
    phone_d = _digits((employee or {}).get("phone") or (employee or {}).get("employee_phone") or phone)
    nm = str((employee or {}).get("name") or name or "")
    for marker in wave2_synthetic_key_markers():
        if marker and (marker in key or marker in nm):
            return True
    for prefix in wave2_synthetic_phone_prefixes():
        if prefix and phone_d.startswith(prefix):
            return True
    return False


def shifts_wave2_applies_to(
    ...
    *,
    ...
    """True when Wave 2 integrity mutations/gates apply to this subject."""
    if not shifts_wave2_enabled_for_company(company_code):
        return False
    if not shifts_wave2_synthetic_only():
        return True
    return is_wave2_synthetic_employee(
        employee, employee_key=employee_key, phone=phone, name=name
    )


def wave2_synthetic_gate_denied(
    *,
    company_code: str | None,
    employee: dict[str, Any] | None,
) -> dict[str, Any] | None:
    """Hard deny when Wave 2 is synthetic-only and subject is not synthetic."""
    if not shifts_wave2_enabled_for_company(company_code):
        return None
    if not shifts_wave2_synthetic_only():
        return None
    if is_wave2_synthetic_employee(employee):
        return None
    return {
        "ok": False,
        "error": "shifts_wave2_synthetic_only_gate",
        "message": "Shifts schedule integrity Wave 2 is synthetic-only for this environment.",
...
def honesty_payload() -> dict[str, Any]:
    return {
        ...
        "synthetic_only": shifts_wave2_synthetic_only(),
        "companies": sorted(shifts_wave2_companies()) or ["*"],
    }
...
def upsert_seasonal_policy(cur: Any, *, company_code: str, policy: dict[str, Any]) -> dict[str, Any]:
    ...
    cur.execute(
        ...
        (
            ...
            _json_dumps(policy.get("metadata") or {}),
        ),
    )
...
def reconcile_lifecycle_future_shifts(
    ...
    *,
    ...
    for shift in rows:
        ...
        if not denied:
            # Also catch terminated/left hub rows even when beyond-end interval
            # evaluation is soft (e.g. require_ack already cleared elsewhere).
            facts = sw1.load_employment_lifecycle_facts(cur, company_code=company, employee=emp)
            hub = str(facts.get("hub_status") or facts.get("lifecycle_state") or "").lower()
            end_d = sw1.effective_employment_end(facts)
            sd = _as_date(shift.get("shift_date"))
            if hub in {"terminated", "left"} and end_d and sd and sd > end_d:
                denied = {
                    "ok": False,
            ...
            elif hub in {"terminated", "left"} and not end_d:
                denied = {
                    "ok": False,
...
# --- Operator jobs (synthetic-scoped, locked, kill-switchable) -----------------

def jobs_enabled() -> bool:
    """Kill switch: WATHEFNI_SHIFTS_INTEGRITY_JOBS=0 disables all Wave 2 operator jobs."""
    raw = os.environ.get("WATHEFNI_SHIFTS_INTEGRITY_JOBS")
    if raw is None or str(raw).strip() == "":
        return True
    return str(raw).strip().lower() in _ON


def try_job_lock(cur: Any, lock_key: int) -> bool:
    cur.execute("SELECT pg_try_advisory_lock(%s) AS ok", (int(lock_key),))
    return bool(dict(cur.fetchone()).get("ok"))


def release_job_lock(cur: Any, lock_key: int) -> None:
    cur.execute("SELECT pg_advisory_unlock(%s)", (int(lock_key),))


def run_lifecycle_reconciliation_job(
    cur: Any,
    *,
    company_code: str = "WATHEFNI",
    limit: int = 200,
) -> dict[str, Any]:
    """Flag lifecycle conflicts for synthetic subjects only. No silent cancel."""
    if not jobs_enabled():
        return {"ok": False, "error": "shifts_wave2_jobs_disabled", "killed": True}
    if not shifts_wave2_enabled_for_company(company_code):
        return {"ok": False, "error": "shifts_wave2_disabled_for_company"}
    if not try_job_lock(cur, JOB_LOCK_LIFECYCLE_RECON):
        return {"ok": False, "error": "job_lock_held", "job": "lifecycle_reconciliation"}
    try:
        full = reconcile_lifecycle_future_shifts(cur, company_code=company_code)
        flagged = []
        for flag in full.get("flagged") or []:
            if not flag:
                continue
            if shifts_wave2_synthetic_only() and not is_wave2_synthetic_employee(
        ...
        release_job_lock(cur, JOB_LOCK_LIFECYCLE_RECON)


def run_leave_reconciliation_job(
    cur: Any,
    *,
    company_code: str = "WATHEFNI",
    limit: int = 200,
) -> dict[str, Any]:
    """Flag approved-leave conflicts for synthetic subjects only. No silent cancel."""
    if not jobs_enabled():
        return {"ok": False, "error": "shifts_wave2_jobs_disabled", "killed": True}
    if not shifts_wave2_enabled_for_company(company_code):
        return {"ok": False, "error": "shifts_wave2_disabled_for_company"}
    if not try_job_lock(cur, JOB_LOCK_LEAVE_RECON):
        return {"ok": False, "error": "job_lock_held", "job": "leave_reconciliation"}
    try:
        full = reconcile_approved_leave_conflicts(cur, company_code=company_code)
        flagged = []
        for flag in full.get("flagged") or []:
            if not flag:
                continue
        ...
        release_job_lock(cur, JOB_LOCK_LEAVE_RECON)


def filter_reminders_synthetic_only(reminders: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not shifts_wave2_synthetic_only():
        return reminders
    return [
        rem
        for rem in reminders
        if is_wave2_synthetic_employee(employee_key=rem.get("employee_key"))
    ]
