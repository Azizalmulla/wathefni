...
def _hr_outcome(status: str, detail: dict[str, Any] | None = None) -> str:
    if status == "will_update" and isinstance(detail, dict) and detail.get("name_change_approved"):
        return "Approved"
    return {
...
def _row_entry(row: dict[str, Any]) -> dict[str, Any]:
    ...
    name_approved = bool(detail.get("name_change_approved")) if isinstance(detail, dict) else False
    applyable = status == "will_update" and name_approved
    return {
        ...
        "changes": changes or {},
        "name_identity_review": bool(detail.get("name_identity_review")) if isinstance(detail, dict) else False,
        "approvable": bool(detail.get("approvable")) if isinstance(detail, dict) else False,
        ...
        "name_change_applied": bool(detail.get("name_change_applied")) if isinstance(detail, dict) else False,
...
def _is_resolved_identity_row(row: dict[str, Any], detail: dict[str, Any] | None = None) -> bool:
    ...
    if status == "updated" and (
        detail.get("name_change_applied")
        or detail.get("name_change_approved")
        or detail.get("name_identity_review")
    ):
        return True
    return False
...
def _is_approved_pending_apply(row: dict[str, Any], detail: dict[str, Any] | None = None) -> bool:
    ...
    if str(row.get("status") or "") != "will_update":
        return False
    detail = detail if detail is not None else _parse_row_detail(row)
    return bool(detail.get("name_change_approved")) and not bool(detail.get("name_change_applied"))
...
def list_review_items(
    ...
    *,
    ...
    for _key, cands in by_identity.items():
        approved = [r for r in cands if _is_approved_pending_apply(r, _parse_row_detail(r))]
        conflicts = [
        ...
        if approved:
            selected.append(max(approved, key=_row_recency_tuple))
            continue
        if conflicts:
            newest_conflict = max(conflicts, key=_row_recency_tuple)
            if resolved:
                newest_resolved = max(resolved, key=_row_recency_tuple)
                if _row_recency_tuple(newest_resolved) >= _row_recency_tuple(newest_conflict):
                    continue
    ...
    for row in sorted(selected, key=_row_recency_tuple, reverse=True):
        ...
        entry["review_identity_key"] = _review_identity_key(row, detail)
        if _is_approved_pending_apply(row, detail):
            entry["reason"] = "Approved — apply to update this person."
            entry["applyable"] = str(row.get("batch_status") or "") in {
                "previewed",
                "partial",
                "committing",
            }
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if str(row.get("status")) != "conflict" or not detail.get("name_identity_review"):
                raise legacy.HTTPException(
                    status_code=409,
                    detail={"error": "row_not_name_identity_review"},
                )
            changes = detail.get("proposed_changes") or detail.get("changes") or {}
            if not isinstance(changes, dict) or not changes:
                raise legacy.HTTPException(status_code=409, detail={"error": "no_proposed_changes"})
...
def apply_approved_name_change(
    ...
    *,
    ...
    Does not require re-upload. Does not transfer approval to other batches.
    """
    company = legacy.require_employee_roster_admin(context)
    if not foundation_enabled(company):
        raise legacy.HTTPException(status_code=403, detail={"error": "employee_migration_foundation_disabled"})
    ...
    with legacy.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            if str(row.get("status")) == "updated" and (
                detail.get("name_change_applied") or detail.get("name_change_approved")
            ):
                # Idempotent: already applied on this batch (even if batch is now committed).
                cur.execute(
                    "SELECT * FROM employee_import_rows WHERE batch_id=%s AND company_code=%s ORDER BY row_number",
                    (batch_id, company),
                )
                ...
                out["applied_employee_key"] = str(row.get("employee_key") or "") or None
                out["message"] = "Approved change was already applied on this import."
                return out
            if batch.get("status") not in {"previewed", "partial", "committing"}:
                raise legacy.HTTPException(
                    status_code=409,
                    detail={
                        "error": "batch_not_applyable",
                        "message": "Apply approved changes on the original import batch.",
                        "status": batch.get("status"),
                    },
                )
            if not _is_approved_pending_apply(row, detail):
                raise legacy.HTTPException(
                    status_code=409,
                    detail={"error": "row_not_approved_pending_apply"},
                )
    ...
    if not employee_key or not changes:
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        raise legacy.HTTPException(status_code=409, detail={"error": "approved_row_incomplete"})
    ...
    try:
        ...
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        raise legacy.HTTPException(status_code=409, detail={"error": "apply_failed", "message": str(exc)[:240]})
    ...
    if upd_status == "conflict":
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        raise legacy.HTTPException(status_code=409, detail={"error": "apply_conflict"})
    if fields and upd_status not in {"updated", "noop"}:
        with legacy.db_connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                ...
                conn.commit()
        raise legacy.HTTPException(status_code=409, detail={"error": "apply_failed", "status": upd_status})
        ...
        try:
            ...
            with legacy.db_connect() as conn:
                with conn.cursor() as cur:
                    ...
                    conn.commit()
            raise legacy.HTTPException(status_code=409, detail={"error": "manager_apply_failed"})
