    ...
    <div className="rounded-2xl border border-line/50 bg-white/55 p-3">
      ...
      <ul className="mt-1 space-y-2 text-[12px] leading-5 text-subtle/85">
        {rows.slice(0, 12).map((r) => {
          ...
          return (
            <li key={`${title}-${r.row}-${r.employee_key || r.phone || ''}`} className="space-y-0.5">
              <p>
...
function ImportSection({
  ...
  const run = async (dryRun: boolean) => {
    ...
    if (!dryRun && !preview) {
      setError('Preview the file before confirming.')
      return
    }
    setBusy(true)
    setError(null)
    try {
      ...
      if (dryRun) {
        ...
        if (added + updated > 0) {
          onNotice(
            `Applied: ${added} added, ${updated} updated.`,
            'success',
          )
          onApplied()
        } else {
          onNotice('No employees were added or updated.', 'info')
        }
      }
    } catch (err) {
  ...
  const approveName = async (row: EmployeeImportRow) => {
    ...
    try {
      ...
      setPreview(res)
      onNotice('Name change approved — it will update when you confirm.', 'success')
    } catch (err) {
      setError(friendlyError(err, 'Could not approve name change.'))
    } finally {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-[13px] leading-6 text-subtle/90">
          Upload a file of your existing team. Preview exact changes, then confirm. We never send invitations or messages, and we never start onboarding from import.
        </p>
      </div>
      <ol className="flex flex-wrap gap-2 text-[11.5px] font-medium uppercase tracking-[0.06em] text-subtle/70">
        <li className={cn('rounded-full px-2.5 py-1', step === 'upload' ? 'bg-[#eee5d4] text-text' : 'bg-white/50')}>1 · Upload</li>
        <li className={cn('rounded-full px-2.5 py-1', step === 'preview' ? 'bg-[#eee5d4] text-text' : 'bg-white/50')}>2 · Preview</li>
      ...
      {summary ? (
        <div className="space-y-2">
          ...
          <OutcomeList title={result ? 'Updated' : 'Will be updated'} rows={updatedRows} tone="info" showChanges />
...
function ReviewSection({
  ...
  const approveName = async (item: EmployeeImportRow) => {
    ...
    try {
      await approveEmployeeImportNameChange(access, item.batch_id, item.row_id)
      onNotice?.('Name change approved — confirm the import to apply it.', 'success')
      await load()
    } catch (err) {
      setError(friendlyError(err, 'Could not approve name change.'))
    } finally {
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-[13px] leading-6 text-subtle/90">
          Ambiguous matches and material name differences stay here until you fix the file or approve the name change. We never update by name alone, and we never change the employee key.
        </p>
      </div>
      {loading ? (
      ...
      <ul className="space-y-2">
        {items.map((item) => (
          <li
            key={`${item.batch_id}-${item.row}-${item.row_id || ''}`}
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11.5px] text-subtle/70">
                  {item.filename || 'File'} · row {item.row}
                  {item.phone ? ` · ${item.phone}` : ''}
                </p>
                {item.approvable && item.name_identity_review && item.batch_status === 'previewed' ? (
                  <button
                    type="button"
...
function HistorySection({
  ...
  const undo = async (batchId: string) => {
    ...
    try {
      ...
      onNotice(
        `Undo finished: ${res.removed} added people removed, ${reverted} updates reversed (fields changed later were left alone).`,
        'success',
      )
      setExpanded(null)
      await load()
