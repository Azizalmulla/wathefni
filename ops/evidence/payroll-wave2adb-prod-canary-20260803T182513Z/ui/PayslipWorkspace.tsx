...
import { payrollPayslipCopy, payslipStatusTone } from '@/posthire/payrollPayslipUx'
...
export function PayslipWorkspace({ access, permissions, onNotice, onAccessIssue }: PayslipWorkspaceProps) {
  ...
  const openDetail = async (id: string) => {
    ...
    try {
      ...
      onNotice(err instanceof Error ? err.message : 'load_failed', 'error')
    }
  }
  ...
  if (error === 'permission') {
    return <BlockedReason title={c.permissionDenied} />
  }
  ...
  return (
    <div className="space-y-5" dir={isAr ? 'rtl' : 'ltr'} data-testid="payslip-workspace">
      <div className="space-y-2">
        <h2 className="text-[22px] font-semibold tracking-tight text-text">{c.title}</h2>
        <p className="max-w-2xl text-[14px] text-subtle">{c.subtitle}</p>
        <div className="rounded-xl border border-amber-200/80 bg-amber-50/70 px-4 py-3 text-[13px] text-amber-950">
          <p>{c.honesty}</p>
          <p className="mt-1">
      ...
      <div className="flex flex-wrap gap-1 border-b border-line/60 pb-1">
        <button
          type="button"
          className={`rounded-lg px-3 py-1.5 text-[13px] font-medium ${tab === 'list' ? 'bg-wf-ink/10 text-text' : 'text-subtle'}`}
          onClick={() => setTab('list')}
        >
          {c.tabList}
        </button>
        <button
          type="button"
          className={`rounded-lg px-3 py-1.5 text-[13px] font-medium ${tab === 'history' ? 'bg-wf-ink/10 text-text' : 'text-subtle'}`}
          onClick={() => setTab('history')}
        >
          {c.tabHistory}
        </button>
        <button type="button" className="ms-auto rounded-lg px-3 py-1.5 text-[13px] text-subtle hover:bg-black/[0.03]" onClick={() => void reload()}>
          {c.refresh}
        </button>
      </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  onNotice(err instanceof DashboardApiError ? String((err.detail as { error?: string })?.error || err.message) : c.permissionDenied, 'error')
                } finally {
                  setBusy(false)
                }
              }}
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      onNotice(err instanceof Error ? err.message : 'revoke_failed', 'error')
                    } finally {
                      setBusy(false)
                    }
                  }}
