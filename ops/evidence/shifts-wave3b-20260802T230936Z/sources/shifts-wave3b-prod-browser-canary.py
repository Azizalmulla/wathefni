...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        # Day/week nav + filters smoke (Calendar-aligned board)
        if page.locator("[data-testid='shifts-filters']").count():
            shot(page, "w3b-prod-filters")
        for label in ("Day", "Week"):
            btn = page.get_by_role("button", name=label)
            if btn.count():
                btn.first.click()
                page.wait_for_timeout(400)
        shot(page, "w3b-prod-nav")
        week_btn = page.get_by_role("button", name="Week")
        if week_btn.count():
