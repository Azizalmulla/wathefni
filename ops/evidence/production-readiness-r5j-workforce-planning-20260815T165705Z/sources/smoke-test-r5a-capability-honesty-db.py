    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                check("customer can enable talent after R5C", blocked_talent is None)
                check(
                    "customer can enable job_architecture after R5D",
                    ready.customer_setup_write_block("wave6_job_architecture") is None,
                )
                check(
                    "customer can enable learning after R5E",
                    ready.customer_setup_write_block("wave6_learning") is None,
                )
                check(
                    "customer can enable benefits after R5F",
                    ready.customer_setup_write_block("wave6_benefits") is None,
                )
                check(
                    "customer can enable employee_relations after R5G",
                    ready.customer_setup_write_block("wave6_employee_relations") is None,
                )
                check(
                    "customer can enable engagement after R5H",
                    ready.customer_setup_write_block("wave6_engagement") is None,
                )
                check(
                    "customer can enable comp_planning after R5I",
                    ready.customer_setup_write_block("wave6_comp_planning") is None,
                )
                check(
                    "customer can enable workforce_planning after R5J",
                    ready.customer_setup_write_block("wave6_workforce_planning") is None,
                )
