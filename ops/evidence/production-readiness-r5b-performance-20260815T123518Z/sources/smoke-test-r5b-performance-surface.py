...
def main() -> int:
    ...
    check("talent still hidden", ready.customer_enableable("talent") is False)
    check("performance is catalog SKU", "performance" in catalog.MODULE_BY_KEY)
    check("talent not catalog SKU", "talent" not in catalog.MODULE_BY_KEY)
    ...
    check("fail-closed keeps talent namespaces", ("talent", "/dashboard/posthire/talent") in ready.all_http_namespaces())
    ...
    check("talent still fail-closed", talent.status_code == 404 and isinstance(detail, dict) and detail.get("error") == "capability_not_released")
