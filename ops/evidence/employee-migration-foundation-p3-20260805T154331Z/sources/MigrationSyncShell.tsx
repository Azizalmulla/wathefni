    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-line/50 bg-white/55 p-3 text-[12px] leading-5 text-subtle/85">
        ...
        <p className="mt-1.5 text-subtle/75">
          Matching: external ID when supplied, otherwise phone. Never by name alone. Safe updates: name, email, job title, department, start date, manager.
        </p>
      </div>
      <input
        type="file"
      ...
      {summary ? (
        <div className="space-y-2">
          ...
          <OutcomeList title="Needs review" rows={reviewRows} tone="warning" />
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p className="mt-1 text-[13px] leading-6 text-subtle/90">
          Ambiguous matches and invalid rows stay here until you fix the file or the employee record. We never update by name alone.
        </p>
      </div>
      {loading ? (
...
export function MigrationSyncShell({
  ...
  const body: ReactNode = useMemo(() => {
    ...
    if (section === 'review') return <ReviewSection access={access} onAccessIssue={onAccessIssue} />
