...
def cleanup_synthetics() -> dict[str, Any]:
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            # Collect all synthetic artifacts for this tag / phone prefix / marker
            cur.execute(
                """
                SELECT shift_id::text AS shift_id FROM shift_assignments
                WHERE company_code=%s AND (
                  employee_key LIKE %s OR employee_key LIKE %s
                  OR coalesce(employee_phone,'') LIKE '965529%%'
                  OR coalesce(source_text,'') LIKE %s
                  OR coalesce(metadata::text,'') LIKE %s
                  OR shift_id::text = ANY(%s)
                )
                """,
                (COMPANY, f"%SHW1B%{TAG}%", f"%SHW1B-SYNTH|%{TAG}%", f"%{TAG}%", f"%{TAG}%", IDS["shift_ids"] or ["00000000-0000-0000-0000-000000000000"]),
            )
            sids = [dict(r)["shift_id"] for r in cur.fetchall()]
            # Never delete orphan allowlist rows — only soft-cancelled; leave them
            sids = [s for s in sids if s not in ORPHAN_ALLOWLIST]
            if sids:
                cur.execute("DELETE FROM shift_events WHERE company_code=%s AND shift_id::text = ANY(%s)", (COMPANY, sids))
                try:
                    cur.execute("DELETE FROM shift_lifecycle_flags WHERE company_code=%s AND shift_id::text = ANY(%s)", (COMPANY, sids))
                except Exception:
                    conn.rollback()
                try:
                    cur.execute("DELETE FROM shift_orphan_quarantine WHERE company_code=%s AND shift_id::text = ANY(%s)", (COMPANY, sids))
                except Exception:
                    conn.rollback()
                try:
                    cur.execute(
                        ...
                        DELETE FROM shift_swap_events WHERE swap_id IN (
                          SELECT swap_id FROM shift_swap_requests WHERE company_code=%s
                            AND (requester_employee_key LIKE %s OR target_employee_key LIKE %s
                                 OR coalesce(reason,'') LIKE %s)
                        )
                        """,
                        (COMPANY, f"%SHW1B%{TAG}%", f"%SHW1B%{TAG}%", f"%{TAG}%"),
                    )
                except Exception:
                    conn.rollback()
                try:
                    cur.execute(
                        """
                        DELETE FROM shift_swap_requests WHERE company_code=%s
                          AND (requester_employee_key LIKE %s OR target_employee_key LIKE %s
                               OR coalesce(reason,'') LIKE %s OR swap_id::text = ANY(%s))
                        """,
                        (COMPANY, f"%SHW1B%{TAG}%", f"%SHW1B%{TAG}%", f"%{TAG}%", IDS["swap_ids"] or ["00000000-0000-0000-0000-000000000000"]),
                    )
                except Exception:
                    conn.rollback()
                    try:
                        ...
                        conn.rollback()
                cur.execute("DELETE FROM shift_assignments WHERE company_code=%s AND shift_id::text = ANY(%s)", (COMPANY, sids))
            if IDS.get("leave_ids"):
                cur.execute("DELETE FROM leave_requests WHERE company_code=%s AND leave_id::text = ANY(%s)", (COMPANY, [str(x) for x in IDS["leave_ids"]]))
            cur.execute(
                """
                DELETE FROM employees WHERE company_code=%s
                  AND (employee_key LIKE %s OR employee_key LIKE %s OR phone LIKE '965529%%')
                  AND (raw_json->>'shw1b')='true'
                """,
                (COMPANY, f"%SHW1B%{TAG}%", f"WATHEFNI-SHW1B%"),
            )
            # Residual scan
            cur.execute(
                """
                ...
                WHERE company_code=%s AND (
                  employee_key LIKE '%%SHW1B%%' OR coalesce(employee_phone,'') LIKE '965529%%'
                  OR coalesce(metadata::text,'') LIKE '%%SHW1B%%'
                ) AND shift_id::text <> ALL(%s)
                """,
                (COMPANY, list(ORPHAN_ALLOWLIST)),
            )
            residual_assign = int(dict(cur.fetchone())["c"])
            cur.execute(
                """
                SELECT count(*) AS c FROM shift_swap_requests
                WHERE company_code=%s AND (
                  requester_employee_key LIKE '%%SHW1B%%' OR target_employee_key LIKE '%%SHW1B%%'
                  OR coalesce(reason,'') LIKE '%%SHW1B%%'
                )
                """,
                (COMPANY,),
            )
            residual_swap = int(dict(cur.fetchone())["c"])
            cur.execute(
                """
                SELECT count(*) AS c FROM employees
                WHERE company_code=%s AND (employee_key LIKE '%%SHW1B%%' OR phone LIKE '965529%%')
                  AND coalesce(raw_json->>'shw1b','')='true'
                """,
                (COMPANY,),
            )
            residual_emp = int(dict(cur.fetchone())["c"])
            conn.commit()
    return {
        "deleted_shift_ids": sids,
        "residual_assignments": residual_assign,
        "residual_swaps": residual_swap,
        "residual_employees": residual_emp,
        "residual_total": residual_assign + residual_swap + residual_emp,
    }
...
def main() -> int:
    ...
    check("synthetic_only", sw1.shifts_authority_synthetic_only())
    ...
    swap_req = app.request_shift_swap(
        {"employee_key": EMP_KEY, "target_phone": PHONE_B, "shift_id": req_sid, "target_shift_id": tgt_sid, "date": swap_day.isoformat(), "reason": f"SHW1B swap {TAG}"},
        company_code=COMPANY,
    ...
    self_dec = app.approve_shift_swap({"swap_id": swap_id}, company_code=COMPANY, created_by_phone=PHONE)
    check("swap self-denial", self_dec.get("error") == "self_swap_decision_forbidden", self_dec.get("error"))
    scoped = app.reject_shift_swap({"swap_id": swap_id}, company_code=COMPANY, created_by_phone=ACTOR, account_id=None)
    check("swap scoped decision", scoped.get("ok") is True, scoped.get("error"))
    replay = app.reject_shift_swap({"swap_id": swap_id}, company_code=COMPANY, created_by_phone=ACTOR, account_id=None)
    check("swap idempotent replay", replay.get("ok") is True and replay.get("idempotent") is True, replay)
    swap2 = app.request_shift_swap(
        {"employee_key": EMP_KEY_B, "target_employee_key": EMP_KEY, "target_phone": PHONE, "shift_id": tgt_sid, "target_shift_id": req_sid, "date": swap_day.isoformat(), "reason": f"SHW1B swap2 {TAG}"},
        company_code=COMPANY,
    ...
    first = app.approve_shift_swap({"swap_id": swap2_id}, company_code=COMPANY, created_by_phone=ACTOR, account_id=None)
    check("swap first wins", first.get("ok") is True, first.get("error"))
    second = app.reject_shift_swap({"swap_id": swap2_id}, company_code=COMPANY, created_by_phone=ACTOR, account_id=None)
    check(
        "swap stale second",
