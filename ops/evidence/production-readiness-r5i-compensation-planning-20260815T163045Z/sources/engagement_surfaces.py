...
def list_history(cur: Any, *, company_code: str, campaign_id: str | None = None) -> dict[str, Any]:
    ...
    params: list[Any] = [company]
    if campaign_id:
        sql += " AND (entity_id=%s OR COALESCE(detail->>'campaign_id','')=%s)"
        params.extend([campaign_id, campaign_id])
    sql += " ORDER BY created_at ASC"
    cur.execute(sql, params)
    history = [dict(r) for r in cur.fetchall()]
    if campaign_id and not history:
        cur.execute(
            """
            SELECT campaign_id, survey_id, survey_version_id, status, privacy_mode,
                   audience_frozen, created_at, launched_at, closed_at
              FROM eng_campaigns
             WHERE company_code=%s AND campaign_id=%s
            """,
            (company, campaign_id),
            ...
            history.append(
                {
                    ...
                    "detail": {
                        "survey_version_id": str(camp.get("survey_version_id") or ""),
                        "privacy_mode": camp.get("privacy_mode"),
                        "reconstructed_from_canonical_row": True,
                    },
                }
            )
            if camp.get("launched_at"):
                history.append(
                    {
                        "action": "launched",
                        "entity_type": "campaign",
                        "entity_id": campaign_id,
                        "created_at": camp.get("launched_at"),
                        "detail": {
                            "survey_version_id": str(camp.get("survey_version_id") or ""),
                            "audience_frozen": bool(camp.get("audience_frozen")),
                            "reconstructed_from_canonical_row": True,
