    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                check("JA enableable after R5D", ready.customer_enableable("job_architecture") is True)
                check("Talent still does not require JA", honesty.get("job_architecture_required") is False if (honesty := surfaces.honesty_payload()) else False)
            conn.commit()
