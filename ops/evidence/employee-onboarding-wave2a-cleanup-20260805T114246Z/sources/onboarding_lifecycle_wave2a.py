...
def _employee_visible(item: dict[str, Any]) -> bool:
    ...
    status = normalize_status(item.get("status"))
    has_evidence = bool(item.get("file_id") or item.get("current_file_id") or item.get("value"))
    meta = item.get("lifecycle_meta") if isinstance(item.get("lifecycle_meta"), dict) else {}
    # Seed checklist due dates do NOT count as assignment — only explicit meta.
    explicitly_assigned = bool(meta.get("explicitly_assigned") or meta.get("applicable"))
    active = status in REVIEW_STATES | COMPLETE_STATES | {
        STATE_REPLACEMENT_REQUIRED,
        STATE_REJECTED,
        STATE_SUBMITTED,
        STATE_IN_PROGRESS,
