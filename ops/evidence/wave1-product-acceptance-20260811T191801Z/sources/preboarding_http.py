...
def register_preboarding_http(app_mod: Any) -> None:
    ...
    def dashboard_preboarding_joining_date(
        ...
        with db_connect() as conn:
            with conn.cursor() as cur:
                pb.ensure_preboarding_schema(cur)
                jd = _parse_date(body.joining_date)
                result = pb.set_joining_date(
                    cur,
                    company_code=company,
                    assignment_id=assignment_id,
                    joining_date=jd,  # type: ignore[arg-type]
                    actor_user_id=context.get("actor_user_id"),
                )
                if result.get("ok") and jd is not None:
                    try:
                        import hire_ready_bridge as _hrb

                        asn = result.get("assignment") or pb.get_assignment(
                            cur, company_code=company, assignment_id=assignment_id
                        )
                        if asn:
                            _hrb.on_joining_date_changed(
                                cur,
                                company_code=company,
                                employee_key=str(asn.get("employee_key") or ""),
                                joining_date=jd,
                                actor_user_id=context.get("actor_user_id"),
                            )
                    except Exception:
    ...
    @app.post("/dashboard/posthire/preboarding/{assignment_id}/convert")
    def dashboard_preboarding_convert(
        assignment_id: str,
        context: dict[str, Any] = Depends(dashboard_context),
    ):
        """Hire→Ready: Preboarding ready → convert same pending_start employment."""
        company = _posthire_read_context(context, "preboarding")
        if _actor_role(context) == "manager":
            raise HTTPException(status_code=403, detail={"error": "permission_denied"})
        with db_connect() as conn:
            with conn.cursor() as cur:
                ...
                try:
                    ...
                    result = _hrb.convert_ready_assignment_to_hire(
                        cur,
                        company_code=company,
                        assignment_id=assignment_id,
                        actor_user_id=context.get("actor_user_id"),
                    ...
                    result = {"ok": False, "error": "convert_failed", "detail": str(exc)}
            conn.commit()
        if not result.get("ok"):
            _raise_gate(result)
        return json_safe(result)
