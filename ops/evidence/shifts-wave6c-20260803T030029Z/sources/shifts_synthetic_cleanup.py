...
Safety rules:
  ...
  - Optional Wave 2 / Wave 4 tables use SAVEPOINT so missing tables never abort the txn.
  - Wave marker families cannot delete each other (distinct markers/phones).
...
Deletion order (children → parents):
  ...
  9b. shift_recurrence_events / exceptions / recurrences / templates (Wave 4 planning)
 10. shift_assignments         (excludes protected IDs; includes template_recurrence provenance)
...
CLEANUP_CONTRACT_VERSION = "1.5.0"
...
DELETION_ORDER: tuple[str, ...] = (
    ...
    "shift_open_shift_claims",
    "shift_open_shifts",
    "shift_notification_acks",
    "shift_notification_deliveries",
    "shift_notification_events",
    "shift_channel_preferences",
    "shift_pam_exports",
    "shift_rotation_events",
    "shift_rotation_assignments",
    "shift_rotation_patterns",
    "shift_compliance_profiles",
    "shift_schedule_draft_rows",
    "shift_schedule_events",
    "shift_schedule_versions",
    "shift_schedule_periods",
    "shift_coverage_rules",
    "shift_recurrence_events",
    "shift_recurrence_exceptions",
    "shift_recurrences",
    "shift_templates",
    "shift_assignments",
    "leave_requests",
...
def wave4_scope(
    *,
    ...
    return CleanupScope(
        company_code=company_code,
        markers=("SHW4", "SHW4-SYNTH|"),
        phone_prefixes=("965532",),
        employee_json_flag="shw4",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw4%",
        seasonal_name_ilike="%SHW4%",
    )
...
def wave4b_scope(
    *,
    ...
    """Production synthetic Wave 4B canary scope — SHW4B / 965533* only."""
    return CleanupScope(
        company_code=company_code,
        markers=("SHW4B", "SHW4B-SYNTH|"),
        phone_prefixes=("965533",),
        employee_json_flag="shw4b",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw4b%",
        seasonal_name_ilike="%SHW4B%",
    )
...
def wave5_scope(
    *,
    ...
    return CleanupScope(
        company_code=company_code,
        markers=("SHW5", "SHW5-SYNTH|"),
        phone_prefixes=("965534",),
        employee_json_flag="shw5",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw5%",
        seasonal_name_ilike="%SHW5%",
    )
...
def wave5b_scope(
    *,
    ...
    """Production synthetic Wave 5B canary scope — SHW5B / 965535* only."""
    return CleanupScope(
        company_code=company_code,
        markers=("SHW5B", "SHW5B-SYNTH|"),
        phone_prefixes=("965535",),
        employee_json_flag="shw5b",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw5b%",
        seasonal_name_ilike="%SHW5B%",
    )
...
def wave6_scope(
    *,
    ...
    """Staging Wave 6A synthetic scope — SHW6 / 965536* only."""
    return CleanupScope(
        company_code=company_code,
        markers=("SHW6", "SHW6-SYNTH|"),
        phone_prefixes=("965536",),
        employee_json_flag="shw6",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw6%",
        seasonal_name_ilike="%SHW6%",
    )
...
def wave6b_scope(
    *,
    ...
    """Production synthetic Wave 6B canary scope — SHW6B / 965537* only."""
    return CleanupScope(
        company_code=company_code,
        markers=("SHW6B", "SHW6B-SYNTH|"),
        phone_prefixes=("965537",),
        employee_json_flag="shw6b",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw6b%",
        seasonal_name_ilike="%SHW6B%",
    )
...
def wave6c_scope(
    *,
    ...
    """Production Wave 6C controlled canary scope — SHW6C / 965538* synthetic subjects only.

    Real controlled-rollout subjects (Talal and the named HR operator) are never in scope:
    their rows are business data, not canary residue.
    """
    return CleanupScope(
        company_code=company_code,
        markers=("SHW6C", "SHW6C-SYNTH|"),
        phone_prefixes=("965538",),
        employee_json_flag="shw6c",
        tag=tag,
        extra_employee_keys=tuple(extra_employee_keys),
        leave_reason_ilike="%shw6c%",
        seasonal_name_ilike="%SHW6C%",
    )
    ...
    cur.execute(
        ...
        WHERE company_code=%s
          AND (
            ...
            OR coalesce(occurrence_key,'') LIKE ANY(%s)
            OR coalesce(generated_from_version,'') LIKE ANY(%s)
          )
        """,
        (
            scope.company_code,
    ...
    *,
    ...
    residual = {
        "assignments": _count(
            ...
            """,
            (scope.company_code, marker_likes, phone_likes, marker_likes, marker_likes, protected),
        ),
        "versions": _count(
            """
        ...
        "templates": _count(
            """
            SELECT count(*) AS c FROM shift_templates
            WHERE company_code=%s AND name LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
        ...
        "recurrences": _count(
            """
            SELECT count(*) AS c FROM shift_recurrences
            WHERE company_code=%s
              AND (name LIKE ANY(%s) OR coalesce(target_key,'') LIKE ANY(%s))
            """,
            (scope.company_code, marker_likes, marker_likes),
            table="shift_recurrences",
        ),
        "recurrence_exceptions": _count(
            """
            SELECT count(*) AS c FROM shift_recurrence_exceptions
            WHERE company_code=%s
              AND recurrence_id IN (
                SELECT recurrence_id FROM shift_recurrences
                WHERE company_code=%s AND (
                  name LIKE ANY(%s) OR coalesce(target_key,'') LIKE ANY(%s)
                )
              )
            """,
            (scope.company_code, scope.company_code, marker_likes, marker_likes),
            table="shift_recurrence_exceptions",
        ),
        "recurrence_events": _count(
            """
            SELECT count(*) AS c FROM shift_recurrence_events
            WHERE company_code=%s
              AND (coalesce(payload::text,'') LIKE ANY(%s)
                   OR recurrence_id IN (
                     SELECT recurrence_id FROM shift_recurrences
                     WHERE company_code=%s AND (
                       name LIKE ANY(%s) OR coalesce(target_key,'') LIKE ANY(%s)
            ...
            """,
            (scope.company_code, marker_likes, scope.company_code, marker_likes, marker_likes),
            table="shift_recurrence_events",
        ),
        "schedule_periods": _count(
            """
            SELECT count(*) AS c FROM shift_schedule_periods
            WHERE company_code=%s AND name LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_schedule_periods",
        ),
        "schedule_versions": _count(
            """
            SELECT count(*) AS c FROM shift_schedule_versions
            WHERE company_code=%s
              AND period_id IN (
                SELECT period_id FROM shift_schedule_periods
                WHERE company_code=%s AND name LIKE ANY(%s)
              )
            """,
            (scope.company_code, scope.company_code, marker_likes),
            table="shift_schedule_versions",
        ),
        "schedule_draft_rows": _count(
            """
            SELECT count(*) AS c FROM shift_schedule_draft_rows
            WHERE company_code=%s
              AND (coalesce(employee_key,'') LIKE ANY(%s)
                   OR coalesce(occurrence_key,'') LIKE ANY(%s)
                   OR period_id IN (
                     SELECT period_id FROM shift_schedule_periods
                     WHERE company_code=%s AND name LIKE ANY(%s)
            ...
            """,
            (scope.company_code, marker_likes, marker_likes, scope.company_code, marker_likes),
            table="shift_schedule_draft_rows",
        ),
        "schedule_events": _count(
            """
            SELECT count(*) AS c FROM shift_schedule_events
            WHERE company_code=%s
              AND (coalesce(payload::text,'') LIKE ANY(%s)
                   OR period_id IN (
                     SELECT period_id FROM shift_schedule_periods
            ...
            """,
            (scope.company_code, marker_likes, scope.company_code, marker_likes),
            table="shift_schedule_events",
        ),
        "open_shifts": _count(
            """
            SELECT count(*) AS c FROM shift_open_shifts
            WHERE company_code=%s
              AND (coalesce(notes,'') LIKE ANY(%s)
                   OR coalesce(assigned_employee_key,'') LIKE ANY(%s)
                   OR coalesce(role,'') LIKE ANY(%s))
            """,
            (scope.company_code, marker_likes, marker_likes, marker_likes),
            table="shift_open_shifts",
        ),
        "open_shift_claims": _count(
            """
            SELECT count(*) AS c FROM shift_open_shift_claims
            WHERE company_code=%s AND employee_key LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_open_shift_claims",
        ),
        "coverage_rules": _count(
            """
            SELECT count(*) AS c FROM shift_coverage_rules
            WHERE company_code=%s AND name LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_coverage_rules",
        ),
        "rotation_patterns": _count(
            """
            SELECT count(*) AS c FROM shift_rotation_patterns
            WHERE company_code=%s AND name LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_rotation_patterns",
        ),
        "rotation_assignments": _count(
            """
            SELECT count(*) AS c FROM shift_rotation_assignments
            WHERE company_code=%s
              AND (name LIKE ANY(%s) OR coalesce(target_key,'') LIKE ANY(%s)
                   OR coalesce(employee_key,'') LIKE ANY(%s))
            """,
            (scope.company_code, marker_likes, marker_likes, marker_likes),
            table="shift_rotation_assignments",
        ),
        "rotation_events": _count(
            """
            SELECT count(*) AS c FROM shift_rotation_events
            WHERE company_code=%s AND coalesce(payload::text,'') LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_rotation_events",
        ),
        "compliance_profiles": _count(
            """
            SELECT count(*) AS c FROM shift_compliance_profiles
            WHERE company_code=%s AND name LIKE ANY(%s)
            """,
            (scope.company_code, marker_likes),
            table="shift_compliance_profiles",
        ),
        "pam_exports": _count(
            """
            SELECT count(*) AS c FROM shift_pam_exports
            WHERE company_code=%s
              AND (fingerprint LIKE ANY(%s)
                   OR coalesce(payload::text,'') LIKE ANY(%s)
                   OR period_id IN (
                     SELECT period_id FROM shift_schedule_periods
                     WHERE company_code=%s AND name LIKE ANY(%s)
                   ))
            """,
            (scope.company_code, marker_likes, marker_likes, scope.company_code, marker_likes),
            table="shift_pam_exports",
        ),
        "notification_events": _count(
            """
            SELECT count(*) AS c FROM shift_notification_events
            WHERE company_code=%s
              AND (employee_key LIKE ANY(%s) OR coalesce(dedupe_key,'') LIKE ANY(%s)
                   OR coalesce(payload::text,'') LIKE ANY(%s))
            """,
            (scope.company_code, marker_likes, marker_likes, marker_likes),
            table="shift_notification_events",
        ),
        "notification_deliveries": _count(
            """
            SELECT count(*) AS c FROM shift_notification_deliveries
            WHERE company_code=%s
              AND event_id IN (
                SELECT event_id FROM shift_notification_events
                WHERE company_code=%s
                  AND (employee_key LIKE ANY(%s) OR coalesce(dedupe_key,'') LIKE ANY(%s))
              )
            """,
            (scope.company_code, scope.company_code, marker_likes, marker_likes),
            table="shift_notification_deliveries",
        ),
        "notification_acks": _count(
            """
            SELECT count(*) AS c FROM shift_notification_acks
            WHERE company_code=%s
              AND event_id IN (
                SELECT event_id FROM shift_notification_events
                WHERE company_code=%s AND employee_key LIKE ANY(%s)
            ...
            """,
            (scope.company_code, scope.company_code, marker_likes),
            table="shift_notification_acks",
        ),
        "channel_preferences": _count(
            """
            SELECT count(*) AS c FROM shift_channel_preferences
            WHERE company_code=%s
              AND (scope_key LIKE ANY(%s) OR coalesce(channel_order::text,'') LIKE ANY(%s))
            """,
            (scope.company_code, marker_likes, marker_likes),
            table="shift_channel_preferences",
        ),
        "employees": 0,
    }
    ...
    *,
    ...
    with db_connect() as conn:
        with conn.cursor() as cur:
            ...
            # 9a Wave 5 publish / open / coverage tables
            deleted["shift_open_shift_claims"] = _exec_optional(
                cur,
                "shift_open_shift_claims",
                """
                DELETE FROM shift_open_shift_claims
                WHERE company_code=%s AND employee_key LIKE ANY(%s)
                """,
                (scope.company_code, marker_likes),
            )
            deleted["shift_open_shifts"] = _exec_optional(
                cur,
                "shift_open_shifts",
                """
                DELETE FROM shift_open_shifts
                WHERE company_code=%s
                  AND (coalesce(notes,'') LIKE ANY(%s)
                       OR coalesce(assigned_employee_key,'') LIKE ANY(%s)
                       OR coalesce(role,'') LIKE ANY(%s)
                ...
                """,
                (scope.company_code, marker_likes, marker_likes, marker_likes, scope.company_code, marker_likes),
            )
            # 9a++ Wave 6B notifications
            deleted["shift_notification_acks"] = _exec_optional(
                cur,
                "shift_notification_acks",
                """
                DELETE FROM shift_notification_acks
                WHERE company_code=%s
                  AND event_id IN (
                    SELECT event_id FROM shift_notification_events
                    WHERE company_code=%s AND employee_key LIKE ANY(%s)
            ...
            deleted["shift_notification_deliveries"] = _exec_optional(
                cur,
                "shift_notification_deliveries",
                """
                DELETE FROM shift_notification_deliveries
                WHERE company_code=%s
                  AND event_id IN (
                    SELECT event_id FROM shift_notification_events
                    WHERE company_code=%s
            ...
            deleted["shift_notification_events"] = _exec_optional(
                cur,
                "shift_notification_events",
                """
                DELETE FROM shift_notification_events
                WHERE company_code=%s
                  AND (employee_key LIKE ANY(%s) OR coalesce(dedupe_key,'') LIKE ANY(%s)
                       OR coalesce(payload::text,'') LIKE ANY(%s))
                """,
            ...
            deleted["shift_channel_preferences"] = _exec_optional(
                cur,
                "shift_channel_preferences",
                """
                DELETE FROM shift_channel_preferences
                WHERE company_code=%s
                  AND (scope_key LIKE ANY(%s) OR coalesce(channel_order::text,'') LIKE ANY(%s))
                """,
                (scope.company_code, marker_likes, marker_likes),
            )
            # 9a+ Wave 6 rotations / compliance / PAM
            deleted["shift_pam_exports"] = _exec_optional(
                cur,
                "shift_pam_exports",
                """
                DELETE FROM shift_pam_exports
                WHERE company_code=%s
                  AND (coalesce(payload::text,'') LIKE ANY(%s)
                       OR period_id IN (
                         SELECT period_id FROM shift_schedule_periods
            ...
            deleted["shift_rotation_events"] = _exec_optional(
                cur,
                "shift_rotation_events",
                """
                DELETE FROM shift_rotation_events
                WHERE company_code=%s AND coalesce(payload::text,'') LIKE ANY(%s)
                """,
                (scope.company_code, marker_likes),
            )
            deleted["shift_rotation_assignments"] = _exec_optional(
                cur,
                "shift_rotation_assignments",
                """
            ...
            deleted["shift_rotation_patterns"] = _exec_optional(
                cur,
                "shift_rotation_patterns",
                """
            ...
            deleted["shift_compliance_profiles"] = _exec_optional(
                cur,
                "shift_compliance_profiles",
                """
                DELETE FROM shift_compliance_profiles
                WHERE company_code=%s AND name LIKE ANY(%s)
                """,
                (scope.company_code, marker_likes),
            )
            deleted["shift_schedule_draft_rows"] = _exec_optional(
                cur,
                "shift_schedule_draft_rows",
                """
                DELETE FROM shift_schedule_draft_rows
                WHERE company_code=%s
                  AND (coalesce(employee_key,'') LIKE ANY(%s)
                       OR coalesce(occurrence_key,'') LIKE ANY(%s)
                       OR period_id IN (
            ...
            deleted["shift_schedule_events"] = _exec_optional(
                cur,
                "shift_schedule_events",
                """
                DELETE FROM shift_schedule_events
                WHERE company_code=%s
                  AND (coalesce(payload::text,'') LIKE ANY(%s)
                       OR period_id IN (
                         SELECT period_id FROM shift_schedule_periods
            ...
            deleted["shift_schedule_versions"] = _exec_optional(
                cur,
                "shift_schedule_versions",
                """
                DELETE FROM shift_schedule_versions
                WHERE company_code=%s
                  AND period_id IN (
                    SELECT period_id FROM shift_schedule_periods
                    WHERE company_code=%s AND name LIKE ANY(%s)
            ...
            deleted["shift_schedule_periods"] = _exec_optional(
                cur,
                "shift_schedule_periods",
                """
                DELETE FROM shift_schedule_periods
                WHERE company_code=%s AND name LIKE ANY(%s)
                """,
                (scope.company_code, marker_likes),
            )
            deleted["shift_coverage_rules"] = _exec_optional(
                cur,
                "shift_coverage_rules",
                """
                DELETE FROM shift_coverage_rules
                WHERE company_code=%s AND name LIKE ANY(%s)
                """,
                (scope.company_code, marker_likes),
            )
            # 9b Wave 4 planning tables (exceptions/events/recurrences/templates)
            deleted["shift_recurrence_events"] = _exec_optional(
                cur,
                "shift_recurrence_events",
                """
                DELETE FROM shift_recurrence_events
                WHERE company_code=%s
                  AND (coalesce(payload::text,'') LIKE ANY(%s)
                       OR recurrence_id IN (
                         SELECT recurrence_id FROM shift_recurrences
            ...
            deleted["shift_recurrence_exceptions"] = _exec_optional(
                cur,
                "shift_recurrence_exceptions",
                """
                DELETE FROM shift_recurrence_exceptions
                WHERE company_code=%s
                  AND recurrence_id IN (
                    SELECT recurrence_id FROM shift_recurrences
                    WHERE company_code=%s AND (
            ...
            deleted["shift_recurrences"] = _exec_optional(
                cur,
                "shift_recurrences",
                """
                DELETE FROM shift_recurrences
                WHERE company_code=%s
                  AND (name LIKE ANY(%s) OR coalesce(target_key,'') LIKE ANY(%s))
                """,
                (scope.company_code, marker_likes, marker_likes),
            )
            deleted["shift_templates"] = _exec_optional(
                cur,
                "shift_templates",
                """
