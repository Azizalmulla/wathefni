...
def is_wave5_synthetic_employee(
    *,
    ...
    if isinstance(raw, dict) and (
        raw.get("shw5") is True
        or raw.get("shw5b") is True
        or str(raw.get("shw5") or "").lower() in _ON
        or str(raw.get("shw5b") or "").lower() in _ON
    ):
        return True
    return False
...
def honesty_payload() -> dict[str, Any]:
    return {
        ...
        "complexity_levels": {
            ...
            "enterprise_later": "wave6_rotations_remote_compliance_pam",
        },
    ...
    try:
        ...
        for row in draft:
            if str(row.get("row_class")) in {"conflict", "cancelled_held", "detached"}:
                ...
                continue
            # Wave 6 planning days — never promote travel/rest/standby to L0
            if str(row.get("row_class")) in {"travel", "rest", "standby"} or str(row.get("day_kind") or "") in {
                "travel",
                "rest",
                "standby",
            }:
                results.setdefault("skipped_non_work", 0)
                results["skipped_non_work"] += 1
                continue
...
def decide_open_shift_claim(
    ...
    *,
    ...
    if not created.get("ok"):
        # create_fn overlap/leave/lifecycle denials are operational gates
        if created.get("conflicts"):
            return {"ok": False, "error": "gate_conflict", "conflict": {"class": "overlap_or_create", "create_result": created}, **honesty_payload()}
