...
def fp(r):
  parts=[str(r.get(k) or "") for k in ("shift_id","employee_key","employee_phone","shift_date","start_time","end_time","status","updated_at","role","location","timezone","source_kind","occurrence_key")]
...
with app.db_connect() as conn:
  with conn.cursor() as cur:
    ...
    try:
      ...
      cur.execute("SELECT exception_id::text, recurrence_id::text, exception_date::text, kind FROM shift_recurrence_exceptions e WHERE company_code='WATHEFNI' ORDER BY 1")
      exc=[dict(r) for r in cur.fetchall()]
    except Exception as e:
      print(json.dumps({"note":"wave4_tables_absent_pre_deploy","error":str(e)[:200]}))
print(json.dumps({
  "db":db,
