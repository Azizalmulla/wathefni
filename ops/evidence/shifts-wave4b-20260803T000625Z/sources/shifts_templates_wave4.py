...
def ensure_shifts_templates_wave4_schema(cur: Any) -> None:
    # Serialize DDL — concurrent CREATE TABLE/TYPE races on pg_type.
    lock_id = JOB_LOCK_MATERIALIZE_BASE - 7
...
def evaluate_planned_conflicts(
    ...
    *,
    ...
    if w2.shifts_wave2_applies_to(company, emp):
        ...
        avail_denied = w2.availability_conflict_denied(
...
def materialize_recurrence(
    ...
    *,
    ...
    try:
        ...
        classified = list(preview.get("classified") or [])
        write_rows: list[dict[str, Any]] = []
        for row in classified:
            cls = str(row.get("class") or "")
            if cls == "unchanged":
                results["unchanged"] += 1
            elif cls in {"detached", "cancelled_held"}:
                results[cls] += 1
            elif cls == "conflict":
                results["conflict"] += 1
                results["conflicts"].append(row)
            elif cls == "newly_generated":
                write_rows.append(row)
            elif cls == "updated_future":
                if reschedule_fn:
                    write_rows.append(row)
                else:
        ...
        # Session advisory lock survives COMMIT; release relation locks so sibling
        # connections in create_fn/reschedule_fn can run ensure-* DDL without deadlock.
        try:
        ...
        for row in write_rows:
            ...
            if cls == "newly_generated":
                ...
                if created.get("ok"):
                    ...
                    # Provenance is stamped inside create_fn when source_kind=template_recurrence.
                    # Do not UPDATE on this cursor between creates — it re-locks relations and
                    # deadlocks sibling create_fn connections running ensure-* DDL.
                else:
                    results["conflict"] += 1
            ...
            if cls == "updated_future" and reschedule_fn:
                ...
                if rs.get("ok"):
                    ...
                    # Stamp on a short transaction, then commit so the next sibling call can proceed.
                    _stamp_provenance(
                        cur,
                        company_code=company,
