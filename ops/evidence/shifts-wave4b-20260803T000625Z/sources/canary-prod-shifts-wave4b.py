...
def main() -> int:
    ...
    check("cleanup_contract_1_1", CLEANUP_CONTRACT_VERSION.startswith("1.1"))
        ...
        with conn.cursor() as cur:
            ...
            for label, rid in (("split_pm", split_id), ("team", team_id), ("site", site_id), ("role", role_id), ("six_on", six_id)):
                m = w4.materialize_recurrence(
                    cur, company_code=COMPANY, recurrence_id=rid, create_fn=app.create_shift_assignment, actor_phone=ACTOR, action_acks=ACKS
                )
