...
def main() -> int:
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE company_code=%s AND (employee_key = ANY(%s) OR name LIKE 'W3D-SYNTH|W3E %')
    ...
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT count(*)::int n FROM employees WHERE company_code=%s AND (name LIKE 'W3D-SYNTH|W3E %' OR employee_key = ANY(%s))",
                (COMPANY, synth_keys),
            )
