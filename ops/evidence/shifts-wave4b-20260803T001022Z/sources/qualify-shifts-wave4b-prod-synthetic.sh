...
\$PY - <<'PY'
import app
from shifts_synthetic_cleanup import CleanupScope, cleanup_synthetic_scope, wave1b_scope, wave2b_scope, wave3b_scope, wave4b_scope
for s in (
  wave4b_scope(), wave3b_scope(), wave2b_scope(), wave1b_scope(),
...
PY
# Service drop-in keeps INTEGRITY_JOBS=0 (timers disabled). W2B canary exercises jobs in-process only.
if [[ -f canary-prod-shifts-wave1b.py ]]; then
  echo '=== W1B ==='; mkdir -p /tmp/shw4b-coexist/w1b; SHW1B_EVID=/tmp/shw4b-coexist/w1b \$PY -u canary-prod-shifts-wave1b.py | tee /tmp/w1b-coexist.out | tail -8; echo W1B_RC=\${PIPESTATUS[0]}
fi
if [[ -f canary-prod-shifts-wave2b.py ]]; then
  echo '=== W2B ==='; mkdir -p /tmp/shw4b-coexist/w2b; WATHEFNI_SHIFTS_INTEGRITY_JOBS=1 SHW2B_EVID=/tmp/shw4b-coexist/w2b \$PY -u canary-prod-shifts-wave2b.py | tee /tmp/w2b-coexist.out | tail -10; echo W2B_RC=\${PIPESTATUS[0]}
fi
...
# Confirm service still has jobs kill switch off
