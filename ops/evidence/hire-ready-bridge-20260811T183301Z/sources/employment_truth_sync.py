...
def truth_sync_company_allowlist() -> set[str]:
    """Staging/company canary allowlist. Empty ⇒ writers stay dark even if flag on."""
...
def writers_enabled_for_company(company_code: str | None) -> bool:
    """Company-scoped writer canary — never globally enable mutations."""
    if not writers_enabled():
        return False
    allow = truth_sync_company_allowlist()
    if not allow:
        return False
    return company_code_norm(company_code) in allow
...
def rollback_guidance() -> dict[str, Any]:
    return {
        "runtime": [
            ...
            "Do not entitle WATHEFNI_EMPLOYMENT_TRUTH_SYNC_COMPANIES until dry-run is clean",
            "Writers require BOTH the global flag AND a non-empty company allowlist (never global-only)",
        ],
        "data": [
...
def dry_run_company(cur: Any, company_code: str, *, limit: int = 200) -> dict[str, Any]:
    ...
    try:
        from employee_lifecycle_wave3 import hub_status_for_lifecycle, is_active_hub_status

        hub_for_pending = hub_status_for_lifecycle("pending_start")
        if hub_for_pending == "active" or is_active_hub_status(hub_for_pending):
            findings.append(
                {
                    "invariant": "P2_joining_vs_active_visibility",
                    "severity": "fail",
                    "code": "hub_status_for_lifecycle_maps_pending_start_to_active",
                    "detail": "BLOCKER: hub_status_for_lifecycle must project pending_start as non-active joining state",
                }
            )
        elif hub_for_pending != "pending_start":
            findings.append(
                {
                    "invariant": "P2_joining_vs_active_visibility",
                    "severity": "fail",
                    "code": "hub_status_for_lifecycle_pending_start_unexpected",
                    "detail": f"expected pending_start, got {hub_for_pending!r}",
                }
            )
    except Exception as exc:
        findings.append(
            {
                "invariant": "P2_joining_vs_active_visibility",
                "severity": "fail",
                "code": "hub_status_helper_unavailable",
                "detail": str(exc.__class__.__name__),
            }
        )
    ...
    invariant_violation_codes = sorted(
        {
            str(f.get("code") or "")
            for f in findings
            if f.get("severity") == "fail"
            and str(f.get("code") or "").startswith(
                ("pending_start_", "hub_status_", "duplicate_active_")
            )
        }
    )
    ...
    return {
        ...
        "version": TRUTH_SYNC_VERSION,
        "writers_enabled": writers_enabled(),
        "writers_enabled_for_company": writers_enabled_for_company(company),
        "truth_sync_company_allowlist": sorted(truth_sync_company_allowlist()),
        "module_behavior": mod,
        "pending_start_invariants_pass": fail_count == 0 and not invariant_violation_codes,
        "counts": {
            ...
            "fail": fail_count,
            "warning": warn_count,
            "invariant_violations": len(invariant_violation_codes),
        },
