...
def cleanup_synthetics() -> dict[str, Any]:
    shift_ids = IDS["shift_ids"] or ["00000000-0000-0000-0000-000000000000"]
    swap_ids = IDS["swap_ids"] or ["00000000-0000-0000-0000-000000000000"]
    with app.db_connect() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                DELETE FROM shift_reminder_queue
                WHERE company_code=%s AND (shift_id::text = ANY(%s) OR employee_key LIKE %s)
                """,
                (COMPANY, shift_ids, f"%{MARKER}%"),
            )
            try:
                cur.execute(
                    """
                    DELETE FROM shift_assignment_versions
                    WHERE company_code=%s AND (shift_id::text = ANY(%s) OR employee_key LIKE %s OR previous_employee_key LIKE %s)
                    """,
                    (COMPANY, shift_ids, f"%{MARKER}%", f"%{MARKER}%"),
                )
            except Exception:
                conn.rollback()
            try:
            ...
            try:
                cur.execute(
                    ...
                    """,
                    (COMPANY, f"%{MARKER}%", f"%{MARKER}%", f"%{MARKER}%", swap_ids),
                )
            except Exception:
                conn.rollback()
            cur.execute(
            ...
            cur.execute(
                """
                DELETE FROM employee_availability_requests WHERE company_code=%s
                  AND (employee_key LIKE %s OR coalesce(employee_phone,'') LIKE '965530%%'
                       OR employee_key LIKE %s OR availability_id::text = ANY(%s))
                """,
                (COMPANY, f"%{MARKER}%", f"%{GATE_KEY.split('-')[1]}%", IDS["availability_ids"] or ["00000000-0000-0000-0000-000000000000"]),
            )
            cur.execute(
                """
                DELETE FROM shift_assignments WHERE company_code=%s
                  AND (shift_id::text = ANY(%s) OR employee_key LIKE %s OR coalesce(employee_phone,'') LIKE '965530%%')
                """,
                (COMPANY, shift_ids, f"%{MARKER}%"),
            )
            if IDS.get("leave_ids"):
                cur.execute(
                    ...
                    (COMPANY, [str(x) for x in IDS["leave_ids"]]),
                )
            cur.execute(
            ...
            try:
                cur.execute(
                    "DELETE FROM shift_seasonal_policies WHERE company_code=%s AND name ILIKE %s",
                    (COMPANY, f"%{MARKER}%"),
                )
            except Exception:
                conn.rollback()
            cur.execute(
                ...
                DELETE FROM employees WHERE company_code=%s
                  AND (employee_key LIKE %s OR phone LIKE '965530%%')
                  AND coalesce(raw_json->>'shw2b','')='true'
                """,
                (COMPANY, f"%{MARKER}%"),
            )
            cur.execute(
            ...
            # Residual scan across every table Wave 2B can touch.
            cur.execute(
                """
                SELECT count(*) AS c FROM shift_assignments
                WHERE company_code=%s AND (employee_key LIKE %s OR coalesce(employee_phone,'') LIKE '965530%%')
                """,
                (COMPANY, f"%{MARKER}%"),
            )
            ...
            try:
                cur.execute(
                    "SELECT count(*) AS c FROM shift_assignment_versions WHERE company_code=%s AND employee_key LIKE %s",
                    (COMPANY, f"%{MARKER}%"),
                )
                residual_versions = int(dict(cur.fetchone())["c"])
            except Exception:
                conn.rollback()
                residual_versions = 0
            try:
                cur.execute(
                    "SELECT count(*) AS c FROM shift_reminder_queue WHERE company_code=%s AND employee_key LIKE %s",
                    (COMPANY, f"%{MARKER}%"),
                )
                residual_reminders = int(dict(cur.fetchone())["c"])
            except Exception:
                conn.rollback()
                residual_reminders = 0
            try:
                cur.execute(
                    "SELECT count(*) AS c FROM shift_reconciliation_flags WHERE company_code=%s AND employee_key LIKE %s",
                    (COMPANY, f"%{MARKER}%"),
                )
                residual_flags = int(dict(cur.fetchone())["c"])
            except Exception:
                conn.rollback()
                residual_flags = 0
            cur.execute(
                ...
                WHERE company_code=%s AND (requester_employee_key LIKE %s OR target_employee_key LIKE %s)
                """,
                (COMPANY, f"%{MARKER}%", f"%{MARKER}%"),
            )
            residual_swap = int(dict(cur.fetchone())["c"])
            cur.execute(
                """
                SELECT count(*) AS c FROM employee_availability_requests
                WHERE company_code=%s AND (employee_key LIKE %s OR coalesce(employee_phone,'') LIKE '965530%%' OR employee_key=%s)
                """,
                (COMPANY, f"%{MARKER}%", GATE_KEY),
            )
            residual_avail = int(dict(cur.fetchone())["c"])
            cur.execute(
                "SELECT count(*) AS c FROM leave_requests WHERE company_code=%s AND coalesce(reason,'') ILIKE %s",
                (COMPANY, "%shw2b%"),
            )
            residual_leave = int(dict(cur.fetchone())["c"])
            try:
                cur.execute(
                    "SELECT count(*) AS c FROM shift_seasonal_policies WHERE company_code=%s AND name ILIKE %s",
                    (COMPANY, f"%{MARKER}%"),
                )
                residual_seasonal = int(dict(cur.fetchone())["c"])
            except Exception:
                conn.rollback()
                residual_seasonal = 0
            cur.execute(
                ...
                WHERE company_code=%s AND (
                  (employee_key LIKE %s AND coalesce(raw_json->>'shw2b','')='true')
                  OR (phone LIKE '965530%%' AND coalesce(raw_json->>'shw2b','')='true')
                  OR employee_key=%s
                )
                """,
                (COMPANY, f"%{MARKER}%", GATE_KEY),
            ...
            conn.commit()
    residual = {
        "assignments": residual_assign,
        "versions": residual_versions,
        "reminders": residual_reminders,
        "reconciliation_flags": residual_flags,
        "swaps": residual_swap,
        "availability": residual_avail,
        "leave": residual_leave,
        "seasonal_policies": residual_seasonal,
        "employees": residual_emp,
    }
    residual["total"] = sum(residual.values())
    return residual
