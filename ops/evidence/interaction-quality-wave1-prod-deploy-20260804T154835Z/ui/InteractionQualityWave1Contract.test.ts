...
describe('Interaction Quality & Loading Integrity Wave 1 contract', () => {
  ...
  it('clears ranking before loading a new job', () => {
    expect(appSrc).toContain("Clear prior job's results immediately")
    expect(appSrc).toMatch(/async function loadCurrentRanking[\s\S]*setRanking\(null\)[\s\S]*setBusy\(true\)/)
  })
})
