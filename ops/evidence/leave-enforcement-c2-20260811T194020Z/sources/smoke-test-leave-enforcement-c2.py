...
def _insert_leave_request(cur, company: str, leave: dict, *, emp_phone: str, status: str = "pending") -> None:
    cur.execute(
        """
        INSERT INTO leave_requests (
          ...
          requested_by_phone, requested_at, metadata, created_at, updated_at,
          row_version, duration_unit, chargeable_days, chargeable_hours, payroll_handoff
        ) VALUES (
          %s,%s,%s,%s,%s,
          %s,%s,%s,%s,%s,%s,
          %s, now(), '{}'::jsonb, now(), now(),
...
def main() -> int:
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            _insert_leave_request(cur, COMPANY, leave_ok, emp_phone=emp_phone)
            ...
            _insert_leave_request(cur, COMPANY, leave_big, emp_phone=emp_phone)
            ...
            leave_direct = {**leave_big, "leave_id": str(uuid.uuid4())}
            _insert_leave_request(cur, COMPANY, leave_direct, emp_phone=emp_phone)
            r_direct = lp.reserve_leave_balance(
                cur, company_code=COMPANY, leave=leave_direct, policy=policy
            )
            ...
            _insert_leave_request(cur, COMPANY, leave_unpaid, emp_phone=emp_phone)
            ...
            _insert_leave_request(cur, COMPANY, leave_ov, emp_phone=emp_phone)
            ...
            leave_ov2 = {**leave_ov, "leave_id": str(uuid.uuid4())}
            _insert_leave_request(cur, COMPANY, leave_ov2, emp_phone=emp_phone)
            ...
            # Override may leave available negative; top-up so cancel/consume proves have headroom
            top = enf.post_balance_adjustment(
                cur,
                company_code=COMPANY,
                employee_key=emp,
                leave_type="annual",
                days=25,
                actor_phone="96552599999",
                reason="canary top-up after override headroom",
            )
            check("balance top-up after override", top.get("ok") is True, top)
            ...
            _insert_leave_request(cur, COMPANY, leave_cancel, emp_phone=emp_phone)
            ...
            _insert_leave_request(cur, COMPANY, leave_cons, emp_phone=emp_phone)
            ...
            _insert_leave_request(cur, COMPANY, leave_obs, emp_phone=emp_phone)
