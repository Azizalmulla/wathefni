# HR mobile terminal qualification — 20260811T091526Z

**Ship (terminal):** `NO-SHIP`
**Reason:** FAIL=5 PASS=84 SKIP=5 — terminal API/contract only; PHYSICAL_ONLY remains for device UI
...
| Leave | Open approve fixture | `GET /dashboard/mobile/leave/6cb79442-e2f7-4ed7-b17b-cdf9b8249840` | status=requested + approve/reject actions | HTTP 200 status=requested actions=['read', 'approve', 'reject'] | **PASS** |
| Leave | Open reject fixture | `GET /dashboard/mobile/leave/b06ef4d2-dc52-4fd0-8783-3e8c5405c509` | status=requested + approve/reject actions | HTTP 200 status=requested actions=['read', 'approve', 'reject'] | **PASS** |
...
| Leave | Confirm Approve | `POST …/decision confirm=true` | completed / approved | HTTP 200 ok=False status=failed | **FAIL** |
...
| Leave | Post-approve detail refresh | `GET /dashboard/mobile/leave/6cb79442-e2f7-4ed7-b17b-cdf9b8249840` | status=approved, allowed_actions=[] | HTTP 200 status=requested actions=['read', 'approve', 'reject'] | **FAIL** |
...
| Leave | Confirm Reject | `POST …/decision confirm=true` | completed / rejected | HTTP 200 keys=ok,status,confirmation,result | **FAIL** |
| Leave | Post-reject detail + reason persistence | `GET /dashboard/mobile/leave/b06ef4d2-dc52-4fd0-8783-3e8c5405c509` | status=rejected + decision_note set | status=requested note_len=0 | **FAIL** |
| Home/Inbox | Priorities after leave decisions | `GET /dashboard/mobile/priorities` | decided leave IDs absent from destinations | HTTP 200 lingering=2 | **FAIL** |
