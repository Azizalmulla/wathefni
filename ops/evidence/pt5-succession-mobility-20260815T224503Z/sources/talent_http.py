...
class MobilityDiscoverBody(BaseModel):
    employee_key: str
    reason: str = "advisory mobility discovery"


class TrajectoryPolicyBody(BaseModel):
...
class TrajectoryEvaluateBody(BaseModel):
    employee_key: str
...
def register_talent_http(app_mod: Any) -> None:
    ...
    def dashboard_talent_map(lens: str = "perf_x_potential", department: str = "", team: str = "", context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context)
        filters = {k: v for k, v in {"department": department, "team": team}.items() if v}
        return _run(
            lambda cur: pt4.build_map(
                cur,
                company_code=company,
                lens=lens,
                filters=filters,
    ...
    def dashboard_talent_map_why(employee_key: str, lens: str = "perf_x_potential", context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context)
        return _run(lambda cur: pt4.get_placement_why(cur, company_code=company, employee_key=employee_key, lens=lens))

    def dashboard_succession_intel(context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context)
        if not _can_succession(context):
            raise HTTPException(status_code=403, detail={"error": "permission_denied"})
        return _run(lambda cur: pt5.succession_intelligence(cur, company_code=company))

    def dashboard_mobility_discover(body: MobilityDiscoverBody, context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context, write=True)
        return _run(
            lambda cur: pt5.discover_mobility(
                cur,
                company_code=company,
                actor_phone=str(context.get("actor_phone") or ""),
                employee_key=body.employee_key,
    ...
    def dashboard_trajectory_publish(body: TrajectoryPolicyBody, context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context, write=True)
        if _role(context) != "hr" and "talent.manage" not in _perms(context):
            raise HTTPException(status_code=403, detail={"error": "permission_denied"})
        return _run(
            lambda cur: pt6.publish_trajectory_policy(
                cur,
                company_code=company,
                actor_phone=str(context.get("actor_phone") or ""),
                min_closed_periods=body.min_closed_periods,
                high_band=body.high_band,
                rules=body.rules,
                reason=body.reason,
            )
        )

    def dashboard_trajectory_evaluate(body: TrajectoryEvaluateBody, context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context, write=True)
        return _run(
            lambda cur: pt6.evaluate_trajectory(
                cur, company_code=company, employee_key=body.employee_key, reason=body.reason
            )
        )

    def dashboard_holder_dependency(employee_key: str, context: dict[str, Any] = Depends(dashboard_context)):
        company = _company(context)
        return _run(lambda cur: pt6.holder_dependency(cur, company_code=company, employee_key=employee_key))

    _bind("/map", dashboard_talent_map)
    _bind("/map/{employee_key}/why", dashboard_talent_map_why)
    _bind("/succession-intelligence", dashboard_succession_intel)
    _bind("/mobility/discover", dashboard_mobility_discover, ("POST",))
    _bind("/trajectory/policy", dashboard_trajectory_publish, ("POST",))
    _bind("/trajectory/evaluate", dashboard_trajectory_evaluate, ("POST",))
    _bind("/capability/holder-dependency/{employee_key}", dashboard_holder_dependency)
