/**
 * Before = live prod. After = local dist via API proxy. No deploy.
 */
...
const PORT = Number(process.env.VISUAL_QUAL_PORT || 4197)
...
const MODE = (process.env.CAPTURE_MODE || 'after').trim() // before | after
if (!TOKEN) process.exit(2)
if (MODE === 'after' && !existsSync(DIST)) process.exit(2)

const PAGES = ['jobs', 'candidates', 'interviews', 'assessments', 'ranking', 'reports', 'ai']
const MIME = {
...
const shots = resolve(EVID, 'screenshots', MODE)
...
const probes = []
const origin = MODE === 'after' ? `http://127.0.0.1:${PORT}` : API_ORIGIN
...
async function capture(pageId, locale, vpName, size) {
  ...
  await page.goto(`${origin}/dashboard/?page=${pageId}`, { waitUntil: 'domcontentloaded', timeout: 60000 })
  ...
  const probe = await page.evaluate(() => {
    const samples = [...document.querySelectorAll('[class*="wf-accent"], [class*="accent-"]')]
      .slice(0, 12)
      .map((el) => el.className.toString().slice(0, 140))
    return {
      dir: document.documentElement.getAttribute('dir') || document.querySelector('main')?.getAttribute('dir'),
      h1: document.querySelector('h1')?.textContent?.trim() || null,
      accentSamples: samples,
      blackPills: document.querySelectorAll('[class*="bg-[#23211d]"]').length,
      bodyHasSoftPriority: /bg-wf-accent-priority-soft/.test(document.body.innerHTML) || !!document.querySelector('[class*="priority-soft"]'),
    }
  })
  probes.push({ page: pageId, locale, viewport: vpName, mode: MODE, ...probe })
  console.log(MODE, pageId, locale, vpName)
  await page.close()
  await context.close()
}
...
writeFileSync(resolve(EVID, 'verify', `${MODE}-probes.json`), JSON.stringify(probes, null, 2))
await browser.close()
if (server) server.close()
console.log('capture_complete', MODE, probes.length)
