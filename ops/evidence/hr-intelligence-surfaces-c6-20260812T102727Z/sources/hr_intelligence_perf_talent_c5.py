...
def runtime_gate_for_company(company_code: str | None) -> dict[str, Any]:
    ...
    raw = str(os.environ.get("WATHEFNI_HR_INTELLIGENCE_PERF_TALENT_COMPANIES") or "").strip()
    allowed = {part.strip().upper() for part in raw.split(",") if part.strip()} if raw else set()
