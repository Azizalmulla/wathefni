...
- Wave 3+ lifecycle/clearance/letters product (settlement **pay path** is in; exit clearance module is not) — see draft `ops/WATHEFNI_HCM_WAVE3_EMPLOYEE_LIFECYCLE_BUILD_CHARTER.md`
...
| C1 | Attendance ingest + projection + correction apply | **QUALIFIED** `ATTENDANCE_TRUTH_FULL_PASS` — stop for owner review before C2 |
| C2 | Leave enforcement + N-step/delegation | **ACCEPTED** `LEAVE_ENFORCEMENT_FULL_PASS` — C2 frozen |
| C3 | Shifts MSS allowlist unlock | **ACCEPTED** `SHIFTS_MSS_FULL_PASS` — C3 frozen |
| C4 | Payroll authoritative finalize + input contracts | **ACCEPTED** `PAYROLL_AUTHORITY_FULL_PASS` — C4 frozen |
| C5 | Payslips release + payment files | **ACCEPTED** `PAYSLIP_PAYMENT_FULL_PASS` — C5 frozen |
| C6 | Final settlement pay path + OT contract | **ACCEPTED** `SETTLEMENT_OT_FULL_PASS` — C6 frozen |
| C7 | Product acceptance / modularity matrix / Setup | **ACCEPTED** `WAVE2_PRODUCT_FULL_PASS` — Wave 2 frozen |
...
- [x] Entitled company: punches ingested → projections → exceptions → corrections applied  
- [x] Leave balances enforce; multi-step approve + delegation  
- [x] Manager can roster within scope without empty allowlist dead-end  
- [x] Payroll finalize authoritative for entitled company; payslips released to app  
- [x] Payment file generation gated, audited, kill-switchable  
- [x] Final settlement path from lifecycle inputs → payroll  
- [ ] Analytics hooks emit canonical facts (hours, leave days, pay components) for Wave 5 *(safe debt — Wave 5)*  
- [x] Modularity matrix proven (independent modules + explicit contracts)  
- [x] Setup Console owns Wave 2 company policies (not env-only)  
