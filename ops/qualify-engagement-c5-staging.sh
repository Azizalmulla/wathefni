#!/usr/bin/env bash
# Wave 6 C5 — Engagement qualify (company-scoped; global OFF).
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/engagement-c5-$STAMP"
REMOTE_STAGE="/tmp/engagement-c5-stage"
...
assert eg.runtime_gate_for_company("WATHEFNI").get("ok") is not True
assert "engagement" in w6.WAVE6_MODULE_KEYS
assert w6.honesty_payload().get("engagement_recognition_out") is True
h = eg.honesty_payload()
assert h["recognition_out_of_mvp"] is True
assert h["min_responses_default_5"] is True
assert h["threshold_upward_only"] is True
assert h["anonymous_no_respondent_answer_map"] is True
assert h["assistant_mutations"] is False
assert eg.recognition_absent_check()["recognition_absent"] is True
src = open("engagement_c5.py").read()
assert "eng_campaigns" in src and "eng_invitations" in src
assert "CREATE TABLE IF NOT EXISTS eng_recognition" not in src
assert "CHECK (min_responses >= 5)" in src
...
card = (root / "Wave6EngagementPoliciesCard.tsx").read_text()
app = (root / "SetupConsoleApp.tsx").read_text()
assert "Wave6EngagementPoliciesCard" in app
assert "Wave6EmployeeRelationsPoliciesCard" in app
assert "wave6_engagement" in card
assert "recognition" in card.lower() or "تقدير" in card
...
cp -a "$REPO_ROOT/ops/ENGAGEMENT_FULL_PASS.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
cp -a "$REPO_ROOT/ops/ENGAGEMENT_C5_FREEZE_AMENDMENT.md" "$LOCAL_EVID/docs/" 2>/dev/null || true

"${SSH[@]}" "rm -rf '$REMOTE_STAGE' && mkdir -p '$REMOTE_STAGE'"
"${SCP[@]}" \
  "$ORCH_SRC/engagement_c5.py" \
  "$ORCH_SRC/setup_console_wave6_policies.py" \
  "$ORCH_SRC/smoke-test-engagement-c5.py" \
...
if grep -Rls "WATHEFNI_ENGAGEMENT_C5=on" /etc/systemd/system/wathefni-orchestrator-staging.service.d 2>/dev/null; then
  echo "UNEXPECTED_SYSTEMD_ENGAGEMENT_ON"; exit 1
fi
echo STAGING_COPY_OK_GLOBAL_ENGAGEMENT_OFF
REMOTE
...
"\$PYBIN" smoke-test-engagement-c5.py
echo STAGING_RC=\$?
REMOTE

log "C1–C4 Wave6 + Waves 1–5 freeze unit regressions"
REG_OK=YES
set +e
...
for f in ja-c1-unit.out ld-c2-unit.out bn-c3-unit.out er-c4-unit.out; do
...
DB_OK=NO
if grep -q 'ENGAGEMENT_FULL_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
if [[ "$GATES_OK" == YES && "$FE_OK" == YES && "$DB_OK" == YES && "$REG_OK" == YES ]]; then
  VERDICT='ENGAGEMENT_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Engagement C5 — Staging Prove

- Stamp: $STAMP
- Gates: $GATES_OK
- Frontend Setup card: $FE_OK
- Staging DB: $DB_OK
- C1–C4 Wave6 + Wave5/Wave4/Wave1 unit freezes: $REG_OK
- Global Engagement flag: remains **off**
- Verdict: **$VERDICT**

Stop before C6 Compensation Planning until owner accepts.
EOF
