#!/usr/bin/env bash
# Production Readiness R5E — Learning & Development product surface qualification.
#
# Proves Wave 6 C2 Learning is a usable product:
# HTTP adapter → HR Web → scoped Manager → Employee App.
# Benefits / ER / Engagement / Comp Planning / Workforce Planning stay unreleased.
# Does not begin R5F.
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/production-readiness-r5e-learning-$STAMP"
REMOTE_STAGE="/tmp/r5e-learning-surface-stage"
...
log "2/8 local R5E + R5D + R5C + R5B + R5A unit contracts"
cd "$ORCH_SRC"
...
cp -a "$DASH_SRC/src/posthire/LearningWorkspace.tsx" "$LOCAL_EVID/dashboard/" 2>/dev/null || true
cp -a "$MOBILE_SRC/src/features/learning/LearningHubView.tsx" "$LOCAL_EVID/mobile/" 2>/dev/null || true
...
test -f '$STG/learning_http.py'
test -f '$STG/learning_surfaces.py'
test -f '$STG/learning_development_c2.py'
test -f '$STG/capability_readiness.py'
python3 -m py_compile '$STG/app.py' '$STG/learning_http.py' '$STG/learning_surfaces.py' '$STG/capability_readiness.py'
...
staging_py smoke-test-r5e-learning-surface-db.py "$LOCAL_EVID/tests/staging-db.out"
...
ck("live learning customer enableable", ready.customer_enableable("learning") is True)
...
ck("live benefits still hidden", ready.customer_enableable("benefits") is False)
...
for path in ("/dashboard/learning/workspace", "/dashboard/learning/catalog", "/app/learning"):
...
log "6/8 regressions (Waves 1–6 + R2–R5D)"
REG_OK=YES
set +e
for t in smoke-test-wave6-product-acceptance smoke-test-job-architecture-c1 smoke-test-learning-development-c2 \
         ...
         smoke-test-r5d-job-architecture-surface \
...
http = Path("wathefni-orchestrator/learning_http.py").read_text(encoding="utf-8")
routes = [line.strip() for line in http.splitlines() if "/dashboard/learning" in line or "/app/learning" in line]
(evid / "api-inventory.md").write_text(
    "# R5E API inventory\\n\\n" + "\\n".join(f"- {r}" for r in routes[:120]) + "\\n",
    encoding="utf-8",
)
...
grep -q 'R5E_LEARNING_SURFACE_UNIT_PASS' "$LOCAL_EVID/tests/r5e-unit.out" \
  ...
  && grep -qE '[0-9]+ passed, 0 failed' "$LOCAL_EVID/tests/r5e-unit.out" && UNIT_OK=YES

DB_OK=NO
grep -q 'R5E_LEARNING_SURFACE_DB_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
VERDICT=FAIL
if [[ "$DASH_OK" == YES && "$MOBILE_OK" == YES && "$UNIT_OK" == YES && "$DB_OK" == YES && "$R5D_DB_OK" == YES && "$R5C_DB_OK" == YES && "$R5B_DB_OK" == YES && "$R5A_DB_OK" == YES && "$LIVE_OK" == YES && "$REG_OK" == YES && "$DEPLOY_OK" == YES ]]; then
  VERDICT='PRODUCTION_READINESS_R5E_LEARNING_SURFACE_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# Production Readiness R5E — Learning Product Surface
...
- Waves 1–6 + R2–R5D regressions: $REG_OK
- Verdict: **$VERDICT**

Scope: R5E Learning product surface over frozen Wave 6 C2 authority.
Benefits, Employee Relations, Engagement, Compensation Planning, and Workforce Planning remain unreleased.
FULL_PASS is not authorisation for production rollout.
Do not begin R5F until owner review.
EOF
