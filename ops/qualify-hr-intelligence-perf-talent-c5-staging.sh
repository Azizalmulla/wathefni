#!/usr/bin/env bash
# Wave 5 C5 — Performance + Talent Intelligence qualify (company-scoped; global OFF).
set -euo pipefail
...
LOCAL_EVID="$REPO_ROOT/ops/evidence/hr-intelligence-perf-talent-c5-$STAMP"
REMOTE_STAGE="/tmp/hr-intelligence-perf-talent-c5-stage"
...
import os, sys
sys.path.insert(0, ".")
os.environ["WATHEFNI_HR_INTELLIGENCE_PERF_TALENT_C5"]="off"
os.environ["WATHEFNI_HR_INTELLIGENCE_REGISTRY_C1"]="off"
import hr_intelligence_perf_talent_c5 as t
...
assert t.PASS_STAMP == "HR_INTELLIGENCE_PERFORMANCE_TALENT_FULL_PASS"
h = t.honesty_payload()
assert h.get("uses_c1_registry_evaluator") is True
assert h.get("hipo_is_explicit_designation_only") is True
assert h.get("recruiting_pool_queried") is False
assert h.get("no_universal_score") is True
assert len(t.ALL_SEMANTIC_KEYS) == 19
...
cp -a "$REPO_ROOT/ops/HR_INTELLIGENCE_PERFORMANCE_TALENT_FULL_PASS.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
cp -a "$REPO_ROOT/ops/HR_INTELLIGENCE_PERF_TALENT_C5_FREEZE_AMENDMENT.md" "$LOCAL_EVID/docs/" 2>/dev/null || true
...
for flag in HR_INTELLIGENCE_PERF_TALENT_C5 HR_INTELLIGENCE_TIME_PAY_C4 HR_INTELLIGENCE_RECRUITING_C3 HR_INTELLIGENCE_WORKFORCE_C2 HR_INTELLIGENCE_REGISTRY_C1; do
...
staging_py smoke-test-hr-intelligence-perf-talent-c5.py "$LOCAL_EVID/tests/staging-db.out"
...
DB_OK=NO
if grep -q 'HR_INTELLIGENCE_PERFORMANCE_TALENT_FULL_PASS' "$LOCAL_EVID/tests/staging-db.out" \
...
ok_stamp() {
  ...
  grep -q "$stamp" "$file" && grep -qE '[0-9]+ passed, 0 failed' "$file"
}
...
VERDICT=FAIL
if [[ "$GATES_OK" == YES && "$DB_OK" == YES && "$C4_OK" == YES && "$C3_OK" == YES && "$C2_OK" == YES && "$C1_OK" == YES ]]; then
  VERDICT='HR_INTELLIGENCE_PERFORMANCE_TALENT_FULL_PASS'
fi

cat > "$LOCAL_EVID/REPORT.md" <<EOF
# HR Intelligence Perf/Talent C5 — Staging Prove

- Stamp: $STAMP
- Gates: $GATES_OK
- Staging DB: $DB_OK
- C4–C1 regressions: $C4_OK / $C3_OK / $C2_OK / $C1_OK
