...
**P1** document-first Kuwait bank certificate OCR proven on real Gulf Bank letter
(`ops/evidence/bank-ess-p1-real-doc-extraction-qual-20260807T081338Z/`) — **frozen**.
**P2** production employee workflow (`CONTRACT_VERSION=bank_ess_v1_p2_production_workflow`):
upload → extract → confirm/correct → proposed → HR → payroll → Apply.
Auth Wave 2 Phase 6: **not started**.
Verdicts: Bank ESS P0/P1 `fully_proven` · P2 shipping on canary ·
...
P1 real-document prove (frozen): MIME sniff for extensionless evidence paths;
HR extraction IBAN/account always masked on review surfaces.

### P2 — Production employee workflow

Same authority model. UX completes the production loop:

1. Upload certificate (primary) → extract → confirm/correct → submit proposed
2. Distinct failure copy: wrong document, missing IBAN, partial, unreadable
3. Correction/resubmit stays document-first (replace certificate or edit)
4. HR review: current vs employee-proposed vs masked extracted fields
5. Payroll approve → Apply → payroll-effective only
