...
8. **Synthetic-only** — do not disable `AUTHORITY_SYNTHETIC_ONLY` / `OPS_SYNTHETIC_ONLY` or enable `CAPTURE_INGEST` without an explicit owner-approved wave.
   - **Amendment (Wave 2 C1):** `ops/ATTENDANCE_TRUTH_C1_FREEZE_AMENDMENT.md` — company-scoped entitled ingest allowlist may be process-scoped for canary prove; **global** ingest stays off; empty allowlist = nobody.
