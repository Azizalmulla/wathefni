...
cd "$ORCH_SRC"
cp -a app.py \
  migration_wave1_cv_foundation.py \
...
mkdir -p "\$OUTDIR"
tr '\0' '\n' < /proc/\$PID/environ | grep -E 'MIGRATION_WAVE1|CAPTURE_INGEST|ASSISTANT_MUTATIONS|SCHEMA_APPLY|LOCAL_HYBRID|GPT_AUTO' | sort | tee "\$OUTDIR/flags.txt"
...
grep -q 'SCHEMA_APPLY=' "\$OUTDIR/flags.txt" && { echo REFUSE_SCHEMA_APPLY_SET; exit 3; } || echo SCHEMA_APPLY_UNSET_OK
