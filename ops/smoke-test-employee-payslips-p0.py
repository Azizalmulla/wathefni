...
def main() -> int:
    ...
    check("official pdf feature gated", honesty.get("official_employee_pdf") is True)
    check(
        "official pdf requires external seal",
        "external_import" in str(honesty.get("official_employee_pdf_requires") or "")
        and "external" in str(honesty.get("official_employee_pdf_requires") or ""),
        honesty.get("official_employee_pdf_requires"),
    )
    check("native preview never official pdf", honesty.get("native_preview_never_official_pdf") is True)
    ...
    try:
        with app.db_connect() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SAVEPOINT ep0_create_period")
                try:
                    ...
                    if not period.get("ok"):
                        raise RuntimeError(str(period.get("error") or "period_create_failed"))
                    cur.execute("RELEASE SAVEPOINT ep0_create_period")
                except Exception:
                    cur.execute("ROLLBACK TO SAVEPOINT ep0_create_period")
                    ...
                    row = cur.fetchone()
                    period = {"ok": True, "period": dict(row)} if row else {"ok": False}
        ...
        ar_honesty = str(((d_ar.json().get("payslip") or {}).get("honesty") or {}).get("ar") or "")
        check(
            "summary AR honesty",
            ("معاينة" in ar_honesty or "غير ملزمة" in ar_honesty or "الرسمي" in ar_honesty) and len(ar_honesty) > 20,
            d_ar.json(),
        )
        ...
        # P0.1: native/preview payslips refuse official PDF download (no soft honesty body).
        dl_body = (dl.text or "").lower()
        check(
            "http download owner",
            dl.status_code in (400, 404, 409, 422)
            and ("official_pdf_not" in dl_body or "not available" in dl_body or "not eligible" in dl_body),
            dl.text[:300],
        )
        check(
            "http download header honesty",
            dl.headers.get("X-Wathefni-Official-Document") in (None, "false")
            or dl.status_code in (400, 404, 409, 422),
        )
