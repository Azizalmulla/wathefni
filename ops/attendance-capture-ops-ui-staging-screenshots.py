...
def main() -> int:
    ...
    print(
        ...
        st,
        {
            "ok": seed.get("ok") if isinstance(seed, dict) else None,
            "error": (seed.get("error") if isinstance(seed, dict) else None)
            or (seed.get("detail") if isinstance(seed, dict) else seed),
        },
    )
    if st != 200 or not (isinstance(seed, dict) and seed.get("ok")):
        # One retry with a freshly minted session (covers brief post-restart races).
        token = mint()
        st, seed = http("POST", "/dashboard/posthire/attendance/capture-ops/seed-synthetic", token, {"tag": PREFIX})
        print(
            "SEED_RETRY",
            ...
            {
                "ok": seed.get("ok") if isinstance(seed, dict) else None,
                "error": (seed.get("detail") if isinstance(seed, dict) else seed),
            },
        )
    if st != 200 or not (isinstance(seed, dict) and seed.get("ok")):
